...
class EditorSelection {
  ...
  static cursor(pos, assoc = 0, bidiLevel, goalColumn) {
    return SelectionRange.create(
      pos,
      pos,
      (assoc == 0 ? 0 : assoc < 0 ? 8 /* RangeFlag.AssocBefore */ : 16) /* RangeFlag.AssocAfter */ |
  ...
  static range(anchor, head, goalColumn, bidiLevel) {
    let flags =
      ...
      (bidiLevel == null ? 7 : Math.min(6, bidiLevel));
    return head < anchor
      ? SelectionRange.create(head, anchor, 32 /* RangeFlag.Inverted */ | 16 /* RangeFlag.AssocAfter */ | flags)
...
class Facet {
  ...
  static define(config = {}) {
    return new Facet(
      ...
      config.compare || (!config.combine ? sameArray$1 : (a, b) => a === b),
      !!config.static,
      config.enables
    );
  }
...
let mac = typeof navigator !== 'undefined' && /Mac/.test(navigator.platform);
let ie$1 =
...
function replaceRange(parent, fromI, fromOff, toI, toOff, insert, breakAtStart, openStart, openEnd) {
  ...
  if (toI < children.length) {
    ...
    if (
      after &&
      (toOff < after.length || (after.breakAfter && (last === null || last === void 0 ? void 0 : last.breakAfter)))
    ) {
...
class LineView extends ContentView {
  ...
  sync(view, track) {
    ...
    if (
      ...
      !this.length ||
      (last.nodeName != 'BR' &&
        ((_a = ContentView.get(last)) === null || _a === void 0 ? void 0 : _a.isEditable) == false &&
...
class MarkDecoration extends Decoration {
  constructor(spec) {
    ...
    super(
      start ? -1 /* Side.InlineIncStart */ : 500000000 /* Side.NonIncStart */,
  ...
  for (let iI = 0; iI <= isolates.length; iI++) {
    ...
    for (let i = from, prev = prevType, prevStrong = prevType; i < to; i++) {
      ...
      if (type == 128 /* T.CS */) {
        ...
        let replace =
          (i && prev == 8) /* T.EN */ || (end < rTo && types[end] == 8) /* T.EN */
            ? prevStrong == 1 /* T.L */
              ? 1 /* T.L */
              : 8 /* T.EN */
            : 256; /* T.NI */
...
function computeOrder(line, direction, isolates) {
  ...
  if (isolates.length) {
    while (line.length > types.length) {
      types[types.length] = 256 /* T.NI */;
    }
  } // Make sure types array has no gaps
  let order = [],
...
class DocView extends ContentView {
  ...
  update(update) {
    ...
    let composition =
      readCompositionAt > -1 ? findCompositionRange(this.view, update.changes, readCompositionAt) : null;
    this.domChanged = null;
  ...
  updateInner(changes, oldLength, composition) {
    ...
    observer.ignore(() => {
      ...
      let track =
        browser.chrome || browser.ios ? { node: observer.selectionRange.focusNode, written: false } : undefined;
  ...
  fixCompositionDOM(composition) {
    let fix = (dom, cView) => {
      cView.flags |=
        ...
        (cView.children.some((c) => c.flags & 7 /* ViewFlag.Dirty */) ? 1 /* ViewFlag.ChildDirty */ : 0);
...
function nextToUneditable(node, offset) {
  ...
  return (
    (offset && node.childNodes[offset - 1].contentEditable == 'false' ? 1 /* NextTo.Before */ : 0) |
...
function doPaste(view, input) {
  ...
  if (linewise) {
    ...
    changes = state.changeByRange((range) => {
      ...
      return { changes: { from: line.from, insert }, range: EditorSelection.cursor(range.from + insert.length) };
    });
  } else if (byLine) {
    changes = state.changeByRange((range) => {
  ...
  if (browser.chrome && browser.android && (pending = PendingKeys.find((key) => key.inputType == event.inputType))) {
    ...
    if (pending.key == 'Backspace' || pending.key == 'Delete') {
      ...
      setTimeout(() => {
        ...
        if (
          (((_a = window.visualViewport) === null || _a === void 0 ? void 0 : _a.height) || 0) > startViewHeight + 10 &&
          view.hasFocus
        ) {
...
class ViewState {
  ...
  getViewport(bias, scrollTarget) {
    ...
    let viewport = new Viewport(
      map.lineAt(visibleTop - marginTop * 1000 /* VP.Margin */, QueryType$1.ByHeight, oracle, 0, 0).from,
      map.lineAt(visibleBottom + (1 - marginTop) * 1000 /* VP.Margin */, QueryType$1.ByHeight, oracle, 0, 0).to
    );
    ...
    if (scrollTarget) {
      ...
      if (head < viewport.from || head > viewport.to) {
        ...
        let block = map.lineAt(head, QueryType$1.ByPos, oracle, 0, 0),
          topPos;
        ...
        viewport = new Viewport(
          map.lineAt(topPos - 1000 /* VP.Margin */ / 2, QueryType$1.ByHeight, oracle, 0, 0).from,
          map.lineAt(topPos + viewHeight + 1000 /* VP.Margin */ / 2, QueryType$1.ByHeight, oracle, 0, 0).to
        );
      }
    }
    return viewport;
  ...
  lineBlockAtHeight(height) {
    return scaleBlock(
      this.heightMap.lineAt(this.scaler.fromDOM(height), QueryType$1.ByHeight, this.heightOracle, 0, 0),
      this.scaler
    );
  }
...
const baseTheme$1$2 = /*@__PURE__*/ buildTheme(
  '.' + baseThemeID,
  {
    '&': {
...
function applyDefaultInsert(view, change, newSel) {
  ...
  if (
    ...
    tr = startState.replaceSelection(
      view.state.toText(before + change.insert.sliceString(0, undefined, view.state.lineBreak) + after)
    );
  } else {
...
class EditorView {
  ...
  get hasFocus() {
    ...
    return (
      (this.dom.ownerDocument.hasFocus() ||
        (browser.safari &&
          ((_a = this.inputState) === null || _a === void 0 ? void 0 : _a.lastContextMenu) > Date.now() - 3e4)) &&
  ...
  scrollSnapshot() {
    ...
    return scrollIntoView$1.of(
      new ScrollTarget(EditorSelection.cursor(ref.from), 'start', 'start', ref.top - scrollTop, scrollLeft, true)
    );
  }
  /**
  ...
  let add = (scope, key, command, preventDefault, stopPropagation) => {
    ...
    let binding =
      ...
      (scopeObj[full] = {
        ...
        stopPropagation: false,
        run:
          ((_b = (_a = scopeObj._any) === null || _a === void 0 ? void 0 : _a.run) === null || _b === void 0
            ? void 0
            : _b.slice()) || [],
      });
    if (command) {
...
function rectanglesForRange(view, className, range) {
  ...
  let leftSide =
    contentRect.left + (lineStyle ? parseInt(lineStyle.paddingLeft) + Math.min(0, parseInt(lineStyle.textIndent)) : 0);
  ...
  if (visualStart && visualEnd && visualStart.from == visualEnd.from) {
    ...
    let between = [];
    if (
      (visualStart || startBlock).to < (visualEnd || endBlock).from - (visualStart && visualEnd ? 1 : 0) ||
...
function hoverTooltip(source, options = {}) {
  ...
  let hoverState = StateField.define({
    ...
    update(value, tr) {
      if (
        value &&
        ((options.hideOnChange && (tr.docChanged || tr.selection)) || (options.hideOn && options.hideOn(tr, value)))
      ) {
        return null;
      }
  ...
  return [
    hoverState,
    ViewPlugin.define(
      (view) => new HoverPlugin(view, source, hoverState, setHover, options.hoverTime || 300 /* Hover.Time */)
    ),
  ...
  if (overlays) {
    for (let scan = node, parent = scan.parent; parent; scan = parent, parent = scan.parent) {
      if (
        ...
        scan.index < 0 &&
        ((_a = parent.enter(pos, side, mode)) === null || _a === void 0 ? void 0 : _a.from) != scan.from
      ) {
        node = parent;
      }
    }
...
class BufferNode extends BaseNode {
  ...
  get prevSibling() {
    ...
    return new BufferNode(
      this.context,
      this._parent,
      buffer.findChild(parentStart, this.index, -1, 0, 4 /* Side.DontCare */)
    );
  }
  get tree() {
    return null;
...
class TreeCursor {
  ...
  parent() {
    ...
    let parent =
      this.mode & IterMode.IncludeAnonymous ? this.buffer.parent : this.buffer.parent.nextSignificantParent();
    this.buffer = null;
  ...
  sibling(dir) {
    ...
    return d < 0
      ? this.yield(this.buffer.parent.nextChild(this.buffer.index + dir, dir, 0, 4 /* Side.DontCare */, this.mode))
      : false;
  }
  /**
...
function buildTree(data) {
  ...
  let length =
    (_a = data.length) !== null && _a !== void 0 ? _a : children.length ? positions[0] + children[0].length : 0;
  ...
  function divide(children, positions, from, to, offset) {
    for (let i = from; i < to; ) {
      ...
      if (i == groupFrom + 1) {
        ...
        localChildren.push(
          balanceRange(balanceType, children, positions, groupFrom, i, groupStart, length, null, mkTree)
        );
      }
    ...
    work(deadline) {
      ...
      let endTime =
        ...
        Math.min(
          this.chunkBudget,
          100 /* Work.Slice */,
          deadline && !isInputPending ? Math.max(25 /* Work.MinSlice */, deadline.timeRemaining() - 5) : 1e9
        );
...
const matchingMark = /*@__PURE__*/ Decoration.mark({ class: 'cm-matchingBracket' }),
...
class FuzzyMatcher {
  ...
  match(word) {
    ...
    if (adjacentTo == len && adjacentStart == 0) {
      return this.ret(
        -200 /* Penalty.CaseFold */ - word.length + (adjacentEnd == word.length ? 0 : -100) /* Penalty.NotFull */,
        [0, adjacentEnd]
      );
    }
    if (direct > -1) {
    ...
    if (adjacentTo == len) {
      return this.ret(-200 /* Penalty.CaseFold */ + -700 /* Penalty.NotStart */ - word.length, [
        adjacentStart,
        adjacentEnd,
      ]);
    }
    ...
    return chars.length == 2
      ? false
      : this.result(
          (any[0] ? -700 /* Penalty.NotStart */ : 0) + -200 /* Penalty.CaseFold */ + -1100 /* Penalty.Gap */,
          any,
          word
        );
  }
  ...
  static build(active, state, id, prev, conf) {
    ...
    if (!options.length) {
      return prev && active.some((a) => a.state == 1 /* State.Pending */)
        ? new CompletionDialog(prev.options, prev.attrs, prev.tooltip, prev.timestamp, prev.selected, true)
        : null;
    }
  ...
  update(tr) {
    ...
    let active = sources.map((source) => {
      let value =
        ...
        new ActiveSource(
          source,
          this.active.some((a) => a.state != 0 /* State.Inactive */) ? 1 /* State.Pending */ : 0 /* State.Inactive */
        );
  ...
  handleUserEvent(tr, type, conf) {
    ...
    if (
      ...
      return new ActiveSource(
        this.source,
        type == 'input' && conf.activateOnTyping ? 1 /* State.Pending */ : 0 /* State.Inactive */
      );
    }
  ...
  class {
    ...
    update(update) {
      ...
      this.debounceUpdate = cState.active.some(
        (a) => a.state == 1 /* State.Pending */ && !this.running.some((q) => q.active.source == a.source)
      )
...
function autocompletion(config = {}) {
  return [completionState, completionConfig.of(config), completionPlugin, completionKeymapExt, baseTheme$3];
}
/**
...
const selectParentSyntax = ({ state, dispatch }) => {
  let selection = updateSel(state.selection, (range) => {
    ...
    for (let cur = stack; cur; cur = cur.next) {
      let { node } = cur;
      if (
        ((node.from < range.from && node.to >= range.to) || (node.to > range.to && node.from <= range.from)) &&
...
class RegExpCursor {
  ...
  constructor(text, query, options, from = 0, to = text.length) {
    ...
    this.re = new RegExp(
      query,
      baseFlags + ((options === null || options === void 0 ? void 0 : options.ignoreCase) ? 'i' : '')
    );
  ...
  next() {
    for (;;) {
      ...
      if (match) {
        ...
        if (
          (this.flat.to >= this.to || match.index + match[0].length <= this.flat.text.length - 10) &&
...
function insideWord(check, state, from, to) {
  return (
    check(state.sliceDoc(from, from + 1)) == CharCategory.Word && check(state.sliceDoc(to - 1, to)) == CharCategory.Word
  );
}
const matchHighlighter = /*@__PURE__*/ ViewPlugin.fromClass(
  class {
    constructor(view) {
      this.decorations = this.getDeco(view);
    }
    ...
    getDeco(view) {
      ...
      if (range.empty) {
        ...
        if (conf.wholeWords) {
          ...
          if (
            !(
              insideWordBoundaries(check, state, range.from, range.to) && insideWord(check, state, range.from, range.to)
            )
          ) {
            return Decoration.none;
          }
...
class RegExpQuery extends QueryType {
  ...
  getReplacement(result) {
    return this.spec.unquote(
      this.spec.replace.replace(/\$([$&\d+])/g, (m, i) =>
        i == '$' ? '$' : i == '&' ? result.match[0] : i != '0' && +i < result.match.length ? result.match[i] : m
      )
    );
  }
  matchAll(state, limit) {
...
function defaultQuery(state, fallback) {
  ...
  return new SearchQuery({
    search: (
      (_a = fallback === null || fallback === void 0 ? void 0 : fallback.literal) !== null && _a !== void 0
        ? _a
        : config.literal
    )
      ? selText
      : selText.replace(/\n/g, '\\n'),
    caseSensitive:
      (_b = fallback === null || fallback === void 0 ? void 0 : fallback.caseSensitive) !== null && _b !== void 0
        ? _b
        : config.caseSensitive,
    literal:
      (_c = fallback === null || fallback === void 0 ? void 0 : fallback.literal) !== null && _c !== void 0
        ? _c
        : config.literal,
    regexp:
      (_d = fallback === null || fallback === void 0 ? void 0 : fallback.regexp) !== null && _d !== void 0
        ? _d
        : config.regexp,
    wholeWord:
      (_e = fallback === null || fallback === void 0 ? void 0 : fallback.wholeWord) !== null && _e !== void 0
        ? _e
        : config.wholeWord,
  });
}
...
class Stack {
  ...
  reduce(action) {
    ...
    if (
      ...
      !((_a = this.p.parser.nodeSet.types[type]) === null || _a === void 0 ? void 0 : _a.isAnonymous)
    ) {
  ...
  canShift(term) {
    for (let sim = new SimulatedStack(this); ; ) {
      let action =
        this.p.parser.stateSlot(sim.state, 4 /* ParseState.DefaultReduce */) ||
  ...
  get deadEnd() {
    ...
    let { parser } = this.p;
    return (
      parser.data[parser.stateSlot(this.state, 1 /* ParseState.Actions */)] == 65535 /* Seq.End */ &&
  ...
  shiftContext(term, start) {
    if (this.curContext) {
      this.updateContext(
        this.curContext.tracker.shift(this.curContext.context, term, this, this.p.stream.reset(start))
      );
    }
  }
    ...
    if (!(side < 0 ? cursor.childBefore(pos) : cursor.childAfter(pos))) {
      for (;;) {
        if ((side < 0 ? cursor.to < pos : cursor.from > pos) && !cursor.type.isError) {
          return side < 0
            ? Math.max(0, Math.min(cursor.to - 1, pos - 25 /* Safety.Margin */))
...
const prqlHighlight = styleTags({
  ...
  Docblock: tags.docString,
  'this that': tags.self,
  null: tags.null,
  Boolean: tags.bool,
  Integer: tags.integer,
  Float: tags.float,
  DeclarationItem: tags.propertyName,
  TypeTerm: tags.typeName,
  Escape: tags.escape,
  String: tags.string,
  FString: tags.special(tags.string),
  RString: tags.special(tags.string),
  SString: tags.special(tags.string),
  TimeUnit: tags.unit,
...
const parser = LRParser.deserialize({
  version: 14,
  states:
    ";SO`QPOOOOQO'#Dc'#DcOqQPO'#DsO$rQPO'#CwO%PQPO'#CvO&mQPO'#C}O'OQPO'#C`OOQO'#Dl'#DlO'TQPO'#DsO'`QPO'#EnOOQO'#Ds'#DsO}QPO'#DsO'TQPO'#DsQOQPOOO'hQPO'#D[OOQO-E7a-E7aO'mQPO,5:_OOQO,5:_,5:_O'sQPO,5:_O'mQPO,5:_O'xQPO'#DfO(QQPO,59cO*yQPO'#CbOOQO'#Cg'#CgO+iQPO'#ESO-wQPO'#CiO/sQPO'#DgOOQO'#ES'#ESO1uQPO'#CjO2VQPO'#CyOOQO'#Dg'#DgO2[QPO'#CxOOQO,59b,59bO2rQPO'#CrO4`QPO'#C|O4hQPO'#DOOOQO'#Ec'#EcO4mOQO'#DUO4xOSO'#DUO5TOQO'#DUO5`OSO'#DUOOQO'#Dx'#DxO5kQPO'#CaO5pQPO'#CpOOQO'#E`'#E`O4cQPO'#DiO5wQPO,59iOOQO'#Cb'#CbOOQO'#Dd'#DdO6YQPO,58zOOQO-E7j-E7jO6bQPO,5;YOOQO,5;Y,5;YOOQO'#D^'#D^O6jQPO,59vOOQO1G/y1G/yO6oQPO1G/yO6tQPO1G/yOOQO,5:Q,5:QOOQO-E7d-E7dOOQO'#Ch'#ChO-wQPO,59QO-wQPO,59QOOQO,59T,59TOOQO'#Cl'#ClOOQO'#Cm'#CmO5pQPO,59VO5pQPO,59VO5pQPO,59VO7PQPO'#ERO7^QPO'#ERO-wQPO'#EROOQO,59U,59UO7hQPO,59UO-wQPO,59eOOQO-E7e-E7eO2rQPO,59^O7mQPO'#EZO7}QPO'#CtO:ZQPO'#CsO:`QPO'#DxOOQO'#EZ'#EZO:|QPO,59^O4`QPO,59hO;XQPO,59hOOQO,59j,59jOOOO'#Dj'#DjO;aOQO,59pOOQO,59p,59pOOOO'#Dk'#DkO;lOSO,59pO;wOQO,59pO<SOSO,59pO-wQPO,58{O<_QPO,59[OOQO,5:T,5:TOOQO-E7g-E7gO>dQPO1G.fOOQO-E7b-E7bO>rQPO1G/bOOQO7+%e7+%eO>}QPO7+%eO?SQPO1G.lOOQO1G.l1G.lODXQPO1G.qOD`QPO1G.qOOQO1G.q1G.qODgQPO,5:mO-wQPO,5:mODqQPO,5:mOERQPO,5:mOEZQPO,5:mOOQO1G.p1G.pOEeQPO1G/POGaQPO1G.xO-wQPO,59gO-wQPO,59_OGlQPO1G.xOGtQPO1G.xOGaQPO1G.xOOQO1G.x1G.xOG{QPO1G/SOOQO1G/S1G/SOHOQ
SOr$ers%Usw$ewx%px#O$e#P;'S$e;'S;=`&[<%lO$e_'^[!hP#XQ#ZSOY$eYZ(SZr$ers%Ust)Ptw$ewx%px#O$e#O#P+`#P;'S$e;'S;=`&[<%lO$e_(Z[#XQ#ZSOY$eYZ(SZr$ers%Ust)Ptw$ewx%px#O$e#O#P+`#P;'S$e;'S;=`&[<%lO$e_)WZ#XQ#ZSOY)PYZ(SZr)Prs)ysw)Pwx,ax#O)P#O#P+e#P;'S)P;'S;=`-y<%lO)P]*OX#ZSOY)yYZ*kZw)ywx+ex#O)y#O#P+e#P;'S)y;'S;=`,Z<%lO)y]*pY#ZSOY%UYZ*kZs%Ust)ytw%Ux#O%U#O#P+`#P;'S%U;'S;=`%j<%lO%UX+eO!fXX+hTOY+eYZ+wZ;'S+e;'S;=`,T<%lO+eX+zRYZ+wst+e#O#P+`X,WP;=`<%l+e],^P;=`<%l)yZ,fX#XQOY,aYZ-RZr,ars+es#O,a#O#P+e#P;'S,a;'S;=`-s<%lO,aZ-WX#XQOY%pYZ-RZr%pst,at#O%p#O#P+`#P;'S%p;'S;=`&U<%lO%pZ-vP;=`<%l,a_-|P;=`<%l)PV.WY#XQ#ZSOr$ers%Usw$ewx%px!_$e!_!`.v!`#O$e#P;'S$e;'S;=`&[<%lO$eV/PWbP#XQ#ZSOr$ers%Usw$ewx%px#O$e#P;'S$e;'S;=`&[<%lO$eV/pV#[R#ZSOr%Urs0Vsw%Ux#O%U#P;'S%U;'S;=`%j<%lO%UV0[V#ZSOr%Urs0qsw%Ux#O%U#P;'S%U;'S;=`%j<%lO%UV0xT#WR#ZSOw%Ux#O%U#P;'S%U;'S;=`%j<%lO%U_1b[#XQ#ZSPXOY2WYZ$eZq2Wqr5ers3Ssw2Wwx4ex#O2W#O#P3v#P;'S2W;'S;=`5_<%lO2W_2aZ#XQ#ZSPXOY2WYZ$eZr2Wrs3Ssw2Wwx4ex#O2W#O#P3v#P;'S2W;'S;=`5_<%lO2W]3ZX#ZSPXOY3SYZ%UZw3Swx3vx#O3S#O#P3v#P;'S3S;'S;=`4_<%lO3SX3{SPXOY3vZ;'S3v;'S;=`4X<%lO3vX4[P;=`<%l3v]4bP;=`<%l3SZ4lX#XQPXOY4eYZ%pZr4ers3vs#O4e#O#P3v#P;'S4e;'S;=`5X<%lO4eZ5[P;=`<%l4e_5bP;=`<%l2W_5pZ#XQ#ZSQXPXOY5eYZ$eZr5ers6csw5ewx7xx#O5e#O#P7X#P;'S5e;'S;=`8t<%lO5e]6lX#ZSQXPXOY6cYZ%UZw6cwx7Xx#O6c#O#P7X#P;'S6c;'S;=`7r<%lO6cX7`SQXPXOY7XZ;'S7X;'S;=`7l<%lO7XX7oP;=`<%l7X]7uP;=`<%l6cZ8RX#XQQXPXOY7xYZ%pZr7xrs7Xs#O7x#O#P7X#P;'S7x;'S;=`8n<%lO7xZ8qP;=`<%l7x_8wP;=`<%l5eV9TW!qP#XQ#ZSOr$ers%Usw$ewx%px#O$e#P;'S$e;'S;=`&[<%lO$eV9tX#XQ#ZSOr$ers%Usv$evw:awx%px#O$e#P;'S$e;
ZP;=`<%l#L`T#MaP;=`<%l#IpR#MiZ#XQOr#Mdrs#N[sw#Mdwx$ gx#O#Md#O#P#N[#P#o#Md#o#p$ }#p;'S#Md;'S;=`$!{<%lO#MdP#N_VOw#N[wx#Khx#o#N[#o#p#Nt#p;'S#N[;'S;=`$ a<%lO#N[P#NwUOw#Ntx#q#Nt#q#r#N[#r;'S#Nt;'S;=`$ Z<%lO#NtP$ ^P;=`<%l#NtP$ dP;=`<%l#N[R$ nT#XQ{POr%ps#O%p#P;'S%p;'S;=`&U<%lO%pR$!SZ#XQOr$ }rs#Ntsw$ }wx%px#O$ }#O#P#Nt#P#q$ }#q#r#Md#r;'S$ };'S;=`$!u<%lO$ }R$!xP;=`<%l$ }R$#OP;=`<%l#Md_$#[a#XQ#ZS!iXOr$ers$$asw$ewx$(Tx!Q$e!Q![#:}![!c$e!c!}#:}!}#O$e#P#R$e#R#S#:}#S#T$e#T#o#:}#o$g$e$g;'S#:};'S;=`#<]<%lO#:}T$$fZ#ZSOr$$ars$%Xsw$$awx$%ox#O$$a#O#P$%o#P#o$$a#o#p$'P#p;'S$$a;'S;=`$'}<%lO$$aT$%`T#ZS|POw%Ux#O%U#P;'S%U;'S;=`%j<%lO%UP$%rVOr$%ors$&Xs#o$%o#o#p$&^#p;'S$%o;'S;=`$&y<%lO$%oP$&^O|PP$&aUOr$&^s#q$&^#q#r$%o#r;'S$&^;'S;=`$&s<%lO$&^P$&vP;=`<%l$&^P$&|P;=`<%l$%oT$'UZ#ZSOr$'Prs%Usw$'Pwx$&^x#O$'P#O#P$&^#P#q$'P#q#r$$a#r;'S$'P;'S;=`$'w<%lO$'PT$'zP;=`<%l$'PT$(QP;=`<%l$$aR$(YZ#XQOr$(Trs$({sw$(Twx$*Wx#O$(T#O#P$({#P#o$(T#o#p$*n#p;'S$(T;'S;=`$+l<%lO$(TP$)OVOw$({wx$&Xx#o$({#o#p$)e#p;'S$({;'S;=`$*Q<%lO$({P$)hUOw$)ex#q$)e#q#r$({#r;'S$)e;'S;=`$)z<%lO$)eP$)}P;=`<%l$)eP$*TP;=`<%l$({R$*_T#XQ|POr%ps#O%p#P;'S%p;'S;=`&U<%lO%pR$*sZ#XQOr$*nrs$)esw$*nwx%px#O$*n#O#P$)e#P#q$*n#q#r$(T#r;'S$*n;'S;=`$+f<%lO$*nR$+iP;=`<%l$*nR$+oP;=`<%l$(TV$+{W!|P#XQ#ZSOr$ers%Usw$ewx%px#O$e#P;'S$e;'S;=`&[<%lO$e_$,nY#TX#XQ#ZSOr$ers%Usw$ewx%px#O$e#P#p$e#p#q$-^#q;'S$e;'S;=`&[<%lO$eV$-gW!wP#XQ#ZSOr$ers%Usw$ewx%px#O$e#P;'S$e;'S;=`&[<%lO$eV$.YW!{P#XQ#ZSOr$ers%Usw$ewx%px#O$e#P;'S$e;'S;=`&[<%lO$e~$.wO#^~",
  tokenizers: [0, 1, 2, 3],
  topRules: { Query: [0, 3] },
  specialized: [{ term: 71, get: (value) => spec_identPart[value] || -1 }],
  tokenPrec: 2306,
});
...
const prqlLanguage = LRLanguage.define({
  name: 'prql',
