...
test.afterEach(async () => {
    if (grafanaContainer) { await grafanaContainer.stop({ remove: true }); }
    if (nginxContainer) { await nginxContainer.stop({ remove: true }); }
});

test.describe("Auth Proxy", () => {
    test("should login the user successfully", async ({ browser, request }) => {
        const page = await createPageWithHttpCredentials(
            "user1",
            "grafana",
            browser
        );
    ...
    test("should return 401 when allow_signup is false", async ({
        browser,
        request,
    }) => {
        const page = await createPageWithHttpCredentials(
...
const createPageWithHttpCredentials = async (
    username: string,
    password: string,
    browser: Browser
) => {
