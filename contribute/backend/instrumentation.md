...
- **`panic: listen tcp 127.0.0.1:6000: bind: address already in use`.** A stale Grafana process is holding the pprof port. Find and kill it:
  ...
  Parallel workspaces default to the same pprof port and collide. Override one with the `GF_DIAGNOSTICS_PROFILING_ADDR` / port env vars or the `-profile-port` flag.
