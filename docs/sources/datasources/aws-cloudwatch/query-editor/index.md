...
Cross-account querying is available in the plugin through the `Logs`, `Metric search` and `Metric Insights` mode. To support cross-account query building in `Metric Insights` builder mode, the cloudwatchMetricInsightsCrossAccount feature toggle should be enabled. Once you have it configured correctly, you'll see a "Monitoring account" badge displayed in the query editor header.
