...
In conjunction with **Group by**, you can set a positive number in the **Limit** input to limit the maximum number of time series returned by the data source. Note that the series returned are always ordered by descending value (total aggregated data over the time period).
