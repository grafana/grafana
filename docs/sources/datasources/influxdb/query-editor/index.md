...
| `$__timeFilter`             | The time range that applies the start and the end of the currently active time selection. |
...
| `$__dateBin(<column>)`      | Applies [date_bin](https://docs.influxdata.com/influxdb/cloud-serverless/reference/sql/functions/time-and-date/#date_bin) function. Column must be a timestamp. |
| `$__dateBinAlias(<column>)` | Applies [date_bin](https://docs.influxdata.com/influxdb/cloud-serverless/reference/sql/functions/time-and-date/#date_bin) function with suffix `_binned`. Column must be a timestamp. |
...
```sql
-- Using time filter macros
SELECT * FROM cpu WHERE time >= $__timeFrom AND time <= $__timeTo
SELECT * FROM cpu WHERE $__timeFilter(time)

-- Using date_bin macros
SELECT $__dateBin(time) from cpu

-- Interpolated result
SELECT date_bin(interval '15 second', time, timestamp '1970-01-01T00:00:00Z') from cpu
```
