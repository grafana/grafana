...
At the top of the editor there is `Run queries` button that will run the queries and `Explain | Builder | Code` tabs to switch between the editor modes. If the query editor is in Builder mode there are additional elements explained in the Builder section.

> **Note:** Run queries button is not present in Explore. To run Prometheus queries, use Explore's `Run query` button.
