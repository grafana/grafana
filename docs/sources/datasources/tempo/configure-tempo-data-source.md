...
The **Trace to logs** setting configures [trace to logs](ref:explore-trace-integration) that is available when you integrate Grafana with Tempo.
...
1. Optional: Switch on the **Filter by trace ID** and/or **Filter by span ID** setting to further filter the logs if your logs consistently contain trace or span IDs.
...
1. Optional: Change the **Span start time shift** and/or **Span end time shift**. The default start time shift is -2 minutes and 2 minutes for end time shift.
