...
-- **Data source -** Target data source.
-- **Tags -** The tags that may be used in the linked queries. The key is the span attribute name. The optional value is the corresponding metric label name (e.g. map `k8s.pod` to `pod`). You may interpolate these tags into your queries using the `$__tags` keyword.

Each linked query consists of:

-- **Link Label -** (Optional) Descriptive label for the linked query.
-- **Query -** Query that runs when navigating from a trace to the metrics data source. Interpolate tags using the `$__tags` keyword. For example, with the tags `k8s.pod=pod` and `cluster` configured the query `requests_total{$__tags}` would result in `requests_total{pod="nginx-554b9", cluster="us-east-1"}`.
