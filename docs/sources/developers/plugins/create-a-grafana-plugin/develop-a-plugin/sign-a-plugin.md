...
1. Submit your plugin for [review]({{< relref "../../publish-a-plugin/publish-or-update-a-plugin.md#publishing-your-plugin-for-the-first-time" >}})
...
With a **private** plugin, you need to add a `rootUrls` flag to the `plugin:sign` command. The `rootUrls` must match the [root_url]({{< relref "/docs/grafana/setup-grafana/configure-grafana#root_url" >}}) configuration. For more information, refer to [Sign a private plugin](#sign-a-private-plugin).
