...
Grouping is configured by using labels in the notification policy that reference the labels that are generated by the alert instances, or that are configured by the user. With notification policies, you can also define how often notifications are sent for each group of alert instances.
