...
```bash
curl -H "Authorization: Bearer glsa_iNValIdinValiDinvalidinvalidinva_lidinVal" -X GET '<grafana_url>/api/access-control/user/permissions' | jq
```
...
```bash
curl -H "Authorization: Bearer glsa_iNValIdinValiDinvalidinvalidinva_lidinVal" -X GET '<grafana_url>/api/access-control/user/permissions' | jq '."dashboards:read"'
```
