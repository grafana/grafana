...
--header 'Authorization: Bearer glsa_iNValIdinValiDinvalidinvalidinva_lidinVal' \
...
- Add the following permissions to the `basic:editor` role, using provisioning or the [RBAC HTTP API](ref:api-rbac-update-a-role):
  ...
  curl -H 'Authorization: Bearer glsa_iNValIdinValiDinvalidinvalidinva_lidinVal' \
  ...
  # Update the role
  curl -H 'Authorization: Bearer glsa_iNValIdinValiDinvalidinvalidinva_lidinVal' -H 'Content-Type: application/json' \
