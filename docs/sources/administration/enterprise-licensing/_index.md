...
**API `/login` endpoint rate limit**: Grafana Cloud users are limited to 10 log in attempts per user via the `/login` API endpoint in a given 10-minute period. The period starts from the first log in attempt.
