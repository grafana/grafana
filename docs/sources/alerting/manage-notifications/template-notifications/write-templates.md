...
You write message templates in Go's templating language, [text/template](https://pkg.go.dev/text/template). In the following topics we will look at how to write message templates, and then look at a number of more complex examples that you might want to use as a reference for customizing your notifications.
...
You cannot write a template that use `{{ .Labels }}` in the second range because here dot refers to the current label, not the current alert:
...
It is possible to fix this using variables, which we will look at next.

## Variables

While most templates written text/template use dot, it is also possible to define variables. Variables in text/template must be created within the template. For example, to create a variable called `$variable` with the current value of dot you would write the following:

```
{{ $variable := . }}
```

You can use `$variable` inside a range or `with` and it will refer to the value of dot at the time the variable was defined, not the current value of dot. This means variables can be used to fix the issue in the previous example:
...
{{ range .Labels.SortedPairs }}
The name of the label is {{ .Name }}, and the value is {{ .Value }}
{{/* this works because $alert refers to the value of dot inside the first range */}}
{{ $alert.SortedPairs }}
{{ end }}
{{ end }}
```
