...
> **Note:** Before v8.2 the configuration of the embedded Alertmanager was shared across organisations. Therefore, users running an earlier version are advised to use the new Grafana 8 Alerts only if they have one organisation otherwise contact points for the Grafana managed alerts will be visible by all organizations.
