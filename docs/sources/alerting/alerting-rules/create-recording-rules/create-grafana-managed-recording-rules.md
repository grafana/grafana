...
[Recording rules](ref:create-recording-rules) allow you to pre-compute expensive queries in advance and save the results as a new set of time series. They then query the pre-computed results instead of real-time data, which can respond faster and reduce system load.
...
First, enable the `grafanaManagedRecordingRules` [feature flag](https://grafana.com/docs/grafana/<GRAFANA_VERSION>/setup-grafana/configure-grafana/feature-toggles/).
