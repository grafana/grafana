...
- For Grafana Mimir and Loki data sources, enable the ruler API by configuring their respective services.
  ...
  - **Grafana Mimir** - use the `/prometheus` prefix. The Prometheus data source supports both Grafana Mimir and Prometheus, and Grafana expects that both the [Query API]({{< relref "/docs/mimir/latest/references/http-api#querier--query-frontend" >}}) and [Ruler API]({{< relref "/docs/mimir/latest/references/http-api#ruler" >}}) are under the same URL. You cannot provide a separate URL for the Ruler API.
