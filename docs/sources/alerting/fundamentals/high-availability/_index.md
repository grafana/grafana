...
While the alert generator evaluates all alert rules on all instances, Grafana makes a best-effort attempt to avoid sending duplicate notifications. Grafana's Alertmanager chooses availability over consistency, which may result in occasional duplicated or out-of-order notifications. Alertmanager takes the opinion that duplicate or out-of-order notifications are better than no notifications. It uses a gossip protocol to share information about notifications between Grafana instances.
