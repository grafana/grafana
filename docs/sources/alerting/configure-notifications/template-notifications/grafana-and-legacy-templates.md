...
description: Compare Grafana and Legacy notification templates, their definitions, functions, and data, and learn how to adapt templates when replacing Legacy integrations.
keywords:
  - grafana
  - alerting
...
Grafana and Legacy notification templates differ in their named definitions, built-in defaults, and functions. Integrations also differ in the data they provide to templates. Use this guide to understand these differences before sharing templates or replacing a Legacy integration.
...
Each integration selects templates from its own version, not from the contact point or the alert's origin. A contact point can contain a mix of Grafana and Legacy integrations. Hosting an integration in Grafana doesn't make it use Grafana templates. All Legacy integrations use Legacy templates.
...
| Built-in templates     | Alertmanager defaults with Grafana additions and overrides, including an overridden `__subject` and additional `default.title` and `default.message` definitions. | Alertmanager defaults, including `__subject`, but not Grafana's `default.title` or `default.message`. |
| Functions              | Alertmanager functions, Mimir helpers, and Grafana's `coll`, `data`, `tmpl`, and `time` namespaces.                                                               | Alertmanager functions and Mimir helpers.                                                             |
| Notification data      | Grafana integrations can provide additional fields, such as dashboard and silence URLs and alert values.                                                          | Legacy integrations use Alertmanager notification data; don't assume Grafana-only fields exist.       |

Grafana and Legacy templates both include Mimir helpers such as `grafanaExploreURL` and `queryFromGeneratorURL`. Using one of these functions doesn't identify whether a template is Grafana or Legacy.
...
A templated integration setting can contain text and Go template expressions, or call a named template. For example, a notification template group can contain this definition:
...
Grafana and Legacy templates have separate _definition namespaces_: the collections of names that integrations can look up. A Grafana integration searches only the Grafana definition namespace; a Legacy integration searches only the Legacy definition namespace. If a definition is missing, the integration doesn't search the other namespace. For example, defining `team.message` only in a Grafana template group doesn't make it available to a Legacy integration.

This separation is independent of the differences in built-in defaults. The same name can exist independently in both definition namespaces. Within one namespace, however, definitions are shared: changing one can affect multiple contact points. Use unique definition names within each namespace to avoid one group overriding another.
...
- **Dot:** This is the data passed to a particular template call. At the notification level, dot contains the alert group. Inside `range .Alerts`, dot refers to one alert.
...
A definition that expects one alert instead can access that alert's labels:
...
The template-testing API uses the field `scope` for the input data: `.` means notification data, `.Alerts` means the alert list, and `.Alert` means one alert. These test labels don't select Grafana or Legacy templates. In particular, `.Alert` in a test result isn't a field on notification data.
...
If an import renames a conflicting template group, it doesn't rewrite the `define` names or template calls inside that group. Check definition names across imports, not just filenames or group titles.

When replacing a Legacy integration with a Grafana integration, adapt its template dependencies for use with Grafana templates:
...
Template errors don't always prevent delivery. Depending on the integration and setting, an error can fail delivery or leave part of the message empty. Check the received content as well as the delivery status.
