...
refs:
  ...
  oncall-oss:
    - pattern: /docs/grafana/
      destination: /docs/oncall/latest/set-up/open-source/#install-grafana-oncall-oss
    - pattern: /docs/grafana-cloud/
...
You can also configure the integration from Grafana OnCall. For more information see [Grafana OnCall documentation](ref:oncall-integration)
{{< /admonition >}}
...
If you are using Grafana OSS, [install and enable the Grafana OnCall plugin](ref:oncall-oss). Also, ensure your version of Grafana is up-to-date and supports the new features.
