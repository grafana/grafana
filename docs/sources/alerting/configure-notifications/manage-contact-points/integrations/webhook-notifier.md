...
| Authentication Header Scheme      | Scheme for the `Authorization` Request Header. Defaults to `Bearer` when left empty. To send only the credentials with no scheme prefix (for example, when an API expects a bare API key), set this field to a single space (` `). |
| Authentication Header Credentials | Credentials for the `Authorization` Request header.                                                                                                                                                                                |
| Extra Headers                     | Additional HTTP headers to include in the request. You can also override the default `Content-Type: application/json` header to specify a different media type for the request payload.                                          |
