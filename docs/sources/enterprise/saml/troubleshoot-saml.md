...
We only support one private key format: PKCS#8. We seen that the keys generated by customers may be in a different format (PKCS#1 or PKCS#12); in that case, it may be necessary to convert the private key format.
