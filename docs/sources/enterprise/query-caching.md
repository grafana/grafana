...
Query caching works for all [Enterprise data sources](https://grafana.com/grafana/plugins/?type=datasource&enterprise=1) and some [built-in data sources]({{< relref "#data-sources-that-work-with-query-caching" >}}). You can enable the cache globally and configure the cache duration (also called Time to Live, or TTL).
