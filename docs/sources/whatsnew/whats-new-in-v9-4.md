...
We have included a new feature flag which allows us to set a new expiration date limit for all newly created service account tokens.

This will not affect existing tokens, however newly created tokens will require an expiration date that doesn't exceeds the configuration option `token_expiration_day_limit`.
