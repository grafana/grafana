...
We have included a new configuration option, disabled by default. This will allow you to require an expiration date limit for all newly created service account tokens.

This change will not affect existing tokens. However, newly created tokens will require an expiration date that doesn't exceed the configuration option `token_expiration_day_limit`.
...
### Alerting: Declare incidents from firing alerts
...
### Alerting: Copy alert rules and notification templates
...
### Alerting: View query definitions for provisioned alerts
...
### Alerting: Export alert rules to use in the provisioning API or files
...
### Alerting: Pause alert rule evaluation
...
### Alerting: Add support for Discord as a contact point receiver
...
### Alerting: Compatibility with AWS Aurora
