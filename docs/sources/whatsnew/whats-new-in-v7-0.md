...
By chaining a simple set of point and click [transformations]({{< relref "../panels-visualizations/transform-data/#transformation-functions" >}}), users will be able join, pivot, filter, re-name and do calculations to get the results they need. Perfect for operations across queries or data sources missing essential data transformations.

[Transformations]({{< relref "../panels-visualizations/transform-data/" >}}) also adds the ability to do math across queries. Lots of data sources do not support this natively, so being able to do it in Grafana is a powerful feature.
...
Learn more about this feature in [Transformations]({{< relref "../panels-visualizations/transform-data/#transformation-functions" >}}).
