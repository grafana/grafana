...
|     {{< figure src="/media/docs/grafana/panels-visualizations/save-to-query-icon.png" max-width="30px" max-height="30px" alt="Save to query library icon" >}}     | Save query to library. Saves the query to the library so it can be reused. Access saved queries by clicking **+ Add query from library**. For more infomation, refer to [Query library](#query-library) (Enterprise and Cloud only).            |
