...
Use this transformation to convert time series result into a table, converting time series data frame into a "Trend" field. "Trend" field can then be rendered using [chart cell type]({{< relref "../../visualizations/table/#chart" >}}), producing an inline chart for each table row. If there are multiple time series queries, each will result in a separate table data frame. These can be joined using join or merge transforms to produce a single table with multiple charts per row.
