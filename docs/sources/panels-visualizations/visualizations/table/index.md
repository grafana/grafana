...
| Max cell height      | <p>Define the maximum height for the row. This option is only displayed if you enable **Wrap text**.</p><p>If you set a maximum height for a specific cell using an override, but the cell appears taller than it should, this is likely because another cell in the row is taller than that one due to text wrapping.</p> |
