...
{{% admonition type="note" %}}
This option is only applicable when Text mode is set to **Value and name**. When wide layout is enabled, the value is shifted to the right when the panel is wide enough. When wide layout is disabled, the value is always rendered underneath the name.
{{% /admonition %}}
