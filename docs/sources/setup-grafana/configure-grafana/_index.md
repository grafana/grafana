...
Caches authentication details and session information in the configured database, Redis or Memcached. Only works if a `database` is configured. This setting does not configure [Query Caching in Grafana Enterprise](../../administration/data-source-management/#query-and-resource-caching).
