...
> With Grafana 10, if `oauth_skip_org_role_update_sync` option
> is set to `false`, users with no mapping will be reset to the default organization role on every login.
