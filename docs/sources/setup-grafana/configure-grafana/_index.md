...
Depending on your OS, your custom configuration file is either the `$WORKING_DIR/conf/config.ini` file or the `/usr/local/etc/grafana/grafana.ini` file. The custom configuration file path can be overridden using the `--config` parameter.
