...
2. Use the [Standard field definitions' display name]({{< relref "../dashboards/configure-panels-visualizations/configure-standard-options/#display-name" >}}) to format the alias. For the preceding example query, you would use `${__field.labels.hostname}` option.
