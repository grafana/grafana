...
| `auto_login`                 | No       | Set to `true` to allow users to automatically log in and skip the login screen. This setting is ignored if multiple auth providers are configured to use auto login.                           | `false`         |
...
| `skip_org_role_sync`         | No       | Set to `true` to stop automatically syncing user roles. This will allow you to manually set organization roles for your users from within Grafana.                                             | `false`         |
