...
This page explains how to configure LDAP authentication in Grafana using the Grafana user interface. For a more detailed information about configuring LDAP authentication using the configuration file, refer to [LDAP authentication](../ldap/).

Benefits of using the Grafana user interface to configure LDAP authentication include:

- No need to manually edit the configuration file.
- Easily test the connection to the LDAP server.
...
1. **Search filter**: LDAP search filter needed to find entries within the directory.
...
1. **Bind DN**: Distinguished name (DN) of the user to bind with.
...
1. **Port**: Port number of the LDAP server. Default is 389.
1. **Timeout**: Time in seconds to wait for a response from the LDAP server.

#### 2. Attributes

Attributes to map LDAP user assertion to Grafana user attributes.
...
1. **Member Of**: Name of the assertion attribute to map to the Grafana user member of.
...
1. **Skip organization role sync**: Skip syncing organization roles. Useful when you want to manage roles manually.
1. **Group search filter**: LDAP search filter needed to find groups within the directory.
1. **Group search base DNS**: List of base DNS to specify where the matching groups are located.
1. **Group name attribute**: Identifies users within group entries.
1. **Manage group mappings**:

   The following fields will become available when managing group mappings. Add a new group mapping by clicking the **Add group mapping** button.
...
Additional security settings for LDAP authentication.

1. **Enable SSL**: This option will enable SSL for the connection to the LDAP server.
...
1. **Encryption key and certificate provision specification**:
   ...
   1. **Path to files**:
      Path in file system to the key and certificate files
...
If you decide to delete all the changes made through the UI and revert to the configuration file settings, click the three dots and click **Reset to default values**.
