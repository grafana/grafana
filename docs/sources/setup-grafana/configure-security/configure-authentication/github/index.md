...
Let's use the following Github teams: `@my-github-organization/my-github-team1` and `@my-github-organization/my-github-team2`.
...
If the user was a member of the `@my-github-organization/admin` group, they would be granted the Grafana server administrator role.

Config:

```ini
role_attribute_path = contains(groups[*], '@my-github-organization/admin') && 'GrafanaAdmin' || 'None'
allow_assign_grafana_admin = true
org_attribute_path = groups
org_mapping = @my-github-organization/my-github-team1:team1:Viewer @my-github-organization/my-github-team2:team2:Editor *:team3:Editor
```

### Example of GitHub configuration in Grafana

Following is an example of a GitHub configuration in the Grafana configuration file.
...
| `org_attribute_path`         | No       | [JMESPath](http://jmespath.org/examples.html) expression to use for Grafana org to role lookup. Grafana will first evaluate the expression using the user information obtained from the UserInfo endpoint. If no value is returned, Grafana creates a JSON data with `groups` key that maps to GitHub teams obtained from GitHub's [`/api/user/teams`](https://docs.github.com/en/rest/teams/teams#list-teams-for-the-authenticated-user) endpoint, and evaluates the expression using this data. The result of the evaluation will be mapped to org roles based on `org_mapping`. For more information on org to role mapping, refer to [Org roles mapping examples]({{< relref "#org-roles-mapping-examples" >}}).        |                                               |
