...
{{< admonition type="note" >}}
Unlike other OAuth providers, the `skip_org_role_sync` option is enabled by default for Google. While it's enabled, Grafana doesn't evaluate `role_attribute_path`, `role_attribute_strict`, `org_mapping`, or `allow_assign_grafana_admin`, so users aren't denied access even if `role_attribute_strict = true`. Set `skip_org_role_sync = false` to use role mapping.
{{< /admonition >}}
...
| `role_attribute_strict`      | No       | Yes                | Set to `true` to deny user login if the Grafana org role cannot be extracted using `role_attribute_path` or `org_mapping`. For more information on user role mapping, refer to [Configure role mapping](#configure-role-mapping). Has no effect while `skip_org_role_sync` is enabled, which it is by default.                                                                                                                                                                                  | `false`                                            |
...
| `skip_org_role_sync`         | No       | Yes                | Set to `true` to stop automatically syncing user roles. This will allow you to set organization roles for your users from within Grafana manually. While enabled, `role_attribute_path`, `role_attribute_strict`, `org_mapping`, and `allow_assign_grafana_admin` have no effect.                                                                                                                                                                                                               | `true`                                             |
