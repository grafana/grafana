...
2. Configure `jwk_set_url` with the URL of your GitLab instance's JSON Web Key Set (JWKS) endpoint. This endpoint provides the public keys used to verify JWT signatures.
   ...
   For GitLab.com, the JWKS endpoint is: `https://gitlab.com/.well-known/jwks.json`

   For self-hosted GitLab instances, the JWKS endpoint is typically: `https://<your-gitlab-domain>/.well-known/jwks.json`
...
| `jwk_set_url`                | No       | Yes                | URL of the JSON Web Key Set (JWKS) endpoint used to verify JWT ID token signatures. Required when ID token validation is enabled. For GitLab.com, use `https://gitlab.com/.well-known/jwks.json`. For self-hosted GitLab instances, use `https://<your-gitlab-domain>/.well-known/jwks.json`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |                                      |
