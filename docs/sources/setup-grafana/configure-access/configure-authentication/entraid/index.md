...
| `token_exchange_timeout`        | No       | Yes                | Timeout duration for the OAuth token exchange request (e.g. `30s`, `1m`). When set, Grafana cancels the token exchange if it does not complete within this duration. Useful in environments with variable network latency. Defaults to no explicit timeout.                                                                                                                                                                                                                                     |                                                      |
