...
labels:
  products:
    ...
    - enterprise
    - oss
menuTitle: Microsoft Entra ID OAuth
title: Configure Microsoft Entra ID OAuth authentication
weight: 800
---
...
1. Click **Certificates & secrets** in the side menu, then add a new entry under the supported client authentication option you want to use. The following are the supported client authentication options with their respective configuration steps.
   ...
   - **Federated credentials**
     ...
     - **_Workload Identity (K8s/AKS)_**
       ...
       1. You may optionally set `workload_identity_token_file` (environment variable `GF_AUTH_AZUREAD_WORKLOAD_IDENTITY_TOKEN_FILE`) under `[auth.azuread]` to a custom token file path in the Grafana server configuration. By default, Grafana reads the path from the `AZURE_FEDERATED_TOKEN_FILE` environment variable. If the environment variable is unset or empty, and a file is not specified, Grafana uses `/var/run/secrets/azure/tokens/azure-identity-token`.
...
| `workload_identity_token_file`  | No       | Yes                | The path to the token file used to authenticate to the OAuth2 provider. This is only required when `client_authentication` is set to `workload_identity`. The token file contains the service account token projected by Kubernetes. It defaults to the value of the `AZURE_FEDERATED_TOKEN_FILE` environment variable, or `/var/run/secrets/azure/tokens/azure-identity-token` if the environment variable is unset or empty.                                                                  | `AZURE_FEDERATED_TOKEN_FILE`, then `/var/run/secrets/azure/tokens/azure-identity-token` |
