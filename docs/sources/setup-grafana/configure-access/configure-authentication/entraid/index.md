...
1. Click **Certificates & secrets** in the side menu, then add a new entry under the supported client authentication option you want to use. The following are the supported client authentication options with their respective configuration steps.
   ...
   - **Federated credentials**
     ...
     - **_Workload Identity (K8s/AKS)_**
       ...
       1. You may optionally set `workload_identity_token_file` under `[auth.azuread]` to `$__env{AZURE_FEDERATED_TOKEN_FILE}` in the Grafana server configuration for this to work. You may alternatively set it through the env var `GF_AUTH_AZUREAD_WORKLOAD_IDENTITY_TOKEN_FILE="${AZURE_FEDERATED_TOKEN_FILE}"`. When unset, Grafana defaults to the legacy path `/var/run/secrets/azure/tokens/azure-identity-token`.
