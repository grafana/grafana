...
1. Click **Certificates & secrets** in the side menu, then add a new entry under the supported client authentication option you want to use. The following are the supported client authentication options with their respective configuration steps.
   ...
   - **Federated credentials**
     ...
     - **_Workload Identity (K8s/AKS)_**
       ...
       1. You may optionally set `workload_identity_token_file` (environment variable `GF_AUTH_AZUREAD_WORKLOAD_IDENTITY_TOKEN_FILE`) under `[auth.azuread]` to a custom token file path in the Grafana server configuration. By default, Grafana reads the path from the `AZURE_FEDERATED_TOKEN_FILE` environment variable.
