...
| `token_exchange_timeout`        | No       | No                 | Timeout in seconds for the OAuth token exchange HTTP request (e.g., the POST to the token endpoint). This can be increased for environments with high network latency to Entra ID (e.g., private clouds, VPNs). Sub-timeouts (dial, TLS handshake) scale proportionally. Defaults to `15`.                                                                                                                                                                                                          | `15`                                                 |
