...
1. In the editor, change the container image under the `kind: Deployment` section.
   ...
   - From
     ...
     - `yaml image: grafana/grafana:12.0.1`

   - To
     - `yaml image: grafana/grafana-dev:12.1.0`
...
1. Add the `change cause` metadata to keep track of things using the commands:
   ...
   ```bash
   kubectl annotate deployment grafana --namespace=my-grafana kubernetes.io/change-cause='using grafana-dev:12.1.0 for testing'
   ```
...
1. To verify, run the `kubectl rollout history` command:
   ...
   2         using grafana-dev:12.1.0 for testing
   ```
