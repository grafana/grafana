...
1. In the **Refresh** list, select when the variable should update options.
   ...
   - **On Time Range Change:** Queries the data source everytime the dashboard loads and when the dashboard time range changes. No matter which dashboard variable query option you choose, the query always runs at dashboard load. Use this option if your variable options query contains a time range filter or is dependent on the dashboard time range. 
