...
1. In the **Query** field, enter a query.
   ...
   - The exact way how the query variable data is extracted from the query results is specific to the given data source, please check the data source's documentation, but usually every string value returned from the data source is used as a possible variable value.
     - Additionally, some data sources allow providing custom "display names" for the values, for example the PostgreSQL, MySQL and the Microsoft SQL Server plugins handle this by looking for fields named `__text` and `__value` in the result. Other data sources look for `text` and `value` or use a different approach. Again, please refer to the documentation for the specific data source plugin.
