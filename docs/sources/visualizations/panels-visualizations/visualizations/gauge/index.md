...
| Limit | If you chose **All values** as your **Show** option, enter the maximum number of rows to display. The default is 25. The maximum is 5,000. |
...
| Segment spacing | Enter a value between 0 and 1 to set the factor that controls the size of the gap between each segment. This option applies only when the **Segments** value is more than `1`. |
| Bar Style | Choose the gauge bar shape when **Segments** is `1`. Choose from **Flat** or **Rounded**. |
| Endpoint marker | When **Bar Style** is **Rounded** and **Segments** is `1`, choose **Point**, **Glow**, or **None**. Glow is only supported in dark mode. |
