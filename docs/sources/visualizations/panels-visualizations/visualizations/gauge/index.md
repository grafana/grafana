...
| Limit | If you chose **All values** as your **Show** option, enter the maximum number of rows to display. The default is `25`, and the maximum is `5000`. |
...
| Bar Style | Choose **Flat** or **Rounded** for the gauge bar. This option is only displayed when **Segments** is set to `1`. |
| Endpoint marker | Choose **Point**, **Glow**, or **None** for the rounded gauge endpoint. **Glow** is only supported in dark mode. This option is only displayed when **Bar Style** is **Rounded** and **Segments** is set to `1`. |
...
If you want to visualize the list of values as a time series along with the calculated gauge value, use the **Show sparkline** switch. Sparkline is on by default for new gauge panels.
