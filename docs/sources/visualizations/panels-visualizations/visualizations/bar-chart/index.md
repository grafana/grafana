...
| X-axis tick label max length | Sets the maximum length of bar chart labels. Labels longer than the maximum length are truncated, and appended with `...`. This option appears when x-axis labels are rotated. |
...
| Group width                      | Controls the width of groups. 1 = Max width, 0 = Min width.       |
...
| Highlight full area on hover     | Controls if the entire surrounding area of the bar is highlighted when you hover over the bar.    |
...
| Transform                        | Transform values for a field override. Choose **Constant** or **Negative Y**. |
| Show thresholds                  | Control whether thresholds are shown for a field override. |
