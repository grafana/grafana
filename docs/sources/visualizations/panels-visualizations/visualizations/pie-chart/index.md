...
| Overflow | Control how overflow for long series names is handled. Choose **Ellipsis** or **Wrap**. This option is only displayed when **Mode** is **Table**. |
| Limit | Limit how many legend items are shown by default. Remaining items are available through **Show all**. |
