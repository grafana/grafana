...
| Overflow | Set how legend items are handled when they overflow the legend area. This option is only displayed when **Mode** is set to **Table**. Choose **Ellipsis** or **Wrap**. |
| Limit | Limits how many items are shown in the legend by default. |
