...
Filter keys and values support variable interpolation. For example, you can use `$var` or `${var}` in a filter value, and Grafana replaces it with the dashboard variable value when the visualization loads.

<!-- prettier-ignore-start -->

| Option | Description |
| ------ | ----------- |
| Filters | Set the initial adhoc span filters. You can filter by text search, duration, service name, span name, status, and span attributes. |
...
| Select critical path | Toggle the switch on to select the spans in the critical path, which helps identify performance bottlenecks and their impact on overall latency. |
