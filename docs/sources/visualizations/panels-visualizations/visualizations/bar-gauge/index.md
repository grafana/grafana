...
| Show name for single series | Off by default, which hides the name when there's only one bar and it has no configured display name. Turn this on to always show the name in that case. Has no effect when **Name placement** is set to **Hidden**. |
