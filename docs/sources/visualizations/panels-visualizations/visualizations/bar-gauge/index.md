...
| Text mode | Controls whether the series name is shown, independently of bar count. Choose from:<ul><li>**Auto** - Grafana's default heuristic: the name is hidden only when there's a single bar with no configured display name.</li><li>**Value and name** and **Name** - Always show the name, even for a single unnamed bar.</li><li>**Value** and **None** - Always hide the name.</li></ul> Has no effect when **Name placement** is set to **Hidden**. |
