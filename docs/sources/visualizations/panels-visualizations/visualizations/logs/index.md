...
| Prettify JSON | Toggle the switch on to pretty print all JSON logs. This setting does not affect logs in any format other than JSON. Only shown when **Wrap lines** is enabled. |
| Enable columns for displayed fields | Show selected displayed fields as columns. Only shown when **Wrap lines** is disabled. |
