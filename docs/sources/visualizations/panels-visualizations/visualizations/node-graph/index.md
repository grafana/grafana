...
The number of nodes shown at a given time is limited to maintain a reasonable visualization performance. Nodes that are not currently visible are hidden behind clickable markers that show the number of hidden nodes that are connected by a particular edge, up to 100. Markers for more than 100 hidden nodes show **>100 nodes** instead of an exact count. You can click on the marker to expand the graph around that node.
