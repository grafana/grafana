...
Node graphs show up to 200 nodes by default. If this limit is crossed, a warning is visible in the upper-right corner, and some nodes are hidden. You can expand hidden parts of the graph by clicking the **Hidden nodes** markers in the graph.
{{< /admonition >}}
...
- **Layered** - Default. Creates a predictable and orderly layout, especially useful for service graphs. When the graph has more than 500 nodes, Grafana shows a warning that the layered layout may be slow.
...
| fixedx         | number        | Fixed x-position for the node. If you use fixed positions, all nodes must have valid `fixedx` and `fixedy` values.                                                                                                                                                                                                                                                       |
| fixedy         | number        | Fixed y-position for the node. If you use fixed positions, all nodes must have valid `fixedx` and `fixedy` values.                                                                                                                                                                                                                                                       |
| isinstrumented | boolean       | Indicates whether the node is instrumented. The default is `true` when omitted.                                                                                                                                                                                                                                                                                          |
