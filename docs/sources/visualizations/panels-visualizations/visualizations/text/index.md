...
| Option | Description |
| ------ | ----------- |
| Mode | Determines how embedded content appears. Dashboard [variables](ref:variables) in the content are expanded in all modes. Choose from:<ul><li>**Markdown** - Formats the content as [markdown](https://en.wikipedia.org/wiki/Markdown).</li><li>**HTML** - Renders the content as [sanitized](https://github.com/grafana/grafana/blob/main/packages/grafana-data/src/text/sanitize.ts) HTML. If you require more direct control over the output, you can set the [disable_sanitize_html](ref:disable-sanitize-html) flag which enables you to directly enter HTML.</li><li>**Code** - Renders content inside a read-only code editor.</li></ul><p>To allow embedding of iframes and other websites, you need to set `allow_embedding = true` in your Grafana `config.ini` or environment variables, depending on your deployment.</p> |
| Content | The text or HTML content to display. Supports dashboard variable interpolation. |
| Language | When you choose **Code** as your text mode, select a syntax-highlighting language: **plaintext** (default), **json**, **yaml**, **xml**, **typescript**, **sql**, **go**, **markdown**, or **html**. |
