...
| Page size (enable pagination) | The **Page size** option lets you paginate the status history visualization to limit how many series are visible at once. The default page size is `20`, so pagination is enabled by default. Clear the field to disable pagination. |

<!-- prettier-ignore-end -->

### Status history field options

The following options are field options under the **Status history** category in field defaults or overrides.
...
| Fill opacity | Controls the fill opacity of status boxes. |
