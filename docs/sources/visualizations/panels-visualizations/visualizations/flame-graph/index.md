...
When you compare two profiles in diff mode, the table has four columns instead: **Symbol**, **Baseline**, **Comparison**, and **Diff**. **Baseline** and **Comparison** show each function's share of total ticks in the two profiles, and **Diff** shows the percentage change between them, colored using the selected [color scheme](#change-color-scheme).
