...
| Group by | Select the label keys used to group alert instances. This option is displayed when **Group mode** is set to **Custom grouping**. |
| Max items | Sets the maximum number of alerts to list. By default, Grafana sets this value to 20. This option is hidden when **Group mode** is set to **Custom grouping**. |
...
- **Importance** - By importance according to the grouping mode, with 1 being the highest. Default grouping uses rule state scores:
  - alerting: 1
  - firing: 1
  - no_data: 2
  - pending: 3
  ...
  - paused: 5
  - inactive: 5
    Custom grouping uses instance state scores:
  - Alerting, Firing, or Error: 1
  - Pending, Recovering, or Inactive: 2
  - NoData: 3
  - Normal: 4
  - Unknown: 5
...
| Folder | Filter Grafana-managed alert rules in the selected folder. This option appears when **Datasource** is empty or set to **-- Grafana --**. |
...
| Recovering | Shows alerts that recently fired and are now recovering. |
