...
| Color blind safe                       | A palette validated to be distinguishable by users with protanopia, deuteranopia, and tritanopia. Located in the **Accessible** group in the color scheme selector.                                                                  |
| Multiple accessible continuous colors (by value) | Grafana automatically assigns colors based on the percentage of a value relative to the min and the max of the field or series. For some visualizations, you also need to choose if the color is set by the **Last**, **Min**, or **Max** value of the field or series. Select from: **Viridis**, **Magma**, **Plasma**, **Inferno**, **Cividis**, and **Yellow-Blue**. Located in the **Accessible** group in the color scheme selector. |
