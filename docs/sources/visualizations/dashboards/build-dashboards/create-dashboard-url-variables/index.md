...
Then provide the key, operator, and value as a pipe-separated list.
...
In this URL, the query parameter `var-filter=key|=|value` applies the variable configured as the `filter` dashboard variable using the `example_key` key, the `=` operator, and the `example_value` value.

{{< admonition type="note" >}}
When sharing URLs with filters, remember to encode the URL.
In the preceding example, replace the pipes (`|`) with `%7C` and the equality operator (`=`) with `%3D`.
{{< /admonition >}}
