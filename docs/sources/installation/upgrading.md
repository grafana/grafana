...
Starting fro grafana v7.0.0, the cookie path does not include the trailing slash if grafana is served from subpath in order to align with [RFC 6265](https://tools.ietf.org/html/rfc6265#section-5.1.4). However, stale session cookies (set before the upgrade) can result in unsuccessful logins because they can not be deleted during the standard login phase due to the changed cookie path. Therefore users experience login problems are adviced to delete manually old session cookies or administrators can fix this for all the users by changing the [`login_cookie_name`]({{< relref "configuration/#login-cookie-name" >}}) so that the old cookie would get ignored.