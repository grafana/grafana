...
The database table _dashboard\_snapshot_, that stores dashboard snapshots, adds a new column _dashboard\_encrypted_ for storing an encrypted snapshot. Any existing snapshots is not migrated to be stored encrypted. Only snapshots created on this version and forward will be stored encrypted.
