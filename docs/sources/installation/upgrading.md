...
If you are using Auth proxy with LDAP a shared cached is used in Grafana so you might want configure [remote_cache] instead. If not
...
> Please note that when migrating a notification channel and later downgrading Grafana to an earlier version, the notification channel will not be able to read stored sensitive settings and by that it does not function as expected.
