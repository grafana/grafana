...
func testKind(t *testing.T, gr func() perKind) perKind {
	t.Helper()
	k := gr()
...
func alertRuleKind(t *testing.T) perKind {
	return testKind(t, func() perKind { return newPerKind(alertrule.ResourceInfo, nil) })
}

func recordingRuleKind(t *testing.T) perKind {
	return testKind(t, func() perKind { return newPerKind(recordingrule.ResourceInfo, nil) })
}
...
func perKindFilterLeaf(field, op string, values ...string) searchv0.WhereNode {
...
func perKindAndNode(children ...searchv0.WhereNode) *searchv0.WhereNode {
...
func validateFor(t *testing.T, k perKind, q *searchv0.SearchQuery) []string {
	t.Helper()
	_, errs := validatePerKindQuery(q, k)
...
func TestPerKindValidateQuery_envelope(t *testing.T) {
...
func TestPerKindValidateQuery_whereShape(t *testing.T) {
	...
	t.Run("accepts one and over leaves", func(t *testing.T) {
		assert.Empty(t, validate(t, whereQuery(perKindAndNode(
			perKindTextLeaf("cpu"),
			perKindFilterLeaf(fieldFolder, perKindFilterOperatorIn, "f1"),
		))))
	})
	...
	t.Run("rejects an empty and", func(t *testing.T) {
		...
		assert.Equal(t, []string{"where"}, validate(t, whereQuery(perKindAndNode())))
	})

	t.Run("rejects an unset and child", func(t *testing.T) {
		assert.Equal(t, []string{"where.and[0]"}, validate(t, whereQuery(perKindAndNode(searchv0.WhereNode{}))))
	})
	...
	t.Run("rejects a node setting more than one key", func(t *testing.T) {
		node := &searchv0.WhereNode{
			...
			Filter: &searchv0.FilterPredicate{Field: fieldFolder, Operator: perKindFilterOperatorIn, Values: []string{"f1"}},
		}
		assert.Equal(t, []string{"where"}, validate(t, whereQuery(node)))
	})

	t.Run("rejects nested and", func(t *testing.T) {
		assert.Equal(t, []string{"where.and[0]"}, validate(t, whereQuery(perKindAndNode(*perKindAndNode(perKindTextLeaf("cpu"))))))
	})
	...
	t.Run("rejects a second text leaf", func(t *testing.T) {
		assert.Equal(t, []string{"where.and[1].text"},
			validate(t, whereQuery(perKindAndNode(perKindTextLeaf("cpu"), perKindTextLeaf("mem")))))
	})
}
...
// asked for.
func TestPerKindValidateQuery_futureWhereNodes(t *testing.T) {
	leaf := perKindFilterLeaf(fieldFolder, perKindFilterOperatorIn, "f1")
...
func TestPerKindValidateQuery_filterLeaf(t *testing.T) {
	...
	t.Run("requires a field", func(t *testing.T) {
		assert.Equal(t, []string{"where.filter.field"}, leafErrs(t, perKindFilterLeaf("", perKindFilterOperatorIn, "x")))
	})
	t.Run("rejects an unknown field", func(t *testing.T) {
		assert.Equal(t, []string{"where.filter.field"}, leafErrs(t, perKindFilterLeaf("bogus", perKindFilterOperatorIn, "x")))
	})
	t.Run("requires at least one value", func(t *testing.T) {
		assert.Equal(t, []string{"where.filter.values"}, leafErrs(t, perKindFilterLeaf(fieldFolder, perKindFilterOperatorIn)))
	})
	t.Run("rejects an unsupported operator", func(t *testing.T) {
		assert.Equal(t, []string{"where.filter.operator"}, leafErrs(t, perKindFilterLeaf(fieldFolder, "Equals", "f1")))
	})
	...
	t.Run("rejects wildcard values", func(t *testing.T) {
		assert.Equal(t, []string{"where.filter.values[0]"}, leafErrs(t, perKindFilterLeaf(fieldFolder, perKindFilterOperatorIn, "f*")))
	})

	// A field a kind declares retrieve-only cannot be filtered.
	t.Run("rejects a field that is not filterable", func(t *testing.T) {
		assert.Equal(t, []string{"where.filter.field"}, leafErrs(t, perKindFilterLeaf(fieldAnnotations, perKindFilterOperatorIn, "x")))
	})
	...
	t.Run("rejects fields the legacy backend cannot filter", func(t *testing.T) {
		for _, name := range []string{fieldTitle, fieldInterval, fieldFor, fieldKeepFiringFor} {
			assert.Equal(t, []string{"where.filter.field"},
				leafErrs(t, perKindFilterLeaf(name, perKindFilterOperatorIn, "x")), "field %q", name)
		}
	})

	t.Run("scalar fields accept exactly one value", func(t *testing.T) {
		assert.Empty(t, leafErrs(t, perKindFilterLeaf(fieldPanelID, perKindFilterOperatorIn, "1")))
		assert.Equal(t, []string{"where.filter.values"}, leafErrs(t, perKindFilterLeaf(fieldPanelID, perKindFilterOperatorIn, "1", "2")))
	})
	...
	t.Run("paused must be a boolean", func(t *testing.T) {
		assert.Empty(t, leafErrs(t, perKindFilterLeaf(fieldPaused, perKindFilterOperatorIn, "true")))
		assert.Equal(t, []string{"where.filter.values[0]"}, leafErrs(t, perKindFilterLeaf(fieldPaused, perKindFilterOperatorIn, "yes")))
	})
	...
	t.Run("normalizes scalar values for both backends", func(t *testing.T) {
		for _, tc := range []struct {
			...
			node := perKindFilterLeaf(tc.field, perKindFilterOperatorIn, tc.value)
	...
	t.Run("panelID must be an integer", func(t *testing.T) {
		assert.Equal(t, []string{"where.filter.values[0]"}, leafErrs(t, perKindFilterLeaf(fieldPanelID, perKindFilterOperatorIn, "x")))
	})

	t.Run("type must name a rule kind", func(t *testing.T) {
		assert.Empty(t, leafErrs(t, perKindFilterLeaf(fieldType, perKindFilterOperatorIn, ruleTypeAlerting)))
		assert.Equal(t, []string{"where.filter.values[0]"}, leafErrs(t, perKindFilterLeaf(fieldType, perKindFilterOperatorIn, "bogus")))
	})

	// The In/NotIn operator already carries negation, so a "!"-prefixed value
	// would double-negate.
	t.Run("rejects a negated labels value", func(t *testing.T) {
		assert.Equal(t, []string{"where.filter.values[0]"}, leafErrs(t, perKindFilterLeaf(fieldLabels, perKindFilterOperatorIn, "!team")))
		assert.Equal(t, []string{"where.filter.values[0]"}, leafErrs(t, perKindFilterLeaf(fieldLabels, perKindFilterOperatorIn, "team!=a")))
	})
	...
	t.Run("rejects server-side expression datasource UIDs", func(t *testing.T) {
		assert.Empty(t, leafErrs(t, perKindFilterLeaf(fieldDatasourceUIDs, perKindFilterOperatorIn, "ds_1", "ds-2")))
		assert.Equal(t, []string{"where.filter.values[1]"},
			leafErrs(t, perKindFilterLeaf(fieldDatasourceUIDs, perKindFilterOperatorIn, "ds1", expr.DatasourceUID)))
	})

	t.Run("rejects invalid datasource UIDs", func(t *testing.T) {
		assert.Equal(t, []string{"where.filter.values[0]"},
			leafErrs(t, perKindFilterLeaf(fieldDatasourceUIDs, perKindFilterOperatorIn, "%")))
	})
}
...
func TestPerKindValidateQuery_fieldsArePerKind(t *testing.T) {
	...
	t.Run("recording-rule fields are unknown to alert rules", func(t *testing.T) {
		for _, name := range []string{fieldMetric, fieldTargetDatasourceUID} {
			q := whereQuery(&searchv0.WhereNode{
				Filter: &searchv0.FilterPredicate{Field: name, Operator: perKindFilterOperatorIn, Values: []string{"x"}},
			})
	...
	t.Run("alert-rule fields are unknown to recording rules", func(t *testing.T) {
		for _, name := range []string{fieldDashboardUID, fieldReceiver, fieldNotificationType, fieldRoutingTree} {
			...
			if name == fieldNotificationType {
				value = perKindNotificationTypeNames()[0]
			}
			q := whereQuery(&searchv0.WhereNode{
				Filter: &searchv0.FilterPredicate{Field: name, Operator: perKindFilterOperatorIn, Values: []string{value}},
			})
...
func TestPerKindValidateQuery_repeatedFilterFields(t *testing.T) {
	t.Run("rejects a repeated field", func(t *testing.T) {
		assert.Equal(t, []string{"where.and[1].filter.field"}, validate(t, whereQuery(perKindAndNode(
			perKindFilterLeaf(fieldFolder, perKindFilterOperatorIn, "f1"),
			perKindFilterLeaf(fieldFolder, perKindFilterOperatorIn, "f2"),
		))))
	})
	...
	t.Run("allows repeated labels", func(t *testing.T) {
		assert.Empty(t, validate(t, whereQuery(perKindAndNode(
			perKindFilterLeaf(fieldLabels, perKindFilterOperatorIn, "team=a"),
			perKindFilterLeaf(fieldLabels, perKindFilterOperatorNotIn, "env=prod"),
		))))
	})
...
func TestPerKindValidateQuery_sort(t *testing.T) {
...
func TestPerKindValidateQuery_returnFields(t *testing.T) {
	...
	t.Run("accepts the defaults", func(t *testing.T) {
		assert.Empty(t, validate(t, fieldsQuery(perKindDefaultReturnFields...)))
	})
...
func TestPerKindValidateQuery_facets(t *testing.T) {
	q := query()
...
func TestPerKindValidateQuery_limits(t *testing.T) {
...
func TestPerKindValidateQuery_continueToken(t *testing.T) {
...
func TestPerKindValidateQuery_labelSelector(t *testing.T) {
...
func TestPerKindValidateQuery_reportsEveryProblem(t *testing.T) {
...
func TestPerKindValidateQuery_returnsLeaves(t *testing.T) {
	t.Run("a single leaf", func(t *testing.T) {
		leaves, errs := validatePerKindQuery(whereQuery(&searchv0.WhereNode{
			Text: &searchv0.TextPredicate{Value: "cpu"},
		}), alertRuleKind(t))
	...
	t.Run("every child of an and", func(t *testing.T) {
		leaves, errs := validatePerKindQuery(whereQuery(perKindAndNode(
			perKindTextLeaf("cpu"),
			perKindFilterLeaf(fieldFolder, perKindFilterOperatorIn, "f1"),
			perKindFilterLeaf(fieldReceiver, perKindFilterOperatorIn, "slack"),
		)), alertRuleKind(t))
	...
	t.Run("nothing when there is no where", func(t *testing.T) {
		leaves, errs := validatePerKindQuery(query(), alertRuleKind(t))
		require.Empty(t, errs)
