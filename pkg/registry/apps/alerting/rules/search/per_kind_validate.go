...
var perKindSelectableLabelKeys = map[string]struct{}{
	model.GroupLabelKey: {},
}

// validatePerKindQuery checks q against the fields declared for k and the restrictions
// above. It returns the flattened where leaves so the caller does not walk the
// tree twice.
func validatePerKindQuery(q *searchv0.SearchQuery, k perKind) ([]searchv0.WhereNode, field.ErrorList) {
...
func validateWhere(where *searchv0.WhereNode, k perKind, p *field.Path) ([]searchv0.WhereNode, field.ErrorList) {
	if where == nil {
...
func validateLeaf(n *searchv0.WhereNode, key string, k perKind, p *field.Path) field.ErrorList {
	switch key {
	case "text":
...
func validateTextLeaf(t *searchv0.TextPredicate, k perKind, p *field.Path) field.ErrorList {
	var errs field.ErrorList
...
func validateFilterLeaf(f *searchv0.FilterPredicate, k perKind, p *field.Path) field.ErrorList {
	...
	case fieldNotificationType:
		...
		if _, ok := perKindValidNotificationTypes[f.Values[0]]; !ok {
			errs = append(errs, field.NotSupported(p.Child("values").Index(0), f.Values[0], perKindNotificationTypeNames()))
		}
	case fieldPanelID:
...
func validateSort(sorts []searchv0.SortField, k perKind, p *field.Path) field.ErrorList {
	var errs field.ErrorList
...
func validateFacets(facets []string, k perKind, p *field.Path) field.ErrorList {
...
func checkSelectableKey(key string, p *field.Path) field.ErrorList {
	if _, ok := perKindSelectableLabelKeys[key]; ok {
		return nil
	}
...
func checkCapability(k perKind, name string, c resource.SearchCapability, p *field.Path) field.ErrorList {
