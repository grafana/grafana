...
func NewSearchHandler(tracer trace.Tracer, dual dualwrite.Service, legacyDashboardSearcher resourcepb.ResourceIndexClient, indexClient resourcepb.ResourceIndexClient, features featuremgmt.FeatureToggles) *SearchHandler {
	searchClient := resource.NewSearchWrapperClient(dualwrite.NewSearchAdapter(dual), dashboardv0alpha1.DashboardResourceInfo.GroupResource(), indexClient, legacyDashboardSearcher, features)
