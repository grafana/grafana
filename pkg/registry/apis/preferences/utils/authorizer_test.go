...
func TestAuthorizer_OrgAdminManagesUserOwned(t *testing.T) {
	resource := map[string][]ResourceOwner{
		"stars": {UserResourceOwner},
	}
	claims := &authn.Claims[authn.AccessTokenClaims]{
		Rest: authn.AccessTokenClaims{
			DelegatedPermissions: []string{"group/stars:*"},
		},
	}
	...
	viewer := &identity.StaticRequester{
		UserUID:           "viewer",
		OrgRole:           identity.RoleViewer,
		AccessTokenClaims: claims,
	}

	editOther := authorizer.AttributesRecord{
		Verb:            "update",
		APIGroup:        "group",
		Resource:        "stars",
		Name:            "user-someoneelse",
		ResourceRequest: true,
	}

	t.Run("org admin can manage another user's stars when enabled", func(t *testing.T) {
		authz := &AuthorizeFromName{
			OrgAdminCanManageUserOwned: true,
			Resource:                   resource,
			AccessClient:               authlib.FixedAccessClient(false),
		}
		ctx := identity.WithRequester(context.Background(), orgAdmin)
		d, _, err := authz.Authorize(ctx, editOther)
		require.NoError(t, err)
		require.Equal(t, authorizer.DecisionAllow, d)
	})

	t.Run("non-admin cannot manage another user's stars", func(t *testing.T) {
		authz := &AuthorizeFromName{
			OrgAdminCanManageUserOwned: true,
			Resource:                   resource,
			AccessClient:               authlib.FixedAccessClient(false),
		}
		ctx := identity.WithRequester(context.Background(), viewer)
		d, reason, err := authz.Authorize(ctx, editOther)
		require.NoError(t, err)
		require.Equal(t, authorizer.DecisionDeny, d)
		require.Equal(t, "your are not the owner of the resource", reason)
	})

	t.Run("org admin denied when override disabled", func(t *testing.T) {
		authz := &AuthorizeFromName{
			Resource:     resource,
			AccessClient: authlib.FixedAccessClient(false),
		}
	...
	t.Run("owner still manages own stars", func(t *testing.T) {
		authz := &AuthorizeFromName{
			OrgAdminCanManageUserOwned: true,
			Resource:                   resource,
			AccessClient:               authlib.FixedAccessClient(false),
