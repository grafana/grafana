...
func TestConnectionTokenMetrics_NilSafe(t *testing.T) {
	...
	assert.NotPanics(t, func() {
		...
		m.recordNearExpiring()
		m.recordExpired()
	})
}

func TestConnectionTokenMetrics_RecordExpiryStates(t *testing.T) {
	...
	assert.Equal(t, 2.0, counterValue(t, reg, "grafana_provisioning_connection_tokens_near_expiring_total"))
...
func TestRepositoryTokenMetrics_RecordExpiryStates(t *testing.T) {
	reg := prometheus.NewPedanticRegistry()
	...
	assert.Equal(t, 1.0, counterValue(t, reg, "grafana_provisioning_repository_tokens_near_expiring_total"))
