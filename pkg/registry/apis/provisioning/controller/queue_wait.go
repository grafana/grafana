...
func (q *queueWaitTracker) startProcessing(key string) (time.Time, bool) {
	q.mu.Lock()
	defer q.mu.Unlock()
	t, ok := q.waiting[key]
	if !ok {
...
func (q *queueWaitTracker) finishProcessing(key string) {
	q.mu.Lock()
	defer q.mu.Unlock()
...
func (q *queueWaitTracker) lag(now time.Time) time.Duration {
	q.mu.Lock()
	defer q.mu.Unlock()
