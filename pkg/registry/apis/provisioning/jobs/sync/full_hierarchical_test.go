	...
	tests := []struct {
	...
	}{
		{
			...
			setupMocks: func(repo *repository.MockRepository, repoResources *resources.MockRepositoryResources, clients *resources.MockResourceClients, progress *jobs.MockJobProgressRecorder, _ *dynamicfake.FakeDynamicClient) {
				...
				repoResources.On("WriteResourceFromFile", mock.Anything, "folder1/file.json", mock.Anything).
		...
		},
		{
			...
			setupMocks: func(repo *repository.MockRepository, repoResources *resources.MockRepositoryResources, clients *resources.MockResourceClients, progress *jobs.MockJobProgressRecorder, _ *dynamicfake.FakeDynamicClient) {
				...
				folderErr := &resources.PathCreationError{Path: "folder1/", Err: fmt.Errorf("permission denied")}
				repoResources.On("WriteResourceFromFile", mock.Anything, "folder1/file1.json", mock.Anything).
					Return("", schema.GroupVersionKind{}, folderErr).Once()
		...
		},
		{
			...
			setupMocks: func(repo *repository.MockRepository, repoResources *resources.MockRepositoryResources, clients *resources.MockResourceClients, progress *jobs.MockJobProgressRecorder, _ *dynamicfake.FakeDynamicClient) {
				...
				repoResources.On("WriteResourceFromFile", mock.Anything, "level1/file1.json", mock.Anything).
					Return("", schema.GroupVersionKind{}, folderErr).Once()
		...
		},
		{
			...
			setupMocks: func(repo *repository.MockRepository, repoResources *resources.MockRepositoryResources, clients *resources.MockResourceClients, progress *jobs.MockJobProgressRecorder, _ *dynamicfake.FakeDynamicClient) {
				...
				repoResources.On("WriteResourceFromFile", mock.Anything, "success/file1.json", mock.Anything).
				...
				repoResources.On("WriteResourceFromFile", mock.Anything, "failure/file2.json", mock.Anything).
					Return("", schema.GroupVersionKind{}, folderErr).Once()
				progress.On("Record", mock.Anything, mock.MatchedBy(func(r jobs.JobResourceResult) bool {
		...
		},
		{
			...
			setupMocks: func(repo *repository.MockRepository, repoResources *resources.MockRepositoryResources, clients *resources.MockResourceClients, progress *jobs.MockJobProgressRecorder, _ *dynamicfake.FakeDynamicClient) {
				...
				folderErr := &resources.PathCreationError{Path: "folder1/", Err: fmt.Errorf("permission denied")}
				repoResources.On("EnsureFolderPathExist", mock.Anything, "folder1/", mock.Anything).Return("", folderErr).Once()
