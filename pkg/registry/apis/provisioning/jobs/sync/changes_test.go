...
func TestChanges(t *testing.T) {
	...
	t.Run("_folder.json records update when parent folder exists in Grafana", func(t *testing.T) {
		source := []repository.FileTreeEntry{
			{Path: "my-folder/", Hash: "abc", Blob: false},
		...
		target := &provisioning.ResourceList{
			Items: []provisioning.ResourceListItem{
				{Path: "my-folder/", Resource: resources.FolderResource.Resource, Group: resources.FolderResource.Group, Hash: "old-hash"},
		...
		// Should have one change: the folder update triggered by _folder.json
		require.Len(t, changes, 1)
	...
	t.Run("_folder.json does not record update when parent folder is new", func(t *testing.T) {
		source := []repository.FileTreeEntry{
			...
			{Path: "new-folder/_folder.json", Hash: "def", Blob: true},
			{Path: "new-folder/dashboard.json", Hash: "ghi", Blob: true},
		}
		target := &provisioning.ResourceList{}
		...
		// Only folder creation and dashboard creation — no _folder.json change
		paths := make([]string, len(changes))
		for i, c := range changes {
			paths[i] = c.Path
		}
		require.NotContains(t, paths, "new-folder/_folder.json")
		require.Contains(t, paths, "new-folder/")
		require.Contains(t, paths, "new-folder/dashboard.json")

		// Verify no update actions — all should be creates
		for _, c := range changes {
			require.Equal(t, repository.FileActionCreated, c.Action)
		}
	})
