...
type SecureValueStorage interface {
	Create(ctx context.Context, tx *db.Session, sv *secretv0alpha1.SecureValue, cb func(*secretv0alpha1.SecureValue, error))
	...
	SecretMetadataHasPendingStatus(ctx context.Context, tx *db.Session, namespace xkube.Namespace, name string, cb func(bool, error))
}
