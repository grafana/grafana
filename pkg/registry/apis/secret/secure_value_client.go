...
func (c *secureValueClient) checkAccess(ctx context.Context, name, verb string) error {
	...
	if req.Verb != utils.VerbList {
		req.Name = name
	}

	authInfo, ok := claims.AuthInfoFrom(ctx)
	if !ok {
		return apierrors.NewForbidden(gr, name, fmt.Errorf("missing auth info in context"))
	}

	decision, err := c.access.Check(ctx, authInfo, req)
	if err != nil {
		return apierrors.NewForbidden(gr, name, fmt.Errorf("failed to check access: %w", err))
	}

	if !decision.Allowed {
		return apierrors.NewForbidden(gr, name, fmt.Errorf("no access to %s", verb))
	}
