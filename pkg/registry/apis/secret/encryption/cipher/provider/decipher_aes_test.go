...
func Test_aesDecipher(t *testing.T) {
	...
	t.Run("aes-cfb", func(t *testing.T) {
		c := aesDecipher{algorithm: cipher.AesCfb}
		...
		decrypted, err := c.Decrypt(ctx, cfbEncryptedCiphertext, "1234")
		require.NoError(t, err)
		assert.Equal(t, []byte("grafana"), decrypted)
	})

	t.Run("aes-gcm", func(t *testing.T) {
		c := aesDecipher{algorithm: cipher.AesGcm}
		...
		decrypted, err := c.Decrypt(ctx, gcmEncryptedCiphertext, "1234")
		require.NoError(t, err)
		assert.Equal(t, []byte("grafana"), decrypted)
	})
}
