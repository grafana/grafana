...
func checkMoveAccess(
	...
	if !writeRes.Allowed {
		...
		if destLabel == folder.RootFolderUID || destLabel == folder.GeneralFolderUID {
			destLabel = folder.GeneralFolderUID
		}
		return dashboards.ErrMoveAccessDenied.Errorf("user does not have permissions to move a folder to folder with UID %s", destLabel)
	}
	...
	if resolveTier(newFolder) > resolveTier(oldFolder) {
		return dashboards.ErrFolderAccessEscalation.Errorf("user cannot move a folder to another folder where they have higher permissions")
	}
