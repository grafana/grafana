...
func admissionTestStore(client pluginv3.AdmissionServiceClient, mutation, validation []app.AdmissionOperation) *kindStore {
	return &kindStore{
...
func TestBuilderAdmissionDispatch(t *testing.T) {
	client := &reviewClient{rsp: allowed(nil)}
	store := admissionTestStore(client,
		[]app.AdmissionOperation{app.AdmissionOperationAny},
		[]app.AdmissionOperation{app.AdmissionOperationAny})
	b := &AppPluginAPIBuilder{kinds: map[schema.GroupVersionResource]*kindStore{
	...
	require.NoError(t, b.Mutate(ctx, attributes(testObject(), nil, admission.Create, ""), nil))
	require.NoError(t, b.Validate(ctx, attributes(testObject(), nil, admission.Create, ""), nil))
	require.Equal(t, 1, client.call)
	...
	require.NoError(t, b.Mutate(ctx, attributes(testObject(), nil, admission.Update, "status"), nil))
	require.NoError(t, b.Validate(ctx, attributes(testObject(), nil, admission.Update, "status"), nil))
	require.Equal(t, 1, client.call)

	// Resources of this group that are not manifest kinds (the settings resource)
	// have no hook to call.
	other := admission.NewAttributesRecord(testObject(), nil, testAdmissionGVK, "default", "thing-1",
...
func TestNewKindStoreAdmissionCapabilities(t *testing.T) {
	gvk := schema.GroupVersionKind{Group: "example-app", Version: "v1alpha1", Kind: "TestKind"}
	client := &reviewClient{}

	newStore := func(t *testing.T, capabilities *app.AdmissionCapabilities) *kindStore {
		t.Helper()
...
func TestNewKindStoreRequiresClientForAdmission(t *testing.T) {
	gvk := schema.GroupVersionKind{Group: "example-app", Version: "v1alpha1", Kind: "TestKind"}
	...
	opts, _ := newKindStoreOpts(t, gvk)
	_, err := newKindStore(gvk, kind, nil, opts, nil)
