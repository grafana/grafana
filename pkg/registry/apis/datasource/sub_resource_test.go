...
func TestSubResourceREST_Connect(t *testing.T) {
	...
	t.Run("forwards headers to plugin", func(t *testing.T) {
		...
		require.Equal(t, "Bearer token123", capturedHeaders.Get("Authorization"))
		require.Equal(t, "application/json", capturedHeaders.Get("Content-Type"))
		require.Equal(t, "application/json", capturedHeaders.Get("Accept"))
