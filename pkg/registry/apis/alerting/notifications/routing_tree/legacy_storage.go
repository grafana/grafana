...
type RouteService interface {
	...
	ResetPolicyTree(ctx context.Context, orgID int64) (definitions.Route, string, error)
}
...
func (s *legacyStorage) Create(ctx context.Context,
	...
	err = s.service.UpdatePolicyTree(ctx, info.OrgID, model, alerting_models.ProvenanceNone, version)
	if err != nil {
		return nil, err
	}

	res, version, err := s.service.GetPolicyTree(ctx, info.OrgID)
	if err != nil {
		return nil, err
	}

	return convertToK8sResource(info.OrgID, res, version, s.namespacer)
}
...
func (s *legacyStorage) DeleteCollection(ctx context.Context, deleteValidation rest.ValidateObjectFunc, _ *metav1.DeleteOptions, _ *internalversion.ListOptions) (runtime.Object, error) {
	...
	_, _, err = s.service.ResetPolicyTree(ctx, info.OrgID) // TODO add support for dry-run option
	return old, err
}
