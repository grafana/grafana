...
// If the caller has users.permissions:read, returns true without checking the specific resource.
func (r *ResourcePermissionsAuthorizer) CanViewTarget(ctx context.Context, authInfo types.AuthInfo, namespace, apiGroup, resource, name string) (bool, error) {
	...
	if ok, err := r.hasUsersPermissionsRead(ctx, authInfo, namespace); err != nil {
		return false, err
	} else if ok {
		return true, nil
	}
