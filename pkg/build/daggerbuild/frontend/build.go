...
func Build(builder *dagger.Container, version string, src *dagger.Directory, nodeModules *dagger.Directory) *dagger.Directory {
	public := WithNodeModules(WithFrontendSource(builder, src), nodeModules).
		WithExec([]string{"/bin/sh", "-c", fmt.Sprintf("pnpm exec lerna version %s --exact --no-git-tag-version --no-push --force-publish -y", version)}).
...
func BuildPlugins(builder *dagger.Container, src *dagger.Directory, nodeModules *dagger.Directory) *dagger.Directory {
	public := WithNodeModules(WithFrontendSource(builder, src), nodeModules).
		WithExec([]string{"/bin/sh", "-c", `if [ -d /src/plugins-bundled ]; then pnpm run plugins:build-bundled; else mkdir /src/plugins-bundled; fi`}).
...
// WithPnpmStore mounts the given cache volume as the pnpm store directory
func WithPnpmStore(container *dagger.Container, vol *dagger.CacheVolume) *dagger.Container {
	pnpmStoreDir := "/pnpm/store"
	c := container.WithEnvVariable("PNPM_STORE_DIR", pnpmStoreDir)
	return c.WithMountedCache(pnpmStoreDir, vol)
}
