...
func TestIntegrationProvisioning(t *testing.T) {
	...
	t.Run("should provide provenance status", func(t *testing.T) {
		require.NoError(t, db.SetProvenance(ctx, &definitions.Route{Name: v0alpha1.UserDefinedRoutingTreeName}, admin.Identity.GetOrgID(), "API"))
...
func TestIntegrationDataConsistency(t *testing.T) {
	...
	createRoute := func(t *testing.T, route definitions.Route) {
		t.Helper()
		route.Name = v0alpha1.UserDefinedRoutingTreeName
	...
	t.Run("all matchers are handled", func(t *testing.T) {
		...
		t.Run("should save into ObjectMatchers", func(t *testing.T) {
			...
			expectedRoutes := cfg.AlertmanagerConfig.Route.Routes
			userDefinedRoutes := expectedRoutes[1].Routes // autogenerated route is the first one
			userDefinedRoutes[0].Match = nil
			userDefinedRoutes[0].ObjectMatchers = definitions.ObjectMatchers{
				ensureMatcher(t, labels.MatchEqual, "oldmatch", "123"),
			}
			userDefinedRoutes[1].MatchRE = nil
			userDefinedRoutes[1].ObjectMatchers = definitions.ObjectMatchers{
				ensureMatcher(t, labels.MatchRegexp, "oldmatchre", ".*"),
			}
			userDefinedRoutes[2].Matchers = nil
			userDefinedRoutes[2].ObjectMatchers = definitions.ObjectMatchers{
				ensureMatcher(t, labels.MatchNotEqual, "matchers", "v"),
			}
	...
	t.Run("correctly save all fields", func(t *testing.T) {
		...
		// User-defined route's root.
		before.AlertmanagerConfig.Route.Routes[1].GroupByStr = []string{"test-123", "test-456", "test-789"}
		before.AlertmanagerConfig.Route.Routes[1].GroupBy = []model.LabelName{"test-123", "test-456", "test-789"}
