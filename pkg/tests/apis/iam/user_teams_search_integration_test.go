...
func TestIntegrationUserTeams(t *testing.T) {
	...
	for _, mode := range modes {
		t.Run(fmt.Sprintf("user teams endpoint with dual writer mode %d", mode), func(t *testing.T) {
			...
			// Create a user that will be bound to the team.
			...
			// Create a second user with no binding.
			...
			// Create a TeamBinding for u1 -> team.
			...
			createdTB, err := tbClient.Resource.Create(ctx, tbObj, metav1.CreateOptions{})
			require.NoError(t, err)
			require.NotNil(t, createdTB)
			...
			t.Run("returns the bound team for the user", func(t *testing.T) {
				...
				require.True(t, containsTeam(res, team.GetName()), "expected response to contain team %q, got %#v", team.GetName(), res.Items)

				item := findTeam(res, team.GetName())
			...
			t.Run("does not return the bound team for a different user", func(t *testing.T) {
				...
				require.Equal(t, http.StatusOK, rsp.Response.StatusCode)
				require.False(t, containsTeam(res, team.GetName()), "did not expect response to contain team %q, got %#v", team.GetName(), res.Items)
			})
		})
	}
}

func TestIntegrationUserTeams_Paging(t *testing.T) {
	testutil.SkipIntegrationTestInShortMode(t)
		...
		t.Run(fmt.Sprintf("DualWriterMode %d", mode), func(t *testing.T) {
			...
			for i := 1; i <= 5; i++ {
				teamObj := createTeamObject(helper,
					fmt.Sprintf("paging-team-%d", i),
					fmt.Sprintf("Paging Team %d", i),
					fmt.Sprintf("paging-team-%d@example.com", i),
				)
			...
			// Create TeamBindings for the user to all teams
			tbClient := helper.GetResourceClient(apis.ResourceClientArgs{
				User:      helper.Org1.Admin,
				Namespace: orgNS,
				GVR:       gvrTeamBindings,
			...
			for _, team := range teams {
				tbObj := createTeamBindingObject(helper, user.GetName(), team.GetName())
			...
			t.Run("paging with page and limit", func(t *testing.T) {
				// Page 1, Limit 2
				res1 := getUserTeamsWithPaging(t, helper, user.GetName(), 1, 2)
				require.Len(t, res1.Items, 2)

				// Page 2, Limit 2
				res2 := getUserTeamsWithPaging(t, helper, user.GetName(), 2, 2)
				require.Len(t, res2.Items, 2)

				// Page 3, Limit 2
				res3 := getUserTeamsWithPaging(t, helper, user.GetName(), 3, 2)
			...
			t.Run("paging with offset and limit", func(t *testing.T) {
				// Offset 0, Limit 2
				res1 := getUserTeamsWithOffset(t, helper, user.GetName(), 0, 2)
				require.Len(t, res1.Items, 2)

				// Offset 2, Limit 2
				res2 := getUserTeamsWithOffset(t, helper, user.GetName(), 2, 2)
				require.Len(t, res2.Items, 2)

				// Offset 4, Limit 2
				res3 := getUserTeamsWithOffset(t, helper, user.GetName(), 4, 2)
				require.Len(t, res3.Items, 1)
