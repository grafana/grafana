...
func runDashboardFieldSelectorTest(t *testing.T, helper *apis.K8sTestHelper, gvr schema.GroupVersionResource) {
	...
	// Update some dashboards to increment their generation
	// Always get the latest version before updating

	// Dashboard-1: Update spec to increase generation to 2
	...
	require.NoError(t, err)
	require.Equal(t, int64(2), updatedDashboard1.GetGeneration()) // Generation should now be 2

	// Dashboard-3: Update spec twice to increase generation to 3
	...
	updatedDashboard3, err := client.Resource.Update(ctx, dashboard3, metav1.UpdateOptions{})
	require.NoError(t, err)
	...
	require.NoError(t, err)
	require.Equal(t, int64(3), finalDashboard3.GetGeneration()) // Generation should now be 3

	// Define test cases for field selectors
	testCases := []struct {
		name          string
		...
		{
			name:          "select by metadata.generation equals 1",
			fieldSelector: "metadata.generation=1",
			expectedCount: 1,
			expectedNames: []string{"dashboard-2"},
		},
		{
			name:          "select by metadata.generation equals 2",
			fieldSelector: "metadata.generation=2",
			expectedCount: 1,
			expectedNames: []string{"dashboard-1"},
		},
		{
			name:          "select by metadata.generation equals 3",
			fieldSelector: "metadata.generation=3",
			expectedCount: 1,
			expectedNames: []string{"dashboard-3"},
		},
		{
			name:          "no matches for generation filter",
			fieldSelector: "metadata.generation=4",
			expectedCount: 0,
			expectedNames: []string{},
		},
	}
	...
	t.Run("field selectors on historical versions", func(t *testing.T) {
		...
		require.Equal(t, int64(3), updatedObj.GetGeneration()) // Generation should now be 3
		...
		require.Equal(t, int64(2), historyObj.GetGeneration()) // Historical generation should be 2
	})
