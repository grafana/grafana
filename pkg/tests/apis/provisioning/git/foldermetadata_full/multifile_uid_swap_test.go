...
// TestIntegrationProvisioning_FullSync_MultiFileUIDSwap verifies that when two
// files swap their dashboard UIDs in a single commit, full sync preserves both
// resources. The deleteOldResource sourcePath guard detects that the old UID is
// now managed by the other file and skips the delete, preventing accidental
// destruction of a resource that was just written by the other file in the same
// sync.
func TestIntegrationProvisioning_FullSync_MultiFileUIDSwap(t *testing.T) {
	...
	require.NoError(t, local.UpdateFile("dashboard_a.json", string(gitcommon.DashboardJSON("swap-full-uid-b", "Dashboard A Swapped", 2))))
	require.NoError(t, local.UpdateFile("dashboard_b.json", string(gitcommon.DashboardJSON("swap-full-uid-a", "Dashboard B Swapped", 2))))
	_, err := local.Git("add", ".")
	...
	common.RequireDashboards(t, helper.DashboardsV1, ctx, map[string]common.ExpectedDashboard{
		"swap-full-uid-a": {Title: "Dashboard B Swapped", SourcePath: "dashboard_b.json"},
		"swap-full-uid-b": {Title: "Dashboard A Swapped", SourcePath: "dashboard_a.json"},
	})
}
