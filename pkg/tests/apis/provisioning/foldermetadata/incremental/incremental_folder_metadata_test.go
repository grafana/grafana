...
func TestIntegrationProvisioning_IncrementalSync_FolderUIDConflict(t *testing.T) {
	...
	// discussion_r3750652263). Unlike the sibling-theft cases above, here the
	// folder stealing the UID is a CHILD of the relocated parent, so the parent's
	// UID is legitimately exempted for the child's ancestor walk. The exemption
	// must be bound to the parent's destination path: when the child leaf resolves
	// the parent UID at a different path, the guard must still fire. Before the fix
	// the exemption was UID-only and leaked to the child leaf, silently repointing
	// the parent's folder object to the child path.
	t.Run("child under a relocated parent stealing the parent UID is rejected", func(t *testing.T) {
		helper := sharedGitHelper(t)

		const repoName = "incr-uid-conflict-child"
		const parentUID = "parent-folder-uid"
		const childUID = "child-folder-uid"

		_, local := helper.CreateGitRepo(t, repoName, map[string][]byte{
			"src/_folder.json":         folderMetadataJSON(parentUID, "Parent Folder"),
			"src/stay.json":            common.DashboardJSON("dash-stay", "Staying Dashboard", 1),
			"src/child/_folder.json":   folderMetadataJSON(childUID, "Child Folder"),
			"src/child/childdash.json": common.DashboardJSON("child-dash", "Child Dashboard", 1),
		})

		common.SyncAndWait(t, helper, common.Repo(repoName), common.Succeeded())
		common.RequireFolderState(t, helper.Folders, parentUID, "Parent Folder", "src", "")
		common.RequireFolderState(t, helper.Folders, childUID, "Child Folder", "src/child", parentUID)

		// In one commit: relocate the parent subtree (src -> dst, parent keeps its
		// UID) AND rewrite the child's _folder.json to claim the parent's UID.
		_, err := local.Git("mv", "src", "dst")
		require.NoError(t, err)
		require.NoError(t, local.CreateFile("dst/child/_folder.json", string(folderMetadataJSON(parentUID, "Child Stealing Parent UID"))))
		_, err = local.Git("add", ".")
		require.NoError(t, err)
		_, err = local.Git("commit", "-m", "relocate parent and steal its UID from the child")
		require.NoError(t, err)
		_, err = local.Git("push")
		require.NoError(t, err)
		...
		require.Equal(t, provisioning.JobStateWarning, jobObj.Status.State,
			"a child reusing the relocating parent's UID must warn, even though the parent's own move is legitimate")
		...
		require.True(t, foundConflict,
			"expected a UID conflict warning for the child, got warnings: %v", jobObj.Status.Warnings)

		// The parent relocated to its own destination path and was NOT hijacked to
		// the child path. Under the pre-fix leaked-exemption bug the parent object
		// would be repointed to dst/child, so this assertion fails on the bug.
		common.RequireFolderState(t, helper.Folders, parentUID, "Parent Folder", "dst", "")
	})
}
