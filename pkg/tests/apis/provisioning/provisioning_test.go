...
func TestIntegrationProvisioning_MoveResources(t *testing.T) {
	...
	t.Run("move file without content change", func(t *testing.T) {
		...
		require.Eventually(t, func() bool {
			...
			return err == nil
		}, 45*time.Second, 100*time.Millisecond, "dashboard should still exist in Grafana after move") // Using Eventually to account for potential delays in dashboards APIs.
	})
