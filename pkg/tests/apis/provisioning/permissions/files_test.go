				...
				for _, grant := range []string{"none", "unrelated", "required"} {
					...
					t.Run(version+"/"+name, func(t *testing.T) {
						...
						if method == "POST" && existing && grant == "required" {
							rsp.Require(t, 409)
							data, err := os.ReadFile(filepath.Join(h.ProvisioningPath, path)) //nolint:gosec // Path is generated by this test inside its provisioning directory.
	...
	for _, version := range pt.Versions {
		for _, grant := range []string{"none", "unrelated", "required"} {
			t.Run(version+"/"+grant, func(t *testing.T) {
				...
				rsp.Require(t, 200)
				data, err := os.ReadFile(filepath.Join(h.ProvisioningPath, path, "_folder.json")) //nolint:gosec // Path is generated by this test inside its provisioning directory.
				require.NoError(t, err)
		...
		for _, newUID := range []bool{false, true} {
			for _, grant := range []string{"none", "delete", "destination", "both"} {
				...
				t.Run(version+"/"+name, func(t *testing.T) {
					...
					_, err := os.Stat(filepath.Join(h.ProvisioningPath, source))
					require.True(t, os.IsNotExist(err))
					data, err := os.ReadFile(filepath.Join(h.ProvisioningPath, dest)) //nolint:gosec // Destination is generated by this test inside its provisioning directory.
					require.NoError(t, err)
