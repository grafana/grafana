...
func TestIntegrationProvisioning_JobWarningResult_ManagedByOtherFile(t *testing.T) {
	...
	helper.WriteToProvisioningPath(t, "dir-a/dashboard.json", common.DashboardJSON("shared-uid", "Dashboard A", 1))
	...
	helper.WriteToProvisioningPath(t, "dir-b/dashboard.json", common.DashboardJSON("shared-uid", "Dashboard B", 1))
	helper.WriteToProvisioningPath(t, "dir-a/dashboard.json", common.DashboardJSON("new-uid", "Dashboard A v2", 2))
	...
	// A's replace and B's create are applied in the same parallel sync phase, so
	// which one wins the race for "shared-uid" is not deterministic:
	//   - if B claims the UID before A's replace-path delete runs, A skips the
	//     now-foreign delete and the job completes with a warning;
	//   - if A's delete runs first, it cleans up the old UID and the job
	//     completes successfully with no warning.
	// Both outcomes are correct. The invariant is that this ownership conflict
	// never fails the job; and whenever a warning is produced, it must be exactly
	// the ownership-conflict warning (not some other unexpected warning).
	require.Empty(t, job2Obj.Status.Errors, "the ownership conflict must not surface as an error")
	require.NotEqual(t, provisioning.JobStateError, job2Obj.Status.State,
		"the ownership conflict must not fail the job")

	if job2Obj.Status.State == provisioning.JobStateWarning {
		found := false
		...
		require.True(t, found,
			"a warning-state sync must be caused by the ownership conflict; got warnings=%v", job2Obj.Status.Warnings)
	}
}
