...
func TestCrossNamespaceIsolation_FolderSync(t *testing.T) {
	...
	// Create org-specific contexts using the common helper
	org1Ctx := helper.OrgContextFor(helper.Org1)
	orgBCtx := helper.OrgContextFor(helper.OrgB)
	...
	t.Run("create repositories in different namespaces", func(t *testing.T) {
		...
		helper.CreateRepoWithOrg(t, common.TestRepo{
			Name:   org1RepoName,
		...
		t.Logf("✓ Created repository '%s' in Org1 (namespace: %s)", org1RepoName, org1Ctx.Namespace)
		...
		t.Logf("✓ Created repository '%s' in OrgB (namespace: %s)", orgBRepoName, orgBCtx.Namespace)
	})
	...
	t.Run("sync repositories and verify success", func(t *testing.T) {
		...
		helper.SyncAndWaitWithOrg(t, org1RepoName, nil, org1Ctx)
		t.Logf("✓ Org1 repository synced successfully")

		// Verify Org1 has folders
		org1Folders := helper.GetFoldersWithOrg(t, org1Ctx)
		...
		helper.SyncAndWaitWithOrg(t, orgBRepoName, nil, orgBCtx)
		t.Logf("✓ OrgB repository synced successfully")

		// Verify OrgB has folders
		orgBFolders := helper.GetFoldersWithOrg(t, orgBCtx)
	...
	t.Run("verify cross-namespace isolation", func(t *testing.T) {
		...
		assert.Equal(t, org1Ctx.Namespace, org1Folder.GetNamespace(), "Org1 folder should be in Org1 namespace")
		...
		assert.Equal(t, orgBCtx.Namespace, orgBFolder.GetNamespace(), "OrgB folder should be in OrgB namespace")
	...
	t.Run("verify no cross-namespace visibility", func(t *testing.T) {
		...
		org1ViewOfOrgBRepos := helper.GetResourceClient(apis.ResourceClientArgs{
			...
			Namespace: orgBCtx.Namespace, // Try to access OrgB namespace
		...
		orgBViewOfOrg1Repos := helper.GetResourceClient(apis.ResourceClientArgs{
			...
			Namespace: org1Ctx.Namespace, // Try to access Org1 namespace
			GVR:       schema.GroupVersionResource{Group: "provisioning.grafana.app", Resource: "repositories", Version: "v0alpha1"},
		})
	...
	t.Run("verify dashboard isolation", func(t *testing.T) {
		org1Dashboards := helper.GetDashboardsWithOrg(t, org1Ctx)
		orgBDashboards := helper.GetDashboardsWithOrg(t, orgBCtx)
		...
		for i := range org1Dashboards.Items {
			...
			assert.Equal(t, org1Ctx.Namespace, dash.GetNamespace(),
		...
		for i := range orgBDashboards.Items {
			...
			assert.Equal(t, orgBCtx.Namespace, dash.GetNamespace(),
				fmt.Sprintf("OrgB dashboard %s should be in OrgB namespace", dash.GetName()))
		}

		t.Logf("✓ Org1 has %d dashboard(s) in namespace '%s'", len(org1Dashboards.Items), org1Ctx.Namespace)
		t.Logf("✓ OrgB has %d dashboard(s) in namespace '%s'", len(orgBDashboards.Items), orgBCtx.Namespace)
	})
