...
func (s *legacyPlaylistService) Create(ctx context.Context, cmd *playlist.CreatePlaylistCommand) (*playlist.Playlist, error) {
	...
	driverName := s.db.GetSqlxSession().DriverName()
	orderColumn := quotedOrderColumn(driverName)
	...
	err := s.db.GetSqlxSession().WithTransaction(ctx, func(tx *session.SessionTx) error {
		...
		id, err := tx.ExecWithReturningId(ctx,
			"INSERT INTO playlist (name, interval, org_id, uid, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?)",
			p.Name, p.Interval, p.OrgId, p.UID, p.CreatedAt, p.UpdatedAt,
		)
		if err != nil {
			return err
		}
		...
		for _, item := range playlistItems {
			if _, err := tx.Exec(
				ctx,
				"INSERT INTO playlist_item (playlist_id, type, value, title, "+orderColumn+") VALUES (?, ?, ?, ?, ?)",
				item.PlaylistId,
				item.Type,
				item.Value,
				item.Title,
...
func quotedOrderColumn(driverName string) string {
