...
import (
	...
	"github.com/grafana/grafana/pkg/tests/storage/unified/migrations/testcases"
...
func runMigrationTestSuite(t *testing.T, testCases []testcases.ResourceMigratorTestCase) {
	...
	t.Run("Step 1: Create data in legacy", func(t *testing.T) {
		...
		env := helper.GetEnv()
		mg := storagemigrator.NewScopedMigrator(env.SQLStore.GetSqlxSession(), "unified_migrator_tests")
...
func verifyRegisteredMigrations(t *testing.T, helper *apis.K8sTestHelper, onlyDefault bool, optOut bool) {
	...
	rows, err := helper.GetEnv().SQLStore.GetSqlxSession().Query(t.Context(), getMigrationsQuery)
...
func verifyKeyPathPopulated(t *testing.T, helper *apis.K8sTestHelper) {
	...
	rows, err := helper.GetEnv().SQLStore.GetSqlxSession().Query(t.Context(), query)
	require.NoError(t, err)
	defer func() {
		require.NoError(t, rows.Close())
	}()
	...
	rowsTotal, err := helper.GetEnv().SQLStore.GetSqlxSession().Query(t.Context(), queryTotal)
	require.NoError(t, err)
	defer func() {
...
func verifyTablesRenamed(t *testing.T, helper *apis.K8sTestHelper, testCases []testcases.ResourceMigratorTestCase) {
	t.Helper()
	session := helper.GetEnv().SQLStore.GetSqlxSession()
	dialect := storagemigrator.NewDialect(session.DriverName())

	for _, tc := range testCases {
		for _, table := range tc.RenameTables() {
			legacyName := table + "_legacy"

			exists, err := storageq.TableExists(t.Context(), session.SqlDB(), dialect, table)
			require.NoError(t, err)
			require.False(t, exists, "original table %q should no longer exist after migration", table)

			exists, err = storageq.TableExists(t.Context(), session.SqlDB(), dialect, legacyName)
			require.NoError(t, err)
...
func cleanupLegacyTables(t *testing.T, testCases []testcases.ResourceMigratorTestCase) {
	...
	testDB, err := testsqlutil.GetTestDB(testsqlutil.GetTestDBType())
	require.NoError(t, err)

	db, err := sql.Open(testDB.DriverName, testDB.ConnStr)
	require.NoError(t, err)
	defer func() { _ = db.Close() }()
	dialect := storagemigrator.NewDialect(testDB.DriverName)

	// Restore renamed tables
	for _, tc := range testCases {
		for _, table := range tc.RenameTables() {
			legacyName := table + "_legacy"
			exists, err := storageq.TableExists(t.Context(), db, dialect, legacyName)
			require.NoError(t, err)
			if exists {
				origExists, err := storageq.TableExists(t.Context(), db, dialect, table)
				require.NoError(t, err)
				if origExists {
					t.Logf("Both %q and %q exist, dropping legacy table", table, legacyName)
					_, err = db.ExecContext(t.Context(), "DROP TABLE "+dialect.Quote(legacyName))
					require.NoError(t, err)
				} else {
					t.Logf("Restoring %q back to %q", legacyName, table)
					_, err = db.ExecContext(t.Context(), dialect.RenameTable(legacyName, table))
					require.NoError(t, err)
				}
			}
		}
