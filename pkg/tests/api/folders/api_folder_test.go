...
func TestIntegrationFolderGracefulDegradation(t *testing.T) {
	testutil.SkipIntegrationTestInShortMode(t)
	...
	t.Run("list folders should succeed when search is down", func(t *testing.T) {
		...
		createResp, err := adminClient.Folders.CreateFolder(&models.CreateFolderCommand{
			Title: "Search Down Folder",
			UID:   "search-down-folder",
		})
		require.NoError(t, err)
		require.Equal(t, http.StatusOK, createResp.Code())

		// List folders — should succeed even with search failing
		listResp, err := adminClient.Folders.GetFolders(nil)
		require.NoError(t, err)
		require.Equal(t, http.StatusOK, listResp.Code())
		require.GreaterOrEqual(t, len(listResp.Payload), 1, "should return at least one folder")

		// Verify our folder is in the list
		found := false
		for _, f := range listResp.Payload {
			if f.UID == "search-down-folder" {
				found = true
				break
			}
		}
	...
	t.Run("delete folder with child dashboard should succeed when search is down", func(t *testing.T) {
		// Create a folder
		createResp, err := adminClient.Folders.CreateFolder(&models.CreateFolderCommand{
			Title: "Folder With Dashboard",
			UID:   "folder-with-dashboard",
		})
		require.NoError(t, err)
		require.Equal(t, http.StatusOK, createResp.Code())

		// Create a dashboard inside the folder via K8s API
		createBody := fmt.Sprintf(`{
			"metadata": {
				"generateName": "integ-test-",
				"annotations": {
					"grafana.app/folder": "%s",
					"grafana.app/grant-permissions": "default"
				}
			},
			"spec": {"title": "test-dashboard-in-folder", "schemaVersion": 42}
		}`, createResp.Payload.UID)
		...
		require.Equal(t, http.StatusCreated, resp.StatusCode, "dashboard creation should succeed")

		// Delete the folder — should succeed even with search down
		_, err = adminClient.Folders.DeleteFolder(folders.NewDeleteFolderParams().WithFolderUID(createResp.Payload.UID))
		require.NoError(t, err)

		// Verify folder is gone
		_, err = adminClient.Folders.GetFolderByUID(createResp.Payload.UID)
		require.Error(t, err, "folder should no longer exist after deletion")
	})
}
...
func TestIntegrationFolderMoveGracefulDegradation(t *testing.T) {
	...
	// Create a parent folder at root
	parentResp, err := adminClient.Folders.CreateFolder(&models.CreateFolderCommand{
		Title: "Parent For Move",
		UID:   "parent-move-degradation",
	})
	require.NoError(t, err)
	require.Equal(t, http.StatusOK, parentResp.Code())

	// Create a child folder at root (not nested yet)
	childResp, err := adminClient.Folders.CreateFolder(&models.CreateFolderCommand{
		Title: "Child For Move",
		UID:   "child-move-degradation",
	})
	require.NoError(t, err)
	require.Equal(t, http.StatusOK, childResp.Code())

	// Move the child folder under the parent — should succeed even with search down
	moveResp, err := adminClient.Folders.MoveFolder(childResp.Payload.UID, &models.MoveFolderCommand{
		ParentUID: parentResp.Payload.UID,
	})
	require.NoError(t, err)
	assert.Equal(t, http.StatusOK, moveResp.Code())
	assert.Equal(t, parentResp.Payload.UID, moveResp.Payload.ParentUID)
}
