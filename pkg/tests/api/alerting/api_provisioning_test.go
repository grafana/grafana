...
func TestIntegrationExportFileProvision(t *testing.T) {
	...
	t.Run("when provisioning alert rules from files", func(t *testing.T) {
		...
		fileProvisionedAlertRules, err := testData.ReadFile(path.Join("test-data", "rules-provisioning-escaped.yaml"))
		require.NoError(t, err)
		...
		err = os.WriteFile(filepath.Join(alertingDir, "rules-provisioning-escaped.yaml"), fileProvisionedAlertRules, 0750)
		require.NoError(t, err)
