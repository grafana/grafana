...
func TestLoadingSettings(t *testing.T) {
	Convey("Testing loading settings from ini file", t, func() {
		...
		Convey("Should be able to override defaults via command line", func() {
			...
			err := cfg.Load(&CommandLineArgs{
				...
				Config: filepath.Join(HomePath, "pkg/internal/setting/testdata/override.ini"),
			})
			So(err, ShouldBeNil)
		...
		Convey("Defaults can be overridden in specified config file", func() {
			if runtime.GOOS == windows {
				...
				err := cfg.Load(&CommandLineArgs{
					HomePath: "../../../",
					Config:   filepath.Join(HomePath, "pkg/internal/setting/testdata/override_windows.ini"),
	...
	Convey("Test reading string values from .ini file", t, func() {
		iniFile, err := ini.Load(path.Join(HomePath, "pkg/internal/setting/testdata/invalid.ini"))
		So(err, ShouldBeNil)
