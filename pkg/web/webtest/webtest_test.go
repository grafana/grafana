...
func TestServer(t *testing.T) {
	...
	routeRegister.Post("/api", routing.Wrap(func(c *model.ReqContext) response.Response {
		actualRequest = c.Req
...
func verifyRequest(t *testing.T, s *Server, req *http.Request, expectedBody string) {
	...
	req = RequestWithWebContext(req, &model.ReqContext{
		IsSignedIn: true,
	})
	require.NotNil(t, req)
...
func TestServerClient(t *testing.T) {
	routeRegister := routing.NewRouteRegister()
	routeRegister.Get("/test", routing.Wrap(func(c *model.ReqContext) response.Response {
		return response.JSON(http.StatusOK, c.SignedInUser)
	}))
