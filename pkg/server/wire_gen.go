...
func Initialize(ctx context.Context, cfg *setting.Cfg, opts Options, apiOpts api.ServerOptions) (*Server, error) {
	...
	pluginassetsService := pluginassets.ProvideService(pluginManagementCfg, pluginscdnService, signatureSignature, inMemory)
	service16 := pluginassets2.NewService(pluginassetsService)
	...
	appInstaller, err := plugins.ProvideAppInstaller(configProvider, eventualRestConfigProvider, pluginstoreService, service16, acimplService, accessClient, featureToggles)
	if err != nil {
		return nil, err
	}
