...
func Initialize(ctx context.Context, cfg *setting.Cfg, opts Options, apiOpts api.ServerOptions) (*Server, error) {
	...
	resourceMigrationService := legacy.ProvideResourceMigrationService(legacyDatabaseProvider, stubProvisioningService, accessControl)
	migrationRegistry := provideMigrationRegistry(resourceMigrationService)
	unifiedMigrator := migrations2.ProvideUnifiedMigrator(resourceMigrationService, resourceClient, migrationRegistry)
	...
	provisioningAPIBuilder, err := provisioning2.RegisterAPIService(cfg, featureToggles, apiserverService, registerer, resourceClient, eventualRestConfigProvider, accessClient, resourceMigrationService, dualwriteService, usageStats, tracingService, v3, v4, repositoryFactory, connectionFactory)
	if err != nil {
		return nil, err
	}
...
func provideMigrationRegistry(
	...
	r.Register(dashboard.FoldersDashboardsMigration(svc))
	r.Register(playlist.PlaylistMigration(svc))
	return r
}
