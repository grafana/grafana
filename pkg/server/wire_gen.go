...
func Initialize(ctx context.Context, cfg *setting.Cfg, opts Options, apiOpts api.ServerOptions) (*Server, error) {
	...
	randRand := rand.ProvideRand()
	databaseDatabase := database2.ProvideDatabase(sqlStore, tracer)
	secureValueMetadataStorage, err := metadata.ProvideSecureValueMetadataStorage(clockClock, randRand, databaseDatabase, tracer, registerer)
	if err != nil {
		return nil, err
	}
