...
func Initialize(ctx context.Context, cfg *setting.Cfg, opts Options, apiOpts api.ServerOptions) (*Server, error) {
	...
	cleanupElector, err := service7.ProvideDashboardCleanupElector(cfg, kv, registerer)
	if err != nil {
		return nil, err
	}
	dashboardServiceImpl, err := service7.ProvideDashboardServiceImpl(cfg, sqlStore, featureToggles, folderPermissionsService, accessControl, acimplService, folderimplService, registerer, quotaService, orgService, v3, dualwriteService, serverLockService, kvStore, k8sHandlerWithFallback, cleanupElector)
	if err != nil {
		return nil, err
	}
