...
func Initialize(ctx context.Context, cfg *setting.Cfg, opts Options, apiOpts api.ServerOptions) (*Server, error) {
	...
	migrationRegistry := migrations2.ProvideMigrationRegistry(migrationDashboardAccessor)
...
func InitializeModuleServer(cfg *setting.Cfg, opts Options, apiOpts api.ServerOptions) (*ModuleServer, error) {
	...
	restConfigProvider := ProvideRestConfigProviderForModuleServer()
	moduleServer, err := NewModule(opts, apiOpts, featureToggles, cfg, storageMetrics, bleveIndexMetrics, registerer, gatherer, tracingService, ossLicensingService, moduleRegisterer, storageBackend, hooksService, restConfigProvider)
	if err != nil {
		return nil, err
	}
