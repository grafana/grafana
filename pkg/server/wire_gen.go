...
func Initialize(cfg *setting.Cfg, opts Options, apiOpts api.ServerOptions) (*Server, error) {
	...
	apiBuilder, err := provisioning2.RegisterAPIService(cfg, featureToggles, apiserverService, registerer, resourceClient, eventualRestConfigProvider, factory, accessClient, legacyMigrator, dualwriteService, usageStats, secretsService, secureValueService, decryptService, tracingService, v2)
	if err != nil {
		return nil, err
	}
...
func InitializeForTest(t sqlutil.ITestDB, testingT interface {
	...
	testEnv, err := ProvideTestEnv(testingT, server, sqlStore, cfg, notificationServiceMock, grpcserverProvider, inMemory, httpclientProvider, oauthtokentestService, featureToggles, resourceClient, idimplService, factory, secretsService, secureValueService, decryptService)
	if err != nil {
		return nil, err
	}
	return testEnv, nil
