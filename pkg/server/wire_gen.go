...
import (
	...
	datasources2 "github.com/grafana/grafana/pkg/extensions/analytics/datasources"
...
func Initialize(cla setting.CommandLineArgs, opts Options, apiOpts api.ServerOptions) (*Server, error) {
	...
	managerManager := manager.ProvideService(cfg, licenseTokenService, requestInterceptor)
	...
	alertEngine := alerting.ProvideAlertEngine(renderingService, inProcBus, requestInterceptor, tsdbService, cfg)
	usageStatsService := usagestats.ProvideService(cfg, inProcBus, sqlStore, alertEngine, licenseTokenService, pluginManager)
	...
	enterpriseAccessControl, err := manager3.ProvideService(cfg, routeRegisterImpl, accessControlStore, licenseTokenService, usageStatsService)
	if err != nil {
		return nil, err
	}
	...
	enterpriseProvisioningServiceImpl, err := service.ProvideService(cfg, enterpriseAccessControl, licenseTokenService, sqlStore, pluginManager, routeRegisterImpl, accessControlProvisionerImpl, provisioningServiceImpl)
	if err != nil {
		return nil, err
	}
	...
	httpServer := api.ProvideHTTPServer(apiOpts, cfg, routeRegisterImpl, inProcBus, renderingService, licenseTokenService, hooksService, cacheService, sqlStore, tsdbService, alertEngine, usageStatsService, requestInterceptor, pluginManager, managerManager, implementation, cacheServiceImpl, userAuthTokenService, cleanUpService, shortURLService, remoteCache, enterpriseProvisioningServiceImpl, loginserviceImplementation, enterpriseAccessControl, dataSourceProxyService, searchService, grafanaLive, gateway, plugincontextProvider, contextHandler, plugindashboardsService, schemaLoaderService, alertNG, libraryPanelService, libraryElementService, notificationService, tracingService, internalMetricsService, quotaService)
	container := backgroundsvcs.ProvideService(httpServer, alertNG, cleanUpService, grafanaLive, gateway, notificationService, renderingService, userAuthTokenService, provisioningServiceImpl, alertEngine, pluginManager, plugindashboardsService, managerManager, internalMetricsService, usageStatsService, tracingService, remoteCache)
	dataSources, err := datasources2.ProvideService(inProcBus, sqlStore, licenseTokenService, cfg, routeRegisterImpl)
	if err != nil {
		return nil, err
	}
	summariesSummaries, err := summaries.ProvideService(inProcBus, sqlStore, licenseTokenService, serverLockService, cfg, routeRegisterImpl)
	if err != nil {
		return nil, err
	}
	viewsViews, err := views.ProvideService(inProcBus, sqlStore, licenseTokenService, serverLockService, cfg, routeRegisterImpl)
	if err != nil {
		return nil, err
	}
	meta, err := analytics.ProvideService(routeRegisterImpl, cfg, licenseTokenService, dataSources, summariesSummaries, viewsViews)
	if err != nil {
		return nil, err
	}
	backgroundsvcsContainer := backgroundsvcs2.ProvideService(container, meta, enterpriseProvisioningServiceImpl)
	server, err := New(opts, cfg, sqlStore, httpServer, enterpriseProvisioningServiceImpl, backgroundsvcsContainer)
	if err != nil {
		return nil, err
	}
	return server, nil
