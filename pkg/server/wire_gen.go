...
func Initialize(ctx context.Context, cfg *setting.Cfg, opts Options, apiOpts api.ServerOptions) (*Server, error) {
	...
	inProcRoundTripper, err := rendering.NewInProcRoundTripper(ossImpl)
	if err != nil {
		return nil, err
	}
	...
	renderingService, err := rendering.ProvideService(cfg, ossImpl, inProcRoundTripper, featureToggles, remoteCache)
	if err != nil {
		return nil, err
	}
	...
	corepluginRegistry := coreplugin.ProvideCoreRegistry(tracer, azuremonitorService, cloudwatchService, graphiteService, influxdbService, lokiService, prometheusService, testdatasourceService, postgresService, mysqlService, grafanadsService)
