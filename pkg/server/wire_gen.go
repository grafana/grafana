...
func Initialize(cfg *setting.Cfg, opts Options, apiOpts api.ServerOptions) (*Server, error) {
	...
	secureValueService := service5.ProvideSecureValueService(tracer, accessClient, databaseDatabase, secureValueMetadataStorage, keeperMetadataStorage, ossKeeperService, registerer)
	inlineSecureValueSupport, err := metadata.ProvideInlineSecureValueSupport(secureValueService, accessClient)
	if err != nil {
		return nil, err
	}
