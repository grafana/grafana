...
func Initialize(ctx context.Context, cfg *setting.Cfg, opts server.Options, apiOpts api.ServerOptions) (*server.Server, error) {
	...
	appPluginAPIBuilder, err := appplugin.RegisterAPIService(apiserverService, middlewareHandler, inMemory, plugincontextProvider, pluginsourcesService, service12, accessControl, decryptService, tracingService, featureToggles, cfg)
	if err != nil {
		return nil, err
	}
