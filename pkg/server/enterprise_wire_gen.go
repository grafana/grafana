...
import (
	...
	encryption3 "github.com/grafana/grafana/pkg/storage/secret/encryption"
...
func Initialize(cfg *setting.Cfg, opts Options, apiOpts api.ServerOptions) (*Server, error) {
	...
	service16 := sources2.ProvideService(sourcesService, pluginManagementCfg, pluginscdnService)
	...
	service17 := service6.ProvideService(sqlStore, secretsService)
	serviceregistrationService := serviceregistration.ProvideService(cfg, featureToggles, registryRegistry, service17)
	...
	pluginstoreService, err := pluginstore.ProvideService(inMemory, service16, loaderLoader)
	if err != nil {
		return nil, err
	}
	...
	service18 := service8.ProvideService(fileStoreManager, pluginService)
	...
	service19, err := service9.ProvideService(sqlStore, secretsService, secretsKVStore, cfg, featureToggles, accessControl, datasourcePermissionsService, quotaService, pluginstoreService, middlewareHandler, baseProvider)
	if err != nil {
		return nil, err
	}
	...
	plugincontextProvider := plugincontext.ProvideService(cfg, cacheService, pluginstoreService, cacheServiceImpl, service19, service17, requestConfigProvider)
	...
	dataSourceSecretMigrationService := migrations3.ProvideDataSourceMigrationService(service19, kvStore, featureToggles)
	...
	navtreeService := navtreeimpl.ProvideService(cfg, accessControl, pluginstoreService, service17, starService, featureToggles, dashboardService, acimplService, kvStore, apikeyService, licenseTokenService, authnService, openFeatureService)
	...
	scopedPluginDatasourceProvider := datasource.ProvideDefaultPluginConfigs(service19, cacheServiceImpl, plugincontextProvider)
	...
	supportbundlesimplService, err := supportbundlesimpl.ProvideService(accessControl, acimplService, bundleregistryService, cfg, featureToggles, httpServer, kvStore, service17, pluginstoreService, routeRegisterImpl, implementation, sqlStore, usageStats, tracingService)
	if err != nil {
		return nil, err
	}
	...
	checkregistryService := checkregistry.ProvideService(service19, pluginstoreService, plugincontextProvider, middlewareHandler, plugincheckerService, repoManager, preinstallImpl, managedPluginService, managedinstanceClient, ssosettingsimplService, cfg, pluginerrsStore)
	...
	importDashboardService := service12.ProvideService(routeRegisterImpl, quotaService, service18, pluginstoreService, libraryPanelService, dashboardService, accessControl, folderimplService, featureToggles)
	dashboardUpdater := service8.ProvideDashboardUpdater(inProcBus, pluginstoreService, service18, importDashboardService, service17, pluginService, dashboardService)
	...
	dashboardsAPIBuilder := dashboard.RegisterAPIService(cfg, featureToggles, apiserverService, dashboardService, dashboardProvisioningService, service19, dashboardServiceImpl, accessControl, accessClient, enterpriseProvisioningServiceImpl, dashboardsStore, registerer, sqlStore, tracingService, resourceClient, dualwriteService, sortService, quotaService, dashboardFolderStoreImpl, libraryPanelService, eventualRestConfigProvider, userService)
	...
	queryAPIBuilder, err := query2.RegisterAPIService(featureToggles, apiserverService, service19, pluginstoreService, accessControl, middlewareHandler, plugincontextProvider, registerer, tracingService, legacyDataSourceLookup)
	if err != nil {
		return nil, err
	}
	...
	teamLBAC := teamlbac.ProvideTeamLBAC(cfg, routeRegisterImpl, service19, accessControl, teamService, usageStats, featureManager)
	...
	alertEnrichmentAPIBuilder := alertenrichment2.RegisterAPIService(featureToggles, apiserverService, licenseTokenService, accessControl)
	extapiregistryService := extapiregistry.ProvideRegistryServiceSink(reportingAPIBuilder, bannersAPIBuilder, sandboxSettingsAPIBuilder, scimAPIBuilder, usageInsightsAPIBuilder, queryLibraryAPIBuilder, scopeAPIBuilder, alertEnrichmentAPIBuilder)
...
func InitializeForTest(t sqlutil.ITestDB, testingT interface {
	...
	navtreeService := navtreeimpl.ProvideService(cfg, accessControl, pluginstoreService, service17, starService, featureToggles, dashboardService, acimplService, kvStore, apikeyService, validLicense, authnService, openFeatureService)
	searchHTTPService := searchV2.ProvideSearchHTTPService(searchService)
	statsService := statsimpl.ProvideService(cfg, sqlStore, dashboardService, folderimplService, orgService, featureToggles)
...
func InitializeModuleServer(cfg *setting.Cfg, opts Options, apiOpts api.ServerOptions) (*ModuleServer, error) {
	...
	moduleServer, err := NewModule(opts, apiOpts, featureToggles, cfg, storageMetrics, bleveIndexMetrics, registerer, gatherer, licenseTokenService)
	if err != nil {
		return nil, err
	}
