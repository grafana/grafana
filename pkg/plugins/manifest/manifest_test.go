...
-----END PGP SIGNATURE-----`
	...
	t.Run("valid manifest", func(t *testing.T) {
		...
		assert.Equal(t, PrivateSignatureType, manifest.SignatureType)
...
func Test_urlMatch_privateGlob(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got, err := UrlMatch(tt.args.specs, tt.args.target, PrivateGlobSignatureType)
			require.NoError(t, err)
...
func Test_urlMatch_private(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got, err := UrlMatch(tt.args.specs, tt.args.target, PrivateSignatureType)
			require.NoError(t, err)
			require.Equal(t, tt.shouldMatch, got)
		})
	}
...
func createV2Manifest(t *testing.T, cbs ...func(*PluginManifest)) *PluginManifest {
	...
	m := &PluginManifest{
		...
		ManifestVersion: "2.0.0",
		SignatureType:   GrafanaSignatureType,
		SignedByOrg:     "grafana",
