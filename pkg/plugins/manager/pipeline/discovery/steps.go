...
// DuplicatePluginVersionValidation is a filter step that will filter out any plugins that are already registered with the
// registry. This includes both the primary plugin and any child plugins, which are matched using the plugin ID and
// version fields.
type DuplicatePluginVersionValidation struct {
	registry registry.Service
	log      log.Logger
}

// NewDuplicatePluginVersionFilterStep returns a new DuplicatePluginVersionValidation.
func NewDuplicatePluginVersionFilterStep(registry registry.Service) *DuplicatePluginVersionValidation {
	return &DuplicatePluginVersionValidation{
		registry: registry,
...
func (d *DuplicatePluginVersionValidation) Filter(ctx context.Context, bundles []*plugins.FoundBundle) ([]*plugins.FoundBundle, error) {
	...
	for _, b := range bundles {
		...
		for _, child := range b.Children {
			_, exists = d.registry.Plugin(ctx, child.JSONData.ID, b.Primary.JSONData.Info.Version)
			if exists {
