...
func NewOrchestrator(cfg *plugins.Cfg, pluginRegistry registry.Service, pluginSources []PluginSource,
	pluginLoader loader.Service) *Orchestrator {
	return &Orchestrator{
		cfg:            cfg,
...
func (m *Orchestrator) loadPlugins(ctx context.Context, class plugins.Class, paths ...string) error {
	if len(paths) == 0 {
		return nil
	}
...
func (m *Orchestrator) registerAndStart(ctx context.Context, p *plugins.Plugin) error {
