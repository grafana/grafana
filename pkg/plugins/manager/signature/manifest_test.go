...
-----END PGP SIGNATURE-----`
	...
	t.Run("valid manifest", func(t *testing.T) {
		s := ProvideService(&config.PluginsCfg{}, statickey.New())
...
func TestCalculate(t *testing.T) {
	t.Run("Validate root URL against App URL for non-private plugin if is specified in manifest", func(t *testing.T) {
		...
		for _, tc := range tcs {
			...
			s := ProvideService(&config.PluginsCfg{GrafanaAppURL: tc.appURL}, statickey.New())
...
func Test_VerifyRevokedKey(t *testing.T) {
	s := ProvideService(&config.PluginsCfg{}, &revokedKeyProvider{})
