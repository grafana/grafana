...
func Test_VerifyRevokedKey(t *testing.T) {
	...
	// This manifest was signed before the key was revoked
	validManifest := `-----BEGIN PGP SIGNED MESSAGE-----
Hash: SHA512
...
-----END PGP SIGNATURE-----
`
	// This manifest was signed after the key was revoked
	invalidManifest := `-----BEGIN PGP SIGNED MESSAGE-----
Hash: SHA512
...

4VJfWfeN
=4rL9
-----END PGP SIGNATURE-----
`

	t.Run("when the key has been revoked but the signature is prior to that", func(t *testing.T) {
		block, _ := clearsign.Decode([]byte(validManifest))
		require.NotNil(t, block, "failed to decode block")
		err := s.validateManifest(context.Background(), *m, block)
		require.NoError(t, err)
	})

	t.Run("when the key has been revoked and the signature was generated after that", func(t *testing.T) {
		block, _ := clearsign.Decode([]byte(invalidManifest))
		require.NotNil(t, block, "failed to decode block")
		err := s.validateManifest(context.Background(), *m, block)
		require.Error(t, err)
