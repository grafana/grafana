...
func TestIntegrationPluginManager(t *testing.T) {
	...
	assets := assetpath.ProvideService(pCfg, pluginscdn.ProvideService(pCfg))
	discovery := pipeline.ProvideDiscoveryStage(pCfg, finder.NewLocalFinder(pCfg.DevMode), reg)
	bootstrap := pipeline.ProvideBootstrapStage(pCfg, signature.ProvideService(pCfg, statickey.New()), assets)
	...
	l := loader.ProvideService(pCfg, lic, signature.NewUnsignedAuthorizer(pCfg),
		...
		assets, angularInspector, &fakes.FakeOauthService{}, discovery, bootstrap)
