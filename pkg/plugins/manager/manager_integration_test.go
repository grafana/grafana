...
func TestIntegrationPluginManager_Run(t *testing.T) {
	...
	authorizer := signature.UnsignedAuthorizer(pmCfg)
	pm, err := ProvideService(cfg, reg, loader.New(pmCfg, license, authorizer,
		provider.ProvideService(coreRegistry)), nil, authorizer)
	require.NoError(t, err)
