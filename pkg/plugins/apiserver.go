...
func GetDatasourceGroupNameFromPluginID(pluginId string) (string, error) {
	...
	shortName, err := GetShortDatasourceGroupNameFromPluginID(pluginId)
	if err != nil {
		return "", err
	}

	return fmt.Sprintf("%s.datasource.grafana.app", shortName), nil
}

func GetShortDatasourceGroupNameFromPluginID(pluginId string) (string, error) {
	if pluginId == "" {
