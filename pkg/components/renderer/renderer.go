...
func RenderToPdf(params *RenderOpts) (string, error) {
	rendererLog.Info("Rendering", "url", params.Url)
	...
	binPath, _ := filepath.Abs(filepath.Join(setting.PhantomDir, executable))
	scriptPath, _ := filepath.Abs(filepath.Join(setting.PhantomDir, "render-pdf.js"))
	pdfPath, _ := filepath.Abs(filepath.Join(setting.ImagesDir, util.GetRandomString(20)))
	pdfPath = pdfPath + ".pdf"

	cmd := exec.Command(binPath, "--ignore-ssl-errors=true", scriptPath, "url="+params.Url, "width="+params.Width,
		"height="+params.Height, "pdf="+pdfPath, "cookiename="+setting.SessionOptions.CookieName,
		"domain="+setting.Domain, "sessionid="+params.SessionId)
	stdout, err := cmd.StdoutPipe()
	...
	case <-time.After(time.Duration(timeout) * time.Second):
		...
		return "", fmt.Errorf("PhantomRenderer::renderToPdf timeout (>%vs)", timeout)
	case <-done:
	}

	rendererLog.Debug("PDF rendered", "path", pdfPath)
	return pdfPath, nil
}