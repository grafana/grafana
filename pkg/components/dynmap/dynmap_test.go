...
func TestFirst(t *testing.T) {
	...
	assert.True(s == testUser && err == nil, "name should be a string")

	s = j.MustGetString("name", "fallback")
	assert.True(s == testUser, "must get string")

	s = j.MustGetString("adsasdas", "fallback")
	assert.True(s == "fallback", "must get string return fallback")

	s, err = j.GetString("name")
	assert.True(s == testUser && err == nil, "name should match")

	s, err = j.GetString("address", "street")
	assert.True(s == testStreet && err == nil, "street should match")
	...
	assert.True(s == testStreet && err == nil, "street mismatching")
	...
	for _, elementValue := range list2Array {
		...
		assert.True(s == testStreet && err == nil, "second fail")
	}
