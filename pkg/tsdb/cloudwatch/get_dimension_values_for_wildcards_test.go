...
func TestGetDimensionValuesForWildcards(t *testing.T) {
	...
	t.Run("Should not change non-wildcard dimension value", func(t *testing.T) {
		...
		queries, err := executor.getDimensionValuesForWildcards(ctx, pluginCtx, "us-east-1", nil, []*models.CloudWatchQuery{query}, tagValueCache, nullLogger)
		assert.Nil(t, err)
	...
	t.Run("Should change wildcard dimension value", func(t *testing.T) {
		...
		queries, err := executor.getDimensionValuesForWildcards(ctx, pluginCtx, "us-east-1", api, []*models.CloudWatchQuery{query}, tagValueCache, nullLogger)
		assert.Nil(t, err)
		assert.Len(t, queries, 1)
	...
	t.Run("Should use cache for previously fetched value", func(t *testing.T) {
		...
		api.On("ListMetricsPagesWithContext").Return(nil)
		_, err := executor.getDimensionValuesForWildcards(ctx, pluginCtx, "us-east-1", api, []*models.CloudWatchQuery{query}, tagValueCache, nullLogger)
		assert.Nil(t, err)
