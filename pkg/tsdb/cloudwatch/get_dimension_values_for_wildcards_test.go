...
func TestGetDimensionValuesForWildcards(t *testing.T) {
	...
	t.Run("MetricSearch query type", func(t *testing.T) {
		t.Run("Should not change non-wildcard dimension value", func(t *testing.T) {
			...
			query.MetricQueryType = models.MetricQueryTypeSearch
			queries, err := executor.getDimensionValuesForWildcards(ctx, "us-east-1", nil, []*models.CloudWatchQuery{query}, tagValueCache, 50, skip)
			assert.Nil(t, err)
			assert.Len(t, queries, 1)
	...
	t.Run("MetricQuery query type", func(t *testing.T) {
		...
		t.Run("Should add an empty string as the value for a dimension without a value", func(t *testing.T) {
			query := getBaseQuery()
			query.MetricName = "Test_MetricName"
			query.Dimensions = map[string][]string{}
			query.Sql.GroupBy = &models.SQLExpressionGroupBy{
				Expressions: []dataquery.QueryEditorGroupByExpression{
					{
						Property: dataquery.QueryEditorProperty{Name: utils.Pointer("Test_DimensionNameWithoutValue"), Type: "string"},
						Type:     "groupBy",
					},
				},
				Type: "and",
			...
			assert.Nil(t, err)
			assert.Len(t, queries, 1)
			assert.Equal(t, map[string][]string{"Test_DimensionNameWithoutValue": {""}}, queries[0].Dimensions)
			api.AssertExpectations(t)
		})
