...
// Fake CloudWatch client for testing
type fakeCWClient struct {
	metrics []*cloudwatch.Metric
}

func (c fakeCWClient) ListMetricsPagesWithContext(ctx context.Context, input *cloudwatch.ListMetricsInput, fn func(*cloudwatch.ListMetricsOutput, bool) bool, opts ...client.RequestOption) error {
	output := &cloudwatch.ListMetricsOutput{Metrics: c.metrics}
	fn(output, true)
	return nil
}
...
func TestQuery_InstanceAttributes(t *testing.T) {
	...
	t.Run("Get instance ID", func(t *testing.T) {
		...
		filterMap := map[string][]string{"tag:Environment": {"production"}}
		...
		expResponse := []suggestData{{Text: instanceID, Value: instanceID, Label: instanceID}}
	...
	t.Run("Get different types", func(t *testing.T) {
					...
					Instances: []*ec2.Instance{
						{
							...
							IamInstanceProfile: &ec2.IamInstanceProfile{Arn: &expectedArn},
						},
					},
				},
			},
		...
		testcases := []struct {
			...
			{"int field", "AmiLaunchIndex", []suggestData{{Text: "3", Value: "3", Label: "3"}}},
			{"bool field", "EbsOptimized", []suggestData{{Text: "true", Value: "true", Label: "true"}}},
			{"nested field", "IamInstanceProfile.Arn", []suggestData{{Text: expectedArn, Value: expectedArn, Label: expectedArn}}},
			{"nil field", "InstanceLifecycle", []suggestData{}},
		}
...
func TestQuery_ResourceARNs(t *testing.T) {
	...
	t.Run("", func(t *testing.T) {
			...
			tagMapping: []*resourcegroupstaggingapi.ResourceTagMapping{
				{
					...
					Tags:        []*resourcegroupstaggingapi.Tag{{Key: aws.String("Environment"), Value: aws.String("production")}},
				},
				{
		...
		tagMap := map[string][]string{"Environment": {"production"}}
...
func TestQuery_DimensionValues(t *testing.T) {
	origNewCWClient := NewCWClient
	t.Cleanup(func() {
		NewCWClient = origNewCWClient
	})

	var cli fakeCWClient
	NewCWClient = func(client.ConfigProvider) models.CWAPIProvider {
		return cli
	}

	t.Run("Get JobId dimensions for EMRServerless", func(t *testing.T) {
		cli = fakeCWClient{
			metrics: []*cloudwatch.Metric{
				{
					Namespace:  aws.String("AWS/EMRServerless"),
					MetricName: aws.String("WorkerCpuUsed"),
					Dimensions: []*cloudwatch.Dimension{
						{Name: aws.String("ApplicationId"), Value: aws.String("app-123")},
						{Name: aws.String("JobId"), Value: aws.String("job-456")},
					},
				},
				{
					Namespace:  aws.String("AWS/EMRServerless"),
					...
					Dimensions: []*cloudwatch.Dimension{
						{Name: aws.String("ApplicationId"), Value: aws.String("app-123")},
						{Name: aws.String("JobId"), Value: aws.String("job-789")},
					},
				},
			},
		}
		...
		executor := newExecutor(im, log.NewNullLogger())
		resp, err := executor.handleGetDimensionValues(
			context.Background(),
			backend.PluginContext{DataSourceInstanceSettings: &backend.DataSourceInstanceSettings{}},
			url.Values{
				"region":        []string{"us-east-1"},
				"namespace":     []string{"AWS/EMRServerless"},
				"metricName":    []string{"WorkerCpuUsed"},
				"dimensionKey":  []string{"JobId"},
		...
		expResponse := []suggestData{
			{Text: "job-456", Value: "job-456", Label: "job-456"},
			{Text: "job-789", Value: "job-789", Label: "job-789"},
		}
		assert.Equal(t, expResponse, resp)
	})
}
