...
func (e *CloudWatchExecutor) executeLogAction(ctx context.Context, queryContext *tsdb.TsdbQuery, query *tsdb.Query) (*data.Frame, error) {
	...
	region := parameters.Get("region").MustString(cloudWatchDefaultRegion)
	...
	case "DescribeLogGroups":
		return e.handleDescribeLogGroups(ctx, logsClient, parameters)
	case "GetLogGroupFields":
		return e.handleGetLogGroupFields(ctx, logsClient, parameters, query.RefId)
	case "StartQuery":
		return e.handleStartQuery(ctx, logsClient, parameters, queryContext.TimeRange, query.RefId)
	case "StopQuery":
		return e.handleStopQuery(ctx, logsClient, parameters)
	case "GetQueryResults":
		return e.handleGetQueryResults(ctx, logsClient, parameters, query.RefId)
	case "GetLogEvents":
		return e.handleGetLogEvents(ctx, logsClient, parameters)
	}

	return nil, fmt.Errorf("unrecognized query model subtype %q", subType)
}
