	...
	Convey("TestMetricDataInputBuilder", t, func() {
		...
		Convey("Time range is valid", func() {
			Convey("End time before start time should result in error", func() {
				_, err := mdpb.build(&tsdb.TsdbQuery{TimeRange: tsdb.NewTimeRange("now-1h", "now-2h")}, []*cloudWatchQuery{})
			...
			Convey("End time equals start time should result in error", func() {
				_, err := mdpb.build(&tsdb.TsdbQuery{TimeRange: tsdb.NewTimeRange("now-1h", "now-1h")}, []*cloudWatchQuery{})
				So(err.Error(), ShouldEqual, "Invalid time range: Start time must be before end time")
			})
		})
		...
		Convey("and testing metricDataInput bulding", func() {
			Convey("first metricDataInput is filled with metric stat queries without ID only", func() {
				res, err := mdpb.build(ctx, []*cloudWatchQuery{inferredSearchExpression, userDefinedSearchExpression, mathExpression, metricStatQueryWithoutID, metricStatQueryWithoutID, metricStatQueryWithoutID, metricStatQueryWithoutID, metricStatQueryWithDefinedID})

				So(err, ShouldBeNil)
				So(*res[0].MetricDataInput.MetricDataQueries[0].MetricStat, ShouldNotBeNil)
				So(*res[0].MetricDataInput.MetricDataQueries[1].MetricStat, ShouldNotBeNil)
				So(*res[0].MetricDataInput.MetricDataQueries[2].MetricStat, ShouldNotBeNil)

				So(*res[1].MetricDataInput.MetricDataQueries[0].Id, ShouldEqual, "metricStatQueryWithID")
				So(*res[1].MetricDataInput.MetricDataQueries[1].Id, ShouldEqual, "mathExpression")
				So(*res[1].MetricDataInput.MetricDataQueries[2].Id, ShouldBeIn, []string{"inferredSearchExpression", "userDefinedSearchExpression"})
				So(*res[1].MetricDataInput.MetricDataQueries[3].Id, ShouldBeIn, []string{"inferredSearchExpression", "userDefinedSearchExpression"})
			})
		})

		Convey("a maximum of 2 search expressions are used in each metricDataInput", func() {
			res, err := mdpb.build(ctx, []*cloudWatchQuery{
			...
			So(*res[0].MetricDataInput.MetricDataQueries[1].MetricStat, ShouldNotBeNil)
			So(*res[0].MetricDataInput.MetricDataQueries[2].MetricStat, ShouldNotBeNil)
			So(*res[0].MetricDataInput.MetricDataQueries[3].MetricStat, ShouldNotBeNil)
			...
			So(*res[2].MetricDataInput.MetricDataQueries[0].Id, ShouldBeIn, []string{"inferredSearchExpression", "userDefinedSearchExpression"})
			So(*res[2].MetricDataInput.MetricDataQueries[1].Id, ShouldBeIn, []string{"inferredSearchExpression", "userDefinedSearchExpression"})

			So(*res[3].MetricDataInput.MetricDataQueries[0].Id, ShouldBeIn, []string{"inferredSearchExpression", "userDefinedSearchExpression"})
			So(*res[3].MetricDataInput.MetricDataQueries[1].Id, ShouldBeIn, []string{"inferredSearchExpression", "userDefinedSearchExpression"})
