	...
	Convey("TestCloudWatchQueryBuilder", t, func() {
		...
		Convey("buildMetricDataQueries", func() {
			...
			Convey("and metric stat", func() {
				...
				Convey("will not used when expression is set in the client", func() {
					dimensions := make(map[string][]string)
					dimensions["InstanceId"] = []string{"i-12345678"}
					query := &CloudWatchQuery{
						RefId:      "A",
				...
				Convey("will be not used when a dimension key has more than one value", func() {
					dimensions := make(map[string][]string)
				...
				Convey("will be not used when a dimension key has a value that is equals to *", func() {
					dimensions := make(map[string][]string)
					dimensions["InstanceId"] = []string{"i-12345678", "*"}
					query := &CloudWatchQuery{
						RefId:      "A",
