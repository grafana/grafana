...
func (e *cloudWatchExecutor) executeTimeSeriesQueryV2(ctx context.Context, req *backend.QueryDataRequest) (*backend.QueryDataResponse, error) {
	plog.Debug("Executing time series query")
	...
	for _, q := range req.Queries {
		...
		requestQueriesByRegion, err := e.parseQueriesV2(req, startTime, endTime)
		if err != nil {
			return nil, err
		}
		...
		for r, q := range requestQueriesByRegion {
			...
			eg.Go(func() error {
				...
				cloudwatchResponses = append(cloudwatchResponses, responses...)
				res, err := e.transformQueryResponsesToQueryResultV2(cloudwatchResponses, requestQueries, startTime, endTime)
				if err != nil {
					for _, query := range requestQueries {
						resultChan <- &backend.DataResponse{
							Frames: data.Frames{data.NewFrame(query.RefId)},
