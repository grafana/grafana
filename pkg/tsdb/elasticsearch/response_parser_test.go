...
func TestResponseParser(t *testing.T) {
	Convey("Elasticsearch response parser test", t, func() {
		...
		Convey("With top_metrics", func() {
			...
			seriesOne := queryRes.Series[0]
			So(seriesOne.Name, ShouldEqual, "Top Metrics @value")
			So(seriesOne.Points, ShouldHaveLength, 2)
			So(seriesOne.Points[0][0].Float64, ShouldEqual, 1)
			So(seriesOne.Points[0][1].Float64, ShouldEqual, 1609459200000)
			So(seriesOne.Points[1][0].Float64, ShouldEqual, 1)
			So(seriesOne.Points[1][1].Float64, ShouldEqual, 1609459210000)

			seriesTwo := queryRes.Series[1]
			So(seriesTwo.Name, ShouldEqual, "Top Metrics @anotherValue")
			So(seriesTwo.Points, ShouldHaveLength, 2)
			So(seriesTwo.Points[0][0].Float64, ShouldEqual, 2)
			So(seriesTwo.Points[0][1].Float64, ShouldEqual, 1609459200000)
			So(seriesTwo.Points[1][0].Float64, ShouldEqual, 2)
