...
func TestTimeSeriesQueryParser(t *testing.T) {
	t.Run("Test time series query parser", func(t *testing.T) {
        ...
        "interval": "10m",
			...
			require.Equal(t, q.BucketAggs[0].Settings.Get("min_doc_count").MustInt64(), 1)
			...
			require.Equal(t, q.BucketAggs[1].Settings.Get("min_doc_count").MustInt64(), 0)
			require.Equal(t, q.BucketAggs[1].Settings.Get("trimEdges").MustInt64(), 0)
		})
	})
}
