...
var newTimeSeriesQueryResponseTransformer = func(responses []*es.SearchResponse, targets []*timeSeriesQueryModel) responseTransformer {
			...
			for _, metric := range target.metrics {
				if metric.id == metricID {
					...
					for name, pipelineAgg := range metric.pipelineVariables {
						for _, m := range target.metrics {
							if m.id == pipelineAgg {
								metricName = strings.Replace(metricName, "params."+name, describeMetric(m.aggType, m.field), -1)
							}
						}
					}
				}
...
func getErrorFromElasticResponse(err map[string]interface{}) *tsdb.QueryResult {
	...
	if rootCauseReason != "" {
		result.Error = fmt.Errorf(rootCauseReason)
	} else if reason != "" {
		result.Error = fmt.Errorf(reason)
	} else {
		result.Error = fmt.Errorf("unkown error response from elasticsearch")
	}
