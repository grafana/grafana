...
func TestIndexPattern(t *testing.T) {
	Convey("Static index patterns", t, func() {
		indexPatternScenario(noInterval, "data-*", pluginmodels.DataTimeRange{}, func(indices []string) {
			So(indices, ShouldHaveLength, 1)
			So(indices[0], ShouldEqual, "data-*")
		})

		indexPatternScenario(noInterval, "es-index-name", pluginmodels.DataTimeRange{}, func(indices []string) {
			So(indices, ShouldHaveLength, 1)
	...
	Convey("Dynamic index patterns", t, func() {
		...
		indexPatternScenario(intervalHourly, "[data-]YYYY.MM.DD.HH", pluginmodels.NewDataTimeRange(from, to), func(indices []string) {
			So(indices, ShouldHaveLength, 1)
			So(indices[0], ShouldEqual, "data-2018.05.15.17")
		})

		indexPatternScenario(intervalHourly, "YYYY.MM.DD.HH[-data]", pluginmodels.NewDataTimeRange(from, to), func(indices []string) {
			So(indices, ShouldHaveLength, 1)
			So(indices[0], ShouldEqual, "2018.05.15.17-data")
		})

		indexPatternScenario(intervalDaily, "[data-]YYYY.MM.DD", pluginmodels.NewDataTimeRange(from, to), func(indices []string) {
			So(indices, ShouldHaveLength, 1)
			So(indices[0], ShouldEqual, "data-2018.05.15")
		})

		indexPatternScenario(intervalDaily, "YYYY.MM.DD[-data]", pluginmodels.NewDataTimeRange(from, to), func(indices []string) {
			So(indices, ShouldHaveLength, 1)
			So(indices[0], ShouldEqual, "2018.05.15-data")
		})

		indexPatternScenario(intervalWeekly, "[data-]GGGG.WW", pluginmodels.NewDataTimeRange(from, to), func(indices []string) {
			So(indices, ShouldHaveLength, 1)
			So(indices[0], ShouldEqual, "data-2018.20")
		})

		indexPatternScenario(intervalWeekly, "GGGG.WW[-data]", pluginmodels.NewDataTimeRange(from, to), func(indices []string) {
			So(indices, ShouldHaveLength, 1)
			So(indices[0], ShouldEqual, "2018.20-data")
		})

		indexPatternScenario(intervalMonthly, "[data-]YYYY.MM", pluginmodels.NewDataTimeRange(from, to), func(indices []string) {
			So(indices, ShouldHaveLength, 1)
			So(indices[0], ShouldEqual, "data-2018.05")
		})

		indexPatternScenario(intervalMonthly, "YYYY.MM[-data]", pluginmodels.NewDataTimeRange(from, to), func(indices []string) {
			So(indices, ShouldHaveLength, 1)
			So(indices[0], ShouldEqual, "2018.05-data")
		})

		indexPatternScenario(intervalYearly, "[data-]YYYY", pluginmodels.NewDataTimeRange(from, to), func(indices []string) {
			So(indices, ShouldHaveLength, 1)
			So(indices[0], ShouldEqual, "data-2018")
		})

		indexPatternScenario(intervalYearly, "YYYY[-data]", pluginmodels.NewDataTimeRange(from, to), func(indices []string) {
			So(indices, ShouldHaveLength, 1)
			So(indices[0], ShouldEqual, "2018-data")
		})

		indexPatternScenario(intervalDaily, "YYYY[-data-]MM.DD", pluginmodels.NewDataTimeRange(from, to), func(indices []string) {
			So(indices, ShouldHaveLength, 1)
			So(indices[0], ShouldEqual, "2018-data-05.15")
		})

		indexPatternScenario(intervalDaily, "[data-]YYYY[-moredata-]MM.DD", pluginmodels.NewDataTimeRange(from, to), func(indices []string) {
			So(indices, ShouldHaveLength, 1)
...
func indexPatternScenario(interval string, pattern string, timeRange pluginmodels.DataTimeRange, fn func(indices []string)) {
