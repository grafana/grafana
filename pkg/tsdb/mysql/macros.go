...
func (m *MySqlMacroEngine) evaluateMacro(name string, args []string) (string, error) {
	...
	case "__timeFilter":
		...
		return fmt.Sprintf("%s >= FROM_UNIXTIME(%d) AND %s <= FROM_UNIXTIME(%d)", args[0], int64(m.TimeRange.GetFromAsSecondsEpoch()), args[0], int64(m.TimeRange.GetToAsSecondsEpoch())), nil
	case "__timeFrom":
		return fmt.Sprintf("FROM_UNIXTIME(%d)", int64(m.TimeRange.GetFromAsSecondsEpoch())), nil
	case "__timeTo":
		return fmt.Sprintf("FROM_UNIXTIME(%d)", int64(m.TimeRange.GetToAsSecondsEpoch())), nil
	case "__timeGroup":
		if len(args) < 2 {
