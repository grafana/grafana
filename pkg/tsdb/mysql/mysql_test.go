...
func TestIntegrationMySQL(t *testing.T) {
	...
	exe, err := sqleng.NewQueryDataHandler(setting.NewCfg(), config, &rowTransformer, newMysqlMacroEngine(logger), logger)
	...
	t.Run("Given a table with event data", func(t *testing.T) {
		...
		t.Run("When row limit set to 1", func(t *testing.T) {
			...
			handler, err := sqleng.NewQueryDataHandler(setting.NewCfg(), config, &queryResultTransformer, newMysqlMacroEngine(logger), logger)
			require.NoError(t, err)
