...
func (s *Service) handleCsvContentScenario(ctx context.Context, req *backend.QueryDataRequest) (*backend.QueryDataResponse, error) {
	...
	for _, q := range req.Queries {
		...
		frame, err := s.loadCsvContent(strings.NewReader(csvContent), alias)
		if err != nil {
			return nil, err
		}
...
func (s *Service) loadCsvFile(fileName string) (*data.Frame, error) {
	...
	return s.loadCsvContent(fileReader, fileName)
}

func (s *Service) loadCsvContent(ioReader io.Reader, name string) (*data.Frame, error) {
