...
func (s *Service) parseResponse(res *http.Response) ([]TargetResponseDTO, error) {
	...
	if res.StatusCode/100 != 2 {
		...
		s.logger.Error("Request failed", "status", res.Status, "error", graphiteError, "body", truncateForLog(body))
	...
	if err != nil {
		...
		if err != nil {
			s.logger.Error("Failed to unmarshal legacy graphite response", "error", err, "status", res.Status, "body", truncateForLog(body))
			return nil, backend.PluginError(err)
		}
