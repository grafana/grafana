...
import (
	...
	"github.com/grafana/grafana/pkg/tsdb/sqleng/convert"
...
func (e *DataSourceHandler) executeQuery(query backend.DataQuery, wg *sync.WaitGroup, queryContext context.Context,
	...
	if qm.Format == dataQueryFormatSeries {
		...
		for i := range qm.columnNames {
			...
			var err error
			if frame, err = convert.ConvertSQLValueColumnToFloat(frame, i); err != nil {
		...
		if qm.FillMissing != nil {
			var err error
			frame, err = resample.Resample(frame, qm.TimeRange, qm.Interval, qm.FillMissing)
			if err != nil {
...
func convertSQLTimeColumnsToEpochMS(frame *data.Frame, qm *dataQueryModel) error {
	if qm.timeIndex != -1 {
		if err := convert.ConvertSQLTimeColumnToEpochMS(frame, qm.timeIndex); err != nil {
			return fmt.Errorf("%v: %w", "failed to convert time column", err)
		}
	}

	if qm.timeEndIndex != -1 {
		if err := convert.ConvertSQLTimeColumnToEpochMS(frame, qm.timeEndIndex); err != nil {
