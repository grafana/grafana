...
func TestIntegrationPostgres(t *testing.T) {
	...
	db, exe, err := newPostgres(context.Background(), cfg, dsInfo, cnn, logger, backend.DataSourceInstanceSettings{})
	...
	t.Run("Given a table with event data", func(t *testing.T) {
		...
		t.Run("When row limit set to 1", func(t *testing.T) {
			...
			_, handler, err := newPostgres(context.Background(), conf, dsInfo, cnn, logger, backend.DataSourceInstanceSettings{})
