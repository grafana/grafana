...
func (s *Service) getTrace(ctx context.Context, pCtx backend.PluginContext, query backend.DataQuery) (*backend.DataResponse, error) {
	...
	if err != nil {
		s.logger.Debug("getTrace error", "error", err)
		return result, err
	}
	...
	if resp.StatusCode != http.StatusOK {
		...
		s.logger.Debug("getTrace error", "error", result.Error)
		return result, nil
	}
	...
	s.logger.Debug("getTrace completed")
	return result, nil
}
...
func (s *Service) createRequest(ctx context.Context, dsInfo *Datasource, traceID string, start int64, end int64) (*http.Request, error) {
	...
	if err != nil {
		s.logger.Debug("createRequest error", "error", err)
		return nil, err
	}
