...
func NewManagedDualWriter(service Service,
	...
	if !status.Runtime {
		if status.ReadUnified {
			if status.WriteLegacy {
				...
				return grafanarest.NewDualWriter(grafanarest.Mode3, legacy, storage, reg, gr.String()), nil
			}
			return storage, nil
		}
		if status.WriteUnified {
			// Write both, read legacy
			return grafanarest.NewDualWriter(grafanarest.Mode2, legacy, storage, reg, gr.String()), nil
		}
		return legacy, nil
	}
	...
	return &dualWriterMode3Managed{
		...
		legacy:  legacy,
		unified: storage,
		target:  grafanarest.NewDualWriter(grafanarest.Mode3, legacy, storage, reg, gr.String()),
		gr:      gr,
	}, nil
}
