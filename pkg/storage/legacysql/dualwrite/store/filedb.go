...
type fileDB struct {
	path    string
	changed int64
	db      map[string]dualwrite.StorageStatus
	mu      sync.RWMutex
	logger  logging.Logger
}

// File implementation while testing -- values are saved in the data directory
func ProvideStorage(cfg *setting.Cfg) dualwrite.StatusStorage {
	path := ""
	if cfg != nil {
		path = filepath.Join(cfg.DataPath, "dualwrite.json")
	}
	return &fileDB{
		db:     make(map[string]dualwrite.StorageStatus),
		path:   path,
		logger: logging.DefaultLogger.With("logger", "fileDB"),
	}
}

func (m *fileDB) Get(ctx context.Context, gr schema.GroupResource) (dualwrite.StorageStatus, bool) {
	m.mu.RLock()
	defer m.mu.RUnlock()
...
func (m *fileDB) Set(ctx context.Context, status dualwrite.StorageStatus) (dualwrite.StorageStatus, error) {
	m.mu.Lock()
	defer m.mu.Unlock()
