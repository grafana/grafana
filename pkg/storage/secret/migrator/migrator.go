...
const (
	...
	TableNameAuditLogConfig    = "secret_audit_log_config"
)
...
func (*SecretDB) AddMigration(mg *migrator.Migrator) {
	...
	tables = append(tables, migrator.Table{
		...
		Columns: []*migrator.Column{
			...
			// Spec
			{Name: "stdout_enable", Type: migrator.DB_Bool, Nullable: false},
			...
			{Name: "loki_protocol", Type: migrator.DB_Text, Nullable: true},
			{Name: "loki_tls", Type: migrator.DB_Bool, Nullable: true},
		},
		Indices: []*migrator.Index{
			{Cols: []string{"namespace"}, Type: migrator.UniqueIndex}, // Each namespace can have only ONE audit log config.
		},
	})
