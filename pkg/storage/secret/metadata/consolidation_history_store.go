...
func (s *consolidationHistoryStore) StartNewConsolidation(ctx context.Context) (record *contracts.ConsolidationRecord, createErr error) {
	start := time.Now()
	...
	if err != nil {
		return nil, fmt.Errorf("execute template %q: %w", sqlConsolidationHistoryCreate.Name(), err)
	}

	rows, err := s.db.QueryContext(ctx, query, req.GetArgs()...)
	if err != nil {
		return nil, fmt.Errorf("inserting consolidation row: %w", err)
	}
	...
	if err := rows.Scan(&id); err != nil {
		return nil, fmt.Errorf("scanning inserted id: %w", err)
	}
	if err := rows.Err(); err != nil {
		return nil, fmt.Errorf("read rows error: %w", err)
	}
	return &contracts.ConsolidationRecord{
		ID:        id,
		Created:   time.Unix(now, 0).UTC(),
		Completed: time.Time{},
	}, nil
}

func (s *consolidationHistoryStore) FinishConsolidation(ctx context.Context, record *contracts.ConsolidationRecord) (finishErr error) {
	if record == nil {
		return contracts.ErrNoConsolidationToFinish
	}
	start := time.Now()
	ctx, span := s.tracer.Start(ctx, "ConsolidationHistoryStore.FinishConsolidation")
	defer span.End()
	...
	defer func() {
		...
		args := []any{"success", success}
		if !success {
			span.SetStatus(codes.Error, "ConsolidationHistoryStore.FinishConsolidation failed")
			span.RecordError(finishErr)
			args = append(args, "error", finishErr)
		}
		logging.FromContext(ctx).Info("ConsolidationHistoryStore.FinishConsolidation", args...)
