...
func TestEncodeMaxVersionEnforcement(t *testing.T) {
	...
	t.Run("codec path: real LegacyCodec down-converts to the cap version (preferred==cap)", func(t *testing.T) {
		// Exercises the actual remediation, not a fake codec: ReorderGroupVersionsForLegacyCodec puts the
		// preferred version first in the group, so when preferred==cap, a real LegacyCodec built from
		// [cap, higher] converts every internal-object write down to the cap version.
...
// capWidget is a hub (unversioned/internal) test type. capWidgetV1/capWidgetV2 are its external
// versions. Feeding a capWidget through a real LegacyCodec has no exact-match target version (the
// hub kind's version never equals an external one), so apimachinery falls back to the first group
// version in the codec's list — the mechanism newCapCodec exercises.
...
// newCapCodec builds a scheme registering capWidget as the hub plus versions[0] and versions[1] as
// its two external spokes, and returns a real serializer.CodecFactory.LegacyCodec(versions...).
