...
// StorageServerOptions contains options for creating a storage server
...
// storageServer implements StorageServer interface for storage operations
...
func NewStorageServer(opts StorageServerOptions) (StorageServer, error) {
	if opts.Backend == nil {
	...
	err := s.Init(ctx)
	if err != nil {
		s.log.Error("storage server init failed", "error", err)
		return nil, err
	}
...
func (s *storageServer) Init(ctx context.Context) error {
	s.once.Do(func() {
		// Call lifecycle hooks
		if s.lifecycle != nil {
			err := s.lifecycle.Init(ctx)
			if err != nil {
				s.initErr = fmt.Errorf("initialize Storage Server: %w", err)
			}
		}
		...
		if s.initErr != nil {
			s.log.Error("error running storage server init", "error", s.initErr)
		}
	})
	return s.initErr
}
...
func (s *storageServer) Stop(ctx context.Context) error {
...
//
//nolint:gocyclo
func (s *storageServer) newEvent(ctx context.Context, user claims.AuthInfo, key *resourcepb.ResourceKey, value, oldValue []byte) (*WriteEvent, *resourcepb.ErrorResult) {
...
func (s *storageServer) isFolderMove(event *WriteEvent) bool {
...
func (s *storageServer) checkFolderMovePermissions(ctx context.Context, user claims.AuthInfo, key *resourcepb.ResourceKey, oldFolder, newFolder string) *resourcepb.ErrorResult {
...
func (s *storageServer) Create(ctx context.Context, req *resourcepb.CreateRequest) (*resourcepb.CreateResponse, error) {
	ctx, span := tracer.Start(ctx, "resource.storageServer.Create")
	defer span.End()
...
func (s *storageServer) create(ctx context.Context, user claims.AuthInfo, req *resourcepb.CreateRequest) (*resourcepb.CreateResponse, error) {
	rsp := &resourcepb.CreateResponse{}
...
func (s *storageServer) Update(ctx context.Context, req *resourcepb.UpdateRequest) (*resourcepb.UpdateResponse, error) {
	ctx, span := tracer.Start(ctx, "resource.storageServer.Update")
	defer span.End()
...
func (s *storageServer) update(ctx context.Context, user claims.AuthInfo, req *resourcepb.UpdateRequest) (*resourcepb.UpdateResponse, error) {
	rsp := &resourcepb.UpdateResponse{}
...
func (s *storageServer) Delete(ctx context.Context, req *resourcepb.DeleteRequest) (*resourcepb.DeleteResponse, error) {
	ctx, span := tracer.Start(ctx, "resource.storageServer.Delete")
	defer span.End()
...
func (s *storageServer) delete(ctx context.Context, user claims.AuthInfo, req *resourcepb.DeleteRequest) (*resourcepb.DeleteResponse, error) {
	rsp := &resourcepb.DeleteResponse{}
	latest := s.backend.ReadResource(ctx, &resourcepb.ReadRequest{
		Key: req.Key,
	})
...
func (s *storageServer) Read(ctx context.Context, req *resourcepb.ReadRequest) (*resourcepb.ReadResponse, error) {
	user, ok := claims.AuthInfoFrom(ctx)
	if !ok || user == nil {
...
func (s *storageServer) read(ctx context.Context, user claims.AuthInfo, req *resourcepb.ReadRequest) (*resourcepb.ReadResponse, error) {
...
func (s *storageServer) List(ctx context.Context, req *resourcepb.ListRequest) (*resourcepb.ListResponse, error) {
	ctx, span := tracer.Start(ctx, "resource.storageServer.List")
	defer span.End()
...
func (s *storageServer) isTrashItemAuthorized(ctx context.Context, iter ListIterator, trashChecker claims.ItemChecker) bool {
	user, ok := claims.AuthInfoFrom(ctx)
	if !ok || user == nil {
		return false
	}
...
func (s *storageServer) initWatcher() error {
	var err error
...
//nolint:gocyclo
func (s *storageServer) Watch(req *resourcepb.WatchRequest, srv resourcepb.ResourceStore_WatchServer) error {
	ctx := srv.Context()
	...
	// don't replay events that happened before the watch was established. Using
	...
	if !req.SendInitialEvents && req.Since == 0 {
		...
		if err != nil {
			// Fallback to the broadcaster's view if the backend lookup fails.
...
func (s *storageServer) IsHealthy(ctx context.Context, req *resourcepb.HealthCheckRequest) (*resourcepb.HealthCheckResponse, error) {
	return s.diagnostics.IsHealthy(ctx, req)
}

// PutBlob implements BlobStoreServer.
func (s *storageServer) PutBlob(ctx context.Context, req *resourcepb.PutBlobRequest) (*resourcepb.PutBlobResponse, error) {
	if s.blob == nil {
...
func (s *storageServer) GetQuotaUsage(ctx context.Context, req *resourcepb.QuotaUsageRequest) (*resourcepb.QuotaUsageResponse, error) {
	if s.overridesService == nil {
...
func (s *storageServer) getPartialObject(ctx context.Context, key *resourcepb.ResourceKey, rv int64) (utils.GrafanaMetaAccessor, *resourcepb.ErrorResult) {
...
// GetBlob implements BlobStoreServer.
func (s *storageServer) GetBlob(ctx context.Context, req *resourcepb.GetBlobRequest) (*resourcepb.GetBlobResponse, error) {
	if s.blob == nil {
...
func (s *storageServer) runInQueue(ctx context.Context, tenantID string, runnable func(ctx context.Context)) error {
...
func (s *storageServer) checkQuota(ctx context.Context, nsr NamespacedResource) {
...
func (s *storageServer) Backend() StorageBackend {
	return s.backend
}

// Access returns the access client
func (s *storageServer) Access() claims.AccessClient {
	return s.access
}

// Blob returns the blob support
func (s *storageServer) Blob() BlobSupport {
	return s.blob
}

// MostRecentRV returns the most recent resource version
func (s *storageServer) MostRecentRV() int64 {
	return s.mostRecentRV.Load()
}

// Broadcaster returns the event broadcaster
func (s *storageServer) Broadcaster() Broadcaster[*WrittenEvent] {
