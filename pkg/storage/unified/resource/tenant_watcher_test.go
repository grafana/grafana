...
func TestReconcileStaleRecords(t *testing.T) {
	t.Run("clears record when tenant is not in live set", func(t *testing.T) {
		tw := newTestTenantWatcher(t)
		collector := &writeEventCollector{}
		tw.writeEvent = collector.append

		// Seed a resource owned by tenant-1 with the pending-delete label.
		pendingLabels := map[string]string{labelPendingDelete: "true"}
		saveTestResource(t, tw.dataStore, "tenant-1", "apps", "dashboards", "dash1", 100, pendingLabels)
		...
		// tenant-1 is not in live set — reconciliation should clear it.
		tw.reconcileStaleRecords(map[string]struct{}{})

		_, err := tw.pendingDeleteStore.Get(t.Context(), "tenant-1")
		assert.ErrorIs(t, err, ErrNotFound, "record should be removed")
		assert.Len(t, collector.all(), 1, "one unlabel write event should occur")
	})

	t.Run("leaves record untouched when tenant is in live set", func(t *testing.T) {
		tw := newTestTenantWatcher(t)
		collector := &writeEventCollector{}
		tw.writeEvent = collector.append
	...
	t.Run("skips orphaned record even when tenant is not in live set", func(t *testing.T) {
		tw := newTestTenantWatcher(t)
		collector := &writeEventCollector{}
		tw.writeEvent = collector.append
		...
		// Orphaned records must be preserved by clearTenantPendingDelete.
		record, err := tw.pendingDeleteStore.Get(t.Context(), "tenant-1")
		require.NoError(t, err)
		assert.True(t, record.Orphaned, "orphaned record should be preserved")
		assert.Empty(t, collector.all(), "no unlabelling for orphaned records")
	})

	t.Run("clears only stale records from a mixed set", func(t *testing.T) {
		tw := newTestTenantWatcher(t)

		require.NoError(t, tw.pendingDeleteStore.Upsert(t.Context(), "tenant-live", PendingDeleteRecord{
			DeleteAfter: "2026-03-01T00:00:00Z", LabelingComplete: true,
		}))
		require.NoError(t, tw.pendingDeleteStore.Upsert(t.Context(), "tenant-stale-1", PendingDeleteRecord{
			DeleteAfter: "2026-03-01T00:00:00Z", LabelingComplete: true,
		}))
		require.NoError(t, tw.pendingDeleteStore.Upsert(t.Context(), "tenant-stale-2", PendingDeleteRecord{
			DeleteAfter: "2026-03-01T00:00:00Z", LabelingComplete: true,
		}))

		tw.reconcileStaleRecords(map[string]struct{}{"tenant-live": {}})

		_, err := tw.pendingDeleteStore.Get(t.Context(), "tenant-live")
		require.NoError(t, err, "live record should be preserved")
		_, err = tw.pendingDeleteStore.Get(t.Context(), "tenant-stale-1")
		assert.ErrorIs(t, err, ErrNotFound)
		_, err = tw.pendingDeleteStore.Get(t.Context(), "tenant-stale-2")
		assert.ErrorIs(t, err, ErrNotFound)
	})

	t.Run("no-op when KV store has no records", func(t *testing.T) {
		tw := newTestTenantWatcher(t)
		tw.reconcileStaleRecords(map[string]struct{}{"tenant-1": {}})
