...
func TestGetFileBuildInfo(t *testing.T) {
	ns := resource.NamespacedResource{
		Namespace: "test",
		Group:     "group",
		Resource:  "resource",
	...
	t.Run("returns nil when size is below file threshold", func(t *testing.T) {
		tmpDir := t.TempDir()
		backend, _ := setupBleveBackend(t, withRootDir(tmpDir), withFileThreshold(100))

		// Size is below threshold (100), so should return nil
		bi, err := backend.GetFileBuildInfo(ns, 50)
		require.NoError(t, err)
		require.Nil(t, bi)
	})

	t.Run("returns nil when no file-based index exists", func(t *testing.T) {
		tmpDir := t.TempDir()
		backend, _ := setupBleveBackend(t, withRootDir(tmpDir), withFileThreshold(5))

		// No index was created, so should return nil
		bi, err := backend.GetFileBuildInfo(ns, 100)
		require.NoError(t, err)
		require.Nil(t, bi)
	})

	t.Run("returns nil when only in-memory index exists", func(t *testing.T) {
		tmpDir := t.TempDir()
		backend, _ := setupBleveBackend(t, withRootDir(tmpDir), withFileThreshold(100))

		// Build an in-memory index (size below threshold)
		idx, err := backend.BuildIndex(context.Background(), ns, 2 /* below threshold */, nil, "test", indexTestDocs(ns, 2, 100), nil, false, time.Time{})
		require.NoError(t, err)
		require.NotNil(t, idx)
		...
		// GetFileBuildInfo should return nil since there's no file-based index
		bi, err := backend.GetFileBuildInfo(ns, 100)
		require.NoError(t, err)
		require.Nil(t, bi)
	})

	t.Run("returns build info from file-based index after eviction", func(t *testing.T) {
		tmpDir := t.TempDir()
		backend, _ := setupBleveBackend(t, withRootDir(tmpDir), withFileThreshold(5))

		// Build a file-based index (size above threshold)
		idx, err := backend.BuildIndex(context.Background(), ns, 100 /* above threshold */, nil, "test", indexTestDocs(ns, 10, 100), nil, false, time.Time{})
		require.NoError(t, err)
		require.NotNil(t, idx)

		// Verify it's file-based
		bleveIdx := idx.(*bleveIndex)
		require.Equal(t, indexStorageFile, bleveIdx.indexStorage)

		// Simulate eviction from cache (as happens when index is closed/evicted)
		// First, remove from cache to prevent cleanup from double-closing
		backend.cacheMx.Lock()
		delete(backend.cache, ns)
		backend.cacheMx.Unlock()

		// Close the index to simulate a closed/evicted index
		err = bleveIdx.stopUpdaterAndCloseIndex()
		require.NoError(t, err)

		// GetFileBuildInfo should return the build info from the file-based index
		bi, err := backend.GetFileBuildInfo(ns, 100)
		require.NoError(t, err)
		require.NotNil(t, bi)

		// Verify build info contains expected data
		require.False(t, bi.BuildTime.IsZero(), "BuildTime should not be zero")
		require.NotNil(t, bi.BuildVersion, "BuildVersion should not be nil")
		require.Equal(t, buildVersion, bi.BuildVersion.String(), "BuildVersion should match")
	})
}
