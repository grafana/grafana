...
func testBleveBackend(t *testing.T, backend *bleveBackend) {
	...
	t.Run("build dashboards", func(t *testing.T) {
				...
				Items: []*resource.BulkIndexItem{
					{
						...
						Doc: &resource.IndexableDocument{
							...
							Fields: map[string]any{
								external.DASHBOARD_PANEL_TYPES:       []string{"timeseries", "table"},
								external.DASHBOARD_ERRORS_TODAY:      25,
								external.DASHBOARD_VIEWS_LAST_1_DAYS: 50,
							},
					...
					},
					{
						...
						Doc: &resource.IndexableDocument{
							...
							Folder: "xxx",
							Fields: map[string]any{
								external.DASHBOARD_PANEL_TYPES:       []string{"timeseries"},
								external.DASHBOARD_ERRORS_TODAY:      40,
								external.DASHBOARD_VIEWS_LAST_1_DAYS: 100,
							},
							Tags: []string{"aa"},
							Labels: map[string]string{
								"region":                           "east",
		...
		rsp, err = index.Search(ctx, NewStubAccessClient(map[string]bool{"dashboards": true}), &resourcepb.ResourceSearchRequest{
			...
			Limit:  100000,
			Fields: []string{external.DASHBOARD_ERRORS_TODAY, external.DASHBOARD_VIEWS_LAST_1_DAYS, "fieldThatDoesntExist"},
			SortBy: []*resourcepb.ResourceSearchRequest_Sort{
				{Field: "fields." + external.DASHBOARD_VIEWS_LAST_1_DAYS, Desc: true},
			},
		}, nil, nil)
		require.NoError(t, err)
		require.Equal(t, 2, len(rsp.Results.Columns))
		require.Equal(t, external.DASHBOARD_ERRORS_TODAY, rsp.Results.Columns[0].Name)
		require.Equal(t, external.DASHBOARD_VIEWS_LAST_1_DAYS, rsp.Results.Columns[1].Name)
