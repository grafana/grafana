...
type VectorBackend interface {
	// ResolveCollection maps a wire-level (group, resource) pair to its
	// embedding_collections catalog entry. found=false means the pair is
	// not provisioned — callers surface NOT_FOUND. Snapshots are cached
	// in-process for up to a minute, so new catalog rows take effect
	// without a restart.
	...
	// is the partition key (Collection.PartitionKey), not the wire name.
