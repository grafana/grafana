...
func TestReconciler_WatchOnlySeedsCheckpoint(t *testing.T) {
	...
	assert.Equal(t, seed, s.seedRV, "established checkpoint ignores further watch notifications")
}
...
// Late writes within KV's lookback window are recovered from storage even
// when a restart discards all watch and retry state after seed persistence.
func TestReconciler_SeedLookback(t *testing.T) {
	for _, restart := range []bool{false, true} {
		t.Run(fmt.Sprintf("restart=%t", restart), func(t *testing.T) {
			...
			vec.onSetLatestRV = func(rv int64) {
				...
				// Deliver after the seed was copied, while persistence is in flight.
				delivered := make(chan struct{})
				go func() {
					s.observeWrite(&resource.WrittenEvent{Key: &late.Key, ResourceVersion: late.ResourceVersion})
					close(delivered)
				}()
				select {
				case <-delivered:
			...
			s.sweep(t.Context())
			assert.True(t, vec.hasUpsertFor("ns", dashRes, "late"))
			assert.True(t, vec.hasUpsertFor("ns", dashRes, "dash"))
			assert.Equal(t, snowflakeRV(100), vec.latestRV)
			assert.Nil(t, st.lastCalledWith[len(st.lastCalledWith)-1], "initial sweep keeps the lookback")
		})
	}
}
