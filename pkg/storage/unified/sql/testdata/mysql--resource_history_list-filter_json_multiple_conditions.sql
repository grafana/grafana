SELECT
    ...
    WHERE kv.`action`  != 3 
        ...
        AND kv.`resource`  = 'dashboards'
            AND JSON_UNQUOTE(JSON_EXTRACT(kv.`value`, '$.metadata.labels.environment')) = 'prod'
            AND (JSON_UNQUOTE(JSON_EXTRACT(kv.`value`, '$.spec.title')) != 'Untitled' OR JSON_UNQUOTE(JSON_EXTRACT(kv.`value`, '$.spec.title')) IS NULL)
