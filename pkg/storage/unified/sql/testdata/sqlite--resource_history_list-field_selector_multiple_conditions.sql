SELECT
    ...
    WHERE kv."action"  != 3 
        ...
        AND kv."resource"  = 'dashboards'
            AND CAST(json_extract(kv."value", '$.metadata.name') AS TEXT) = 'dashboard-1'
            AND (CAST(json_extract(kv."value", '$.spec.title') AS TEXT) != 'Untitled' OR CAST(json_extract(kv."value", '$.spec.title') AS TEXT) IS NULL)
