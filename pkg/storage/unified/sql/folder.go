...
type folderInfo struct {
	...
	Title string `json:"title"` // original display name
...
func (b *backend) updateFolderTree(ctx context.Context, tx db.Tx, namespace string) error {
	_, err := tx.ExecContext(ctx, "DELETE FROM resource_folder WHERE namespace=?", namespace)
	if err != nil {
		return err
	}

	group, err := b.dialect.Ident("group") // group must be quoted!
	if err != nil {
		return err
	}
	query := "SELECT guid,name,folder,name" +
		" FROM resource" +
		" WHERE " + group + "=? AND resource=? AND namespace=?" +
		" ORDER BY slug asc"
	args := []interface{}{"folder.grafana.app", "folders", namespace}
...
func insertFolderInfo(ctx context.Context, tx db.Tx, namespace string, folder *folderInfo, isDetached bool) error {
