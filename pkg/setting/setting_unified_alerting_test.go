...
func TestCfg_ReadUnifiedAlertingSettings(t *testing.T) {
	...
	// It sets the correct defaults.
	{
		...
		require.True(t, cfg.UnifiedAlerting.GrafanaManagedAlertsEnabled)
	}
	...
	// With peers set, it correctly parses them.
	{
		...
		require.NoError(t, err)
		_, err = s.NewKey("grafana_managed_alerts_enabled", "false")
		require.NoError(t, err)
		...
		require.False(t, cfg.UnifiedAlerting.GrafanaManagedAlertsEnabled)
	}
