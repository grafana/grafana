...
type Cfg struct {
	...
	ProvisioningAllowInsecureToken            bool // allow http:// repository URLs together with a token (cleartext credentials); local/dev only
...
func (cfg *Cfg) readProvisioningSettings(iniFile *ini.File) error {
	...
	cfg.ProvisioningAllowInsecureToken = iniFile.Section("provisioning").Key("allow_insecure_token").MustBool(false)
