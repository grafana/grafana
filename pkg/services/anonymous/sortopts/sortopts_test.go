...
func TestSorter_Filters(t *testing.T) {
	require.Equal(t, SortOptionsByQueryParam["userAgent-asc"].Filter[0].OrderBy(), "LOWER(anon_device.user_agent) ASC")
	require.Equal(t, SortOptionsByQueryParam["userAgent-desc"].Filter[0].OrderBy(), "LOWER(anon_device.user_agent) DESC")
}
...
func TestSorter_OrderBy(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "anon_device.user_agent case sensitive desc",
			fields: fields{
				Field:         "email",
				LowerCase:     false,
				Descending:    true,
