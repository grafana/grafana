...
func TestIasdntegration_SQLStore_RemoveOrgUser(t *testing.T) {
	...
	t.Run("tt.desc", func(t *testing.T) {
		...
		_, err := usrSvc.Create(context.Background(), createFirstUserCmd)
		require.NoError(t, err)
		...
		_, err = orgUserStore.GetUserOrgList(context.Background(), &org.GetUserOrgListQuery{
			UserID: 1,
		})
		require.NoError(t, err)
		...
		// get metric usage
		stats, err := orgUserStore.GetUsageMetrics(context.Background())
		require.NoError(t, err)
		assert.Equal(t, int64(1), stats.UserAccountsWithNoRole)
	})
}
