...
func TestBundler_Build_bundlesPanelScreenshot(t *testing.T) {
	png := append(append([]byte{}, pngMagic...), []byte("pixel-data")...)

	blob, err := NewBundler().Build(nil, &harcapture.Buffer{}, nil, nil, nil, nil, nil,
		WithPanelScreenshot(png, nil))
	require.NoError(t, err)

	files := readTarGz(t, blob)
	// Stored byte-for-byte: panel.png is the picture the user's browser produced, so re-encoding or
	// otherwise altering it would defeat the point of shipping it.
	require.Equal(t, png, files["panel.png"])
	require.NotContains(t, files, "panel-png-error.txt")
}

func TestBundler_Build_omitsPanelScreenshotWhenNotRequested(t *testing.T) {
	// No option supplied: the artifact is simply absent, with no error marker -- a bundle generated by a
	// client that never captured a screenshot is not a failure.
	blob, err := NewBundler().Build(nil, &harcapture.Buffer{}, nil, nil, nil, nil, nil)
	require.NoError(t, err)
...
func TestBundler_Build_recordsPanelScreenshotError(t *testing.T) {
	// A client-side capture failure is recorded, and must not cost the bundle its other artifacts.
	blob, err := NewBundler().Build(nil, &harcapture.Buffer{}, json.RawMessage(`{"id":1}`), nil, nil, nil, nil,
		WithPanelScreenshot(nil, errors.New("panel screenshot timed out after 10000ms")))
	require.NoError(t, err)

	files := readTarGz(t, blob)
	require.NotContains(t, files, "panel.png")
	require.Contains(t, string(files["panel-png-error.txt"]), "timed out after 10000ms")
	require.Contains(t, files, "panel.json", "an unrelated artifact still ships")
}

func TestBundler_Build_rejectsNonPNGScreenshot(t *testing.T) {
	// The bytes are client-supplied, so something that isn't a PNG must not be bundled under a .png
	// name -- a reader opening panel.png should never get a file their image viewer cannot decode.
	blob, err := NewBundler().Build(nil, &harcapture.Buffer{}, nil, nil, nil, nil, nil,
		WithPanelScreenshot([]byte("<html>not a png</html>"), nil))
	require.NoError(t, err)

	files := readTarGz(t, blob)
	require.NotContains(t, files, "panel.png")
	require.Contains(t, string(files["panel-png-error.txt"]), "not a PNG")
}

func TestBundler_Build_rejectsOversizedScreenshot(t *testing.T) {
	oversized := append(append([]byte{}, pngMagic...), bytes.Repeat([]byte("x"), maxPanelScreenshotBytes)...)

	blob, err := NewBundler().Build(nil, &harcapture.Buffer{}, nil, nil, nil, nil, nil,
		WithPanelScreenshot(oversized, nil))
	require.NoError(t, err)

	files := readTarGz(t, blob)
	// Dropped whole rather than truncated: a partial PNG would not decode, so a marker is more useful.
	require.NotContains(t, files, "panel.png")
	require.Contains(t, string(files["panel-png-error.txt"]), "exceeds")
}

func TestBuildDashboard_bundlesPerPanelScreenshots(t *testing.T) {
	png := append(append([]byte{}, pngMagic...), []byte("panel-1-pixels")...)

	blob, err := NewBundler().BuildDashboard(json.RawMessage(`{"uid":"d1"}`), []DashboardPanel{
		{ID: 1, Title: "Visible", HARBuffer: &harcapture.Buffer{}, ScreenshotPNG: png},
		// Off-screen panels are the normal case on a tall dashboard: no bytes, a reason instead.
		{ID: 2, Title: "Off screen", HARBuffer: &harcapture.Buffer{},
			ScreenshotErr: errors.New("Panel not in DOM")},
		// Nothing requested at all -- neither an image nor an error belongs in the manifest.
		{ID: 3, Title: "No capture", HARBuffer: &harcapture.Buffer{}},
	})
	require.NoError(t, err)

	files := readTarGz(t, blob)
	require.Equal(t, png, files["panels/1-visible/panel.png"], "the captured panel gets its own image")
	require.NotContains(t, files, "panels/2-off-screen/panel.png")
	require.NotContains(t, files, "panels/3-no-capture/panel.png")

	entries := manifestPanelsByID(t, files)
	require.Equal(t, len(png), entries[1].ScreenshotBytes)
	require.Empty(t, entries[1].ScreenshotError)
	// The gap is explained in the manifest rather than left silent, so a reader knows why panel 2 has
	// no image without having to guess.
	require.Contains(t, entries[2].ScreenshotError, "Panel not in DOM")
	require.Zero(t, entries[2].ScreenshotBytes)
	require.Empty(t, entries[3].ScreenshotError)
	require.Zero(t, entries[3].ScreenshotBytes)
}

func TestBuildDashboard_rejectsNonPNGPanelScreenshot(t *testing.T) {
	blob, err := NewBundler().BuildDashboard(json.RawMessage(`{"uid":"d1"}`), []DashboardPanel{
		{ID: 1, Title: "Bad", HARBuffer: &harcapture.Buffer{}, ScreenshotPNG: []byte("nope")},
	})
	require.NoError(t, err)

	files := readTarGz(t, blob)
	require.NotContains(t, files, "panels/1-bad/panel.png")
	require.Contains(t, manifestPanelsByID(t, files)[1].ScreenshotError, "not a PNG")
}

// manifestPanelsByID decodes manifest.json and keys its panel entries by id.
func manifestPanelsByID(t *testing.T, files map[string][]byte) map[int64]manifestPanelEntry {
	...
	out := map[int64]manifestPanelEntry{}
	for _, e := range manifest.Panels {
		out[e.ID] = e
	}
	return out
}
...
func TestBundler_Build_bundlesPanelData(t *testing.T) {
	...
	require.NotContains(t, files, "paneldata-error.txt")
}
...
func TestBundler_Build_rejectsInvalidPanelData(t *testing.T) {
	...
	files := readTarGz(t, blob)
	require.NotContains(t, files, "paneldata.json")
	require.Contains(t, string(files["paneldata-error.txt"]), "not valid JSON")
	require.Contains(t, files, "panel.json", "an unrelated artifact still ships")
}

func TestBundler_Build_rejectsOversizedPanelData(t *testing.T) {
