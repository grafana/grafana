...
// AutoAuthorizerRegistration is an interface for dynamically registering authorizers for APIs.
// It mimics the autoregister.AutoAPIServiceRegistration interface, but for authorizers.
type AutoAuthorizerRegistration interface {
	// AddAuthorizerForGroup registers an authorizer for an API.
	AddAuthorizerForGroup(gv schema.GroupVersion)
	// RemoveAuthorizerForGroup removes the authorizer for an API.
