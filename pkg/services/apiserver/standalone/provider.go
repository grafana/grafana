...
type APIServerProvider interface {
	GetAPIServerBuilder(group string, version string) (builder.APIGroupBuilder, error)
}

// Zero dependency provider for testing
func GetDummyAPIProvider() APIServerProvider {
	return &DummyAPIProvider{}
}

type DummyAPIProvider struct{}

func (p *DummyAPIProvider) GetAPIServerBuilder(group string, version string) (builder.APIGroupBuilder, error) {
	if version != "v0alpha1" {
