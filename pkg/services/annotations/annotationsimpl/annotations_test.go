...
func TestIntegrationAnnotationListingWithInheritedRBAC(t *testing.T) {
	...
	setupFolderStructure := func() (db.DB, dashboards.DashboardService) {
		...
		fStore := folderimpl.ProvideStore(sql)
		folderStore := folderimpl.ProvideDashboardFolderStore(sql)
		folderSvc, err := folderimpl.ProvideService(fStore, ac, bus.ProvideBus(tracing.InitializeTracerForTest()), dashStore,
			folderStore, sql, features, supportbundlestest.NewFakeBundleService(), cfg, nil, tracing.InitializeTracerForTest(), nil)
		require.NoError(t, err)
