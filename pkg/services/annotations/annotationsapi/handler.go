...
//
// Summing double counts a tag on any annotation that exists in both stores. That only happens
// for a record written to the new store whose legacy copy was not removed, so counts can read
// slightly high until legacy is retired.
