...
// GetByLegacyID returns the annotation for annotationID, including one that has been
// soft-deleted (with metadata.deletionTimestamp set). It goes through the /search route
// so tombstones are visible; the plain List path hides them. Callers map a tombstone to
// ErrGone. Returns ErrNotFound when no row exists at all.
//
...
// tags. It includes soft-deleted annotations (with metadata.deletionTimestamp set) so the
// caller can suppress un-purged legacy copies; the caller decides what to do with them.
