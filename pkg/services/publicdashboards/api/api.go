...
func (api *Api) ListPublicDashboards(c *model.ReqContext) response.Response {
...
func (api *Api) GetPublicDashboard(c *model.ReqContext) response.Response {
...
func (api *Api) CreatePublicDashboard(c *model.ReqContext) response.Response {
	// exit if we don't have a valid dashboardUid
	dashboardUid := web.Params(c.Req)[":dashboardUid"]
	if !tokens.IsValidShortUID(dashboardUid) {
...
func (api *Api) UpdatePublicDashboard(c *model.ReqContext) response.Response {
	// exit if we don't have a valid dashboardUid
	dashboardUid := web.Params(c.Req)[":dashboardUid"]
	if !tokens.IsValidShortUID(dashboardUid) {
...
func (api *Api) DeletePublicDashboard(c *model.ReqContext) response.Response {
	uid := web.Params(c.Req)[":uid"]
	if !tokens.IsValidShortUID(uid) {
		return response.Err(ErrInvalidUid.Errorf("UpdatePublicDashboard: invalid Uid %s", uid))
	}
