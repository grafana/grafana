...
func TestGetManagedStreams(t *testing.T) {
	...
	require.NoError(t, err)
	require.Len(t, managedChannels, 9) // 9 hardcoded testdata streams.
	...
	managedChannels, err = runner.GetManagedChannels(1)
	require.NoError(t, err)
	require.Len(t, managedChannels, 12) // 12 hardcoded testdata streams + 3 test channels.
	require.Equal(t, "stream/test1/cpu1", managedChannels[9].Channel)
	require.Equal(t, "stream/test1/cpu2", managedChannels[10].Channel)
	require.Equal(t, "stream/test2/cpu1", managedChannels[11].Channel)
	...
	managedChannels, err = runner.GetManagedChannels(1)
	require.NoError(t, err)
	require.Len(t, managedChannels, 12) // Not affected by other org.
}
