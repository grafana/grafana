...
func (s *service) extensionsServerConfig(apiConfig apiServerConfig) (*apiextensionsapiserver.Config, error) {
	serverConfig := *apiConfig.Config
	...
	if err := apiConfig.APIEnablement.ApplyTo(&serverConfig, apiextensionsapiserver.DefaultAPIResourceConfigSource(), apiextensionsapiserver.Scheme); err != nil {
		return nil, err
	}
	...
	config := &apiextensionsapiserver.Config{
		...
		ExtraConfig: apiextensionsapiserver.ExtraConfig{
			...
			MasterCount:          1,
			ServiceResolver:      &serviceResolver{apiConfig.SharedInformers.Core().V1().Services().Lister()},
			AuthResolverWrapper:  webhook.NewDefaultAuthenticationInfoResolverWrapper(apiConfig.ProxyTransport, apiConfig.Config.EgressSelector, serverConfig.LoopbackClientConfig, oteltrace.NewNoopTracerProvider()),
		},
	}
