...
func (l *LibraryElementService) createHandler(c *models.ReqContext) response.Response {
	...
	if cmd.FolderUID != nil {
		if *cmd.FolderUID == "" {
			...
			folder, err := l.folderService.Get(ctx, &folder.GetFolderQuery{OrgID: c.OrgID, UID: cmd.FolderUID})
	...
	if element.FolderID != 0 {
		ctx := appcontext.WithUser(c.Req.Context(), c.SignedInUser)
		folder, err := l.folderService.Get(ctx, &folder.GetFolderQuery{OrgID: c.OrgID, ID: &element.FolderID})
		if err != nil {
