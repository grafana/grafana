...
func TestIntegration_FolderConsumer_FoldersInUse(t *testing.T) {
	...
	t.Run("reports nothing until the org is marked healed", func(t *testing.T) {
	...
	t.Run("reports folders once the org is marked healed", func(t *testing.T) {
		c, store, heal := consumerSetup(t)
		healInsert(t, store, "panel", 2, "f2")
		require.NoError(t, heal.kv.Set(context.Background(), healOrgID, healKVNamespace, healKVKey, healKVDone))
	...
	// The marker gates cleanup, not the toggle: enabling the heal is not the same as it
	// having finished.
