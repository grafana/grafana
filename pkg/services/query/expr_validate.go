...
func (s *ServiceImpl) ValidatePipeline(ctx context.Context, user identity.Requester, reqDTO dtos.MetricRequest) (*queryV0.PipelineValidation, error) {
	...
	return s.expressionService.ValidatePipeline(ctx, &exprReq)
}

func ValidatePipeline(ctx context.Context, log log.Logger, dscache datasources.CacheService, exprService *expr.Service, reqDTO dtos.MetricRequest, qsDatasourceClientBuilder dsquerierclient.QSDatasourceClientBuilder, headers map[string]string) (*queryV0.PipelineValidation, error) {
	s := &ServiceImpl{
		log:                        log,
	...
	return s.ValidatePipeline(ctx, user, reqDTO)
}
