...
func (dc *DatasourceProvisioner) apply(ctx context.Context, cfg *configs) error {
	...
	for _, ds := range cfg.Datasources {
		...
		dsResult, err := dc.store.GetDataSource(ctx, cmd)
		...
		if errors.Is(err, datasources.ErrDataSourceNotFound) {
			...
			inserted, err := dc.store.AddDataSource(ctx, insertCmd)
			if err != nil {
				return err
			}

			for _, correlation := range ds.Correlations {
				if insertCorrelationCmd, err := makeCreateCorrelationCommand(correlation, inserted.UID, insertCmd.OrgID); err == nil {
			...
			updateCmd := createUpdateCommand(ds, dsResult.ID)
			...
			for _, correlation := range ds.Correlations {
				if insertCorrelationCmd, err := makeCreateCorrelationCommand(correlation, dsResult.UID, updateCmd.OrgID); err == nil {
					correlationsToInsert = append(correlationsToInsert, insertCorrelationCmd)
				} else {
					dc.log.Error("failed to parse correlation", "correlation", correlation)
					return err
...
func (dc *DatasourceProvisioner) deleteDatasources(ctx context.Context, dsToDelete []*deleteDatasourceConfig) error {
	for _, ds := range dsToDelete {
		...
		result, err := dc.store.GetDataSource(ctx, getDsQuery)
		if err != nil && !errors.Is(err, datasources.ErrDataSourceNotFound) {
			return err
		}
