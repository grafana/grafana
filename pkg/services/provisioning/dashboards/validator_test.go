...
func TestDuplicatesValidator(t *testing.T) {
	...
	t.Run("Duplicates validator should restrict write access only for readers with duplicates", func(t *testing.T) {
		...
		fakeService.On("SaveProvisionedDashboard", mock.Anything, mock.Anything, mock.Anything).Return(&dashboards.Dashboard{}, nil).Times(6)
		fakeStore := &fakeDashboardStore{}
		...
		reader3, err := NewDashboardFileReader(cfg3, logger, nil, fakeStore, featuremgmt.WithFeatures(), fakeFoldersService)
		...
		r, err = NewDashboardFileReader(cfg3, logger, nil, fakeStore, featuremgmt.WithFeatures(), fakeFoldersService)
		require.NoError(t, err)
