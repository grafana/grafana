...
func TestDashboardFileReader(t *testing.T) {
	...
	t.Run("Reading dashboards from disk", func(t *testing.T) {
		t.Run("Can read default dashboard", func(t *testing.T) {
			...
			reader, err := NewDashboardFileReader(cfg, logger, nil, fakeStore, featuremgmt.WithFeatures(), fakeFoldersService)
		...
		t.Run("Can read default dashboard and replace old version in database", func(t *testing.T) {
			...
			reader, err := NewDashboardFileReader(cfg, logger, nil, fakeStore, featuremgmt.WithFeatures(), nil)
			reader.dashboardProvisioningService = fakeService
			require.NoError(t, err)
		...
		t.Run("Invalid configuration should return error", func(t *testing.T) {
			...
			_, err := NewDashboardFileReader(cfg, logger, nil, nil, featuremgmt.WithFeatures(), nil)
		...
		t.Run("Two dashboard providers should be able to provisioned the same dashboard without uid", func(t *testing.T) {
			...
			reader1, err := NewDashboardFileReader(cfg1, logger, nil, fakeStore, featuremgmt.WithFeatures(), fakeFoldersService)
			...
			reader2, err := NewDashboardFileReader(cfg2, logger, nil, fakeStore, featuremgmt.WithFeatures(), fakeFoldersService)
	...
	t.Run("Should not create new folder if folder name is missing", func(t *testing.T) {
		...
		r, err := NewDashboardFileReader(cfg, logger, nil, nil, featuremgmt.WithFeatures(), nil)
		require.NoError(t, err)
	...
	t.Run("can get or Create dashboard folder", func(t *testing.T) {
		...
		r, err := NewDashboardFileReader(cfg, logger, nil, fakeStore, featuremgmt.WithFeatures(), nil)
		require.NoError(t, err)
