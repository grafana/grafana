...
// fakeLibraryPanelServiceCall records the arguments of one ImportLibraryPanelsForDashboard call.
type fakeLibraryPanelServiceCall struct {
...
// fakeLibraryPanelService records calls made to it instead of touching a database, so tests
// can assert that dashboard provisioning asks it to import a dashboard's embedded library panels.
type fakeLibraryPanelService struct {
	calls []fakeLibraryPanelServiceCall
}

var _ librarypanels.Service = (*fakeLibraryPanelService)(nil)

func (f *fakeLibraryPanelService) ImportLibraryPanelsForDashboard(_ context.Context, _ identity.Requester, libraryPanels *simplejson.Json, panels []any, folderID int64, folderUID string) error {
	f.calls = append(f.calls, fakeLibraryPanelServiceCall{libraryPanels: libraryPanels, panels: panels, folderID: folderID, folderUID: folderUID})
	return nil
}
...
func TestIntegrationDashboardFileReader(t *testing.T) {
	...
	t.Run("Reading dashboards from disk", func(t *testing.T) {
		...
		t.Run("Dashboard referencing a library panel creates the missing library panel", func(t *testing.T) {
			setup()
			cfg.Options["path"] = withLibraryPanel
			...
			require.Len(t, libPanelService.calls, 1)
			call := libPanelService.calls[0]
			assert.Equal(t, "lib-panel-uid-1", call.libraryPanels.Get("lib-panel-uid-1").Get("uid").MustString())
			assert.Equal(t, "My Library Panel", call.libraryPanels.Get("lib-panel-uid-1").Get("name").MustString())
			require.Len(t, call.panels, 1)
			panel := call.panels[0].(map[string]any)
