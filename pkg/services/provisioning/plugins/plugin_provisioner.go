...
func Provision(ctx context.Context, configDirectory string, store Store, pluginRegistry plugins.Registry, pluginSettings pluginsettings.Service) error {
	logger := log.New("provisioning.plugins")
	ap := PluginProvisioner{
		log:            logger,
		cfgProvider:    newConfigReader(logger, pluginRegistry),
		store:          store,
