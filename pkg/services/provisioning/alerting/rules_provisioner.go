...
func (prov *defaultAlertRuleProvisioner) Provision(ctx context.Context,
	...
	for _, file := range files {
		// When the file opts out of provenance, rules are created without it so they
		// remain editable in the UI (they're still reset to file state on reprovision).
		provenance := alert_models.ProvenanceFile
		if file.DisableProvenance {
			provenance = alert_models.ProvenanceNone
		}
		for _, group := range file.Groups {
			...
			for _, rule := range group.Rules {
				...
				err = prov.provisionRule(ctx, u, rule, provenance)
				if err != nil {
					return err
				}
			}
		...
		for _, deleteRule := range file.DeleteRules {
			err := prov.ruleService.DeleteAlertRule(ctx, provisionerUser(deleteRule.OrgID), deleteRule.UID, provenance)
			if err != nil {
				return err
			}
		}
...
func (prov *defaultAlertRuleProvisioner) provisionRule(
	...
	provenance alert_models.Provenance) error {
	...
	if err != nil && !errors.Is(err, alert_models.ErrAlertRuleNotFound) {
		...
		_, err = prov.ruleService.CreateAlertRule(ctx, user, rule, provenance)
	} else {
		prov.logger.Debug("updating rule", "uid", rule.UID, "org", rule.OrgID)
		_, err = prov.ruleService.UpdateAlertRule(ctx, user, rule, provenance)
	}
	return err
}
