...
func TestMapperRegistry_ExtGrafanaAppWildcard(t *testing.T) {
	reg := NewMapperRegistry()

	t.Run("matches any resource under ext.grafana.app groups", func(t *testing.T) {
		for _, tc := range []struct {
			group    string
			resource string
		}{
			{"customcrdtest.ext.grafana.app", "widgets"},
			{"myapp.ext.grafana.app", "foos"},
			{"another.ext.grafana.app", "bars"},
		} {
			mapping, ok := reg.Get(tc.group, tc.resource)
			require.True(t, ok, "Get(%q, %q) should find mapping", tc.group, tc.resource)
			require.NotNil(t, mapping)

			// The dynamic translation should use the actual K8s resource name
			assert.Equal(t, tc.resource, mapping.Resource())
			assert.Equal(t, tc.resource+":uid:", mapping.Prefix())
			assert.True(t, mapping.HasFolderSupport())

			action, ok := mapping.Action("list")
			assert.True(t, ok)
			assert.Equal(t, tc.resource+":read", action)
		}
	})

	t.Run("does not match nested-dot groups", func(t *testing.T) {
		_, ok := reg.Get("nested.dot.ext.grafana.app", "widgets")
		assert.False(t, ok)
	})

	t.Run("GetAll returns empty for wildcard-resource-only groups", func(t *testing.T) {
		all := reg.GetAll("customcrdtest.ext.grafana.app")
		assert.Empty(t, all, "GetAll should not return the wildcard resource template")
	})
}

func TestMapper_WildcardResource(t *testing.T) {
	m := mapper{
		"test.grafana.app": {
			"known":             newResourceTranslation("known", "uid", false, nil),
			wildcardResourceKey: translation{attribute: "uid", folderSupport: true},
		},
	}
	var reg MapperRegistry = m

	t.Run("exact resource match preferred over wildcard", func(t *testing.T) {
		mapping, ok := reg.Get("test.grafana.app", "known")
		require.True(t, ok)
		assert.Equal(t, "known", mapping.Resource())
	})

	t.Run("wildcard fallback for unknown resource", func(t *testing.T) {
		mapping, ok := reg.Get("test.grafana.app", "dynamic")
		require.True(t, ok)
		assert.Equal(t, "dynamic", mapping.Resource())
		assert.Equal(t, "dynamic:uid:", mapping.Prefix())
		assert.True(t, mapping.HasFolderSupport())

		action, ok := mapping.Action("get")
		assert.True(t, ok)
		assert.Equal(t, "dynamic:read", action)
	})

	t.Run("GetAll excludes wildcard template", func(t *testing.T) {
		all := reg.GetAll("test.grafana.app")
		require.Len(t, all, 1)
		assert.Equal(t, "known", all[0].Resource())
	})
}
