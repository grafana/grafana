...
func TestService_checkPermission(t *testing.T) {
	...
	testCases := []testCase{
		...
		{
			name:        "permissions:type:delegate requires delegate scope on the target action",
			permissions: []accesscontrol.Permission{},
			check: checkRequest{
		...
		{
			name: "permissions:type:delegate passes when caller holds delegate on the same action",
			permissions: []accesscontrol.Permission{
				{
					Action: "users.roles:add",
...
func TestService_mapping(t *testing.T) {
	...
	testCases := []testCase{
		...
		{
			name: "permissions resource uses folder field as RBAC action override",
			input: &authzv1.CheckRequest{
				Group:    "iam.grafana.app",
				Resource: "permissions",
				Name:     "delegate",
...
func TestService_BatchCheck(t *testing.T) {
	...
	testCases := []testCase{
		...
		{
			name: "permissions resource uses folder field as RBAC action in batch checks",
			req: &authzv1.BatchCheckRequest{
				Namespace: "org-12",
				Subject:   "user:test-uid",
				Checks: []*authzv1.BatchCheckItem{
