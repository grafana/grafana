...
func ProvideService(cfg *setting.Cfg, features featuremgmt.FeatureToggles, authenticator interceptors.Authenticator, tracer tracing.Tracer, registerer prometheus.Registerer) (Provider, error) {
	s := &gPRCServerService{
		...
		enabled:     features.IsEnabledGlobally(featuremgmt.FlagGrpcServer) || cfg.GRPCServer.Enabled,
	...
	opts := []grpc.ServerOption{
		...
		grpc.ChainUnaryInterceptor(
			...
			interceptors.LoggingUnaryInterceptor(s.logger, s.enabled), // needs to be registered after tracing interceptor to get trace id
