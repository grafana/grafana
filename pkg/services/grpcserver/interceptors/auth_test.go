...
func initAuth(role org.RoleType, permissions ...[]accesscontrol.Permission) (jwt.PluginAuthService, Authenticator) {
	...
	verification, err := jwt.ProvideVerificationService(cfg)
	if err != nil {
		panic(err)
	}
	features := featuremgmt.WithFeatures(featuremgmt.FlagJwtTokenGeneration)
	pluginAuth, err := jwt.ProvidePluginAuthService(cfg, features, kvstore.NewFakeSecretsKVStore(), verification)
	if err != nil {
		panic(err)
	}
