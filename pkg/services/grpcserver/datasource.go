...
func (s *DatasourceServer) getDataSourceFromQuery(ctx context.Context, user *models.SignedInUser, skipCache bool, query *simplejson.Json) (*datasources.DataSource, error) {
...
// TODO: can this be shared with the query service?
func (s *DatasourceServer) decryptSecureJsonDataFn(ctx context.Context) func(ds *datasources.DataSource) map[string]string {
	return func(ds *datasources.DataSource) map[string]string {
		decryptedJsonData, _ := s.dataSourceService.DecryptedValues(ctx, ds)
