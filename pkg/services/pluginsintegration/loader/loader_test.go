...
func TestLoader_Load_Signature_RootURL(t *testing.T) {
	t.Run("Private signature verification ignores trailing slash in root URL", func(t *testing.T) {
		...
		procPrvdr := fakes.NewFakeBackendProcessProvider()
		procMgr := fakes.NewFakeProcessManager()
		cfg := &config.Cfg{GrafanaAppURL: defaultAppURL, Features: featuremgmt.WithManager()}
...
func TestLoader_AngularClass(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			l := newLoaderWithOpts(t, &config.Cfg{AngularSupportEnabled: true, Features: featuremgmt.WithManager()}, loaderDepOpts{
...
func TestLoader_Load_Angular(t *testing.T) {
	...
	for _, cfgTc := range []struct {
		...
		{name: "angular support enabled", cfg: &config.Cfg{AngularSupportEnabled: true, Features: featuremgmt.WithManager()}},
		{name: "angular support disabled", cfg: &config.Cfg{AngularSupportEnabled: false, Features: featuremgmt.WithManager()}},
	} {
...
func newLoader(t *testing.T, cfg *config.Cfg, reg registry.Service, proc process.Manager,
	...
	return ProvideService(pipeline.ProvideDiscoveryStage(cfg,
		finder.NewLocalFinder(false, featuremgmt.WithManager()), reg),
