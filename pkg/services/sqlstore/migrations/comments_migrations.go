...
func addCommentGroupMigrations(mg *Migrator) {
	commentGroupTable := Table{
		...
		Columns: []*Column{
			...
			{Name: "content_type", Type: DB_NVarchar, Length: 10, Nullable: false},
		...
		Indices: []*Index{
			{Cols: []string{"org_id", "content_type", "object_id"}, Type: UniqueIndex},
		},
	}
	mg.AddMigration("create comment group table", NewAddTableMigration(commentGroupTable))
	mg.AddMigration("add index comment_group.org_id_content_type_object_id", NewAddIndexMigration(commentGroupTable, commentGroupTable.Indices[0]))
}
