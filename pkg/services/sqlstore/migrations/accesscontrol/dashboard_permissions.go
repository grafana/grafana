...
AddManagedFolderAlertActionsRepeatMigrator ensures that alerting permissions that have already been added won't get added twice.
*/
func AddManagedFolderAlertActionsRepeatMigrationFixed(mg *migrator.Migrator) {
	mg.AddMigration(managedFolderAlertActionsRepeatMigratorFixedID, &managedFolderAlertActionsRepeatMigratorFixed{})
}

type managedFolderAlertActionsRepeatMigratorFixed struct {
	migrator.MigrationBase
}

func (m *managedFolderAlertActionsRepeatMigratorFixed) SQL(dialect migrator.Dialect) string {
	return CodeMigrationSQL
}

func (m *managedFolderAlertActionsRepeatMigratorFixed) Exec(sess *xorm.Session, mg *migrator.Migrator) error {
	var ids []interface{}
	if err := sess.SQL("SELECT id FROM role WHERE name LIKE 'managed:%'").Find(&ids); err != nil {
		return err
	}
	...
	for id, a := range mapped {
		for scope, p := range a {
			...
			//
			// see issue for explanation of the issue
			if hasFolderView(p) {
