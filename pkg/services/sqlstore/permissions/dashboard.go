...
func (f *accessControlDashboardPermissionFilter) buildClauses() {
	...
	// useSelfContainedPermissions is true if the signed in user's permissions are populated and the accessActionSets feature flag is not enabled
	useSelfContainedPermissions := len(f.user.GetPermissions()) > 0 && !f.features.IsEnabledGlobally(featuremgmt.FlagAccessActionSets)
