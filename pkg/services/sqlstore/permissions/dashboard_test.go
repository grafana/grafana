	...
	for _, tt := range tests {
		...
		for _, features := range []featuremgmt.FeatureToggles{featuremgmt.WithFeatures()} {
			...
			t.Run(tt.desc+" with features "+strings.Join(keys, ","), func(t *testing.T) {
				filter := permissions.NewAccessControlDashboardPermissionFilter(usr, tt.permission, tt.queryType, features, recursiveQueriesAreSupported, store.GetDialect(), 7)
	...
	for _, tc := range testCases {
		...
		for _, features := range []featuremgmt.FeatureToggles{featuremgmt.WithFeatures()} {
			...
			t.Run(tc.desc+" with features "+strings.Join(keys, ","), func(t *testing.T) {
				...
				require.NoError(t, err)
				filter := permissions.NewAccessControlDashboardPermissionFilter(usr, tc.permission, tc.queryType, features, recursiveQueriesAreSupported, db.GetDialect(), 7)
				var result []string
