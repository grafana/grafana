...
func TestIntegration_DashboardPermissionFilter(t *testing.T) {
	...
	for _, tt := range tests {
		...
		for _, features := range []*featuremgmt.FeatureManager{featuremgmt.WithManager(), featuremgmt.WithManager(featuremgmt.FlagPermissionsFilterRemoveSubquery)} {
...
func TestIntegration_DashboardNestedPermissionFilter(t *testing.T) {
	...
	for _, tc := range testCases {
		...
		for _, features := range []*featuremgmt.FeatureManager{featuremgmt.WithManager(tc.features...), featuremgmt.WithManager(append(tc.features, featuremgmt.FlagPermissionsFilterRemoveSubquery)...)} {
			m := features.GetEnabled(context.Background())
			keys := make([]string, 0, len(m))
			for k := range m {
				keys = append(keys, k)
