...
// TestDBConfig holds connection details for a temporary test database.
...
// CreateTemporaryTestDB creates an isolated temporary database for use in tests.
// For SQLite, a unique temp file is created. For MySQL/Postgres, a unique database
// is created on the configured server. Cleanup (file removal or DROP DATABASE) is
// registered via tb.Cleanup automatically.
func CreateTemporaryTestDB(tb TestingTB) (*TestDBConfig, error) {
	tb.Helper()
	tdb, err := createTemporaryDatabase(tb)
	if err != nil {
		return nil, err
	}
	...
	// Populate host/port/user/password/database for MySQL and Postgres
	// so callers can configure a Grafana server's [database] section.
	dbType := getTestDBType()
	switch dbType {
	case "mysql":
		cfg.Host = env("MYSQL_HOST", "localhost")
		cfg.Port = env("MYSQL_PORT", "3306")
		cfg.User = env("MYSQL_USER", "root")
		cfg.Password = env("MYSQL_PASSWORD", "rootpass")
		// Extract the database name from the connection string's path component.
		// The name was generated by createTemporaryDatabase as "grafana_test_<hex>".
		cfg.Database = extractMySQLDatabase(tdb.Conn)
	case "postgres":
		cfg.Host = env("POSTGRES_HOST", "localhost")
		cfg.Port = env("POSTGRES_PORT", "5432")
		cfg.User = env("POSTGRES_USER", "grafanatest")
		cfg.Password = env("POSTGRES_PASSWORD", "grafanatest")
		cfg.Database = extractPostgresDatabase(tdb.Conn)
	}

	return cfg, nil
}

func extractMySQLDatabase(connStr string) string {
...
func extractPostgresDatabase(connStr string) string {
	// Format: user=X password=Y host=H port=P dbname=NAME sslmode=S
