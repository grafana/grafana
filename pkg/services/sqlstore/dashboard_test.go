...
func TestDashboardDataAccess(t *testing.T) {
	...
	Convey("Testing DB", t, func() {
		...
		Convey("Given saved dashboard", func() {
			...
			Convey("Validate dashboard before save", func() {
				Convey("Should return not found error if no dashboard is found by id", func() {
					cmd := m.SaveDashboardCommand{
						OrgId:     1,
						Overwrite: true,
						Dashboard: simplejson.NewFromAny(map[string]interface{}{
				...
				Convey("Should not be able to overwrite dashboard by id in another org", func() {
				...
				Convey("Should be able to validate dashboard before save using uid and return id", func() {
					cmd := m.SaveDashboardCommand{
						OrgId: 1,
						Dashboard: simplejson.NewFromAny(map[string]interface{}{
							"uid":   savedDash.Uid,
				...
				Convey("Should be able to update dashboard with the same title and folder id", func() {
					firstCmd := m.SaveDashboardCommand{
						OrgId: 1,
						Dashboard: simplejson.NewFromAny(map[string]interface{}{
							"uid":   "randomHash",
							"title": "folderId",
				...
				Convey("Should be able to validate before update using uid without id and overwrite", func() {
					cmd := m.SaveDashboardCommand{
						OrgId: 1,
						Dashboard: simplejson.NewFromAny(map[string]interface{}{
							"uid":     savedDash.Uid,
