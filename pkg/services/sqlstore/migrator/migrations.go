...
// This method adds series of migrations to convert existing unique key to PRIMARY KEY.
func ConvertUniqueKeyToPrimaryKey(mg *Migrator, tableName string, uniqueKey Index, finalTable Table) {
	if tableName == "" {
	...
	mysqlMigration1 := NewRawSQLMigration("").Mysql(fmt.Sprintf(`
	  ...
	  DROP INDEX %s,
	  ADD PRIMARY KEY (%s);
	`, tableName, uniqueKey.XName(tableName), strings.Join(uniqueKey.Cols, ",")))
	mysqlMigration1.Condition = &IfColumnExistsCondition{TableName: tableName, ColumnName: "my_row_id"}
	mg.AddMigration(fmt.Sprintf("drop my_row_id and add primary key to %s table if my_row_id exists (auto-generated mysql column)", tableName), mysqlMigration1)
	...
	mg.AddMigration(fmt.Sprintf("drop %s unique index from %s table if it exists (mysql)", uniqueKey.XName(tableName), tableName), mysqlMigration2)
	...
	mg.AddMigration(fmt.Sprintf("add primary key to %s table if it doesn't exist (mysql)", tableName), mysqlMigration3)

	// postgres and sqlite statements are idempotent so we can have only one condition-less migration
	migration := addPrimaryKeyMigration{tableName: tableName, uniqueKey: uniqueKey, table: finalTable}

	mg.AddMigration(fmt.Sprintf("add primary key to %s table (postgres and sqlite)", tableName), &migration)
}
