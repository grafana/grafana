...
const BENCHMARK_DASHBOARD_COUNT = 10_000
...
func Benchmark_DashboardIndex(b *testing.B) {
	d := []dashboard{}

	for i := 0; i < BENCHMARK_DASHBOARD_COUNT; i++ {
	...
	b.Run("applyDashboardEvent delete", func(b *testing.B) {
		b.ResetTimer()
		index, _, _, loader := initTestIndexFromDashes(b, d)

		// clear the dashboards in the loader to force a delete
	...
	b.Run("applyDashboardEvent update", func(b *testing.B) {
		b.ResetTimer()
	...
	b.Run("reIndexFromScratch", func(b *testing.B) {
		b.ResetTimer()
		index, _, _, _ := initTestIndexFromDashes(b, d)
		for i := 0; i < b.N; i++ {
