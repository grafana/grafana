...
func TestFeatureToggleSetup(t *testing.T) {
	...
	assert.Equal(t, map[string]bool{"a": true, "b": true, "c": true}, ft.GetEnabled(context.Background()))

	// Explicit values
	ft = WithFeatures("a", true, "b", false)
	assert.True(t, ft.IsEnabled("a"))
	assert.False(t, ft.IsEnabled("b"))
	assert.Equal(t, map[string]bool{"a": true}, ft.GetEnabled(context.Background()))
