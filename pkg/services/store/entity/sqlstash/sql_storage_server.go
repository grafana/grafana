...
func (s *sqlEntityServer) getReadFields(withBody, withStatus bool) []string {
	fields := []string{
		"guid",
		"key",
...
func (s *sqlEntityServer) poller(stream chan *entity.Entity) {
	...
	t := s.clock.NewTicker(5 * time.Second)
	defer t.Stop()
...
func (s *sqlEntityServer) poll(ctx context.Context, since int64, out chan *entity.Entity) (int64, error) {
	...
	fields := s.getReadFields(rr.WithBody, rr.WithStatus)
