...
func (s *sqlEntityServer) Delete(ctx context.Context, r *entity.DeleteEntityRequest) (*entity.DeleteEntityResponse, error) {
	...
	ret := &entity.DeleteEntityResponse{
		Status: entity.DeleteEntityResponse_ERROR,
	}
	...
	err = s.sqlDB.WithTx(ctx, txOpts, func(_ context.Context, tx db.Tx) error {
		...
		entityDelete, err := readEntity(tx, s.sqlDialect, key, r.PreviousVersion, true, true)
		...
		if _, err = tmplDBExec(tx, sqlEntityLabelsDelete, delLabelsReq); err != nil {
		...
		if _, err = tmplDBExec(tx, sqlEntityDelete, delEntityReq); err != nil {
	...
	if err != nil {
		...
		ctxLogger.Error("delete entity tx", "msg", err.Error())
		return ret, nil
	}
