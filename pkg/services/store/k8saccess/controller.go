...
func (s *k8sDashboardService) Run(ctx context.Context) error {
	...
	dashboardInformer.AddEventHandler(cache.ResourceEventHandlerFuncs{
		AddFunc: func(obj interface{}) {
			...
			if existing, err := s.orig.GetDashboard(context.Background(), &dashboards.GetDashboardQuery{UID: dto.Dashboard.UID, OrgID: dto.OrgID}); err == nil && existing.Version >= dto.Dashboard.Version {
			...
			_, err = s.orig.SaveDashboard(context.Background(), dto, true)
			if err != nil {
				s.log.Error("orig.SaveDashboard failed", err)
				return
			}
		...
		DeleteFunc: func(obj interface{}) {
			...
			existing, err := s.orig.GetDashboard(context.Background(), &dashboards.GetDashboardQuery{UID: dto.Dashboard.UID, OrgID: dto.OrgID})
			// no dashboard found, nothing to delete
			if err != nil {
				return
			}

			if err := s.orig.DeleteDashboard(ctx, existing.ID, existing.OrgID); err != nil {
...
func (s *k8sDashboardService) IsDisabled() bool {
...
func (s *k8sDashboardService) getSignedInUser(ctx context.Context, orgID int64, userID int64) (*user.SignedInUser, error) {
