...
type crawlerScheduleOptions struct {
	...
	crawlerMode      thumbs.CrawlerMode
...
func ProvideService(cfg *setting.Cfg, features featuremgmt.FeatureToggles,
	...
	gl *live.GrafanaLive, db db.DB, authSetupService thumbs.CrawlerAuthSetupService,
	dashboardService dashboards.DashboardService, searchService searchV2.SearchService,
	dsPermissionsService permissions.DatasourcePermissionsService, licensing models.Licensing) thumbs.Service {
	if !features.IsEnabled(featuremgmt.FlagDashboardPreviews) {
		return &dummyService{}
	}
	logger := log.New("previews_service")
	dashboardThumbsService := provideDashboardThumbService(db)
	...
	t := &thumbService{
		...
		scheduleOptions: crawlerScheduleOptions{
			...
			crawlerMode:      thumbs.CrawlerModeThumbs,
			thumbnailKind:    thumbs.ThumbnailKindDefault,
...
func (hs *thumbService) parseImageReq(c *models.ReqContext, checkSave bool) *thumbs.PreviewRequest {
	params := web.Params(c.Req)

	kind, err := thumbs.ParseThumbnailKind(params[":kind"])
	if err != nil {
...
func (hs *thumbService) UpdateThumbnailState(c *models.ReqContext) {
	...
	var body = &thumbs.UpdateThumbnailStateRequest{}
	...
	err = hs.thumbnailRepo.UpdateThumbnailState(c.Req.Context(), body.State, thumbs.DashboardThumbnailMeta{
		DashboardUID: req.UID,
		OrgId:        req.OrgID,
		Theme:        req.Theme,
		Kind:         thumbs.ThumbnailKindDefault,
	})
...
func (hs *thumbService) GetImage(c *models.ReqContext) {
	...
	res, err := hs.thumbnailRepo.GetThumbnail(c.Req.Context(), thumbs.DashboardThumbnailMeta{
		DashboardUID: req.UID,
		OrgId:        req.OrgID,
		Theme:        req.Theme,
		Kind:         thumbs.ThumbnailKindDefault,
...
func (hs *thumbService) hasAccessToPreview(c *models.ReqContext, res *thumbs.DashboardThumbnail, req *thumbs.PreviewRequest) bool {
...
func (hs *thumbService) GetDashboardPreviewsSetupSettings(c *models.ReqContext) thumbs.DashboardPreviewsSetupConfig {
	return hs.getDashboardPreviewsSetupSettings(c.Req.Context())
}

func (hs *thumbService) getDashboardPreviewsSetupSettings(ctx context.Context) thumbs.DashboardPreviewsSetupConfig {
...
func (hs *thumbService) getSystemRequirements(ctx context.Context) thumbs.DashboardPreviewsSystemRequirements {
	...
	if err != nil {
		...
		return thumbs.DashboardPreviewsSystemRequirements{
			Met: false,
		}
	}
...
func (hs *thumbService) SetImage(c *models.ReqContext) {
	...
	_, err = hs.thumbnailRepo.SaveFromBytes(c.Req.Context(), fileBytes, getMimeType(handler.Filename), thumbs.DashboardThumbnailMeta{
		DashboardUID: req.UID,
		OrgId:        req.OrgID,
		Theme:        req.Theme,
		Kind:         req.Kind,
	}, thumbs.DashboardVersionForManualThumbnailUpload, dsUids)
...
func (hs *thumbService) StartCrawler(c *models.ReqContext) response.Response {
	...
	if cmd.Mode == "" {
		cmd.Mode = thumbs.CrawlerModeThumbs
	}

	go hs.runOnDemandCrawl(context.Background(), cmd.Theme, cmd.Mode, thumbs.ThumbnailKindDefault, rendering.AuthOpts{
		OrgID:   c.OrgID,
		UserID:  c.UserID,
		OrgRole: c.OrgRole,
	})
...
func (hs *thumbService) runOnDemandCrawl(parentCtx context.Context, theme models.Theme, mode thumbs.CrawlerMode, kind thumbs.ThumbnailKind, authOpts rendering.AuthOpts) {
	if !hs.canRunCrawler {
		return
	}
