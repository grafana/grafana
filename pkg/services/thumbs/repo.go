...
func (r *sqlThumbnailRepository) saveFromFile(ctx context.Context, filePath string, meta thumbsmodel.DashboardThumbnailMeta, dashboardVersion int, dsUids []string) (int64, error) {
...
func (r *sqlThumbnailRepository) saveFromBytes(ctx context.Context, content []byte, mimeType string, meta thumbsmodel.DashboardThumbnailMeta, dashboardVersion int, dsUids []string) (int64, error) {
...
func (r *sqlThumbnailRepository) updateThumbnailState(ctx context.Context, state thumbsmodel.ThumbnailState, meta thumbsmodel.DashboardThumbnailMeta) error {
	return r.store.UpdateThumbnailState(ctx, &thumbsmodel.UpdateThumbnailStateCommand{
		State:                  state,
		DashboardThumbnailMeta: meta,
	})
}

func (r *sqlThumbnailRepository) getThumbnail(ctx context.Context, meta thumbsmodel.DashboardThumbnailMeta) (*thumbsmodel.DashboardThumbnail, error) {
	query := &thumbsmodel.GetDashboardThumbnailCommand{
		DashboardThumbnailMeta: meta,
	}
	return r.store.GetThumbnail(ctx, query)
}

func (r *sqlThumbnailRepository) findDashboardsWithStaleThumbnails(ctx context.Context, theme thumbsmodel.Theme, kind thumbsmodel.ThumbnailKind) ([]*thumbsmodel.DashboardWithStaleThumbnail, error) {
	return r.store.FindDashboardsWithStaleThumbnails(ctx, &thumbsmodel.FindDashboardsWithStaleThumbnailsCommand{
