...
func TestAuthenticate(t *testing.T) {
	...
	t.Run("no clients registered returns UNSUPPORTED_CREDENTIALS", func(t *testing.T) {
		svc := NewService(tracing.InitializeTracerForTest())

		resp, err := svc.Authenticate(context.Background(), req)
		require.NoError(t, err)
		assert.Equal(t, authnv1.AuthenticateCode_AUTHENTICATE_CODE_UNSUPPORTED_CREDENTIALS, resp.Code)
	})
	...
	t.Run("single client Test false returns UNSUPPORTED_CREDENTIALS", func(t *testing.T) {
		svc := NewService(tracing.InitializeTracerForTest())
		svc.RegisterClient(&mockClient{
			name:       "test-client",
			testResult: false,
	...
	t.Run("client returns UNSUPPORTED_CREDENTIALS mid-auth falls through to next", func(t *testing.T) {
		svc := NewService(tracing.InitializeTracerForTest())
		svc.RegisterClient(&mockClient{
			name:       "declining-client",
			testResult: true,
			authResponse: &authnv1.AuthenticateResponse{
				Code: authnv1.AuthenticateCode_AUTHENTICATE_CODE_UNSUPPORTED_CREDENTIALS,
			},
		})
		svc.RegisterClient(&mockClient{
	...
	t.Run("all clients decline via UNSUPPORTED_CREDENTIALS returns UNSUPPORTED_CREDENTIALS", func(t *testing.T) {
		svc := NewService(tracing.InitializeTracerForTest())
		svc.RegisterClient(&mockClient{
			name:       "client-a",
			testResult: true,
