...
func (om *OrgMigration) migrateAndSaveDashboard(ctx context.Context, dashID int64, alerts []*legacymodels.Alert) (*migmodels.DashboardUpgrade, migmodels.OrgMigrationSummary, error) {
	summary := migmodels.OrgMigrationSummary{}
	if du := om.state.PopDashboardUpgrade(dashID); du != nil {
...
func (om *OrgMigration) migrateOrg(ctx context.Context) (migmodels.OrgMigrationSummary, error) {
	om.log.Info("Migrating alerts for organisation")

	summary, err := om.migrateOrgAlerts(ctx)
	if err != nil {
		om.log.Warn("Failed to migrate alerts", "error", err)
		om.state.AddError(fmt.Errorf("migrate alerts: %w", err).Error())
	}

	channelSummary, err := om.migrateOrgChannels(ctx)
	if err != nil {
...
func (om *OrgMigration) migrateOrgAlerts(ctx context.Context) (migmodels.OrgMigrationSummary, error) {
	summary := migmodels.OrgMigrationSummary{}
	...
	for dashID, alerts := range mappedAlerts {
		du, sum, err := om.migrateAndSaveDashboard(ctx, dashID, alerts)
		if err != nil {
			return summary, err
		}
...
func (om *OrgMigration) migrateOrgChannels(ctx context.Context) (migmodels.OrgMigrationSummary, error) {
	summary := migmodels.OrgMigrationSummary{}
