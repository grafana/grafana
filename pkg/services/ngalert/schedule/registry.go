...
type alertRulesRegistry struct {
	...
	folderTitles models.FolderTitleMap
	mu           sync.Mutex
}

// all returns all rules in the registry.
func (r *alertRulesRegistry) all() ([]*models.AlertRule, models.FolderTitleMap) {
	r.mu.Lock()
	defer r.mu.Unlock()
...
func (r *alertRulesRegistry) set(rules []*models.AlertRule, folders models.FolderTitleMap) diff {
	r.mu.Lock()
	defer r.mu.Unlock()
