...
func jitterOffsetInTicks(interval int64, group ngmodels.AlertRuleGroupKey, baseInterval time.Duration, strategy JitterStrategy) int64 {
	if strategy == JitterNever {
		return 0
	}

	itemFrequency := interval / int64(baseInterval.Seconds())
	offset := jitterHash(group.OrgID, group.RuleGroup, group.NamespaceUID) % uint64(itemFrequency)
...
func jitterHash(orgID int64, groupName string, namespaceUID string) uint64 {
	ls := data.Labels{
		"name":  groupName,
		"file":  namespaceUID,
