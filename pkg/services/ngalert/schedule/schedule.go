	...
	for {
		...
		case ctx := <-evalCh:
			...
			evaluate := func(attempt int64) error {
				...
				err = sch.sendAlerts(alertRule.OrgID, alerts)
				if err != nil {
...
func (sch *schedule) sendAlerts(orgID int64, alerts apimodels.PostableAlerts) error {
	return sch.notifier.PutAlerts(orgID, alerts)
}
