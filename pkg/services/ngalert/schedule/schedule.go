		...
		case ctx := <-evalCh:
			...
			evaluate := func(attempt int64) error {
				...
				for _, r := range results {
					sch.log.Info("alert rule result", "title", alertRule.Title, "key", key, "attempt", attempt, "now", ctx.now, "duration", end.Sub(start), "instance", r.Instance, "state", r.State.String())
					cmd := models.SaveAlertInstanceCommand{DefinitionOrgID: key.OrgID, DefinitionUID: key.UID, State: models.InstanceStateType(r.State.String()), Labels: models.InstanceLabels(r.Instance), LastEvalTime: ctx.now}
					err := sch.store.SaveAlertInstance(&cmd)
					if err != nil {
						sch.log.Error("failed saving alert instance", "title", alertRule.Title, "key", key, "attempt", attempt, "now", ctx.now, "instance", r.Instance, "state", r.State.String(), "error", err)
					}
				}
				transitionedStates := stateTracker.ProcessEvalResults(key.UID, results, condition)
