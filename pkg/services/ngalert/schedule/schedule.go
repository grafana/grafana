...
func (sch *schedule) ruleRoutine(grafanaCtx context.Context, key ngmodels.AlertRuleKey, evalCh <-chan *evaluation, updateCh <-chan ruleVersionAndPauseStatus) error {
	...
	evaluate := func(ctx context.Context, attempt int64, e *evaluation, span tracing.Span, retry bool) error {
			...
			func() {
				...
				for attempt := int64(1); attempt <= sch.maxAttempts; attempt++ {
					...
					if tracingCtx.Err() != nil {
						span.SetStatus(codes.Error, "rule evaluation cancelled")
						span.End()
						logger.Error("Skip evaluation and updating the state because the context has been cancelled", "version", ctx.rule.Version, "attempt", attempt, "now", ctx.scheduledAt)
						return
					}

					retry := attempt < sch.maxAttempts
					err := evaluate(tracingCtx, attempt, ctx, span, retry)
					...
					logger.Error("Failed to evaluate rule", "version", ctx.rule.Version, "attempt", attempt, "now", ctx.scheduledAt)
					select {
					case <-tracingCtx.Done():
						logger.Error("Context has been cancelled while backing off", "version", ctx.rule.Version, "attempt", attempt, "now", ctx.scheduledAt)
						return
