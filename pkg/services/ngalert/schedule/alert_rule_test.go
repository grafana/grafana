...
func blankRuleForTests(ctx context.Context, key models.AlertRuleKeyWithGroup) *alertRule {
	...
	return newAlertRule(ctx, key, nil, false, 0, 0, 0, nil, st, nil, nil, nil, log.NewNopLogger(), nil, featuremgmt.WithFeatures(), nil, nil)
}
...
func TestRuleRoutine(t *testing.T) {
	...
	t.Run("when a message is sent to update channel", func(t *testing.T) {
		...
		require.Greaterf(t, expectedToBeSent, 0, "State manager was expected to return at least one state that can be expired.AlertRuleKey")
	...
	t.Run("when evaluation fails", func(t *testing.T) {
		...
		sch.maxAttempts = 3
		sch.initialRetryDelay = 1 * time.Second
		sch.maxRetryDelay = 1 * time.Second
		ruleStore.PutRule(context.Background(), rule)
