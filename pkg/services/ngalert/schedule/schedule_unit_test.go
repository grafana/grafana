...
func TestProcessTicks(t *testing.T) {
	...
	evaluator := eval.NewEvaluatorFactory(setting.UnifiedAlertingSettings{}, cacheServ, expr.ProvideService(&setting.Cfg{ExpressionsEnabled: true}, nil, nil, featuremgmt.TestFeatureToggles(), nil, tracing.InitializeTracerForTest()), &pluginstore.FakePluginStore{})
...
func setupScheduler(t *testing.T, rs *fakeRulesStore, is *state.FakeInstanceStore, registry *prometheus.Registry, senderMock *SyncAlertsSenderMock, evalMock eval.EvaluatorFactory) *schedule {
	...
	if evalMock == nil {
		evaluator = eval.NewEvaluatorFactory(setting.UnifiedAlertingSettings{}, nil, expr.ProvideService(&setting.Cfg{ExpressionsEnabled: true}, nil, nil, featuremgmt.TestFeatureToggles(), nil, tracing.InitializeTracerForTest()), &pluginstore.FakePluginStore{})
	}
