...
func (api *API) authorize(method, path string) web.Handler {
	...
	case http.MethodDelete + "/api/alertmanager/grafana/api/v2/silence/{SilenceId}": // Delete endpoint is used for silence expiration.
		eval = ac.EvalAll(
			ac.EvalAny(ac.EvalPermission(ac.ActionAlertingInstanceRead), ac.EvalPermission(ac.ActionAlertingSilencesRead)),
			ac.EvalAny(ac.EvalPermission(ac.ActionAlertingInstanceUpdate), ac.EvalPermission(ac.ActionAlertingSilencesWrite)),
		)
	...
	case http.MethodPost + "/api/alertmanager/grafana/api/v2/silences":
		eval = ac.EvalAll(
			ac.EvalAny(ac.EvalPermission(ac.ActionAlertingInstanceRead), ac.EvalPermission(ac.ActionAlertingSilencesRead)),
			ac.EvalAny(ac.EvalPermission(ac.ActionAlertingInstanceCreate), ac.EvalPermission(ac.ActionAlertingInstanceUpdate), ac.EvalPermission(ac.ActionAlertingSilencesCreate), ac.EvalPermission(ac.ActionAlertingSilencesWrite)),
		)
