...
// ConvertPrometheusSrv converts Prometheus rules into Grafana rules
// and returns them in a Prometheus-compatible format.
//
// This service allows rules from Mimir to be imported into Grafana.
// It works similarly to the provisioning API: once a rule group is created,
// it is marked as "provisioned" (using a provenance mechanism)
// and cannot be edited in the UI.
//
// Note: Only rule groups originally imported from Prometheus-compatible sources are returned.
// Rule groups created directly in Grafana (or other APIs) are excluded
// because their original rule definitions are unavailable.
// When converting a rule group from Prometheus to Grafana, the YAML definition of the rule is saved
// with the Grafana rule and used for read requests.
...
// RouteConvertPrometheusGetRules returns all Grafana-managed alert rules across all namespaces (folders)
...
// RouteConvertPrometheusDeleteNamespace deletes all rule groups imported from a Prometheus-compatible source
...
func (srv *ConvertPrometheusSrv) RouteConvertPrometheusGetNamespace(c *contextmodel.ReqContext, namespaceTitle string) response.Response {
	...
	if errors.Is(err, dashboards.ErrFolderAccessDenied) {
		// If there is no folder, GetNamespaceInRootByTitle returns ErrFolderAccessDenied.
...
func (srv *ConvertPrometheusSrv) RouteConvertPrometheusGetRuleGroup(c *contextmodel.ReqContext, namespaceTitle string, group string) response.Response {
	...
	namespace, err := srv.ruleStore.GetNamespaceInRootByTitle(c.Req.Context(), namespaceTitle, c.SignedInUser.GetOrgID(), c.SignedInUser)
	if errors.Is(err, dashboards.ErrFolderAccessDenied) {
		// If there is no such folder, GetNamespaceInRootByTitle returns ErrFolderAccessDenied.
		// We should return 404 in this case, otherwise mimirtool does not work correctly.
		return response.Empty(http.StatusNotFound)
	}
	if err != nil {
...
//
// If a rule group with the same name already exists and was not originally imported from a Prometheus-compatible source,
