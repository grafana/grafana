...
func getDatasourceByUID(ctx *model.ReqContext, cache datasources.CacheService, expectedType apimodels.Backend) (*datasources.DataSource, error) {
...
// The request context
// 1. overwrites the underlying response writer used by a *model.ReqContext because AlertingProxy needs to intercept
...
func (p *AlertingProxy) createProxyContext(ctx *model.ReqContext, request *http.Request, response *response.NormalResponse) *model.ReqContext {
	cpy := *ctx
	cpyMCtx := *cpy.Context
