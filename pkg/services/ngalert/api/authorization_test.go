...
func TestAuthorizeRuleChanges(t *testing.T) {
	...
	for _, testCase := range testCases {
		t.Run(testCase.name, func(t *testing.T) {
			...
			result, err := authorizeRuleChanges(folder, groupChanges, func(evaluator ac.Evaluator) bool {
...
func TestAuthorizeRuleDelete(t *testing.T) {
	folder := randFolder()
	namespace := randNamespace()
	namespaceIdScope := dashboards.ScopeFoldersProvider.GetResourceScope(strconv.FormatInt(folder.Id, 10))
