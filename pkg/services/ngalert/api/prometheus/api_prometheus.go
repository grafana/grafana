...
func (srv PrometheusSrv) RouteGetRuleStatuses(c *contextmodel.ReqContext) response.Response {
	...
	}, srv.applyRuleStatus, srv.applyAlertState)

	return response.JSON(ruleResponse.HTTPStatusCode(), ruleResponse)
}

func (srv *PrometheusSrv) applyRuleStatus(source *ngmodels.AlertRule, toMutate *apimodels.AlertingRule) {
	status, ok := srv.status.Status(source.GetKey())
	...
	toMutate.Rule.Health = status.Health
	toMutate.Rule.LastError = errorOrEmpty(status.LastError)
	toMutate.Rule.LastEvaluation = status.EvaluationTimestamp
	toMutate.Rule.EvaluationTime = status.EvaluationDuration.Seconds()
}

func (srv *PrometheusSrv) applyAlertState(
	source *ngmodels.AlertRule,
	toMutate *apimodels.AlertingRule,
	stateFilterSet map[eval.State]struct{},
	matchers labels.Matchers,
	labelOptions []ngmodels.LabelOption,
) (map[string]int64, map[string]int64) {
	states := srv.manager.GetStatesForRuleUID(source.OrgID, source.UID)
...
func toRuleGroup(log log.Logger, groupKey ngmodels.AlertRuleGroupKey, folderFullPath string, rules []*ngmodels.AlertRule, limitAlerts int64, stateFilterSet map[eval.State]struct{}, matchers labels.Matchers, labelOptions []ngmodels.LabelOption, ruleStatusMutator RuleStatusMutator, ruleAlertStateMutator RuleAlertStateMutator) (*apimodels.RuleGroup, map[string]int64) {
	...
	for _, rule := range rules {
		...
		if len(rule.NotificationSettings) > 0 {
			alertingRule.Rule.NotificationSettings = (*apimodels.AlertRuleNotificationSettings)(&rule.NotificationSettings[0])
		}
		...
		if alertingRule.Rule.Health == "error" || alertingRule.Rule.Health == "nodata" {
			rulesTotals[alertingRule.Rule.Health] += 1
		}
		...
		newGroup.EvaluationTime = alertingRule.Rule.EvaluationTime
		newGroup.LastEvaluation = alertingRule.Rule.LastEvaluation
	}
