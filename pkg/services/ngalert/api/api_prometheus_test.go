...
func TestRouteGetAlertStatuses(t *testing.T) {
	...
	t.Run("with no alerts", func(t *testing.T) {
		...
		c := &model.ReqContext{Context: &web.Context{Req: req}, SignedInUser: &user.SignedInUser{OrgID: orgID}}
...
func TestRouteGetRuleStatuses(t *testing.T) {
	...
	require.NoError(t, err)
	c := &model.ReqContext{Context: &web.Context{Req: req}, SignedInUser: &user.SignedInUser{OrgID: orgID, OrgRole: org.RoleViewer}}
