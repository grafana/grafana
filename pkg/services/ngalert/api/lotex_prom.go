...
func (p *LotexProm) RouteGetAlertStatuses(ctx *model.ReqContext) response.Response {
...
func (p *LotexProm) RouteGetRuleStatuses(ctx *model.ReqContext) response.Response {
	endpoints, err := p.getEndpoints(ctx)
	if err != nil {
		return ErrResp(http.StatusInternalServerError, err, "")
	}
...
func (p *LotexProm) getEndpoints(ctx *model.ReqContext) (*promEndpoints, error) {
