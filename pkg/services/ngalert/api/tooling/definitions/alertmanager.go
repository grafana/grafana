...
func (m MergeResult) LogContext() []any {
	if len(m.RenameResources.Receivers) == 0 && len(m.RenameResources.TimeIntervals) == 0 {
		return nil
	}
	logCtx := make([]any, 0, 4)
	if len(m.RenameResources.Receivers) > 0 {
		rcvBuilder := strings.Builder{}
		for from, to := range m.RenameResources.Receivers {
	...
	if len(m.RenameResources.TimeIntervals) > 0 {
		intervalBuilder := strings.Builder{}
		for from, to := range m.RenameResources.TimeIntervals {
