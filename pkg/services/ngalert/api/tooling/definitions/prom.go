...
type GetGrafanaRuleStatusesParams struct {
	...
	// Filter rules by their static labels (not alert instance labels). Each value is a JSON-encoded Prometheus matcher (for example, {"type":0,"name":"severity","value":"critical"}).
	// in: query
	// required: false
