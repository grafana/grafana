...
func (c *EvaluationCoordinator) Updates(ctx context.Context) <-chan bool {
	...
	go func() {
		...
		if current {
			c.log.Info("Primary node, starting alert rule evaluation")
		} else {
			c.log.Info("Secondary node, alert rule evaluation disabled")
		}
		...
		for {
			...
			case <-ticker.C:
				...
				if next {
					c.log.Info("Became primary node, starting alert rule evaluation")
				} else {
					c.log.Info("Became secondary node, stopping alert rule evaluation")
				}
				current = next
				sendLatest(updates, current)
			}
