...
func NewAlertmanager(ctx context.Context, cfg AlertmanagerConfig, store stateStore, crypto Crypto, autogenFn AutogenFn, metrics *metrics.RemoteAlertmanager, tracer tracing.Tracer) (*Alertmanager, error) {
	...
	// this has one known side-effect - autogenerated notification policies.
	// Anyway, the default flag is sent to remote Alertmanager for informational purpose.
	// So, we can tolerate edge cases when user assigns a default contact point to a rule and set overrides.
	err = func() error {
		defaultCfg, err := am.parseConfiguration(ctx, []byte(cfg.DefaultConfig))
		if err != nil {
