...
func NewAlertmanager(
	...
	err = func() error {
		...
		// nil = the org-independent default hash; AddAutogenConfig fills the autogen
		// root with the Alertmanager route defaults, so an org that overrides timings
		// correctly reads as non-default.
		if err := notifier.AddAutogenConfig(ctx, am.log, emptyAutogenRuleStore{}, am.orgID, &parsed.AlertmanagerConfig, notifier.IgnoreInvalidReceivers, am.features, nil); err != nil {
