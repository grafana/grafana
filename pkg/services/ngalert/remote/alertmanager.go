...
func NewAlertmanager(ctx context.Context, cfg AlertmanagerConfig, store stateStore, crypto Crypto, autogenFn AutogenFn, metrics *metrics.RemoteAlertmanager, tracer tracing.Tracer) (*Alertmanager, error) {
	...
	err = func() error {
		defaultCfg, err := am.buildConfiguration(ctx, []byte(cfg.DefaultConfig), 0, util.Pointer(true))
		if err != nil {
...
func (am *Alertmanager) CompareAndSendConfiguration(ctx context.Context, config *models.AlertConfiguration) error {
	payload, err := am.buildConfiguration(ctx, []byte(config.AlertmanagerConfiguration), config.CreatedAt, nil)
	if err != nil {
...
func (am *Alertmanager) buildConfiguration(ctx context.Context, raw []byte, createdAtEpoch int64, isDefaultPtr *bool) (remoteClient.UserGrafanaConfig, error) {
	...
	if err != nil {
		return remoteClient.UserGrafanaConfig{}, fmt.Errorf("unable to marshall configuration: %w", err)
	}
...
func (am *Alertmanager) SaveAndApplyConfig(ctx context.Context, cfg *apimodels.PostableUserConfig) error {
	...
	payload, err := am.buildConfiguration(ctx, rawCopy, time.Now().Unix(), nil)
	if err != nil {
		return fmt.Errorf("unable to build configuration: %w", err)
	}
...
func (am *Alertmanager) SaveAndApplyDefaultConfig(ctx context.Context) error {
	...
	payload, err := am.buildConfiguration(ctx, []byte(am.defaultConfig), time.Now().Unix(), util.Pointer(true))
	if err != nil {
		return fmt.Errorf("unable to build default configuration: %w", err)
	}
