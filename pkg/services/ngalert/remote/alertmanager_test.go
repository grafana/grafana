	...
	tests := []struct {
		...
		},
		{
			...
			expCfg: func() *client.UserGrafanaConfig {
				...
				// never inherits from the user's root route; here no override is set, so
				// it carries the Alertmanager route defaults.
				autogenGroupWait := model.Duration(dispatch.DefaultRouteOpts.GroupWait)
				autogenGroupInterval := model.Duration(dispatch.DefaultRouteOpts.GroupInterval)
				autogenRepeatInterval := model.Duration(dispatch.DefaultRouteOpts.RepeatInterval)
				testAutogenRoutes.AlertmanagerConfig.Route.Routes = append([]*definition.Route{
					{
						...
						GroupWait:      &autogenGroupWait,
						GroupInterval:  &autogenGroupInterval,
						RepeatInterval: &autogenRepeatInterval,
