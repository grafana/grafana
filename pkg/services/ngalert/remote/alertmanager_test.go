...
func TestNewAlertmanager(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(tt *testing.T) {
			...
			am, err := NewAlertmanager(context.Background(), cfg, nil, secretsService, NoopAutogenFn, m, tracing.InitializeTracerForTest())
			if test.expErr != "" {
...
func TestApplyConfig(t *testing.T) {
	...
	m := metrics.NewRemoteAlertmanagerMetrics(prometheus.NewRegistry())
	am, err := NewAlertmanager(ctx, cfg, fstore, secretsService, NoopAutogenFn, m, tracing.InitializeTracerForTest())
	require.NoError(t, err)
	...
	am, err = NewAlertmanager(context.Background(), cfg, fstore, secretsService, NoopAutogenFn, m, tracing.InitializeTracerForTest())
	require.NoError(t, err)
	require.NoError(t, am.ApplyConfig(ctx, config))
	require.Equal(t, 2, configSyncs)
	...
	_, err = NewAlertmanager(context.Background(), cfg, fstore, secretsService, errAutogenFn, m, tracing.InitializeTracerForTest())
...
func TestCompareAndSendConfiguration(t *testing.T) {
	...
	require.NoError(t, err)
	testValue := []byte("test")
	secretsService := fakes.NewMockService(t)
	secretsService.EXPECT().Decrypt(mock.Anything, mock.Anything).RunAndReturn(
...
func TestIntegrationRemoteAlertmanagerGetStatus(t *testing.T) {
	...
	secretsService := secretsManager.SetupTestService(t, fakes.NewFakeSecretsStore())
	m := metrics.NewRemoteAlertmanagerMetrics(prometheus.NewRegistry())
	am, err := NewAlertmanager(ctx, cfg, nil, secretsService, NoopAutogenFn, m, tracing.InitializeTracerForTest())
	require.NoError(t, err)
