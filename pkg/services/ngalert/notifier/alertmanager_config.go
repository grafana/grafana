...
func (moa *MultiOrgAlertmanager) gettableUserConfigFromAMConfigString(ctx context.Context, orgID int64, config string, withAutogen bool) (definitions.GettableUserConfig, error) {
	...
	if withAutogen {
		// TODO: To ensure that the routes are correct, we should be getting this directly from the in-memory AM.
		// Since this is not currently simple, we validate the notification settings in a similar way to when we POST.
