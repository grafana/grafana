...
func (moa *MultiOrgAlertmanager) SaveAndApplyAlertmanagerConfiguration(ctx context.Context, org int64, config definitions.PostableUserConfig) error {
	...
	// SaveAndApplyAlertmanagerConfiguration does not allow to save ExtraConfigurations,
	// so we need to ensure that the previous configuration is not lost.
	if previousConfig != nil {
		previousCfg, err := Load([]byte(previousConfig.AlertmanagerConfiguration))
		if err == nil && len(previousCfg.ExtraConfigs) > 0 {
			config.ExtraConfigs = previousCfg.ExtraConfigs
		}
	}
...
func (moa *MultiOrgAlertmanager) modifyAndApplyExtraConfiguration(
	...
	currentConfigRecord, err := moa.configStore.GetLatestAlertmanagerConfiguration(ctx, org)
	if err != nil {
		return fmt.Errorf("failed to get current configuration: %w", err)
	}

	cfg, err := Load([]byte(currentConfigRecord.AlertmanagerConfiguration))
	if err != nil {
