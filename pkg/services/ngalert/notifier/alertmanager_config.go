...
func (moa *MultiOrgAlertmanager) gettableUserConfigFromAMConfigString(ctx context.Context, orgID int64, config string, withAutogen bool) (definitions.GettableUserConfig, error) {
	...
	if withAutogen {
		// TODO: To ensure that this route is correct, we either need to get this directly from the in-memory AM or we
		// have to validate the notification settings in a similar way to when we POST. Otherwise, broken settings (e.g.
		// a receiver that doesn't exist) will cause th config displayed to be different than the config used.
		validator := NewNotificationSettingsValidator(cfg.AlertmanagerConfig)
		autogenRoute, err := newAutogeneratedRoute(ctx, moa.logger, orgID, moa.configStore, cfg.AlertmanagerConfig.Route.Receiver, cfg.AlertmanagerConfig.Receivers, validator, true)
		if err != nil {
