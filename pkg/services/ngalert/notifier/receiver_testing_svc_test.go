...
func TestReceiverTestingService_TestByIntegrationUID(t *testing.T) {
	...
	authorizedUser := &user.SignedInUser{OrgID: orgID, OrgRole: org.RoleNone, Permissions: map[int64]map[string][]string{
		orgID: {
			accesscontrol.ActionAlertingReceiversTestCreate: []string{models.ScopeReceiversProvider.GetResourceAllScope()},
			accesscontrol.ActionAlertingReceiversUpdate:     []string{models.ScopeReceiversProvider.GetResourceAllScope()},
