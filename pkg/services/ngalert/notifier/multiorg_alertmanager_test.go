...
func setupMam(t *testing.T, cfg *setting.Cfg) *MultiOrgAlertmanager {
	...
	mam, err := NewMultiOrgAlertmanager(cfg, cs, orgStore, kvStore, provStore, decryptFn, m.GetMultiOrgAlertmanagerMetrics(), nil, log.New("testlogger"), secretsService, &featuremgmt.FeatureManager{})
	require.NoError(t, err)
	return mam
}
