...
func TestAddAutogenConfig(t *testing.T) {
	...
	// defaultRoute is the definition.Route form of rootRoute, i.e. the tree AddAutogenConfig actually mutates.
	defaultRoute := func() *definition.Route {
		return v1.RouteToDB(rootRoute())
	}
	...
	withChildRoutes := func(route *definition.Route, children ...*definition.Route) *definition.Route {
	...
	basicContactRoute := func(receiver string) *definition.Route {
	...
	testCases := []struct {
		...
		},
		{
			...
			storeSettings:  []models.ContactPointRouting{},
			expRoute: withChildRoutes(defaultRoute(), &definition.Route{
				Receiver:       "default",
		...
		},
		{
			...
			expRoute: withChildRoutes(defaultRoute(), &definition.Route{
				...
				ObjectMatchers: matcher(models.AutogeneratedRouteLabel, "true"),
				Routes: []*definition.Route{
					withChildRoutes(basicContactRoute("receiver1"), &definition.Route{
						Receiver:            "receiver1",
					...
					withChildRoutes(basicContactRoute("receiver2"), &definition.Route{
						Receiver:       "receiver2",
						ObjectMatchers: matcher(models.AutogeneratedRouteSettingsHashLabel, "63ad04d6c21c3aec"),
						GroupWait:      new(model.Duration(2 * time.Minute)),
					}),
					withChildRoutes(basicContactRoute("receiver5"), &definition.Route{
						Receiver:            "receiver5",
					...
					withChildRoutes(basicContactRoute("receiver4"), &definition.Route{
						Receiver:       "receiver4",
						ObjectMatchers: matcher(models.AutogeneratedRouteSettingsHashLabel, "9bbbec5f72627ae5"),
						GroupByStr:     []string{models.FolderTitleLabel, model.AlertNameLabel, "custom"},
					}),
					withChildRoutes(basicContactRoute("receiver3"), &definition.Route{
						Receiver:       "receiver3",
	...
	for _, tt := range testCases {
		t.Run(tt.name, func(t *testing.T) {
			...
			if !cmp.Equal(tt.expRoute, route, cOpt...) {
				t.Errorf("Unexpected Route: %v", cmp.Diff(tt.expRoute, route, cOpt...))
			}
		})
	}
}
