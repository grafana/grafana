...
// AddAutogenConfig creates the autogenerated configuration and adds it to route, which is the
// definition.Route form of cfg's routing tree (built and owned by the caller, e.g. via
// legacy_storage.WithManagedRoutes) since that's the type the notification engine operates on.
// cfg is still used to derive the autogenerated settings themselves (receivers, default receiver name).
// If skipInvalid is true, then invalid notification settings are skipped, otherwise an error is returned.
func AddAutogenConfig(ctx context.Context, logger log.Logger, store autogenRuleStore, orgId int64, cfg *v1.AMConfigV1, route *definition.Route, invalidReceiverAction InvalidReceiversAction, features featuremgmt.FeatureToggles) error {
	autogenRoute, err := newAutogeneratedRoute(ctx, logger, store, orgId, cfg, invalidReceiverAction, features)
	if err != nil {
		return err
	}

	err = autogenRoute.addToRoute(route)
	if err != nil {
		return err
	}
...
func newAutogeneratedRoute(ctx context.Context, logger log.Logger, store autogenRuleStore, orgId int64, cfg *v1.AMConfigV1, invalidReceiverAction InvalidReceiversAction, features featuremgmt.FeatureToggles) (autogeneratedRoute, error) {
	...
	newAutogenRoute, err := generateRouteFromSettings(cfg.AlertmanagerConfig.GetRoute().Receiver, notificationSettings)
	if err != nil {
...
func generateRouteFromSettings(defaultReceiver string, settings map[data.Fingerprint]models.ContactPointRouting) (autogeneratedRoute, error) {
	...
	autoGenRoot := &definition.Route{
		Receiver:       defaultReceiver,
		ObjectMatchers: definition.ObjectMatchers{rootMatcher},
		Continue:       false, // We explicitly don't continue toward user-created routes if this matches.
	}

	receiverRoutes := make(map[string]*definition.Route)
	for _, fingerprint := range keys {
		...
		if !ok {
			...
			receiverRoute = &definition.Route{
				Receiver:       s.Receiver,
				ObjectMatchers: definition.ObjectMatchers{contactMatcher},
				Continue:       false,
		...
		receiverRoute.Routes = append(receiverRoute.Routes, &definition.Route{
			Receiver:       s.Receiver,
			ObjectMatchers: definition.ObjectMatchers{settingMatcher},
...
func (ar *autogeneratedRoute) addToRoute(route *definition.Route) error {
	if route == nil {
	...
	route.Routes = append([]*definition.Route{ar.Route}, route.Routes...)
	return nil
}
...
func RemoveAutogenConfigIfExists(route *definition.Route) {
	route.Routes = slices.DeleteFunc(route.Routes, func(route *definition.Route) bool {
		return IsAutogeneratedRoot(route)
	})
}

// IsAutogeneratedRoot returns true if the route is the root of an autogenerated route.
func IsAutogeneratedRoot(route *definition.Route) bool {
