...
func AddAutogenConfig(ctx context.Context, logger log.Logger, store autogenRuleStore, orgId int64, cfg apiAlertingConfig, skipInvalid bool) error {
...
func newAutogeneratedRoute(ctx context.Context, logger log.Logger, store autogenRuleStore, orgId int64, cfg apiAlertingConfig, skipInvalid bool) (autogeneratedRoute, error) {
	...
	for receiver := range cfg.ReceiverNames() {
		setting := models.NewDefaultNotificationSettings(receiver)
...
func (ar *autogeneratedRoute) addToRoute(route *definitions.Route) error {
	...
	RemoveAutogenConfig(route)

	route.Routes = append([]*definitions.Route{ar.Route}, route.Routes...)
	return nil
}

// RemoveAutogenConfig removes all top-level autogenerated routes from the provided route.
func RemoveAutogenConfig(route *definitions.Route) {
