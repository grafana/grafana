...
func generateRouteFromSettings(defaultReceiver string, settings map[data.Fingerprint]models.NotificationSettings) (autogeneratedRoute, error) {
	...
	normalizeRoute(autoGenRoot)
	return autogeneratedRoute{
		Route: autoGenRoot,
	}, nil
}

// normalizeRoute normalizes the given route and its children recursively in accordance with upstream Route.ValidateChild().
func normalizeRoute(r *definitions.Route) {
	r.GroupBy = nil
	r.GroupByAll = false
