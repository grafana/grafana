...
type ErrorRuleAutogenValidation struct {
	Err      error
	RuleUID  string
	Receiver string
}

func (e ErrorRuleAutogenValidation) Error() string {
	return fmt.Sprintf("invalid notification settings for rule %s: %s", e.RuleUID, e.Err.Error())
}

func (e ErrorRuleAutogenValidation) Unwrap() error {
	return e.Err
}

func newErrorRuleAutogenValidation(ruleUID, receiver string, err error) ErrorRuleAutogenValidation {
	return ErrorRuleAutogenValidation{
		Err:      err,
		RuleUID:  ruleUID,
		Receiver: receiver,
	}
	...
	for ruleKey, ruleSettings := range settings {
		for _, setting := range ruleSettings {
			...
			if err = validator.Validate(setting); err != nil {
				...
				return autogeneratedRoute{}, newErrorRuleAutogenValidation(ruleKey.UID, setting.Receiver, err)
			}
			fp := setting.Fingerprint()
