...
func newAlertmanager(orgID int64, cfg *setting.Cfg, store store.AlertingStore, kvStore kvstore.KVStore, peer ClusterPeer, m *metrics.Alertmanager) (*Alertmanager, error) {
	...
	am.silences, err = createSilences(silencesFilePath, m.Registerer)
	if err != nil {
...
//createSilences initializes a new instance of silence.Silences with silences taken from the file by path silencesFilePath and specific metrics registerer.
