...
type alertmanager struct {
	...
	appliedAutogenFingerprint data.Fingerprint
}
...
func (am *alertmanager) SaveAndApplyDefaultConfig(ctx context.Context) error {
	...
	am.Base.WithLock(func() {
		...
		err = am.Store.SaveAlertmanagerConfigurationWithCallback(ctx, cmd, func() error {
			autogenRoute, err := am.AddAutogenConfig(ctx, cfg, false)
			if err != nil {
				return err
			}
			...
			am.appliedAutogenFingerprint = autogenRoute.Fingerprint
			return nil
		})
		if err != nil {
			outerErr = nil
...
func (am *alertmanager) SaveAndApplyConfig(ctx context.Context, cfg *apimodels.PostableUserConfig) error {
	// Remove autogenerated config from the user config before saving it.
	// TODO: Feature Flag.
	RemoveAutogenConfig(cfg)
...
func (am *alertmanager) AddAutogenConfig(ctx context.Context, cfg *apimodels.PostableUserConfig, skipInvalid bool) (*autogeneratedRoute, error) {
	// Create autogenerated side of the policy tree.
	// TODO: Should be implemented outside of any specific alertmanager so it applies to remote alertmanagers as well.
	validator := NewNotificationSettingsValidator(cfg.AlertmanagerConfig)
	autogenRoute, err := newAutogeneratedRoute(ctx, am.logger, am.orgID, am.Store, cfg.AlertmanagerConfig.Route.Receiver, cfg.AlertmanagerConfig.Receivers, validator, skipInvalid)
	if err != nil {
		return nil, err
	}

	err = autogenRoute.AddToConfig(cfg)
	if err != nil {
		return nil, err
	}

	return &autogenRoute, nil
}
...
func (am *alertmanager) ApplyConfig(ctx context.Context, dbCfg *ngmodels.AlertConfiguration) error {
	...
	var outerErr error
	am.Base.WithLock(func() {
		autogenRoute, err := am.AddAutogenConfig(ctx, cfg, true)
		if err != nil {
			outerErr = err
			return
		}
		// Note: Autogen here causes alert_configuration_history to update last_applied more often. Since we update
		// last_applied when autogen changes but user-created config remains the same. To fix this however, the local
		// alertmanager needs to be able to tell the difference between user-created and autogen config, which may
		// introduce cross-cutting complexity.
