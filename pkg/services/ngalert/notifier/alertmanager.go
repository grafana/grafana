...
func (am *alertmanager) applyConfig(ctx context.Context, cfg *apimodels.PostableUserConfig) (bool, error) {
	...
	autogenRoute, err := newAutogeneratedRoute(ctx, am.logger, am.orgID, am.Store, cfg.AlertmanagerConfig.Route.Receiver, NewNotificationSettingsValidator(cfg.AlertmanagerConfig))
	if err != nil {
		return false, err
	}
	autogenRouteChanged := false
	if autogenRoute.Fingerprint != am.appliedAutogenFingerprint {
		autogenRouteChanged = true
	}

	// If neither the configuration nor templates have changed, we've got nothing to do.
	if !amConfigChanged && !templatesChanged && !autogenRouteChanged {
