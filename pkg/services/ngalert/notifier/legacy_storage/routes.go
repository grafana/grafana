...
// GeneratedSubRoute returns the route to use as a managed route's child in the Alertmanager routing
// tree sent to the notification engine. It returns definition.Route (rather than v1.Route) because
// that's the type the notification engine and its autogenerated-route insertion operate on.
func GeneratedSubRoute(r *v1.ManagedRoute) *definition.Route {
	amRoute := v1.RouteToDB(new(ManagedRouteToRoute(r)))
	...
	if !models.IsDefaultRoutingTreeName(r.GetUID()) {
		// Set label matcher.
		amRoute.ObjectMatchers = definition.ObjectMatchers{managedRouteMatcher(r.GetUID())}
	}
	return amRoute
}
...
// WithManagedRoutes returns cfg's default route with each of cfg's managed routes spliced in as a
// generated child route. It returns definition.Route because the result is about to be handed to the
// notification engine, which (along with autogenerated-route insertion) operates on that type, not
// v1.Route.
func WithManagedRoutes(cfg *v1.AMConfigV1) *definition.Route {
	root := v1.RouteToDB(cfg.GetDefaultRoute())
	if len(cfg.ManagedRoutes) <= 1 {
		// ManagedRoutes always contains at least the default route itself; if that's all there is,
		// we just return the original root.
		return root
	}
	newManagedRoutes := make([]*definition.Route, 0, len(root.Routes)+len(cfg.ManagedRoutes))
	for _, k := range slices.Sorted(maps.Keys(cfg.ManagedRoutes)) {
		// On the off chance that the route is nil or invalid managed route with the restricted name, we skip it.
		if cfg.ManagedRoutes[k] == nil || models.IsDefaultRoutingTreeName(k) {
			continue
		}
		newManagedRoutes = append(newManagedRoutes, GeneratedSubRoute(v1.NewManagedRoute(k, cfg.ManagedRoutes[k])))
	}

	// Add the default routing tree at the end.
	newManagedRoutes = append(newManagedRoutes, root.Routes...)
	root.Routes = newManagedRoutes
	return root
}

func (rev *ConfigRevision) GetManagedRoute(name string) *v1.ManagedRoute {
	// Echo the requested name (canonical or alias) so the response preserves the name the client
	// used, while GetUID/ResourceID canonicalize for identity purposes.
	route, ok := rev.Config.ManagedRoutes[models.CanonicalizeRoutingTreeName(name)]
	if !ok {
		return nil
	}
...
func (rev *ConfigRevision) GetManagedRoutes() v1.ManagedRoutes {
	managedRoutes := make(v1.ManagedRoutes, 0, len(rev.Config.ManagedRoutes))
	for _, k := range slices.Sorted(maps.Keys(rev.Config.ManagedRoutes)) {
		// On the off chance that the route is nil (e.g. malformed input), we skip it.
		if rev.Config.ManagedRoutes[k] == nil {
			continue
		}
...
func (rev *ConfigRevision) DeleteManagedRoute(name string) {
	delete(rev.Config.ManagedRoutes, models.CanonicalizeRoutingTreeName(name))
}
...
func (rev *ConfigRevision) UpdateNamedRoute(name string, subtree v1.Route) (*v1.ManagedRoute, error) {
	name = models.CanonicalizeRoutingTreeName(name)
	if name == "" {
...
func (rev *ConfigRevision) ResetUserDefinedRoute(defaultCfg *v1.AMConfigV1) (*v1.ManagedRoute, error) {
	defaultRoute := defaultCfg.GetDefaultRoute()
	// Ensure the new default receiver exists and if not, create it.
	if err := rev.validateReceiverReferences(*defaultRoute); err != nil {
		// Default receiver doesn't exist, create it.
		defaultRcvUID := v1.ReceiverUID(defaultRoute.Receiver) // TODO: This could work with static UIDs but a predetermined UID might make more sense.
		defaultRcv, ok := defaultCfg.Receivers[defaultRcvUID]
		if !ok {
			return nil, fmt.Errorf("inconsistent default configuration: default receiver %q not found", defaultRoute.Receiver)
		}
	...
	return rev.UpdateNamedRoute(models.DefaultRoutingTreeName, *defaultRoute)
}
