...
// PostableAPIConfigToNotificationsConfiguration converts cfg into the notification engine's configuration
// type. routingTree is the already-built definition.Route tree to use (e.g. built via
// legacy_storage.WithManagedRoutes and AddAutogenConfig) rather than being derived from
// cfg.AlertmanagerConfig.Route here, since callers need control over exactly when managed routes and
// autogenerated routes are merged in relative to each other.
func PostableAPIConfigToNotificationsConfiguration(
	cfg v1.AMConfigV1,
	routingTree *definition.Route,
