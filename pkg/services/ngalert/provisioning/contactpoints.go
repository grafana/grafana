...
func (ecp *ContactPointService) UpdateContactPoint(ctx context.Context, orgID int64, contactPoint apimodels.EmbeddedContactPoint, provenance models.Provenance) error {
	...
	// Managing this is complex.

	configModified := false
	// Receivers have an extra layer of hierarchy that we need to flatten.
groupLoop:
	for _, receiverGroup := range revision.cfg.AlertmanagerConfig.Receivers {
		...
		for i, grafanaReceiver := range receiverGroup.GrafanaManagedReceivers {
			if grafanaReceiver.UID == mergedReceiver.UID {
				...
				if grafanaReceiver.Name == mergedReceiver.Name && receiverGroup.Name == grafanaReceiver.Name {
					receiverGroup.GrafanaManagedReceivers[i] = mergedReceiver
					configModified = true
					break groupLoop
				}
				...
				if len(receiverGroup.GrafanaManagedReceivers) == 1 {
					receiverGroup.Name = mergedReceiver.Name
					receiverGroup.GrafanaManagedReceivers[i] = mergedReceiver
					configModified = true
					break groupLoop
				}
				...
				for i, candidateExistingGroup := range revision.cfg.AlertmanagerConfig.Receivers {
					// If so, put our modified receiver into that group. Done!
					if candidateExistingGroup.Name == mergedReceiver.Name {
						...
						candidateExistingGroup.GrafanaManagedReceivers = append(candidateExistingGroup.GrafanaManagedReceivers, mergedReceiver)
						configModified = true
						break groupLoop
					}
				}
				...
				revision.cfg.AlertmanagerConfig.Receivers = append(revision.cfg.AlertmanagerConfig.Receivers, newGroup)
				configModified = true
				break groupLoop
			}
		}
...
func (ecp *ContactPointService) stitchReceiver(revision cfgRevision, target *apimodels.PostableGrafanaReceiver) error {
receiverGroups:
	for _, receiverGroup := range revision.cfg.AlertmanagerConfig.Receivers {
		// Does the current group contain the grafana receiver we're interested in?
		for i, grafanaReceiver := range receiverGroup.GrafanaManagedReceivers {
			if grafanaReceiver.UID == target.UID {
				...
				break receiverGroups
			}
		}
	}
	return fmt.Errorf("contact point with uid '%s' not found", target.UID)
}
