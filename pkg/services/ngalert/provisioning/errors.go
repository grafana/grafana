...
var (
	...
	ErrTemplateNotFound = errutil.NotFound("alerting.notifications.template.notFound")
	ErrTemplateInvalid  = errutil.BadRequest("alerting.notifications.template.invalidFormat").MustTemplate("Invalid format of the submitted template", errutil.WithPublic("Template is in invalid format. Correct the payload and try again."))
	ErrTemplateExists   = errutil.BadRequest("alerting.notifications.template.nameExists", errutil.WithPublicMessage("Template file with this name already exists. Use a different name or update existing one."))
