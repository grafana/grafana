...
func alertInstanceMigration(mg *migrator.Migrator) {
	alertInstance := migrator.Table{
		...
		PrimaryKeys: []string{"org_id", "alert_definition_uid", "labels_hash"},
		Indices: []*migrator.Index{
			{Cols: []string{"org_id", "alert_definition_uid", "current_state"}, Type: migrator.IndexType},
		},
	}

	// create table
	mg.AddMigration("create alert_instance table", migrator.NewAddTableMigration(alertInstance))
	mg.AddMigration("add index in alert_instance table on org_id, alert_definition_uid and current_state columns", migrator.NewAddIndexMigration(alertInstance, alertInstance.Indices[0]))
}
