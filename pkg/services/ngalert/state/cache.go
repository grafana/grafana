...
func expandAnnotationsAndLabels(ctx context.Context, log log.Logger, alertRule *ngModels.AlertRule, result eval.Result, extraLabels data.Labels, externalURL *url.URL) (data.Labels, data.Labels) {
	...
	// Include the expanded alert rule-level labels in the $labels namespace when
	// templating annotations. This allows annotations (e.g. summary, description)
	// to reference rule-level static labels via {{ $labels.myLabel }}.
	// We intentionally do NOT include rule labels when templating rule labels
	// themselves to avoid circular references and breaking changes.
