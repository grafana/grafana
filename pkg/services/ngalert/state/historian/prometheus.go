...
// metricEmittingStates defines which evaluation states should emit ALERTS metrics
var metricEmittingStates = map[eval.State]struct{}{
	eval.Alerting:   {},
	eval.Pending:    {},
	eval.Recovering: {},
}
...
func (b *RemotePrometheusBackend) Record(ctx context.Context, rule history_model.RuleMeta, transitions []state.StateTransition) <-chan error {
	...
	go func() {
		...
		if err := b.promWriter.WriteDatasource(ctx, b.cfg.DatasourceUID, alertMetricName, st.LastEvaluationTime, frames, st.OrgID, nil); err != nil {
