...
// String implements the fmt.Stringer interface. It returns the labels
// formatted as k=v, comma separated, in increasing order.
...
// Ref contains the labels and value of the RefID for a series.
type Ref struct {
	Labels Labels
	Value  float64
}
...
func (r Ref) String() string {
	return fmt.Sprintf("labels: %s, value: %s",
		r.Labels,
		strconv.FormatFloat(r.Value, 'f', -1, 64))
}

// Value is the old $values variable that will be removed in Grafana 10 in favour
// of $refs.
type Value Ref

// String implements the fmt.Stringer interface to print the value of each RefID
