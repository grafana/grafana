...
var (
	defaultFuncs = template.FuncMap{
		"graphLink": graphLink,
		"filter":    filterLabelsFunc,
		"filterRe":  filterLabelsReFunc,
		"remove":    removeLabelsFunc,
		"removeRe":  removeLabelsReFunc,
		"tableLink": tableLink,
	}
)
...
var (
	...
	filterLabelsFunc = func(m Labels, match string) Labels {
		res := make(Labels)
		for k, v := range m {
			if k == match {
				res[k] = v
	...
	filterLabelsReFunc = func(m Labels, pattern string) Labels {
	...
	removeLabelsFunc = func(m Labels, match string) Labels {
		res := make(Labels)
		for k, v := range m {
			if k != match {
				res[k] = v
	...
	removeLabelsReFunc = func(m Labels, pattern string) Labels {
		re := regexp.MustCompile(pattern)
		res := make(Labels)
		for k, v := range m {
			if !re.MatchString(k) {
