...
func (a *State) NeedsSending(evaluatedAt time.Time, resendDelay time.Duration, resolvedRetention time.Duration) bool {
	...
	if a.State == eval.Normal && (a.ResolvedAt == nil || evaluatedAt.Sub(*a.ResolvedAt) > resolvedRetention) {
		return false
	}

	// We should send, and re-send notifications, each time LastSentAt is <= evaluatedAt + resendDelay.
	// This can include normal->normal transitions that were resolved in recent past evaluations.
	return a.LastSentAt == nil || !a.LastSentAt.Add(resendDelay).After(evaluatedAt)
}
