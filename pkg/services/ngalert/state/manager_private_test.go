...
func TestManager_SetPersister(t *testing.T) {
	testCases := []struct {
		name string
	}{
		{name: "updates persister"},
	}

	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			initialPersister := &fakePersister{name: "initial"}
			mgr := NewManager(ManagerCfg{
				Clock: clock.NewMock(),
				Log:   log.NewNopLogger(),
			}, initialPersister)

			newPersister := &fakePersister{name: "new"}
			mgr.SetPersister(newPersister)

			require.Equal(t, newPersister, mgr.persister)
		})
	}
}
