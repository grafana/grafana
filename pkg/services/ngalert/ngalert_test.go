...
import (
	...
	ngtestutil "github.com/grafana/grafana/pkg/services/ngalert/testutil"
...
func TestAlertmanagerIntegration_ExtraDedupStage(t *testing.T) {
	...
	t.Run("assert no flapping alerts when stopOnExtraDedup is enabled", func(t *testing.T) {
		s, err := ngtestutil.NewAlertmanagerScenario()
		require.NoError(t, err)
		defer s.Close()

		s.Start(t, 20, "15s", true)
		s.Provision(t, ngtestutil.ProvisionCfg{
			AlertRuleConfig: ngtestutil.AlertRuleConfig{
				PendingPeriod:                  "30s",
				GroupEvaluationIntervalSeconds: 10,
			},
			NotificationPolicyCfg: ngtestutil.NotificationPolicyCfg{
				GroupWait:      "30s",
				GroupInterval:  "1m",
				RepeatInterval: "30m",
			},
		...
		for {
			...
			case <-ticker.C:
				...
				if st.State != ngtestutil.AlertStateNormal {
					continue
				}
