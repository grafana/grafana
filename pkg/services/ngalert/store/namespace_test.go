...
func TestIntegration_GetNamespaceByTitle(t *testing.T) {
	...
	t.Run("should return error when folder with title doesn't exist under specified parent", func(t *testing.T) {
		nonExistentTitle := "non-existent-folder"
		_, err := store.GetNamespaceByTitle(context.Background(), nonExistentTitle, 1, u, parentUID)
		require.ErrorIs(t, err, dashboards.ErrFolderAccessDenied)
	})
}
...
func TestIntegration_GetNamespaceChildren(t *testing.T) {
	...
	t.Run("should return error when folder does not exist", func(t *testing.T) {
		nonExistentUID := uuid.NewString()
		_, err := store.GetNamespaceChildren(context.Background(), nonExistentUID, 1, admin)
		require.ErrorIs(t, err, dashboards.ErrFolderAccessDenied)
	})

	t.Run("should return empty array for folders with no children", func(t *testing.T) {
		// Root Folder 2 has no children
		_, err := store.GetNamespaceChildren(context.Background(), rootFolder2, 1, admin)
		require.ErrorIs(t, err, dashboards.ErrFolderAccessDenied)
	})

	t.Run("should return error for a different org", func(t *testing.T) {
		differentOrgID := int64(999)
		_, err := store.GetNamespaceChildren(context.Background(), rootFolder1, differentOrgID, admin)
		require.ErrorIs(t, err, dashboards.ErrFolderAccessDenied)
	})
