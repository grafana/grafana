...
func TestJsmNotifier(t *testing.T) {
	...
	t.Run("Parsing alert notification from settings", func(t *testing.T) {
		t.Run("empty settings should return error", func(t *testing.T) {
			...
			_, err := NewJsmNotifier(model, encryptionService.GetDecryptedValue, nil)
			require.Error(t, err)
		})
		...
		t.Run("settings should trigger incident", func(t *testing.T) {
          ...
          "apiKey": "abcdefgh0123456789"
			...
			not, err := NewJsmNotifier(model, encryptionService.GetDecryptedValue, nil)
			jsmNotifier := not.(*JsmNotifier)

			require.Nil(t, err)
			require.Equal(t, "jsm_testing", jsmNotifier.Name)
			require.Equal(t, "jsm", jsmNotifier.Type)
			require.Equal(t, "abcdefgh0123456789", jsmNotifier.APIKey)
		})
	})
	...
	t.Run("Handling notification tags", func(t *testing.T) {
		...
		t.Run("alert payload should include tag pairs only as an array in the tags key when sendAsTags is not set", func(t *testing.T) {
          ...
          "apiKey": "abcdefgh0123456789"
			...
			notifier, notifierErr := NewJsmNotifier(model, encryptionService.GetDecryptedValue, notificationService) // unhandled error
			...
			alertErr := jsmNotifier.createAlert(evalContext)
