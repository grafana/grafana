	...
	alerting.RegisterNotifier(&alerting.NotifierPlugin{
		...
		Options: []alerting.NotifierOption{
			...
			},
			{
				...
				Description:  "Send the notification tags to Jira Service Management as either Extra Properties, Tags or both",
...
const (
	jsmAlertURL = "https://api.atlassian.com/jsm/ops/integration/v2/alerts"
)

// NewJsmNotifier is the constructor for Jira Service Management.
func NewJsmNotifier(model *models.AlertNotification, fn alerting.GetDecryptedValueFn, ns notifications.Service) (alerting.Notifier, error) {
	...
	return &JsmNotifier{
		...
		log:              log.New("alerting.notifier.jsm"),
	}, nil
}

// JsmNotifier is responsible for sending
// alert notifications to Jira Service Management
...
// Notify sends an alert notification to Jira Service Management.
func (on *JsmNotifier) Notify(evalContext *alerting.EvalContext) error {
	var err error
...
func (on *JsmNotifier) createAlert(evalContext *alerting.EvalContext) error {
	on.log.Info("Creating Jira Service Management alert", "ruleId", evalContext.Rule.ID, "notification", on.Name)
	...
	if err := on.NotificationService.SendWebhookSync(evalContext.Ctx, cmd); err != nil {
		on.log.Error("Failed to send notification to Jira Service Management", "error", err, "body", string(body))
	}

	return nil
}

func (on *JsmNotifier) closeAlert(evalContext *alerting.EvalContext) error {
	on.log.Info("Closing Jira Service Management alert", "ruleId", evalContext.Rule.ID, "notification", on.Name)
