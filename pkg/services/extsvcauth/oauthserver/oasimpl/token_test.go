...
func TestOAuth2ServiceImpl_handleJWTBearer(t *testing.T) {
	...
	client1WithPerm := func(perms []ac.Permission) *oauthserver.OAuthClient {
		client := *client1
...
func TestOAuth2ServiceImpl_HandleTokenRequest(t *testing.T) {
	tests := []struct {
		name            string
		tweakTestClient func(*oauthserver.OAuthClient)
		...
		},
		{
			...
			tweakTestClient: func(es *oauthserver.OAuthClient) {
...
func setupHandleTokenRequestEnv(t *testing.T, env *TestEnv, opt func(*oauthserver.OAuthClient)) {
	now := time.Now()
