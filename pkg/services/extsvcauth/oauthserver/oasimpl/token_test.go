...
func TestOAuth2ServiceImpl_HandleTokenRequest(t *testing.T) {
	tests := []struct {
		name            string
		tweakTestClient func(*oauthserver.Client)
		...
		},
		{
			...
			tweakTestClient: func(es *oauthserver.Client) {
...
func setupHandleTokenRequestEnv(t *testing.T, env *TestEnv, opt func(*oauthserver.Client)) {
	now := time.Now()
