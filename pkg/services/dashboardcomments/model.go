...
type Message struct {
	...
	AuthorType   string    `xorm:"author_type"`
...
func (c *AddMessageCommand) Validate() error {
	if c.OrgID == 0 || c.ThreadID == 0 {
		return ErrValidationFailed
	}
	if c.AuthorType == "" {
		c.AuthorType = AuthorTypeUser
	}
	if c.AuthorType != AuthorTypeUser && c.AuthorType != AuthorTypeAssistant {
		return ErrValidationFailed
	}
	if c.AuthorType == AuthorTypeUser && c.AuthorUserID == 0 {
		return ErrValidationFailed
	}
	if c.AuthorType == AuthorTypeAssistant {
		c.AuthorUserID = 0
	}
	if c.Body == "" {
		return ErrEmptyBody
