...
func (s *ServiceImpl) getOrgAdminNode(c *model.ReqContext) (*navtree.NavLink, error) {
...
func (s *ServiceImpl) getServerAdminNode(c *model.ReqContext) *navtree.NavLink {
	hasAccess := ac.HasAccess(s.accessControl, c)
...
func (s *ServiceImpl) ReqCanAdminTeams(c *model.ReqContext) bool {
	return c.OrgRole == org.RoleAdmin || (s.cfg.EditorsCanAdmin && c.OrgRole == org.RoleEditor)
}

func enableServiceAccount(s *ServiceImpl, c *model.ReqContext) bool {
	hasAccess := ac.HasAccess(s.accessControl, c)
