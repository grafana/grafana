...
func TestAddAssistantWatchersToAlerting(t *testing.T) {
	httpReq, _ := http.NewRequest(http.MethodGet, "", nil)
	reqCtx := &contextmodel.ReqContext{SignedInUser: &user.SignedInUser{}, Context: &web.Context{Req: httpReq}}
	const watchersAction = "grafana-assistant-app.watcher-agents:read"
	permissions := []ac.Permission{
		{Action: pluginaccesscontrol.ActionAppAccess, Scope: "*"},
		{Action: watchersAction},
	}

	// Mirrors the real plugin: the Watchers page is an include guarded by an RBAC action.
	assistantApp := pluginstore.Plugin{
		JSONData: plugins.JSONData{
			ID: assistantAppID, Name: "Assistant", Type: plugins.TypeApp,
			Includes: []*plugins.Includes{
				{Name: "Home", Path: assistantAppHomePath, Type: "page", AddToNav: true, DefaultNav: true},
				{Name: "Workspace", Path: assistantAppHomePath + "/workspace", Type: "page", AddToNav: true},
				{Name: "Watchers", Path: assistantWatchersPath, Type: "page", AddToNav: true, Action: watchersAction},
			},
		},
	}
	...
	newAlertingTree := func(alertRulesID string) *navtree.NavTreeRoot {
		treeRoot := &navtree.NavTreeRoot{}
		treeRoot.AddSection(&navtree.NavLink{
			Text: "Alerting", Id: navtree.NavIDAlerting,
			Children: []*navtree.NavLink{
				{Text: "Alert rules", Id: alertRulesID, Url: "/alerting/list"},
				{Text: "Silences", Id: "silences", Url: "/alerting/silences"},
			},
		})
		return treeRoot
	}

	for _, alertRulesID := range []string{"alert-list", "alert-rules"} {
		t.Run("Should add Watchers right below Alert rules ("+alertRulesID+")", func(t *testing.T) {
			service := newService(assistantApp)
			treeRoot := newAlertingTree(alertRulesID)
			require.NoError(t, service.addAppLinks(treeRoot, reqCtx))

			alertingNode := treeRoot.FindById(navtree.NavIDAlerting)
			require.NotNil(t, alertingNode)
			require.Len(t, alertingNode.Children, 3)

			watchers := alertingNode.Children[1]
			require.Equal(t, "Watchers", watchers.Text)
			require.Equal(t, "standalone-plugin-page-assistant-watchers", watchers.Id)
			require.Equal(t, "/a/grafana-assistant-app/watchers", watchers.Url)
			require.True(t, watchers.IsNew)
		})
	}

	t.Run("Should not add Watchers when the Assistant plugin is not available", func(t *testing.T) {
		service := newService()
	...
	t.Run("Should not add Watchers when the user has no access to the Assistant plugin", func(t *testing.T) {
		service := newService(assistantApp)
		service.accessControl = accesscontrolmock.New().WithPermissions([]ac.Permission{})
		treeRoot := newAlertingTree("alert-list")
		require.NoError(t, service.addAppLinks(treeRoot, reqCtx))
	...
	t.Run("Should not add Watchers when there is no Alerting section", func(t *testing.T) {
		service := newService(assistantApp)
		treeRoot := &navtree.NavTreeRoot{}
		require.NoError(t, service.addAppLinks(treeRoot, reqCtx))
	...
	t.Run("Should not add Watchers when the user lacks the RBAC action guarding the page", func(t *testing.T) {
		service := newService(assistantApp)
		service.accessControl = accesscontrolmock.New().WithPermissions([]ac.Permission{
			{Action: pluginaccesscontrol.ActionAppAccess, Scope: "*"},
		})
	...
	t.Run("Should not add Watchers when the plugin version has no Watchers page", func(t *testing.T) {
		olderApp := assistantApp
		olderApp.Includes = assistantApp.Includes[:2]
		service := newService(olderApp)
		treeRoot := newAlertingTree("alert-list")
		require.NoError(t, service.addAppLinks(treeRoot, reqCtx))
