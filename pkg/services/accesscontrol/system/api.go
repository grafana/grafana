...
func newApi(ac accesscontrol.AccessControl, router routing.RouteRegister, manager *System) *api {
...
func (a *api) registerEndpoints() {
	...
	a.router.Group(fmt.Sprintf("/api/access-control/system/%s", a.manager.options.Resource), func(r routing.RouteRegister) {
		idScope := accesscontrol.Scope(a.manager.options.Resource, "id", accesscontrol.Parameter(":resourceID"))
		actionWrite, actionRead := fmt.Sprintf("%s.permissions:write", a.manager.options.Resource), fmt.Sprintf("%s.permissions:read", a.manager.options.Resource)
...
func (a *api) getDescription(c *models.ReqContext) response.Response {
	return response.JSON(http.StatusOK, &Description{
		...
		Assignments: a.manager.options.Assignments,
	})
}
...
func (a *api) getPermissions(c *models.ReqContext) response.Response {
	...
	permissions, err := a.manager.GetPermissions(c.Req.Context(), c.OrgId, resourceID)
	if err != nil {
	...
	for _, p := range permissions {
		if permission, ok := a.manager.MapActions(p); ok {
			teamAvatarUrl := ""
			if p.TeamId != 0 {
...
func (a *api) setUserPermission(c *models.ReqContext) response.Response {
	...
	permission, err := a.manager.SetUserPermission(c.Req.Context(), c.OrgId, userID, resourceID, a.manager.MapPermission(cmd.Permission))
	if err != nil {
		return response.Error(http.StatusBadRequest, "failed to set user permission", err)
	}

	translated, _ := a.manager.MapActions(*permission)
...
func (a *api) setTeamPermission(c *models.ReqContext) response.Response {
	...
	permission, err := a.manager.SetTeamPermission(c.Req.Context(), c.OrgId, teamID, resourceID, a.manager.MapPermission(cmd.Permission))
	if err != nil {
...
func (a *api) setBuiltinRolePermission(c *models.ReqContext) response.Response {
	...
	permission, err := a.manager.SetBuiltinRolePermission(c.Req.Context(), c.OrgId, builtInRole, resourceID, a.manager.MapPermission(cmd.Permission))
	if err != nil {
