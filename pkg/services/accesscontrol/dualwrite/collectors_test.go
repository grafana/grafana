...
func TestTupleStringWithoutCondition(t *testing.T) {
	t.Run("should return tuple string without condition", func(t *testing.T) {
		...
		// Verify condition was preserved
		require.NotNil(t, tuple.Condition)
		assert.Equal(t, "test_condition", tuple.Condition.Name)

		// Verify result doesn't contain condition
		assert.NotContains(t, result, "test_condition")
		assert.Contains(t, result, "user:1")
		assert.Contains(t, result, "member")
		assert.Contains(t, result, "team:1")
	})

	t.Run("should work with tuple without condition", func(t *testing.T) {
		tuple := &openfgav1.TupleKey{
			User:     "user:1",
			Relation: "member",
			Object:   "team:1",
