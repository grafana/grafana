...
// BasicRoleAggregatorServiceName is the service name used by both the noop and
// the real (enterprise) basic-role aggregator so the module dependency graph
// can reference it by a stable key.
...
// IsDisabled always returns true — OSS does not aggregate fixed-role
// permissions into the four basic GlobalRoles (Enterprise-only RBAC concept).
func (s *NoopBasicRoleAggregator) IsDisabled() bool { return true }

// Run satisfies the registry.BackgroundService interface.
func (s *NoopBasicRoleAggregator) Run(_ context.Context) error {
	return nil
}
