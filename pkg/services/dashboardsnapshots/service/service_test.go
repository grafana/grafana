...
func TestValidateDashboardExists(t *testing.T) {
	...
	require.NoError(t, err)
	dashSvc, err := dashsvc.ProvideDashboardServiceImpl(cfg, dashboardStore, folderimpl.ProvideDashboardFolderStore(sqlStore), nil, nil, nil, acmock.New(), client.NewNoopZanzanaClient(), foldertest.NewFakeService(), folder.NewFakeStore(), nil)
	require.NoError(t, err)
