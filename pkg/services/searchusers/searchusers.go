...
type Service interface {
	SearchUsers(c *model.ReqContext) response.Response
	SearchUsersWithPaging(c *model.ReqContext) response.Response
}
...
func (s *OSSService) SearchUsers(c *model.ReqContext) response.Response {
...
// 404: notFoundError
// 500: internalServerError
func (s *OSSService) SearchUsersWithPaging(c *model.ReqContext) response.Response {
	result, err := s.SearchUser(c)
	if err != nil {
		return response.Error(500, "Failed to fetch users", err)
	}
...
func (s *OSSService) SearchUser(c *model.ReqContext) (*user.SearchUserQueryResult, error) {
