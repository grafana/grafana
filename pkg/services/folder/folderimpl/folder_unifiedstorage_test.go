...
func TestIntegrationFolderServiceViaUnifiedStorage(t *testing.T) {
	...
	dual := dualwrite.ProvideService(featuremgmt.WithFeatures(), nil, nil)
	k8sCli := client.NewK8sHandler(dual, request.GetNamespaceMapper(cfg), v0alpha1.FolderResourceInfo.GroupVersionResource(), restCfgProvider.GetRestConfig, dashboardStore, userService)
