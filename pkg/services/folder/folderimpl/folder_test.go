...
func TestIntegrationProvideFolderService(t *testing.T) {
	...
	t.Run("should register scope resolvers", func(t *testing.T) {
		...
		ProvideService(
			...
			nil, nil, nil, db, featuremgmt.WithFeatures(), supportbundlestest.NewFakeBundleService(), nil, cfg, nil, tracing.InitializeTracerForTest(), nil, dualwrite.ProvideTestService())
	...
	t.Run("Should get descendant counts", func(t *testing.T) {
		...
		t.Run("With nested folder feature flag on", func(t *testing.T) {
			...
			dashSrv, err := dashboardservice.ProvideDashboardServiceImpl(cfg, dashStore, folderStore, featuresFlagOn, folderPermissions, ac, serviceWithFlagOn, nestedFolderStore, nil,
				client.MockTestRestConfig{}, nil, quotaService, nil, publicDashboardFakeService, nil, dualwrite.ProvideTestService())
			require.NoError(t, err)
		...
		t.Run("With nested folder feature flag off", func(t *testing.T) {
			...
			dashSrv, err := dashboardservice.ProvideDashboardServiceImpl(cfg, dashStore, folderStore, featuresFlagOff,
				folderPermissions, ac, serviceWithFlagOff, nestedFolderStore, nil, client.MockTestRestConfig{}, nil, quotaService, nil, publicDashboardFakeService, nil, dualwrite.ProvideTestService())
			require.NoError(t, err)
			dashSrv.RegisterDashboardPermissions(dashboardPermissions)
	...
	t.Run("Should delete folders", func(t *testing.T) {
		...
		for _, tc := range testCases {
			t.Run(tc.desc, func(t *testing.T) {
				...
				dashSrv, err := dashboardservice.ProvideDashboardServiceImpl(cfg, dashStore, folderStore, tc.featuresFlag, folderPermissions, ac, tc.service, tc.service.store, nil,
					client.MockTestRestConfig{}, nil, quotaService, nil, publicDashboardFakeService, nil, dualwrite.ProvideTestService())
				require.NoError(t, err)
				dashSrv.RegisterDashboardPermissions(dashboardPermissions)
