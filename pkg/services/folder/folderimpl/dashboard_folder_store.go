...
func newDashboardFolderStore(sqlStore db.DB, absoluteMaxDepth int) *DashboardFolderStoreImpl {
	return &DashboardFolderStoreImpl{store: sqlStore, absoluteMaxDepth: absoluteMaxDepth}
}
...
func (d *DashboardFolderStoreImpl) Get(ctx context.Context, q folder.GetFolderQuery) (*folder.Folder, error) {
	...
	err := d.store.WithDbSession(ctx, func(sess *db.Session) error {
		...
		if q.WithFullpath {
			s.WriteString(fmt.Sprintf(`, %s AS fullpath`, getFullpathSQL(d.store.GetDialect(), d.absoluteMaxDepth)))
		}
		if q.WithFullpathUIDs {
			s.WriteString(fmt.Sprintf(`, %s AS fullpath_uids`, getFullapathUIDsSQL(d.store.GetDialect(), d.absoluteMaxDepth)))
		}
		s.WriteString(" FROM folder f0")
		s.WriteString(" INNER JOIN dashboard d ON f0.uid = d.uid AND f0.org_id = d.org_id")
		if q.WithFullpath || q.WithFullpathUIDs {
			s.WriteString(getFullpathJoinsSQL(d.absoluteMaxDepth))
		}
		switch {
		case q.UID != nil:
