...
// GetPluginIDFromMeta provides a mock function with given fields: obj
func (_m *FakeDashboardStore) GetPluginIDFromMeta(obj utils.GrafanaMetaAccessor) string {
	ret := _m.Called(obj)

	if len(ret) == 0 {
		panic("no return value specified for GetPluginIDFromMeta")
	}

	var r0 string
	if rf, ok := ret.Get(0).(func(utils.GrafanaMetaAccessor) string); ok {
		r0 = rf(obj)
	} else {
		r0 = ret.Get(0).(string)
	}
