...
// GetSharedDashboardUIDsQuery provides a mock function with given fields: ctx, query
func (_m *FakeDashboardService) GetSharedDashboardUIDsQuery(ctx context.Context, query *FindPersistedDashboardsQuery) (bool, error) {
	...
	if len(ret) == 0 {
		panic("no return value specified for GetSharedDashboardUIDsQuery")
	}

	var r0 bool
	var r1 error
	if rf, ok := ret.Get(0).(func(context.Context, *FindPersistedDashboardsQuery) (bool, error)); ok {
		return rf(ctx, query)
	}
	if rf, ok := ret.Get(0).(func(context.Context, *FindPersistedDashboardsQuery) bool); ok {
		r0 = rf(ctx, query)
	} else {
		r0 = ret.Get(0).(bool)
	}
