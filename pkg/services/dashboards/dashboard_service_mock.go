...
func (_m *FakeDashboardService) BuildPublicDashboardMetricRequest(ctx context.Context, accessToken string, panelId int64) (dtos.MetricRequest, *models.SignedInUser, error) {
	...
	var r1 *models.SignedInUser
	if rf, ok := ret.Get(1).(func(context.Context, string, int64) *models.SignedInUser); ok {
		r1 = rf(ctx, accessToken, panelId)
	} else {
		if ret.Get(1) != nil {
			r1 = ret.Get(1).(*models.SignedInUser)
		}
	}

	var r2 error
	if rf, ok := ret.Get(2).(func(context.Context, string, int64) error); ok {
		r2 = rf(ctx, accessToken, panelId)
	} else {
		r2 = ret.Error(2)
	}
