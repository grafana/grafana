...
func Test_GetSnapshotStatusFromGMS(t *testing.T) {
	...
	setupTest := func(ctx context.Context) (service *Service, snapshotUID string, sessionUID string) {
		...
		_, err = s.store.CreateSnapshot(ctx, cloudmigration.CloudMigrationSnapshot{
			UID:            uid,
			SessionUID:     sess.UID,
...
func Test_OnlyQueriesStatusFromGMSWhenRequired(t *testing.T) {
	...
	uid := uuid.NewString()
	_, err = s.store.CreateSnapshot(context.Background(), cloudmigration.CloudMigrationSnapshot{
		UID:            uid,
		SessionUID:     sess.UID,
		Status:         cloudmigration.SnapshotStatusCreating,
		GMSSnapshotUID: "gms uid",
	...
	for _, status := range []cloudmigration.SnapshotStatus{
		...
		require.Eventually(
			t,
			func() bool {
				cms, _ := s.store.GetSnapshotByUID(context.Background(), sess.OrgID, sess.UID, snapshotUID, 0, 0)
