...
func createFolderWithACL(t *testing.T, sqlStore db.DB, title string, user *user.SignedInUser,
	...
	dashboardStore, err := database.ProvideDashboardStore(sqlStore, cfg, featuremgmt.WithFeatures(), tagimpl.ProvideService(sqlStore, cfg), quotaService)
	require.NoError(t, err)
	s := folderimpl.ProvideService(ac, bus.ProvideBus(tracing.InitializeTracerForTest()), cfg, dashboardStore, nil, features, folderPermissions)
...
func testScenario(t *testing.T, desc string, fn func(t *testing.T, sc scenarioContext)) {
	...
	t.Run(desc, func(t *testing.T) {
		...
		folderService := folderimpl.ProvideService(ac, bus.ProvideBus(tracing.InitializeTracerForTest()), cfg, dashboardStore, nil, features, folderPermissions)
