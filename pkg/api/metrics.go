...
func (hs *HTTPServer) QueryMetricsV2(c *models.ReqContext, reqDTO dtos.MetricRequest) response.Response {
	if len(reqDTO.Queries) == 0 {
		return utils.Error(400, "No queries found in query", nil)
	}
	...
	for i, query := range reqDTO.Queries {
		...
		if err != nil {
			...
			return utils.Error(400, "Query missing data source ID", nil)
		}
		...
		if i == 0 && !hasExpr {
			...
			if err != nil {
				...
				if errors.Is(err, models.ErrDataSourceAccessDenied) {
					return utils.Error(403, "Access denied to data source", err)
				}
				if errors.Is(err, models.ErrDataSourceNotFound) {
					return utils.Error(400, "Invalid data source ID", err)
				}
				return utils.Error(500, "Unable to load data source metadata", err)
			}
		}
	...
	if !hasExpr {
		...
		if err != nil {
			return utils.Error(500, "Metric request error", err)
		}
	} else {
		if !hs.Cfg.IsExpressionsEnabled() {
			return utils.Error(404, "Expressions feature toggle is not enabled", nil)
		}

		resp, err = expr.WrapTransformData(c.Req.Context(), request)
		if err != nil {
			return utils.Error(500, "Transform request error", err)
		}
	}
...
func (hs *HTTPServer) QueryMetrics(c *models.ReqContext, reqDto dtos.MetricRequest) response.Response {
	...
	if err != nil {
		return utils.Error(400, "Query missing datasourceId", nil)
	}

	ds, err := hs.DatasourceCache.GetDatasource(datasourceId, c.SignedInUser, c.SkipCache)
	if err != nil {
		if errors.Is(err, models.ErrDataSourceAccessDenied) {
			return utils.Error(403, "Access denied to datasource", err)
		}
		return utils.Error(500, "Unable to load datasource meta data", err)
	}
	...
	return utils.JSON(statusCode, &resp)
}
...
func GenerateError(c *models.ReqContext) response.Response {
	var array []string
	// nolint: govet
	return utils.JSON(200, array[20])
}
...
func GenerateSQLTestData(c *models.ReqContext) response.Response {
	if err := bus.Dispatch(&models.InsertSQLTestDataCommand{}); err != nil {
		return utils.Error(500, "Failed to insert test data", err)
	}

	return utils.JSON(200, &util.DynMap{"message": "OK"})
}
