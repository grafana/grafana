...
func TestUserTokenApiEndpoint(t *testing.T) {
	...
	Convey("When gets auth tokens for a user", t, func() {
		...
		getUserAuthTokensInternalScenario("Should be successful", currentToken, func(sc *scenarioContext) {
			...
			uaOne := resultOne.Get("userAgent")
			browserOne := uaOne.Get("browser")
			So(browserOne.Get("family").MustString(), ShouldEqual, "Chrome")
			So(browserOne.Get("version").MustString(), ShouldEqual, "72.0")
			deviceOne := uaOne.Get("device")
			So(deviceOne.Get("family").MustString(), ShouldEqual, "Other")
			So(deviceOne.Get("brand").MustString(), ShouldEqual, "")
			osOne := uaOne.Get("os")
			...
			uaTwo := resultTwo.Get("userAgent")
			browserTwo := uaTwo.Get("browser")
			So(browserTwo.Get("family").MustString(), ShouldEqual, "Mobile Safari")
			So(browserTwo.Get("version").MustString(), ShouldEqual, "11.0")
			deviceTwo := uaTwo.Get("device")
			So(deviceTwo.Get("family").MustString(), ShouldEqual, "iPhone")
			So(deviceTwo.Get("brand").MustString(), ShouldEqual, "Apple")
			osTwo := uaTwo.Get("os")
