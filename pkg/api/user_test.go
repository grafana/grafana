...
func TestIntegrationUserAPIEndpoint_userLoggedIn(t *testing.T) {
	...
	loggedInUserScenario(t, "When calling GET on", "api/users/1", "api/users/:id", func(sc *scenarioContext) {
		...
		authInfoStore, err := authinfoimpl.ProvideStore(context.Background(), legacysql.NewDatabaseProvider(sqlStore), secretsService, sc.cfg, nil, tracing.InitializeTracerForTest())
