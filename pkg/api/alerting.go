...
func GetAlertStatesForDashboard(c *models.ReqContext) response.Response {
	...
	if dashboardID == 0 {
		return utils.Error(400, "Missing query parameter dashboardId", nil)
	}
	...
	if err := bus.Dispatch(&query); err != nil {
		return utils.Error(500, "Failed to fetch alert states", err)
	}
...
func GetAlerts(c *models.ReqContext) response.Response {
	...
	if dashboardQuery != "" || len(dashboardTags) > 0 || len(stringFolderIDs) > 0 {
		...
		if err != nil {
			return utils.Error(500, "List alerts failed", err)
		}
		...
		if len(dashboardIDs) == 0 {
			return utils.JSON(200, []*models.AlertListItemDTO{})
		}
	}
...
func AlertTest(c *models.ReqContext, dto dtos.AlertTestCommand) response.Response {
	if _, idErr := dto.Dashboard.Get("id").Int64(); idErr != nil {
		return utils.Error(400, "The dashboard needs to be saved at least once before you can test an alert rule", nil)
	}
	...
	if err := bus.Dispatch(&backendCmd); err != nil {
		...
		if errors.As(err, &validationErr) {
			return utils.Error(422, validationErr.Error(), nil)
		}
		if errors.Is(err, models.ErrDataSourceAccessDenied) {
			return utils.Error(403, "Access denied to datasource", err)
		}
		return utils.Error(500, "Failed to test rule", err)
	}
...
func GetAlertNotifiers(c *models.ReqContext) response.Response {
	return utils.JSON(200, alerting.GetNotifiers())
}
...
func GetAlertNotificationLookup(c *models.ReqContext) response.Response {
	...
	if err != nil {
		return utils.Error(500, "Failed to get alert notifications", err)
	}
...
func GetAlertNotificationByID(c *models.ReqContext) response.Response {
	...
	if query.Id == 0 {
		return utils.Error(404, "Alert notification not found", nil)
	}
	...
	return utils.JSON(200, dtos.NewAlertNotification(query.Result))
}
...
func CreateAlertNotification(c *models.ReqContext, cmd models.CreateAlertNotificationCommand) response.Response {
	...
	if err := bus.Dispatch(&cmd); err != nil {
		if errors.Is(err, models.ErrAlertNotificationWithSameNameExists) || errors.Is(err, models.ErrAlertNotificationWithSameUIDExists) {
			return utils.Error(409, "Failed to create alert notification", err)
		}
		return utils.Error(500, "Failed to create alert notification", err)
	}

	return utils.JSON(200, dtos.NewAlertNotification(cmd.Result))
}
...
func UpdateAlertNotification(c *models.ReqContext, cmd models.UpdateAlertNotificationCommand) response.Response {
	...
	if err != nil {
		return utils.Error(500, "Failed to update alert notification", err)
	}

	if err := bus.Dispatch(&cmd); err != nil {
		if errors.Is(err, models.ErrAlertNotificationNotFound) {
			return utils.Error(404, err.Error(), err)
		}
		return utils.Error(500, "Failed to update alert notification", err)
	}
	...
	if err := bus.Dispatch(&query); err != nil {
		return utils.Error(500, "Failed to get alert notification", err)
	}
...
func UpdateAlertNotificationByUID(c *models.ReqContext, cmd models.UpdateAlertNotificationWithUidCommand) response.Response {
	...
	if err := bus.Dispatch(&cmd); err != nil {
		if errors.Is(err, models.ErrAlertNotificationNotFound) {
			return utils.Error(404, err.Error(), nil)
		}
		return utils.Error(500, "Failed to update alert notification", err)
	}
...
func DeleteAlertNotification(c *models.ReqContext) response.Response {
	...
	if err := bus.Dispatch(&cmd); err != nil {
		...
		return utils.Error(500, "Failed to delete alert notification", err)
	}

	return utils.Success("Notification deleted")
}
...
func NotificationTest(c *models.ReqContext, dto dtos.NotificationTestCommand) response.Response {
	...
	if err := bus.DispatchCtx(c.Req.Context(), cmd); err != nil {
		...
		return utils.Error(500, "Failed to send alert notifications", err)
	}

	return utils.Success("Test notification sent")
}
...
func PauseAlert(c *models.ReqContext, dto dtos.PauseAlertCommand) response.Response {
	...
	if err := bus.Dispatch(&query); err != nil {
		return utils.Error(500, "Get Alert failed", err)
	}

	guardian := guardian.New(query.Result.DashboardId, c.OrgId, c.SignedInUser)
	if canEdit, err := guardian.CanEdit(); err != nil || !canEdit {
		if err != nil {
			return utils.Error(500, "Error while checking permissions for Alert", err)
		}

		return utils.Error(403, "Access denied to this dashboard and alert", nil)
	}
...
func PauseAllAlerts(c *models.ReqContext, dto dtos.PauseAllAlertsCommand) response.Response {
	...
	if err := bus.Dispatch(&updateCmd); err != nil {
		return utils.Error(500, "Failed to pause alerts", err)
	}
