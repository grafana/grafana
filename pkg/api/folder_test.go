...
// TestGetFolderByUID_ViewPermissionPreventsCanSave verifies that when a user has only "View" permission
// on a folder (FoldersRead + AlertingRuleRead but no Write/Update/Create/Delete), the folder DTO
// has CanSave false so that alert rule editing is prevented in the UI (fixes #119080).
func TestGetFolderByUID_ViewPermissionPreventsCanSave(t *testing.T) {
	folderUID := "view-only-folder"
	f := &folder.Folder{UID: folderUID, Title: "View Only Folder", OrgID: 1}
	folderScope := dashboards.ScopeFoldersProvider.GetResourceScopeUID(folderUID)

	folderService := &foldertest.FakeService{ExpectedFolder: f}
	server := SetupAPITestServer(t, func(hs *HTTPServer) {
		hs.Cfg = setting.NewCfg()
		hs.folderService = folderService
		hs.QuotaService = quotatest.New(false, nil)
	...
	// View-only: FoldersRead and AlertingRuleRead, but no FoldersWrite or AlertingRule Create/Update/Delete
	viewOnlyPermissions := []accesscontrol.Permission{
		{Action: dashboards.ActionFoldersRead, Scope: folderScope},
		{Action: accesscontrol.ActionAlertingRuleRead, Scope: folderScope},
	}
	req := server.NewRequest(http.MethodGet, "/api/folders/"+folderUID, nil)
	req = webtest.RequestWithSignedInUser(req, &user.SignedInUser{
		UserID: 1, OrgID: 1,
		Permissions: map[int64]map[string][]string{
			1: accesscontrol.GroupScopesByActionContext(context.Background(), viewOnlyPermissions),
		},
	})
	...
	var body dtos.Folder
	require.NoError(t, json.NewDecoder(resp.Body).Decode(&body))
	assert.False(t, body.CanSave, "View-only permission must not grant CanSave; alert rule editing must be prevented")
	assert.False(t, body.CanEdit, "View-only permission must not grant CanEdit")
}
