...
func TestAnnotations_AccessControl(t *testing.T) {
	tests := []accessControlTestCase{
		{
			expectedCode: http.StatusOK,
			desc:         "Annotation should return 200 for user with correct permissions",
			url:          "/api/annotations",
			method:       http.MethodGet,
			permissions: []*accesscontrol.Permission{
				{
					Action: accesscontrol.AnnotationRead,
				},
			},
		},
		{
			expectedCode: http.StatusForbidden,
			desc:         "Annotation should return 403 for user without required permissions",
			url:          "/api/annotations",
			method:       http.MethodGet,
			permissions: []*accesscontrol.Permission{
				{
