...
func pluginAssetScenario(t *testing.T, desc string, url string, urlPattern string,
	...
	t.Run(fmt.Sprintf("%s %s", desc, url), func(t *testing.T) {
		...
		hs := HTTPServer{
			Cfg:             cfg,
			pluginStore:     store.New(pluginRegistry, &fakes.FakeSourceRegistry{}, &fakes.FakeLoader{}),
	...
	for _, tc := range tcs {
		t.Run(tc.desc, func(t *testing.T) {
			server := SetupAPITestServer(t, func(hs *HTTPServer) {
				...
				hs.pluginStore = store.New(pluginRegistry, &fakes.FakeSourceRegistry{}, &fakes.FakeLoader{})
