...
func GetPendingOrgInvites(c *models.ReqContext) response.Response {
	...
	if err := bus.Dispatch(&query); err != nil {
		return utils.Error(500, "Failed to get invites from db", err)
	}
...
func AddOrgInvite(c *models.ReqContext, inviteDto dtos.AddInviteForm) response.Response {
	if !inviteDto.Role.IsValid() {
		return utils.Error(400, "Invalid role specified", nil)
	}
	...
	if err := bus.Dispatch(&userQuery); err != nil {
		if !errors.Is(err, models.ErrUserNotFound) {
			return utils.Error(500, "Failed to query db for existing user check", err)
		}
	} else {
	...
	if setting.DisableLoginForm {
		return utils.Error(400, "Cannot invite when login is disabled.", nil)
	}
	...
	if err != nil {
		return utils.Error(500, "Could not generate random string", err)
	}
	cmd.Role = inviteDto.Role
	cmd.RemoteAddr = c.Req.RemoteAddr

	if err := bus.Dispatch(&cmd); err != nil {
		return utils.Error(500, "Failed to save invite to database", err)
	}
	...
	if inviteDto.SendEmail && util.IsEmail(inviteDto.LoginOrEmail) {
		...
		if err := bus.Dispatch(&emailCmd); err != nil {
			if errors.Is(err, models.ErrSmtpNotEnabled) {
				return utils.Error(412, err.Error(), err)
			}

			return utils.Error(500, "Failed to send email invite", err)
		}

		emailSentCmd := models.UpdateTempUserWithEmailSentCommand{Code: cmd.Result.Code}
		if err := bus.Dispatch(&emailSentCmd); err != nil {
			return utils.Error(500, "Failed to update invite with email sent info", err)
		}

		return utils.Success(fmt.Sprintf("Sent invite to %s", inviteDto.LoginOrEmail))
	}

	return utils.Success(fmt.Sprintf("Created invite for %s", inviteDto.LoginOrEmail))
}
...
func inviteExistingUserToOrg(c *models.ReqContext, user *models.User, inviteDto *dtos.AddInviteForm) response.Response {
	...
	if err := bus.Dispatch(&createOrgUserCmd); err != nil {
		if errors.Is(err, models.ErrOrgUserAlreadyAdded) {
			return utils.Error(412, fmt.Sprintf("User %s is already added to organization", inviteDto.LoginOrEmail), err)
		}
		return utils.Error(500, "Error while trying to create org user", err)
	}
	...
	if inviteDto.SendEmail && util.IsEmail(user.Email) {
		...
		if err := bus.Dispatch(&emailCmd); err != nil {
			return utils.Error(500, "Failed to send email invited_to_org", err)
		}
	}
...
func RevokeInvite(c *models.ReqContext) response.Response {
	...
	return utils.Success("Invite revoked")
}
...
func GetInviteInfoByCode(c *models.ReqContext) response.Response {
	...
	if err := bus.Dispatch(&query); err != nil {
		if errors.Is(err, models.ErrTempUserNotFound) {
			return utils.Error(404, "Invite not found", nil)
		}
		return utils.Error(500, "Failed to get invite", err)
	}
	...
	return utils.JSON(200, dtos.InviteInfo{
		Email:     invite.Email,
		Name:      invite.Name,
...
func (hs *HTTPServer) CompleteInvite(c *models.ReqContext, completeInvite dtos.CompleteInviteForm) response.Response {
	...
	invite := query.Result
	if invite.Status != models.TmpUserInvitePending {
		return utils.Error(412, fmt.Sprintf("Invite cannot be used in status %s", invite.Status), nil)
	}
	...
	if err := bus.Dispatch(&cmd); err != nil {
		if errors.Is(err, models.ErrUserAlreadyExists) {
			return utils.Error(412, fmt.Sprintf("User with email '%s' or username '%s' already exists", completeInvite.Email, completeInvite.Username), err)
		}
	...
	if err != nil {
		return utils.Error(500, "failed to accept invite", err)
	}
...
func updateTempUserStatus(code string, status models.TempUserStatus) (bool, response.Response) {
	...
	if err := bus.Dispatch(&updateTmpUserCmd); err != nil {
		return false, utils.Error(500, "Failed to update invite status", err)
	}
...
func applyUserInvite(user *models.User, invite *models.TempUserDTO, setActive bool) (bool, response.Response) {
	...
	if err := bus.Dispatch(&addOrgUserCmd); err != nil {
		if !errors.Is(err, models.ErrOrgUserAlreadyAdded) {
			return false, utils.Error(500, "Error while trying to create org user", err)
		}
	}
	...
	if setActive {
		...
		if err := bus.Dispatch(&models.SetUsingOrgCommand{OrgId: invite.OrgId, UserId: user.Id}); err != nil {
			return false, utils.Error(500, "Failed to set org as active", err)
		}
	}
