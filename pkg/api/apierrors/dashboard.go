...
func ToDashboardErrorResponse(ctx context.Context, pluginRegistry plugins.PublicRegistry, err error) response.Response {
	...
	if ok := errors.As(err, &pluginErr); ok {
		...
		// look up plugin name
		if plugin, exists := pluginRegistry.Plugin(ctx, pluginErr.PluginId); exists {
