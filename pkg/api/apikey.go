...
// @PERCONA
func (hs *HTTPServer) GetAPIKeyCurrent(c *contextmodel.ReqContext) response.Response {
	query := apikey.GetByIDQuery{ApiKeyID: c.ApiKeyID}
	apikey, err := hs.apiKeyService.GetApiKeyById(c.Req.Context(), &query)
	...
	if apikey.Expires != nil {
		v := time.Unix(*apikey.Expires, 0)
		expiration = &v
	}
