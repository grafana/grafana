...
func TestDSRouteRule(t *testing.T) {
	...
	Convey("DataSourceProxy", t, func() {
		...
		Convey("When datasource hasn't been updated, encrypted JSON should be fetched from cache", func() {
			m.ClearDSDecryptionCache()
		...
		Convey("When datasource is updated, encrypted JSON should not be fetched from cache", func() {
			m.ClearDSDecryptionCache()
			...
			req, err = http.NewRequest(http.MethodGet, "http://grafana.com/sub", nil)
			So(err, ShouldBeNil)
			// Change datasource update time, invalidating cache item
			ds.Updated = time.Now()
			ds.SecureJsonData = securejsondata.GetEncryptedJsonData(map[string]string{
				"password": "",
			})
