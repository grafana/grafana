...
func TestDataSourceProxy_routeRule(t *testing.T) {
	...
	t.Run("Plugin with routes", func(t *testing.T) {
		...
		t.Run("When matching route path", func(t *testing.T) {
			...
			dsService, err := datasourceservice.ProvideService(nil, secretsService, secretsStore, cfg, features, acmock.New(), acmock.NewMockedPermissionsService(), quotaService)
			require.NoError(t, err)
			proxy, err := NewDataSourceProxy(ds, routes, ctx, "api/v4/some/method", cfg, httpClientProvider,
				&oauthtoken.Service{}, dsService, tracer, features)
			require.NoError(t, err)
		...
		t.Run("When matching route path and has dynamic url", func(t *testing.T) {
			...
			dsService, err := datasourceservice.ProvideService(nil, secretsService, secretsStore, cfg, features, acmock.New(), acmock.NewMockedPermissionsService(), quotaService)
			require.NoError(t, err)
			proxy, err := NewDataSourceProxy(ds, routes, ctx, "api/common/some/method", cfg, httpClientProvider, &oauthtoken.Service{}, dsService, tracer, features)
			require.NoError(t, err)
		...
		t.Run("When matching route path with no url", func(t *testing.T) {
			...
			dsService, err := datasourceservice.ProvideService(nil, secretsService, secretsStore, cfg, features, acmock.New(), acmock.NewMockedPermissionsService(), quotaService)
			require.NoError(t, err)
			proxy, err := NewDataSourceProxy(ds, routes, ctx, "", cfg, httpClientProvider, &oauthtoken.Service{}, dsService, tracer, features)
			require.NoError(t, err)
		...
		t.Run("When matching route path and has dynamic body", func(t *testing.T) {
			...
			dsService, err := datasourceservice.ProvideService(nil, secretsService, secretsStore, cfg, features, acmock.New(), acmock.NewMockedPermissionsService(), quotaService)
			require.NoError(t, err)
			proxy, err := NewDataSourceProxy(ds, routes, ctx, "api/body", cfg, httpClientProvider, &oauthtoken.Service{}, dsService, tracer, features)
			require.NoError(t, err)
		...
		t.Run("Validating request", func(t *testing.T) {
			t.Run("plugin route with valid role", func(t *testing.T) {
				...
				dsService, err := datasourceservice.ProvideService(nil, secretsService, secretsStore, cfg, features, acmock.New(), acmock.NewMockedPermissionsService(), quotaService)
				require.NoError(t, err)
				proxy, err := NewDataSourceProxy(ds, routes, ctx, "api/v4/some/method", cfg, httpClientProvider, &oauthtoken.Service{}, dsService, tracer, features)
				require.NoError(t, err)
			...
			t.Run("plugin route with admin role and user is editor", func(t *testing.T) {
				...
				dsService, err := datasourceservice.ProvideService(nil, secretsService, secretsStore, cfg, features, acmock.New(), acmock.NewMockedPermissionsService(), quotaService)
				require.NoError(t, err)
				proxy, err := NewDataSourceProxy(ds, routes, ctx, "api/admin", cfg, httpClientProvider, &oauthtoken.Service{}, dsService, tracer, features)
				require.NoError(t, err)
				err = proxy.validateRequest()
				require.Error(t, err)
			})
	...
	t.Run("Plugin with multiple routes for token auth", func(t *testing.T) {
		...
		t.Run("When creating and caching access tokens", func(t *testing.T) {
			...
			t.Run("first call should add authorization header with access token", func(t *testing.T) {
				...
				dsService, err := datasourceservice.ProvideService(nil, secretsService, secretsStore, cfg, features, acmock.New(), acmock.NewMockedPermissionsService(), quotaService)
				require.NoError(t, err)
				proxy, err := NewDataSourceProxy(ds, routes, ctx, "pathwithtoken1", cfg, httpClientProvider, &oauthtoken.Service{}, dsService, tracer, features)
				require.NoError(t, err)
				...
				t.Run("second call to another route should add a different access token", func(t *testing.T) {
					...
					dsService, err := datasourceservice.ProvideService(nil, secretsService, secretsStore, cfg, features, acmock.New(), acmock.NewMockedPermissionsService(), quotaService)
					require.NoError(t, err)
					proxy, err := NewDataSourceProxy(ds, routes, ctx, "pathwithtoken2", cfg, httpClientProvider, &oauthtoken.Service{}, dsService, tracer, features)
					require.NoError(t, err)
	...
	t.Run("When proxying graphite", func(t *testing.T) {
		...
		dsService, err := datasourceservice.ProvideService(nil, secretsService, secretsStore, cfg, features, acmock.New(), acmock.NewMockedPermissionsService(), quotaService)
		require.NoError(t, err)
		proxy, err := NewDataSourceProxy(ds, routes, ctx, "/render", &setting.Cfg{BuildVersion: "5.3.0"}, httpClientProvider, &oauthtoken.Service{}, dsService, tracer, features)
		require.NoError(t, err)
	...
	t.Run("When proxying InfluxDB", func(t *testing.T) {
		...
		dsService, err := datasourceservice.ProvideService(nil, secretsService, secretsStore, cfg, features, acmock.New(), acmock.NewMockedPermissionsService(), quotaService)
		require.NoError(t, err)
		proxy, err := NewDataSourceProxy(ds, routes, ctx, "", &setting.Cfg{}, httpClientProvider, &oauthtoken.Service{}, dsService, tracer, features)
		require.NoError(t, err)
	...
	t.Run("When proxying a data source with keep cookies specified", func(t *testing.T) {
		...
		dsService, err := datasourceservice.ProvideService(nil, secretsService, secretsStore, cfg, features, acmock.New(), acmock.NewMockedPermissionsService(), quotaService)
		require.NoError(t, err)
		proxy, err := NewDataSourceProxy(ds, pluginRoutes, ctx, "", &setting.Cfg{}, httpClientProvider, &oauthtoken.Service{}, dsService, tracer, features)
		require.NoError(t, err)
	...
	t.Run("When proxying a custom datasource", func(t *testing.T) {
		...
		require.NoError(t, err)
		proxy, err := NewDataSourceProxy(ds, routes, ctx, "/path/to/folder/", &setting.Cfg{}, httpClientProvider, &oauthtoken.Service{}, dsService, tracer, features)
		require.NoError(t, err)
		req, err := http.NewRequest(http.MethodGet, "http://grafana.com/sub", nil)
	...
	t.Run("When proxying a datasource that has OAuth token pass-through enabled", func(t *testing.T) {
		...
		dsService, err := datasourceservice.ProvideService(nil, secretsService, secretsStore, cfg, features, acmock.New(), acmock.NewMockedPermissionsService(), quotaService)
		require.NoError(t, err)
		proxy, err := NewDataSourceProxy(ds, routes, ctx, "/path/to/folder/", &setting.Cfg{}, httpClientProvider, &mockAuthToken, dsService, tracer, features)
		require.NoError(t, err)
		req, err = http.NewRequest(http.MethodGet, "http://grafana.com/sub", nil)
		require.NoError(t, err)
...
func TestDataSourceProxy_requestHandling(t *testing.T) {
	...
	t.Run("When response header Set-Cookie is not set should remove proxied Set-Cookie header", func(t *testing.T) {
		...
		dsService, err := datasourceservice.ProvideService(nil, secretsService, secretsStore, cfg, features, acmock.New(), acmock.NewMockedPermissionsService(), quotaService)
		require.NoError(t, err)
		proxy, err := NewDataSourceProxy(ds, routes, ctx, "/render", &setting.Cfg{}, httpClientProvider, &oauthtoken.Service{}, dsService, tracer, features)
		require.NoError(t, err)
...
func TestNewDataSourceProxy_MSSQL(t *testing.T) {
	...
	for _, tc := range tcs {
		t.Run(tc.description, func(t *testing.T) {
			...
			secretsStore := secretskvs.NewSQLSecretsKVStore(sqlStore, secretsService, log.New("test.logger"))
			quotaService := quotatest.New(false, nil)
			tures := featuremgmt.WithFeatures()
			dsService, err := datasourceservice.ProvideService(nil, secretsService, secretsStore, cfg, features, acmock.New(), acmock.NewMockedPermissionsService(), quotaService)
			require.NoError(t, err)
			p, err := NewDataSourceProxy(&ds, routes, &ctx, "api/method", cfg, httpclient.NewProvider(), &oauthtoken.Service{}, dsService, tracer, features)
			if tc.err == nil {
				require.NoError(t, err)
...
func Test_PathCheck(t *testing.T) {
	...
	dsService, err := datasourceservice.ProvideService(nil, secretsService, secretsStore, cfg, features, acmock.New(), acmock.NewMockedPermissionsService(), quotaService)
	require.NoError(t, err)
	proxy, err := NewDataSourceProxy(&datasources.DataSource{}, routes, ctx, "b", &setting.Cfg{}, httpclient.NewProvider(), &oauthtoken.Service{}, dsService, tracer, features)
	require.NoError(t, err)
