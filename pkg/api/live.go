...
func (hs *HTTPServer) LiveHTTPHandler(c *models.ReqContext) {
	...
	if addr.IsValid() {
		id := addr.ToChannelID()
		channel, err := hs.Live.GetChannelHandler(c.Req.Context(), id)
		if err == nil {
	...
	if addr.Namespace != "" {
		ns, err := hs.Live.GetChannelHandlerFactory(c.Req.Context(), addr.Scope, addr.Namespace)
		if err != nil {
			c.JSON(400, util.DynMap{
				"message": err.Error(),
			})
