...
func (hs *HTTPServer) registerRoutes() {
	...
	r.Get("/org/users", authorize(reqOrgAdmin, eval.Permission(accesscontrol.ActionOrgUsersRead, accesscontrol.ScopeUsersAll)), hs.Index)
	r.Get("/org/users/new", reqOrgAdmin, hs.Index)
	r.Get("/org/users/invite", authorize(reqOrgAdmin, eval.Permission(accesscontrol.ActionUsersCreate, eval.ScopeNone)), hs.Index)
	...
	r.Group("/api", func(apiRoute routing.RouteRegister) {
		...
		apiRoute.Group("/users", func(usersRoute routing.RouteRegister) {
			...
			usersRoute.Get("/", authorize(reqGrafanaAdmin, eval.Permission(accesscontrol.ActionUsersRead, accesscontrol.ScopeGlobalUsersAll)), routing.Wrap(SearchUsers))
			usersRoute.Get("/search", authorize(reqGrafanaAdmin, eval.Permission(accesscontrol.ActionUsersRead, accesscontrol.ScopeGlobalUsersAll)), routing.Wrap(SearchUsersWithPaging))
			usersRoute.Get("/:id", authorize(reqGrafanaAdmin, eval.Permission(accesscontrol.ActionUsersRead, userIDScope)), routing.Wrap(GetUserByID))
			usersRoute.Get("/:id/teams", authorize(reqGrafanaAdmin, eval.Permission(accesscontrol.ActionUsersTeamRead, userIDScope)), routing.Wrap(GetUserTeams))
			usersRoute.Get("/:id/orgs", authorize(reqGrafanaAdmin, eval.Permission(accesscontrol.ActionUsersRead, userIDScope)), routing.Wrap(GetUserOrgList))
			// query parameters /users/lookup?loginOrEmail=admin@example.com
			usersRoute.Get("/lookup", authorize(reqGrafanaAdmin, eval.Permission(accesscontrol.ActionUsersRead, accesscontrol.ScopeGlobalUsersAll)), routing.Wrap(GetUserByLoginOrEmail))
			usersRoute.Put("/:id", authorize(reqGrafanaAdmin, eval.Permission(accesscontrol.ActionUsersWrite, userIDScope)), bind(models.UpdateUserCommand{}), routing.Wrap(UpdateUser))
			usersRoute.Post("/:id/using/:orgId", authorize(reqGrafanaAdmin, eval.Permission(accesscontrol.ActionUsersWrite, userIDScope)), routing.Wrap(UpdateUserActiveOrg))
		})
		...
		// current org
		apiRoute.Group("/org", func(orgRoute routing.RouteRegister) {
			userIDScope := eval.Combine("users", eval.Parameter(":userId"))
			...
			orgRoute.Get("/users", authorize(reqOrgAdmin, eval.Permission(accesscontrol.ActionOrgUsersRead, accesscontrol.ScopeUsersAll)), routing.Wrap(hs.GetOrgUsersForCurrentOrg))
			orgRoute.Get("/users/search", authorize(reqOrgAdmin, eval.Permission(accesscontrol.ActionOrgUsersRead, accesscontrol.ScopeUsersAll)), routing.Wrap(hs.SearchOrgUsersWithPaging))
			orgRoute.Post("/users", authorize(reqOrgAdmin, eval.Permission(accesscontrol.ActionOrgUsersAdd, accesscontrol.ScopeUsersAll)), quota("user"), bind(models.AddOrgUserCommand{}), routing.Wrap(AddOrgUserToCurrentOrg))
			orgRoute.Patch("/users/:userId", authorize(reqOrgAdmin, eval.Permission(accesscontrol.ActionOrgUsersRoleUpdate, userIDScope)), bind(models.UpdateOrgUserCommand{}), routing.Wrap(UpdateOrgUserForCurrentOrg))
		...
		apiRoute.Group("/orgs/:orgId", func(orgsRoute routing.RouteRegister) {
			...
			orgsRoute.Get("/users", authorize(reqGrafanaAdmin, eval.Permission(accesscontrol.ActionOrgUsersRead, accesscontrol.ScopeUsersAll)), routing.Wrap(hs.GetOrgUsers))
			orgsRoute.Post("/users", authorize(reqGrafanaAdmin, eval.Permission(accesscontrol.ActionOrgUsersAdd, accesscontrol.ScopeUsersAll)), bind(models.AddOrgUserCommand{}), routing.Wrap(AddOrgUser))
			orgsRoute.Patch("/users/:userId", authorize(reqGrafanaAdmin, eval.Permission(accesscontrol.ActionOrgUsersRoleUpdate, userIDScope)), bind(models.UpdateOrgUserCommand{}), routing.Wrap(UpdateOrgUser))
			orgsRoute.Delete("/users/:userId", authorize(reqGrafanaAdmin, eval.Permission(accesscontrol.ActionOrgUsersRemove, userIDScope)), routing.Wrap(RemoveOrgUser))
	...
	r.Group("/api/admin/users", func(adminUserRoute routing.RouteRegister) {
		...
		adminUserRoute.Post("/:id/disable", authorize(reqGrafanaAdmin, eval.Permission(accesscontrol.ActionUsersDisable, userIDScope)), routing.Wrap(hs.AdminDisableUser))
		adminUserRoute.Post("/:id/enable", authorize(reqGrafanaAdmin, eval.Permission(accesscontrol.ActionUsersEnable, userIDScope)), routing.Wrap(AdminEnableUser))
		adminUserRoute.Get("/:id/quotas", authorize(reqGrafanaAdmin, eval.Permission(accesscontrol.ActionUsersQuotasList, userIDScope)), routing.Wrap(GetUserQuotas))
		adminUserRoute.Put("/:id/quotas/:target", authorize(reqGrafanaAdmin, eval.Permission(accesscontrol.ActionUsersQuotasUpdate, userIDScope)), bind(models.UpdateUserQuotaCmd{}), routing.Wrap(UpdateUserQuota))

		adminUserRoute.Post("/:id/logout", authorize(reqGrafanaAdmin, eval.Permission(accesscontrol.ActionUsersLogout, userIDScope)), routing.Wrap(hs.AdminLogoutUser))
		adminUserRoute.Get("/:id/auth-tokens", authorize(reqGrafanaAdmin, eval.Permission(accesscontrol.ActionUsersAuthTokenList, userIDScope)), routing.Wrap(hs.AdminGetUserAuthTokens))
		adminUserRoute.Post("/:id/revoke-auth-token", authorize(reqGrafanaAdmin, eval.Permission(accesscontrol.ActionUsersAuthTokenUpdate, userIDScope)), bind(models.RevokeAuthTokenCmd{}), routing.Wrap(hs.AdminRevokeUserAuthToken))
	})
