...
func (hs *HTTPServer) RemoveTeamMember(c *models.ReqContext) response.Response {
	...
	if _, err := hs.TeamPermissionsService.SetUserPermission(c.Req.Context(), orgId, types.User{ID: userId}, teamIDString, ""); err != nil {
...
var addOrUpdateTeamMember = func(ctx context.Context, resourcePermissionService *resourcepermissions.Service, userID, orgID, teamID int64, permission string) error {
	teamIDString := strconv.FormatInt(teamID, 10)
	if _, err := resourcePermissionService.SetUserPermission(ctx, orgID, types.User{ID: userID}, teamIDString, permission); err != nil {
