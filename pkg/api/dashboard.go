...
func dashboardGuardianResponse(err error) response.Response {
	...
	return utils.Error(403, "Access denied to this dashboard", nil)
}
...
func (hs *HTTPServer) GetDashboard(c *models.ReqContext) response.Response {
	...
	if dash.Data != nil {
		...
		if isEmptyData {
			return utils.Error(500, "Error while loading dashboard, dashboard data is invalid", nil)
		}
	}
	...
	if err != nil {
		return utils.Error(500, "Error while checking if dashboard was starred by user", err)
	}
	...
	if dash.FolderId > 0 {
		...
		if err := bus.Dispatch(&query); err != nil {
			return utils.Error(500, "Dashboard folder could not be read", err)
		}
	...
	if err != nil {
		return utils.Error(500, "Error while checking if dashboard is provisioned", err)
	}
...
func getDashboardHelper(orgID int64, slug string, id int64, uid string) (*models.Dashboard, response.Response) {
	...
	if err := bus.Dispatch(&query); err != nil {
		return nil, utils.Error(404, "Dashboard not found", err)
	}
...
func DeleteDashboardBySlug(c *models.ReqContext) response.Response {
	...
	if err := bus.Dispatch(&query); err != nil {
		return utils.Error(500, "Failed to retrieve dashboards by slug", err)
	}

	if len(query.Result) > 1 {
		return utils.JSON(412, util.DynMap{"status": "multiple-slugs-exists", "message": models.ErrDashboardsWithSameSlugExists.Error()})
	}
	...
	if err != nil {
		...
		if ok := errors.As(err, &dashboardErr); ok {
			if errors.Is(err, models.ErrDashboardCannotDeleteProvisionedDashboard) {
				return utils.Error(dashboardErr.StatusCode, dashboardErr.Error(), err)
			}
		}

		return utils.Error(500, "Failed to delete dashboard", err)
	}
...
func (hs *HTTPServer) PostDashboard(c *models.ReqContext, cmd models.SaveDashboardCommand) response.Response {
	...
	if newDashboard {
		...
		if err != nil {
			return utils.Error(500, "failed to get quota", err)
		}
		if limitReached {
			return utils.Error(403, "Quota reached", nil)
		}
	}
...
func dashboardSaveErrorToApiResponse(err error) response.Response {
	...
	if ok := errors.As(err, &dashboardErr); ok {
		if body := dashboardErr.Body(); body != nil {
			return utils.JSON(dashboardErr.StatusCode, body)
		}
		if errors.Is(dashboardErr, models.ErrDashboardUpdateAccessDenied) {
			return utils.Error(dashboardErr.StatusCode, dashboardErr.Error(), err)
		}
		return utils.Error(dashboardErr.StatusCode, dashboardErr.Error(), nil)
	}
	...
	if ok := errors.As(err, &pluginErr); ok {
		...
		return utils.JSON(412, util.DynMap{"status": "plugin-dashboard", "message": message})
	}

	return utils.Error(500, "Failed to save dashboard", err)
}
...
func (hs *HTTPServer) GetHomeDashboard(c *models.ReqContext) response.Response {
	...
	if prefsQuery.Result.HomeDashboardId != 0 {
		...
		if err == nil {
			...
			return utils.JSON(200, &dashRedirect)
		}
	...
	if err != nil {
		return utils.Error(500, "Failed to load home dashboard", err)
	}
	defer func() {
...
func GetDashboardVersions(c *models.ReqContext) response.Response {
	...
	if err := bus.Dispatch(&query); err != nil {
		return utils.Error(404, fmt.Sprintf("No versions found for dashboardId %d", dashID), err)
	}
...
func GetDashboardVersion(c *models.ReqContext) response.Response {
	...
	if err := bus.Dispatch(&query); err != nil {
		return utils.Error(500, fmt.Sprintf("Dashboard version %d not found for dashboardId %d", query.Version, dashID), err)
	}
	...
	return utils.JSON(200, dashVersionMeta)
}
...
func CalculateDashboardDiff(c *models.ReqContext, apiOptions dtos.CalculateDiffOptions) response.Response {
	...
	if err != nil {
		if errors.Is(err, models.ErrDashboardVersionNotFound) {
			return utils.Error(404, "Dashboard version not found", err)
		}
		return utils.Error(500, "Unable to compute diff", err)
	}

	if options.DiffType == dashdiffs.DiffDelta {
		return utils.Respond(200, result.Delta).Header("Content-Type", "application/json")
	}

	return utils.Respond(200, result.Delta).Header("Content-Type", "text/html")
}
...
func (hs *HTTPServer) RestoreDashboardVersion(c *models.ReqContext, apiCmd dtos.RestoreDashboardVersionCommand) response.Response {
	...
	if err := bus.Dispatch(&versionQuery); err != nil {
		return utils.Error(404, "Dashboard version not found", nil)
	}
