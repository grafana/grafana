...
The OpenAPI v2 specification is generated automatically from the annotated Go code using [go-swagger](https://github.com/go-swagger/go-swagger). The code annotation rules can be found [here](https://goswagger.io/use/spec.html).
...
The following defines the route parameters for the `updateServiceAccount` endpoint. The route expects:
...
The following defines the response for the `updateServiceAccount` endpoint in case of a successful `200` response.
