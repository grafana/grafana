...
func addOrgUserHelper(cmd models.AddOrgUserCommand) response.Response {
	...
	if err != nil {
		return utils.Error(404, "User not found", nil)
	}
	...
	if err := bus.Dispatch(&cmd); err != nil {
		if errors.Is(err, models.ErrOrgUserAlreadyAdded) {
			return utils.JSON(409, util.DynMap{
				"message": "User is already member of this organization",
				"userId":  cmd.UserId,
			})
		}
		return utils.Error(500, "Could not add user to organization", err)
	}
...
func (hs *HTTPServer) GetOrgUsersForCurrentOrg(c *models.ReqContext) response.Response {
	...
	if err != nil {
		return utils.Error(500, "Failed to get users for current organization", err)
	}
...
func (hs *HTTPServer) GetOrgUsersForCurrentOrgLookup(c *models.ReqContext) response.Response {
	...
	if !isAdmin {
		return utils.Error(403, "Permission denied", nil)
	}
...
func (hs *HTTPServer) GetOrgUsers(c *models.ReqContext) response.Response {
	...
	if err != nil {
		return utils.Error(500, "Failed to get users for organization", err)
	}
...
func updateOrgUserHelper(cmd models.UpdateOrgUserCommand) response.Response {
	...
	if err := bus.Dispatch(&cmd); err != nil {
		if errors.Is(err, models.ErrLastOrgAdmin) {
			return utils.Error(400, "Cannot change role so that there is no organization admin left", nil)
		}
		return utils.Error(500, "Failed update org user", err)
	}

	return utils.Success("Organization user updated")
}
...
func removeOrgUserHelper(cmd *models.RemoveOrgUserCommand) response.Response {
	if err := bus.Dispatch(cmd); err != nil {
		if errors.Is(err, models.ErrLastOrgAdmin) {
			return utils.Error(400, "Cannot remove last organization admin", nil)
		}
		return utils.Error(500, "Failed to remove user from organization", err)
	}
	...
	return utils.Success("User removed from organization")
}
