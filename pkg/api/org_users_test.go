	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			sc := setupHTTPServerWithCfg(t, false, cfg, func(hs *HTTPServer) {
				...
				require.NoError(t, err)
				hs.userService, err = userimpl.ProvideService(
					hs.SQLStore, hs.orgService, cfg, teamimpl.ProvideService(hs.SQLStore.(*sqlstore.SQLStore), cfg),
					localcache.ProvideService(), quotatest.New(false, nil), nil, featuremgmt.WithFeatures())
				require.NoError(t, err)
			})
