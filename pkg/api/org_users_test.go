...
func TestOrgUsersAPIEndpoint_userLoggedIn(t *testing.T) {
	...
	loggedInUserScenario(t, "When calling GET as an editor with no team / folder permissions on", "api/org/users/lookup", func(sc *scenarioContext) {
		bus.AddHandler("test", func(query *models.GetOrgUsersQuery) error {
			query.Result = mockResult
			return nil
		})
	...
	loggedInUserScenarioWithRole(t, "When calling GET as an admin on", "GET", "api/org/users/lookup", "api/org/users/lookup", models.ROLE_ADMIN, func(sc *scenarioContext) {
		bus.AddHandler("test", func(query *models.GetOrgUsersQuery) error {
			query.Result = mockResult
			return nil
		})
	...
	t.Run("Given there is one hidden user", func(t *testing.T) {
