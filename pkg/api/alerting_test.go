...
func TestAlertingApiEndpoint(t *testing.T) {
	...
	loggedInUserScenarioWithRole(t, "When calling GET on", "GET",
		...
		"/api/alerts", models.ROLE_EDITOR, func(sc *scenarioContext) {
			...
			assert.Equal(t, 1, searchQuery.DashboardIds[0])
			assert.Equal(t, 2, searchQuery.DashboardIds[1])
			assert.Equal(t, 3, searchQuery.FolderIds[0])
			assert.Equal(t, "abs", searchQuery.Tags[0])
			assert.Equal(t, "dbQuery", searchQuery.Title)

			require.NotNil(t, getAlertsQuery)
			assert.Equal(t, 1, getAlertsQuery.DashboardIDs[0])
			assert.Equal(t, 2, getAlertsQuery.DashboardIDs[1])
			assert.Equal(t, 5, getAlertsQuery.Limit)
