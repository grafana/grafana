...
func normalizePreferredEncodings(preferred []string) []string {
	if len(preferred) == 0 {
		return []string{setting.DefaultContentEncoding}
	}
	...
	for _, encoding := range preferred {
		...
		if _, ok := setting.SupportedContentEncodings[encoding]; !ok {
			continue
		}
...
func negotiateEncoding(acceptEncoding string, preferredEncodings []string) string {
	if acceptEncoding == "" {
		return ""
	}
	...
	bestEncoding := ""
	bestQuality := 0.0
	bestPreference := len(preferredEncodings) // lower is better

	for _, c := range candidates {
		for i, supported := range preferredEncodings {
...
// encoding accepted by the client.
// preferredEncodings defines server-side priority and accepted values from: zstd, br, deflate, gzip.
// If preferredEncodings is empty or invalid, gzip is used by default.
func Compressor(preferredEncodings []string) func(http.Handler) http.Handler {
	preferredEncodings = normalizePreferredEncodings(preferredEncodings)
	...
	return func(next http.Handler) http.Handler {
		return http.HandlerFunc(func(rw http.ResponseWriter, req *http.Request) {
			...
			encoding := negotiateEncoding(req.Header.Get("Accept-Encoding"), preferredEncodings)
			if encoding == "" {
				next.ServeHTTP(rw, req)
				return
			}
...
func Gziper() func(http.Handler) http.Handler {
	return Compressor([]string{setting.DefaultContentEncoding})
}
