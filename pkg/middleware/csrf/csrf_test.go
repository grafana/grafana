...
func TestCSRF_Check(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			getCfg: func() *setting.Cfg {
				cfg := setting.NewCfg()
				cfg.SectionWithEnvOverrides("security").Key("csrf_skip_check_if_login_cookie_not_present").SetValue("true")
				return cfg
			},
		...
		},
		{
			...
			getCfg: func() *setting.Cfg {
				cfg := setting.NewCfg()
				cfg.SectionWithEnvOverrides("security").Key("csrf_skip_check_if_login_cookie_not_present").SetValue("false")
				return cfg
			},
			request:        postRequest(t, "grafana.localhost", map[string]string{"X-Forwarded-Host": "grafana.org", "Origin": "https://grafana.org"}, false),
			expectedStatus: http.StatusForbidden,
...
func TestProvideCSRFFilter(t *testing.T) {
	...
	tests := []struct {
		...
		expectedSkipCSRFCheckIfLoginCookieNotPresent bool
	}{
		{
			getInput: func() *setting.Cfg {
				return setting.NewCfg()
			},
			// Should default to true when config value is not set.
			expectedSkipCSRFCheckIfLoginCookieNotPresent: true,
		},
		{
			getInput: func() *setting.Cfg {
				cfg := setting.NewCfg()
			...
			expectedSkipCSRFCheckIfLoginCookieNotPresent: false,
		},
		{
			getInput: func() *setting.Cfg {
				cfg := setting.NewCfg()
	...
	for _, tc := range tests {
		...
		assert.Equal(t, tc.expectedSkipCSRFCheckIfLoginCookieNotPresent, csrf.skipCSRFCheckIfLoginCookieNotPresent)
	}
}
