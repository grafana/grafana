...
func TestLogger(t *testing.T) {
	...
	newLoggerScenario(t, "Logger with contextual arguments", func(t *testing.T, sCtx *scenarioContext) {
		...
		rootLogger := New("root")
		rootLogger.DebugCtx(ctx, "hello root")
		childLogger := rootLogger.New("childKey", "childValue")
		childLogger.ErrorCtx(ctx, "hello child")

		ctx = WithContextualArgs(ctx, "ctxKey", "ctxValue")
		rootLogger.DebugCtx(ctx, "hello contextual root")
		childLogger.ErrorCtx(ctx, "hello contextual child")

		require.Len(t, sCtx.loggedArgs, 4)
		...
		require.Equal(t, "t", sCtx.loggedArgs[2][2].(string))
		require.Equal(t, sCtx.mockedTime.Format(time.RFC3339Nano), sCtx.loggedArgs[2][3].(fmt.Stringer).String())
		require.Equal(t, level.Key().(string), sCtx.loggedArgs[2][4].(string))
		require.Equal(t, level.DebugValue(), sCtx.loggedArgs[2][5].(level.Value))
		require.Equal(t, "msg", sCtx.loggedArgs[2][6].(string))
		require.Equal(t, "hello contextual root", sCtx.loggedArgs[2][7].(string))
		require.Equal(t, "ctxKey", sCtx.loggedArgs[2][8].(string))
		require.Equal(t, "ctxValue", sCtx.loggedArgs[2][9].(string))
		...
		require.Equal(t, "t", sCtx.loggedArgs[3][4].(string))
		require.Equal(t, sCtx.mockedTime.Format(time.RFC3339Nano), sCtx.loggedArgs[3][5].(fmt.Stringer).String())
		require.Equal(t, level.Key().(string), sCtx.loggedArgs[3][6].(string))
		require.Equal(t, level.ErrorValue(), sCtx.loggedArgs[3][7].(level.Value))
		require.Equal(t, "msg", sCtx.loggedArgs[3][8].(string))
		require.Equal(t, "hello contextual child", sCtx.loggedArgs[3][9].(string))
		require.Equal(t, "ctxKey", sCtx.loggedArgs[3][10].(string))
		require.Equal(t, "ctxValue", sCtx.loggedArgs[3][11].(string))
	})
}
