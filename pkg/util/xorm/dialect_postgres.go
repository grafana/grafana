...
func (db *postgres) Init(d *coreDB, uri *coreUri, drivername, dataSourceName string) error {
	err := db.coreBase.Init(d, db, uri, drivername, dataSourceName)
	if err != nil {
		return err
	}
	if db.Schema == "" {
...
func (db *postgres) SqlType(c *coreColumn) string {
	var res string
...
func (db *postgres) ModifyColumnSql(tableName string, col *coreColumn) string {
...
func (db *postgres) DropIndexSql(tableName string, index *coreIndex) string {
	quote := db.Quote
	idxName := index.Name
	...
	if db.coreUri.Schema != "" {
		idxName = db.coreUri.Schema + "." + idxName
	}
...
func (db *postgres) GetColumns(tableName string) ([]string, map[string]*coreColumn, error) {
	args := []any{tableName}
...
WHERE c.relkind = 'r'::char AND c.relname = $1%s AND f.attnum > 0 ORDER BY f.attnum;`
	...
	for rows.Next() {
		...
		case "character varying", "character":
			col.SQLType = coreSQLType{Name: Varchar, DefaultLength: 0, DefaultLength2: 0}
		case "timestamp without time zone":
			col.SQLType = coreSQLType{Name: DateTime, DefaultLength: 0, DefaultLength2: 0}
		case "timestamp with time zone":
			col.SQLType = coreSQLType{Name: TimeStampz, DefaultLength: 0, DefaultLength2: 0}
		case "double precision":
			col.SQLType = coreSQLType{Name: Double, DefaultLength: 0, DefaultLength2: 0}
		case "boolean":
			col.SQLType = coreSQLType{Name: Bool, DefaultLength: 0, DefaultLength2: 0}
		case "time without time zone":
			col.SQLType = coreSQLType{Name: Time, DefaultLength: 0, DefaultLength2: 0}
		case "oid":
			col.SQLType = coreSQLType{Name: BigInt, DefaultLength: 0, DefaultLength2: 0}
		default:
			col.SQLType = coreSQLType{Name: strings.ToUpper(dataType), DefaultLength: 0, DefaultLength2: 0}
		}
...
func (db *postgres) GetTables() ([]*coreTable, error) {
	args := []any{}
...
func (db *postgres) GetIndexes(tableName string) (map[string]*coreIndex, error) {
	args := []any{tableName}
	...
	for rows.Next() {
		...
		index := &coreIndex{Name: indexName, Type: indexType, Cols: make([]string, 0)}
...
func (db *postgres) Filters() []coreFilter {
	return []coreFilter{&coreIdFilter{}, &coreQuoteFilter{}, &coreSeqFilter{Prefix: "$", Start: 1}}
}
...
func (p *pqDriver) Parse(driverName, dataSourceName string) (*coreUri, error) {
	db := &coreUri{DbType: POSTGRES}
	var err error
...
func (pgx *pqDriverPgx) Parse(driverName, dataSourceName string) (*coreUri, error) {
