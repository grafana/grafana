...
func (s *SocialBase) extractRoleAndAdmin(rawJSON []byte, groups []string, legacy bool) (models.RoleType, bool) {
...
func (s *SocialBase) defaultRole(legacy bool) models.RoleType {
	...
	if s.autoAssignOrgRole != "" && !legacy {
		...
		return models.RoleType(s.autoAssignOrgRole)
	}
...
func getRoleFromSearch(role string) (models.RoleType, bool) {
	if strings.EqualFold(role, RoleGrafanaAdmin) {
		return models.ROLE_ADMIN, true
	}

	return models.RoleType(cases.Title(language.Und).String(role)), false
}
