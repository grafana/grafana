...
func TestSocialAzureAD_SkipOrgRole(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "Grafana Admin and Editor roles in claim, skipOrgRoleSync enabled should not get roles, skipOrgRoleSyncBase disabled",
			fields: fields{
				SocialBase:      newSocialBase("azuread", &oauth2.Config{}, &OAuthInfo{AllowAssignGrafanaAdmin: true}, "", false, *featuremgmt.WithFeatures()),
		...
		{
			name: "Grafana Admin and Editor roles in claim, skipOrgRoleSync enabled and skipOrgRoleSyncBase enabled, should not get roles assigned",
			fields: fields{
				SocialBase:      newSocialBase("azuread", &oauth2.Config{}, &OAuthInfo{AllowAssignGrafanaAdmin: true}, "", true, *featuremgmt.WithFeatures()),
				skipOrgRoleSync: true,
			},
			claims: &azureClaims{
				Email:             "me@example.com",
		...
		{
			name: "Grafana Admin and Editor roles in claim, skipOrgRoleSync disabled and skipOrgRoleSyncBase enabled, should not get roles assigned",
			fields: fields{
				SocialBase:      newSocialBase("azuread", &oauth2.Config{}, &OAuthInfo{AllowAssignGrafanaAdmin: true}, "", true, *featuremgmt.WithFeatures()),
				skipOrgRoleSync: false,
			},
