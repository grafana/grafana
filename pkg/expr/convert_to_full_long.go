...
func convertNumericWideToFullLong(frames data.Frames) (data.Frames, error) {
	...
	fields := []*data.Field{
		data.NewField("__metric__", nil, metricCol),
		data.NewField("value", nil, valueCol),
	}
	for _, k := range labelKeys {
		fields = append(fields, data.NewField(k, nil, labelValues[k]))
	}

	out := data.NewFrame("numeric_full_long", fields...)
	out.Meta = &data.FrameMeta{Type: data.FrameTypeNumericLong}
...
func convertTimeSeriesMultiToFullLong(frames data.Frames) (data.Frames, error) {
	...
	fields := []*data.Field{
		...
		data.NewField("__metric__", nil, metrics),
	}
	for _, k := range labelKeys {
		fields = append(fields, data.NewField(k, nil, labels[k]))
	}

	out := data.NewFrame("ts_full_long", fields...)
	out.Meta = &data.FrameMeta{Type: data.FrameTypeTimeSeriesLong}
	return data.Frames{out}, nil
}
