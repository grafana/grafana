...
func TestThresholdEvaluator(t *testing.T) {
	var tests = []struct {
	...
	}{
		{
			...
			inputNumber: newNumber(util.Pointer(3)),
			expected:    true,
		},
		{
...
func TestRangedEvaluator(t *testing.T) {
	var tests = []struct {
		...
		},
		{
			...
			evaluator:   &rangedEvaluator{"within_range", 1, 100},
			inputNumber: newNumber(util.Pointer(300)),
			expected:    false,
		},
		{
		...
		// outside
		{
			...
			inputNumber: newNumber(util.Pointer(1000)),
			expected:    true,
		},
		{
			name:        "value 50 is outside range 1, 100: false",
			evaluator:   &rangedEvaluator{"outside_range", 1, 100},
			inputNumber: newNumber(util.Pointer(50)),
			expected:    false,
		},
		{
