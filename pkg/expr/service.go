...
// If any nodes are broken (e.g. missing dependencies), it returns an error rather than
// a degraded pipeline. This preserves safety for callers such as alerting that cannot
...
func (s *Service) BuildPipeline(ctx context.Context, req *Request) (DataPipeline, error) {
	...
	if len(brokenNodes) > 0 {
		for _, nodeErr := range brokenNodes {
			// Record SQL metrics for the broken node error before returning,
			// matching the behavior of the non-degraded path where
			// instrumentSQLError fires on buildPipeline failure.
