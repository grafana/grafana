...
describe('useDefaultDataSourceInstanceListItem', () => {
  ...
  it('updates the default item when the cache is reloaded', async () => {
    const { result } = renderHook(() => useDefaultDataSourceInstanceListItem('test-db'));
    await waitFor(() => expect(result.current.item?.name).toBe('Bravo'));

    backendGet.mockResolvedValue({
      datasources: {
        Alpha: ds({ id: 1, uid: 'uid-alpha', name: 'Alpha', type: 'test-db', isDefault: true }),
        Bravo: ds({ id: 2, uid: 'uid-bravo', name: 'Bravo', type: 'test-db', isDefault: false }),
      },
