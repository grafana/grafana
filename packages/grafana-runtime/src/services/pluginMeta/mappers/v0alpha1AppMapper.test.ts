...
describe('v0alpha1AppMapper', () => {
  describe.each(PLUGIN_IDS)('when called for pluginId:$pluginId', ({ pluginId }) => {
    ...
    it('should map extensions property correctly', () => {
      ...
      // expect(result[pluginId].extensions.addedFunctions).toEqual(apps[pluginId].extensions.addedFunctions);  //TODO: add back when backend starts to send addedFunctions in result
