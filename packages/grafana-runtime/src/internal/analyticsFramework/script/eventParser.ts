...
export const parseEvents = (file: SourceFile, eventNamespaces: Map<string, EventNamespace>): EventData[] => {
  ...
  for (const variableDecl of variableDecls) {
    ...
    // Only interested in calls to functions returned by defineFeatureEvents
    ...
    // Events should be defined with a single string literal argument (e.g. defineFeatureEvents('click'))
