...
function focusTriggersKeyboardModality(node: any) {
  ...
  if (tagName === 'INPUT' && (inputTypesWhitelist as any)[type] && !node.readOnly) {
    return true;
  }
...
export function useIsFocusVisible<T>(): {
   ...
   * Should be called if a blur event is fired
   */
  function handleFocusVisible(event: React.FocusEvent<any>) {
