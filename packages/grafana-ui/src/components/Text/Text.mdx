...
2. The parent element is not a Text component: the user has to add `overflow: hidden` and `text-overflow: ellipsis` to it. In this case, the user should wrap up this container with a Tooltip, so when the text is truncated its content can still be seen hovering on the text.
