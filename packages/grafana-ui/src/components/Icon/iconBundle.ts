...
//
// After adding icon to correct directory and to
// `packages/grafana-ui/src/components/Icon/cached.json`,
// see @grafana/ui/scripts/generate-icon-bundle.js.
// To run the generateIconBundle function, change directories in terminal
// to grafana-ui, run `yarn generate-icon-bundle`, then in code editor add
// icon key to `availableIconsIndex` const in `packages/grafana-data/src/types/icon.ts`
