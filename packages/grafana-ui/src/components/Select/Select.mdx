...
There are for properties for each option:
...
Like regular Select but handles data asynchronously. Use the `loadOptions` prop for the async function that loads the options. If `defaultOptions` is a boolean set to `true`, `loadOptions` will be called when the component is mounted
