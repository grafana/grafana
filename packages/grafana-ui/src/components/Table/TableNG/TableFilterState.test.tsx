...
it('updates one predicate without rewriting disabled, compound, other-frame or column configs', () => {
  ...
  api.set('table:view', [selected, disabled, compound, elsewhere, organize]);
  ...
  expect(api.get('table:view')[0].options.filters[0].config.options.values).toEqual(['beta']);
  expect(api.get('table:view').slice(1)).toEqual([disabled, compound, elsewhere, organize]);
  expect(api.get('table:view')[2]).toBe(compound);
  act(() => result.current!.clearFilter(props.data.fields[0]));
  expect(api.get('table:view')).toEqual([disabled, compound, elsewhere, organize]);
  act(() => result.current!.clearFilters());
  expect(api.get('table:view')).toEqual([disabled, elsewhere, organize]);
});

it('restores checkbox controls from JSON and replaces an open draft only when its predicate changes', async () => {
  const { props, api, filter } = setup();
  api.set('table:view', JSON.parse(JSON.stringify([filter('Name', ['alpha', 'beta'])])));
  render(<TableNG {...props} />);
  const user = userEvent.setup();
  await user.click(screen.getByRole('button', { name: 'Filter Name' }));
  expect(screen.getByRole('checkbox', { name: 'alpha' })).toBeChecked();
  await user.click(screen.getByRole('checkbox', { name: 'alpha' }));
  act(() => api.set('table:view', [...api.get('table:view'), filter('Other', ['x'])]));
  expect(screen.getByRole('checkbox', { name: 'alpha' })).not.toBeChecked();
  act(() => api.set('table:view', [filter('Name', ['alpha']), filter('Other', ['x'])]));
  expect(screen.getByRole('checkbox', { name: 'alpha' })).toBeChecked();
  expect(screen.getByRole('checkbox', { name: 'beta' })).not.toBeChecked();
  await user.click(screen.getByRole('checkbox', { name: 'beta' }));
  await user.click(screen.getByRole('button', { name: 'Cancel' }));
  expect(api.get('table:view')[0].options.filters[0].config.options.values).toEqual(['alpha']);
  expect(
    screen
      .getAllByRole('row')
      .slice(1)
