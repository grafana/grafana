...
describe('PillCell', () => {
  ...
  describe('Color by hash (classic palette)', () => {
    it('single value', () => {
      expectHTML(
        render(<PillCell field={fieldWithValues(['["value1","value2","value3"]'])} rowIdx={0} />),
        `<span class="${pillClass}" style="background-color: rgb(63, 43, 91); color: rgb(255, 255, 255);">value1</span>`
      );
    });

    it('empty string', () => {
      expectHTML(render(<PillCell field={fieldWithValues([''])} rowIdx={0} />), '');
    });

    it('null', () => {
      const { container } = render(<PillCell field={fieldWithValues([])} rowIdx={0} />);
  ...
  describe('Color by value mappings', () => {
    it('CSV values', () => {
      ...
      expectHTML(
        render(<PillCell field={field} rowIdx={0} />),
        `
