...
It is a wrapper around [TanStack Table](https://tanstack.com/table/v8/docs/overview).
...
Column headers can be customized using strings, React elements, or renderer functions.
...
`CellProps` and `SortByFn` use the TanStack Table v8 row and context APIs. Consumers migrating from react-table v7
must replace table-instance properties such as `rows` and `row.values` with their TanStack Table equivalents.
The `value` and `cell.value` aliases remain available for compatibility.
