import { ArrayVector, FieldType, LogLevel, LogRowModel, MutableDataFrame } from '@grafana/data';
...
describe('getAllFields', () => {
  it('should filter out field with labels name and other type', () => {
    ...
    logRow.dataFrame = new MutableDataFrame({
      refId: 'A',
      fields: [
        testStringField,
        {
...
const createLogRow = (overrides?: Partial<LogRowModel>): LogRowModel => {
  const defaultLogRowProperties = {
    rowIndex: 0,
    entryFieldIndex: 10,
  ...
  return { ...defaultLogRowProperties, ...overrides };
};
