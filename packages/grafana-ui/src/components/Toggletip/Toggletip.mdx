...
Toggletips, similar to [Tooltips](/docs/mdx-tooltip--docs), provide contextual support for users when needed. They are hidden by default, a UI trigger or text link are clicked to set them to their visible state.
...
- Do not use when a [Tooltip](/docs/mdx-tooltip--docs) with clarifying text would suffice.
