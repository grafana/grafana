...
Options may include a `description` string, which is included in type-ahead filtering alongside the label. By default the description renders underneath the label. Set `descriptionPosition="right"` to render it right-aligned on the same line as the label, giving a table-like two column layout.
