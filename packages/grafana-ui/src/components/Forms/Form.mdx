...
Additionally the value to for the `onChange` prop is passed as an array. Check [react-hook-form docs](https://react-hook-form.com/api/#Controller)
for more prop options.

```jsx
{/* In case of Select the value has to be returned as an object with a `value` key */}
...
<Field label="SelectExample">
  <Forms.InputControl
    ...
    {/* Pass the name of the on change handler */}
    onChangeName='onSelectChange'
  />
</Field>
