...
export function Form<T>({
  ...
  const { handleSubmit, register, control, trigger, getValues, formState, watch } = useForm<T>({
    mode: validateOn,
    defaultValues,
  });
  ...
  return (
    <form
      ...
      {...htmlProps}
    >
      {children({ register, errors: formState.errors, control, getValues, formState, watch })}
    </form>
  );
}
