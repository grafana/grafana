...
- When it still redirects the user but it is not surrounded by text - for example, being in a row with more buttons -, please use the LinkButton component.
...
- **Inline**: It is part of a broader text. It will be surrounded by text, before, after or in both directions. Although it can show an icon at its end, we encouragedly advice not to do that.
...
- _Initial appearence_: Blue and underlined at first. <br />- _On hover_: blue and not underlined
...
- _Initial appearence_: Blue and not underlined at first. - _On hover_: blue and underlined Through the color prop the user
will be able to change this value, but the rest will be kept.{' '}

_Example_: A user renders an external and standalone _text link_ setting the color value to 'primary'.
The text link will be 'white' in dark theme or 'black' in light theme, as set by the user, and not underlined at first while blue and underlined when onhover, as it inherits from the standalone default behaviour.
