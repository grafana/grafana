...
describe('walkTree with HiDPI (devicePixelRatio=2)', () => {
  ...
  it('should produce coordinates exactly 2x those from DPR=1', () => {
    ...
    // Collect DPR=2 render data
    const dpr2Data: RenderData[] = [];
    walkTree(
      root,
      'children',
      container,
      ...
      container.getCollapsedMap(),
      (item, x, y, width, height, label, muted) => {
        dpr2Data.push({ item, x, y, width, height, label, muted });
      }
    );

    // Switch to DPR=1 and collect reference data
    Object.defineProperty(window, 'devicePixelRatio', { value: 1, configurable: true, writable: true });
    const dpr1Data: RenderData[] = [];
    walkTree(
      root,
      'children',
      container,
      ...
      container.getCollapsedMap(),
      (item, x, y, width, height, label, muted) => {
        dpr1Data.push({ item, x, y, width, height, label, muted });
      }
    );
