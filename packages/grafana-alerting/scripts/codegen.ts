...
const outputFiles = VERSIONS.reduce<OutputFiles>((acc, version) => {
  ...
  const exportName = `alertingAPI${version}`;

  // ℹ️ these snapshots are generated by running "go test pkg/tests/apis/openapi_test.go" and "sripts/process-specs.sh",
  // see the README in the "openapi_snapshots" directory
  const schemaFile = resolve(`../../../data/openapi/notifications.alerting.grafana.app-${version}.json`);
  ...
  const outputPath = resolve(`../src/grafana/api/${version}/api.gen.ts`);
