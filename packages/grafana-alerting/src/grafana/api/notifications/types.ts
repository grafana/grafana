/**
 * Enhanced and type-narrowed versions of the types generated by the RTKQ codegen package.
 *
 * The underlying generated types are imported from the v1beta1 client unconditionally. This is the
 * single centralized "type pin" for the notifications API: every consumer of `ContactPoint`,
 * `EnhancedListReceiverApiResponse`, etc. resolves to the v1beta1-shaped versions regardless of
 * which version the `alerting.notificationsAPIV1Beta1` feature toggle selects at runtime. The
 * dispatcher in `./index.ts` is what makes that choice safe — it casts the v0alpha1 client to the
 * v1beta1 shape so runtime callers receive the same nominally-typed objects as the type system.
 *
 * When migrating to v2, change this import to `…/v2` AND update the cast in `./index.ts` together.
 * Do not "fix" this import to match the active toggle — that would defeat the centralization and
 * force every consumer to handle union types.
 */
import { type MergeDeep, type MergeExclusive, type OverrideProperties } from 'type-fest';
