...
export const addTagTypes = ['API Discovery', 'Correlation'] as const;
const injectedRtkApi = api
  ...
  .injectEndpoints({
    endpoints: (build) => ({
      ...
      deletecollectionCorrelation: build.mutation<
        DeletecollectionCorrelationApiResponse,
        DeletecollectionCorrelationApiArg
      >({
        query: (queryArg) => ({
          url: `/correlations`,
          method: 'DELETE',
      ...
      getCorrelation: build.query<GetCorrelationApiResponse, GetCorrelationApiArg>({
        query: (queryArg) => ({
          url: `/correlations/${queryArg.name}`,
          params: {
            pretty: queryArg.pretty,
          },
        }),
      ...
      replaceCorrelation: build.mutation<ReplaceCorrelationApiResponse, ReplaceCorrelationApiArg>({
        query: (queryArg) => ({
          url: `/correlations/${queryArg.name}`,
          method: 'PUT',
          body: queryArg.correlation,
...
export type ListCorrelationApiResponse = /** status 200 OK */ CorrelationList;
export type ListCorrelationApiArg = {
...
export type CreateCorrelationApiResponse = /** status 200 OK */
  | Correlation
  | /** status 201 Created */ Correlation
  | /** status 202 Accepted */ Correlation;
...
export type DeletecollectionCorrelationApiResponse = /** status 200 OK */ Status;
export type DeletecollectionCorrelationApiArg = {
  /** If 'true', then the output is pretty printed. Defaults to 'false' unless the user-agent indicates a browser or command-line HTTP tool (curl and wget). */
  pretty?: string;
  /** The continue option should be set when retrieving more results from the server. Since this value is server defined, clients may only use the continue value from a previous query result with identical query parameters (except for the value of continue) and the server may reject a continue value it does not recognize. If the specified continue value is no longer valid whether due to expiration (generally five to fifteen minutes) or a configuration change on the server, the server will respond with a 410 ResourceExpired error together with a continue token. If the client needs a consistent list, it must restart their list without the continue field. Otherwise, the client may send another list request with the token received with the 410 error, the server will respond with a list starting from the next key, but from the latest snapshot, which is inconsistent from the previous list results - objects that are created, modified, or deleted after the first list request will be included in the response, as long as their keys are after the "next key".
...
export type GetCorrelationApiResponse = /** status 200 OK */ Correlation;
...
export type ReplaceCorrelationApiResponse = /** status 200 OK */ Correlation | /** status 201 Created */ Correlation;
export type ReplaceCorrelationApiArg = {
  /** name of the Correlation */
  name: string;
  /** If 'true', then the output is pretty printed. Defaults to 'false' unless the user-agent indicates a browser or command-line HTTP tool (curl and wget). */
  pretty?: string;
...
export type DeleteCorrelationApiResponse = /** status 200 OK */ Status | /** status 202 Accepted */ Status;
...
export type UpdateCorrelationApiResponse = /** status 200 OK */ Correlation | /** status 201 Created */ Correlation;
