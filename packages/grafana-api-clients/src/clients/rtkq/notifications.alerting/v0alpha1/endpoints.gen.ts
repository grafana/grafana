...
const injectedRtkApi = api
  ...
  .injectEndpoints({
    endpoints: (build) => ({
      ...
      createInhibitionRule: build.mutation<CreateInhibitionRuleApiResponse, CreateInhibitionRuleApiArg>({
        query: (queryArg) => ({
          url: `/inhibitionrules`,
          method: 'POST',
          body: queryArg.inhibitionRule,
          params: {
            pretty: queryArg.pretty,
        ...
        invalidatesTags: ['InhibitionRule'],
      }),
      deletecollectionInhibitionRule: build.mutation<
        DeletecollectionInhibitionRuleApiResponse,
        DeletecollectionInhibitionRuleApiArg
      >({
        query: (queryArg) => ({
          url: `/inhibitionrules`,
          method: 'DELETE',
      ...
      getInhibitionRule: build.query<GetInhibitionRuleApiResponse, GetInhibitionRuleApiArg>({
        query: (queryArg) => ({
          url: `/inhibitionrules/${queryArg.name}`,
          params: {
            pretty: queryArg.pretty,
          },
        }),
      ...
      replaceInhibitionRule: build.mutation<ReplaceInhibitionRuleApiResponse, ReplaceInhibitionRuleApiArg>({
        query: (queryArg) => ({
          url: `/inhibitionrules/${queryArg.name}`,
          method: 'PUT',
          body: queryArg.inhibitionRule,
      ...
      deleteInhibitionRule: build.mutation<DeleteInhibitionRuleApiResponse, DeleteInhibitionRuleApiArg>({
        query: (queryArg) => ({
          url: `/inhibitionrules/${queryArg.name}`,
          method: 'DELETE',
          params: {
      ...
      updateInhibitionRule: build.mutation<UpdateInhibitionRuleApiResponse, UpdateInhibitionRuleApiArg>({
        query: (queryArg) => ({
          url: `/inhibitionrules/${queryArg.name}`,
          method: 'PATCH',
          body: queryArg.patch,
...
export type ListInhibitionRuleApiResponse = /** status 200 OK */ InhibitionRuleList;
export type ListInhibitionRuleApiArg = {
...
export type CreateInhibitionRuleApiResponse = /** status 200 OK */
  | InhibitionRule
  | /** status 201 Created */ InhibitionRule
  | /** status 202 Accepted */ InhibitionRule;
export type CreateInhibitionRuleApiArg = {
  /** If 'true', then the output is pretty printed. Defaults to 'false' unless the user-agent indicates a browser or command-line HTTP tool (curl and wget). */
  pretty?: string;
...
export type DeletecollectionInhibitionRuleApiResponse = /** status 200 OK */ Status;
export type DeletecollectionInhibitionRuleApiArg = {
  /** If 'true', then the output is pretty printed. Defaults to 'false' unless the user-agent indicates a browser or command-line HTTP tool (curl and wget). */
  pretty?: string;
  /** The continue option should be set when retrieving more results from the server. Since this value is server defined, clients may only use the continue value from a previous query result with identical query parameters (except for the value of continue) and the server may reject a continue value it does not recognize. If the specified continue value is no longer valid whether due to expiration (generally five to fifteen minutes) or a configuration change on the server, the server will respond with a 410 ResourceExpired error together with a continue token. If the client needs a consistent list, it must restart their list without the continue field. Otherwise, the client may send another list request with the token received with the 410 error, the server will respond with a list starting from the next key, but from the latest snapshot, which is inconsistent from the previous list results - objects that are created, modified, or deleted after the first list request will be included in the response, as long as their keys are after the "next key".
...
export type GetInhibitionRuleApiResponse = /** status 200 OK */ InhibitionRule;
export type GetInhibitionRuleApiArg = {
  /** name of the InhibitionRule */
  name: string;
  /** If 'true', then the output is pretty printed. Defaults to 'false' unless the user-agent indicates a browser or command-line HTTP tool (curl and wget). */
  pretty?: string;
};
export type ReplaceInhibitionRuleApiResponse = /** status 200 OK */
  | InhibitionRule
  | /** status 201 Created */ InhibitionRule;
export type ReplaceInhibitionRuleApiArg = {
  /** name of the InhibitionRule */
  name: string;
  /** If 'true', then the output is pretty printed. Defaults to 'false' unless the user-agent indicates a browser or command-line HTTP tool (curl and wget). */
  pretty?: string;
...
export type DeleteInhibitionRuleApiResponse = /** status 200 OK */ Status | /** status 202 Accepted */ Status;
export type DeleteInhibitionRuleApiArg = {
  /** name of the InhibitionRule */
  name: string;
  /** If 'true', then the output is pretty printed. Defaults to 'false' unless the user-agent indicates a browser or command-line HTTP tool (curl and wget). */
  pretty?: string;
...
export type UpdateInhibitionRuleApiResponse = /** status 200 OK */
  | InhibitionRule
  | /** status 201 Created */ InhibitionRule;
export type UpdateInhibitionRuleApiArg = {
  /** name of the InhibitionRule */
  name: string;
  /** If 'true', then the output is pretty printed. Defaults to 'false' unless the user-agent indicates a browser or command-line HTTP tool (curl and wget). */
  pretty?: string;
...
export type InhibitionRuleMatcher = {
  label: string;
  type: '=' | '!=' | '=~' | '!~';
  value: string;
};
export type InhibitionRuleSpec = {
...
export type InhibitionRuleList = {
  /** APIVersion defines the versioned schema of this representation of an object. Servers should convert recognized schemas to the latest internal value, and may reject unrecognized values. More info: https://git.k8s.io/community/contributors/devel/sig-architecture/api-conventions.md#resources */
  apiVersion?: string;
...
export const {
  ...
  useDeletecollectionInhibitionRuleMutation,
  useGetInhibitionRuleQuery,
  useLazyGetInhibitionRuleQuery,
  useReplaceInhibitionRuleMutation,
  useDeleteInhibitionRuleMutation,
  useUpdateInhibitionRuleMutation,
