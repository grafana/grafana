...
export const addTagTypes = ['API Discovery', 'CustomTheme'] as const;
const injectedRtkApi = api
  ...
  .injectEndpoints({
    endpoints: (build) => ({
      ...
      listCustomTheme: build.query<ListCustomThemeApiResponse, ListCustomThemeApiArg>({
        query: (queryArg) => ({
          url: `/customthemes`,
          params: {
      ...
      createCustomTheme: build.mutation<CreateCustomThemeApiResponse, CreateCustomThemeApiArg>({
        query: (queryArg) => ({
          url: `/customthemes`,
          method: 'POST',
          body: queryArg.customTheme,
          params: {
            pretty: queryArg.pretty,
      ...
      deletecollectionCustomTheme: build.mutation<
        DeletecollectionCustomThemeApiResponse,
        DeletecollectionCustomThemeApiArg
      >({
        query: (queryArg) => ({
          url: `/customthemes`,
          method: 'DELETE',
      ...
      replaceCustomTheme: build.mutation<ReplaceCustomThemeApiResponse, ReplaceCustomThemeApiArg>({
        query: (queryArg) => ({
          url: `/customthemes/${queryArg.name}`,
          method: 'PUT',
          body: queryArg.customTheme,
      ...
      deleteCustomTheme: build.mutation<DeleteCustomThemeApiResponse, DeleteCustomThemeApiArg>({
        query: (queryArg) => ({
          url: `/customthemes/${queryArg.name}`,
          method: 'DELETE',
          params: {
      ...
      updateCustomTheme: build.mutation<UpdateCustomThemeApiResponse, UpdateCustomThemeApiArg>({
        query: (queryArg) => ({
          url: `/customthemes/${queryArg.name}`,
          method: 'PATCH',
          body: queryArg.patch,
...
export type ListCustomThemeApiResponse = /** status 200 OK */ CustomThemeList;
export type ListCustomThemeApiArg = {
...
export type CreateCustomThemeApiResponse = /** status 200 OK */
  | CustomTheme
  | /** status 201 Created */ CustomTheme
  | /** status 202 Accepted */ CustomTheme;
export type CreateCustomThemeApiArg = {
  /** If 'true', then the output is pretty printed. Defaults to 'false' unless the user-agent indicates a browser or command-line HTTP tool (curl and wget). */
  pretty?: string;
...
export type DeletecollectionCustomThemeApiResponse = /** status 200 OK */ Status;
export type DeletecollectionCustomThemeApiArg = {
  /** If 'true', then the output is pretty printed. Defaults to 'false' unless the user-agent indicates a browser or command-line HTTP tool (curl and wget). */
  pretty?: string;
  /** The continue option should be set when retrieving more results from the server. Since this value is server defined, clients may only use the continue value from a previous query result with identical query parameters (except for the value of continue) and the server may reject a continue value it does not recognize. If the specified continue value is no longer valid whether due to expiration (generally five to fifteen minutes) or a configuration change on the server, the server will respond with a 410 ResourceExpired error together with a continue token. If the client needs a consistent list, it must restart their list without the continue field. Otherwise, the client may send another list request with the token received with the 410 error, the server will respond with a list starting from the next key, but from the latest snapshot, which is inconsistent from the previous list results - objects that are created, modified, or deleted after the first list request will be included in the response, as long as their keys are after the "next key".
...
export type ReplaceCustomThemeApiResponse = /** status 200 OK */ CustomTheme | /** status 201 Created */ CustomTheme;
export type ReplaceCustomThemeApiArg = {
  /** name of the CustomTheme */
  name: string;
  /** If 'true', then the output is pretty printed. Defaults to 'false' unless the user-agent indicates a browser or command-line HTTP tool (curl and wget). */
  pretty?: string;
...
export type DeleteCustomThemeApiResponse = /** status 200 OK */ Status | /** status 202 Accepted */ Status;
export type DeleteCustomThemeApiArg = {
  /** name of the CustomTheme */
  name: string;
  /** If 'true', then the output is pretty printed. Defaults to 'false' unless the user-agent indicates a browser or command-line HTTP tool (curl and wget). */
  pretty?: string;
...
export type UpdateCustomThemeApiResponse = /** status 200 OK */ CustomTheme | /** status 201 Created */ CustomTheme;
export type UpdateCustomThemeApiArg = {
  /** name of the CustomTheme */
  name: string;
  /** If 'true', then the output is pretty printed. Defaults to 'false' unless the user-agent indicates a browser or command-line HTTP tool (curl and wget). */
  pretty?: string;
...
export const {
  ...
  useDeleteCustomThemeMutation,
  useUpdateCustomThemeMutation,
} = injectedRtkApi;
