...
const injectedRtkApi = api
  ...
  .injectEndpoints({
    endpoints: (build) => ({
      ...
      getFolderStatus: build.query<GetFolderStatusApiResponse, GetFolderStatusApiArg>({
        query: (queryArg) => ({
          url: `/folders/${queryArg.name}/status`,
          params: {
            pretty: queryArg.pretty,
          },
        }),
      ...
      replaceFolderStatus: build.mutation<ReplaceFolderStatusApiResponse, ReplaceFolderStatusApiArg>({
        query: (queryArg) => ({
          url: `/folders/${queryArg.name}/status`,
          method: 'PUT',
          body: queryArg.folder,
      ...
      updateFolderStatus: build.mutation<UpdateFolderStatusApiResponse, UpdateFolderStatusApiArg>({
        query: (queryArg) => ({
          url: `/folders/${queryArg.name}/status`,
          method: 'PATCH',
          body: queryArg.patch,
...
export type GetFolderStatusApiResponse = /** status 200 OK */ Folder;
export type GetFolderStatusApiArg = {
  /** name of the Folder */
  name: string;
  /** If 'true', then the output is pretty printed. Defaults to 'false' unless the user-agent indicates a browser or command-line HTTP tool (curl and wget). */
  pretty?: string;
};
export type ReplaceFolderStatusApiResponse = /** status 200 OK */ Folder | /** status 201 Created */ Folder;
export type ReplaceFolderStatusApiArg = {
  /** name of the Folder */
  name: string;
  /** If 'true', then the output is pretty printed. Defaults to 'false' unless the user-agent indicates a browser or command-line HTTP tool (curl and wget). */
  pretty?: string;
...
export type UpdateFolderStatusApiResponse = /** status 200 OK */ Folder | /** status 201 Created */ Folder;
export type UpdateFolderStatusApiArg = {
  /** name of the Folder */
  name: string;
  /** If 'true', then the output is pretty printed. Defaults to 'false' unless the user-agent indicates a browser or command-line HTTP tool (curl and wget). */
  pretty?: string;
...
export type CascadeDeleteStatus = {
...
export type FolderStatus = {
  /** CascadeDelete tracks async cascade-deletion progress for this folder's subtree. PoC: the only field on FolderStatus today. */
  cascadeDelete?: CascadeDeleteStatus;
};
export type Folder = {
  /** APIVersion defines the versioned schema of this representation of an object. Servers should convert recognized schemas to the latest internal value, and may reject unrecognized values. More info: https://git.k8s.io/community/contributors/devel/sig-architecture/api-conventions.md#resources */
  apiVersion?: string;
  ...
  spec: FolderSpec;
  // Hand-adjusted post-codegen: the OpenAPI spec marks `status` required (a real GET always
  // returns one), but a lot of pre-existing frontend code and test fixtures construct partial
  // Folder objects without it. Making this required broke ~9 unrelated files across the repo.
  // Widening it back to optional here avoids that broad, out-of-scope collateral; a real
  // regeneration pass should reconcile this properly (see AGENTS.md's generate-apis script).
  status?: FolderStatus;
};
...
export const {
  ...
  useGetFolderStatusQuery,
  useLazyGetFolderStatusQuery,
  useReplaceFolderStatusMutation,
