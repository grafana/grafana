...
const injectedRtkApi = api
  ...
  .injectEndpoints({
    endpoints: (build) => ({
      ...
      deletecollectionSnapshot: build.mutation<DeletecollectionSnapshotApiResponse, DeletecollectionSnapshotApiArg>({
        query: (queryArg) => ({
          url: `/snapshots`,
          method: 'DELETE',
          params: {
      ...
      deleteSnapshotsDeleteByDeleteKey: build.mutation<
        DeleteSnapshotsDeleteByDeleteKeyApiResponse,
        DeleteSnapshotsDeleteByDeleteKeyApiArg
      >({
      ...
      replaceSnapshot: build.mutation<ReplaceSnapshotApiResponse, ReplaceSnapshotApiArg>({
        query: (queryArg) => ({
          url: `/snapshots/${queryArg.name}`,
          method: 'PUT',
          body: queryArg.snapshot,
      ...
      updateSnapshot: build.mutation<UpdateSnapshotApiResponse, UpdateSnapshotApiArg>({
        query: (queryArg) => ({
          url: `/snapshots/${queryArg.name}`,
          method: 'PATCH',
          body: queryArg.patch,
...
export type CreateSnapshotApiResponse = /** status 200 OK */
  | Snapshot
  | /** status 201 Created */ Snapshot
  | /** status 202 Accepted */ Snapshot;
...
export type DeletecollectionSnapshotApiResponse = /** status 200 OK */ Status;
export type DeletecollectionSnapshotApiArg = {
  /** If 'true', then the output is pretty printed. Defaults to 'false' unless the user-agent indicates a browser or command-line HTTP tool (curl and wget). */
  pretty?: string;
  /** The continue option should be set when retrieving more results from the server. Since this value is server defined, clients may only use the continue value from a previous query result with identical query parameters (except for the value of continue) and the server may reject a continue value it does not recognize. If the specified continue value is no longer valid whether due to expiration (generally five to fifteen minutes) or a configuration change on the server, the server will respond with a 410 ResourceExpired error together with a continue token. If the client needs a consistent list, it must restart their list without the continue field. Otherwise, the client may send another list request with the token received with the 410 error, the server will respond with a list starting from the next key, but from the latest snapshot, which is inconsistent from the previous list results - objects that are created, modified, or deleted after the first list request will be included in the response, as long as their keys are after the "next key".
...
export type DeleteSnapshotsDeleteByDeleteKeyApiResponse = unknown;
export type DeleteSnapshotsDeleteByDeleteKeyApiArg = {
...
export type ReplaceSnapshotApiResponse = /** status 200 OK */ Snapshot | /** status 201 Created */ Snapshot;
export type ReplaceSnapshotApiArg = {
  /** name of the Snapshot */
  name: string;
  /** If 'true', then the output is pretty printed. Defaults to 'false' unless the user-agent indicates a browser or command-line HTTP tool (curl and wget). */
  pretty?: string;
...
export type UpdateSnapshotApiResponse = /** status 200 OK */ Snapshot | /** status 201 Created */ Snapshot;
export type UpdateSnapshotApiArg = {
  /** name of the Snapshot */
  name: string;
  /** If 'true', then the output is pretty printed. Defaults to 'false' unless the user-agent indicates a browser or command-line HTTP tool (curl and wget). */
  pretty?: string;
...
export const {
  ...
  useDeletecollectionSnapshotMutation,
  usePostSnapshotsCreateMutation,
  useDeleteSnapshotsDeleteByDeleteKeyMutation,
  useGetSnapshotQuery,
  useLazyGetSnapshotQuery,
  useReplaceSnapshotMutation,
