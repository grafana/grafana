...
import { TraceKeyValuePair, TraceLink, TraceSpan, TraceSpanReference } from '../../types/trace';
import { autoColor, createStyle, Theme, useTheme } from '../../Theme';
import { uAlignIcon, uTxEllipsis } from '../../uberUtilityStyles';

function getTraceName(span: TraceSpan, ref: TraceSpanReference) {
...
// export for tests
export function KeyValuesSummary(props: { data?: TraceSpanReference[]; span: TraceSpan }) {
  ...
  return (
    <ul className={styles.summary}>
      {data.map((item, i) => (
        ...
        <li className={styles.summaryItem} key={`${item.traceID}-${i}`}>
          <span className={styles.summaryLabel}>{getTraceName(span, item)}</span>
...
export default function AccordianKeyValues(props: AccordianKeyValuesProps) {
  const { className, data, span, highContrast, interactive, isOpen, label, linksGetter, onToggle } = props;
  ...
  return (
    <div className={cx(className, uTxEllipsis)}>
      ...
      >
        ...
        {!isOpen && <KeyValuesSummary data={data} span={span} />}
      </div>
      {isOpen && (
        <KeyValuesTable
          data={data.map((ref) => {
            return { key: getTraceName(span, ref), value: ref.spanID };
          })}
