...
import { PromQail, promQailTestIds } from './PromQail';
...
describe('PromQail', () => {
  ...
  it('displays a prompt when the user knows what they want to query', async () => {
    ...
    const aiPrompt = screen.getByTestId(promQailTestIds.clickForAi);
  ...
  it('does not display a prompt when choosing historical', async () => {
    ...
    const historicalPrompt = screen.getByTestId(promQailTestIds.clickForHistorical);
...
function clickSecurityButton() {
  const securityInfoButton = screen.getByTestId(promQailTestIds.securityInfoButton);
