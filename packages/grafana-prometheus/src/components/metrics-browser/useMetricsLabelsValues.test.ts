...
describe('useMetricsLabelsValues', () => {
  ...
  describe('memory usage with large datasets', () => {
    it('should handle large metric lists efficiently', async () => {
      // Create a large array of metrics (1000 items)
      const largeMetricList = Array.from({ length: 1000 }, (_, i) => `metric${i}`);

      // Mock API to return the large list
      (mocks.mockLanguageProvider.fetchSeriesValuesWithMatch as jest.Mock).mockImplementation(
        (_timeRange: TimeRange, label: string) => {
          if (label === METRIC_LABEL) {
            return Promise.resolve(largeMetricList);
          }
          return Promise.resolve([]);
        }
      );

      // Use performance.mark/measure to track memory usage if available
      ...
      if (typeof performance.mark === 'function') {
        ...
        performance.measure('render-duration', 'before-render', 'after-render');
      }
      ...
      // Select a metric to test rendering with selection
      if (typeof performance.mark === 'function') {
        performance.mark('before-selection');
      }

      await act(async () => {
        await result.current.handleSelectedMetricChange('metric500');
      });

      if (typeof performance.mark === 'function') {
        performance.mark('after-selection');
        performance.measure('selection-duration', 'before-selection', 'after-selection');
      }

      // Verify selection was made
      expect(result.current.selectedMetric).toBe('metric500');
    });

    it('should handle large label value lists efficiently', async () => {
      // Create a large array of label values (1000 items)
      const largeLabelValueList = Array.from({ length: 1000 }, (_, i) => `value${i}`);

      // Mock API to return the large list for a specific label
      (mocks.mockLanguageProvider.fetchSeriesValuesWithMatch as jest.Mock).mockImplementation(
        (_timeRange: TimeRange, label: string) => {
          if (label === 'job') {
            return Promise.resolve(largeLabelValueList);
          }
          if (label === METRIC_LABEL) {
            return Promise.resolve(['metric1', 'metric2']);
          }
      ...
      if (typeof performance.mark === 'function') {
        performance.mark('before-render-large-labels');
      }

      const { result } = await renderHookWithInit(mocks);

      if (typeof performance.mark === 'function') {
        performance.mark('after-render-large-labels');
        performance.measure('render-large-labels-duration', 'before-render-large-labels', 'after-render-large-labels');
      }

      // Make sure job is in the selected label keys
      await act(async () => {
        await result.current.handleSelectedLabelKeyChange('job');
      });
      ...
      // Wait for label values to load
      await waitFor(() => {
        expect(result.current.labelValues.job?.length).toBe(1000);
      });

      // Test selecting multiple values
      if (typeof performance.mark === 'function') {
        performance.mark('before-multi-selection');
      }

      // Select 10 values instead of 50 to make the test faster
      for (let i = 0; i < 10; i++) {
        await act(async () => {
          await result.current.handleSelectedLabelValueChange('job', `value${i}`, true);
        });
      }

      if (typeof performance.mark === 'function') {
        performance.mark('after-multi-selection');
        performance.measure('multi-selection-duration', 'before-multi-selection', 'after-multi-selection');
      }

      // Verify 10 selections were made
      expect(result.current.selectedLabelValues.job.length).toBe(10);
    });
  });
