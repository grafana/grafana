...
function getAllLabels(frames: DataFrame[]): Labels[] {
  ...
  return frames.map(
    (frame: DataFrame) => frame.fields.filter(
      (field: Field) => field.labels?.__name__ && field.labels?.__name__ === field.name
    ...
    if (target && target.legendFormat === '__auto') {
      f.fields.forEach((field) => {
        if (field.labels?.__name__ && field.labels?.__name__ === field.name) {
          ...
          const commonLabels = framesWithSameRefId.length == 1 ? {} : findCommonLabels(getAllLabels(framesWithSameRefId));
