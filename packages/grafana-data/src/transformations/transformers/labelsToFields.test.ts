...
describe('Labels as Columns', () => {
  ...
  it('data frame with two labels', async () => {
    ...
    await expect(transformDataFrame([cfg], [source])).toCompleteWith(received => {
  ...
  it('two data frames with 1 value and 1 label', async () => {
    ...
    await expect(transformDataFrame([cfg], [oneValueOneLabelA, oneValueOneLabelB])).toCompleteWith(received => {
      const data = received[0];
      const result = toDataFrameDTO(data[0]);
