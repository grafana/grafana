...
import { DataTransformContext, DataTransformerConfig, DataTransformerInfo } from '../../types/transformations';
    ...
    return outerSource.pipe(
      ...
      map((data) => {
        ...
        const outFrames = data.map((frame) => {
          ...
          const field = {
            name: getNameFromOptions(options, ctx.originalTransformations?.[0]),
            type: FieldType.number,
            config: {},
            values,
          };
...
export function getNameFromOptions(
  options: CalculateFieldTransformerOptions,
  originalTransformation?: DataTransformerConfig
) {
  ...
  switch (options.mode) {
    ...
    case CalculateFieldMode.BinaryOperation: {
      const aliasOptionsBinary = originalTransformation?.options.binary ?? options.binary;
      const alias = `${aliasOptionsBinary?.left?.matcher?.options ?? aliasOptionsBinary?.left?.fixed ?? ''} ${aliasOptionsBinary?.operator ?? ''} ${aliasOptionsBinary?.right?.matcher?.options ?? aliasOptionsBinary?.right?.fixed ?? ''}`;
