 ...
 * @alpha
 */
export interface HeatmapTransformerOptions {
...
export const heatmapFieldInfo = {
  ...
  combine: {
    name: 'Combine series',
    description: 'combine all series into a single heatmap',
  },
};
 ...
 * @alpha
 */
export const heatmapTransformer: DataTransformerInfo<HeatmapTransformerOptions> = {
  id: DataTransformerID.heatmap,
  name: 'Heatmap',
  description: 'Calculate a heatmap from input data',
  defaultOptions: {
    fields: {},
  },
  ...
  operator: (options) => (source) =>
    source.pipe(
      map((data) => {
        ...
        const hist = buildHeatmap(data, options);
        if (hist == null) {
          return [];
        }
        return [heatmapFieldsToFrame(hist)];
      })
    ),
};
 ...
 * @internal
 */
export const heatmapFrameBucketMinFieldName = 'BucketMin';

/**
 * @internal
 */
export const heatmapFrameBucketMaxFieldName = 'BucketMax';

/**
 * @alpha
 */
export interface HeatmapFields {
  bucketMin: Field;
  bucketMax: Field;
...
/**
 * Given a frame, find the explicit heatmap fields
 *
 * @alpha
 */
export function getHeatmapFields(frame: DataFrame): HeatmapFields | undefined {
  let bucketMin: Field | undefined = undefined;
  let bucketMax: Field | undefined = undefined;
  const counts: Field[] = [];
  for (const field of frame.fields) {
    if (field.name === heatmapFrameBucketMinFieldName) {
      bucketMin = field;
    } else if (field.name === heatmapFrameBucketMaxFieldName) {
      bucketMax = field;
 ...
 * @alpha
 */
export function buildHeatmap(frames: DataFrame[], options?: HeatmapTransformerOptions): HeatmapFields | null {
  ...
  if (!bucketSize) {
    ...
    // choose bucket
    for (let i = 0; i < heatmapBucketSizes.length; i++) {
      let _bucketSize = heatmapBucketSizes[i];
  ...
  for (const frame of frames) {
    for (const field of frame.fields) {
      if (field.type === FieldType.number) {
        let fieldHist = heatmap(field.values.toArray(), getBucket, histFilter, histSort) as AlignedData;
        heatmaps.push(fieldHist);
        counts.push({
          ...field,
          config: {
            ...field.config,
  ...
  const bucketMin: Field = {
    name: heatmapFrameBucketMinFieldName,
  ...
  const bucketMax = {
    ...bucketMin,
    name: heatmapFrameBucketMaxFieldName,
 ...
 * @internal
 */
export function heatmapFieldsToFrame(info: HeatmapFields, theme?: GrafanaTheme2): DataFrame {
