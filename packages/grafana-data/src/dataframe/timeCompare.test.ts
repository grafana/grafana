...
describe('isTimeCompareFrame', () => {
  it('should return true for a frame flagged as a time shift query', () => {
    const frame = toDataFrame({ refId: 'A-compare', fields: [] });
    frame.meta = { timeCompare: { diffMs: -86400000, isTimeShiftQuery: true } };

    expect(isTimeCompareFrame(frame)).toBe(true);
  });

  it('should return false when timeCompare meta is present but isTimeShiftQuery is false', () => {
    const frame = toDataFrame({ refId: 'A', fields: [] });
    frame.meta = { timeCompare: { diffMs: 0, isTimeShiftQuery: false } };

    expect(isTimeCompareFrame(frame)).toBe(false);
  });

  it('should return false for a frame with no timeCompare meta', () => {
    expect(isTimeCompareFrame(toDataFrame({ refId: 'A', fields: [] }))).toBe(false);
  });

  it('should return false for an undefined frame', () => {
    // calculateFieldDisplayName passes an optional frame, so undefined must not throw.
    expect(isTimeCompareFrame(undefined)).toBe(false);
  });
});

describe('withComparisonSuffix', () => {
  it('should append the comparison suffix to a display name', () => {
    expect(withComparisonSuffix('cpu usage')).toBe('cpu usage (comparison)');
  });
});
