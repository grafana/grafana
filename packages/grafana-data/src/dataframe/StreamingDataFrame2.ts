...
export class StreamingDataFrame implements DataFrame {
  ...
  private labelsFieldIndex = -1; // haslabels?
  ...
  // for consistency and math, move time field to [0], & remove labels field
  private niceStruct(struct: any[]) {
    let niceStruct = [struct[this.timeFieldIndex]];

    struct.forEach((f, idx) => {
      if (idx !== this.timeFieldIndex && idx !== this.labelsFieldIndex) {
        niceStruct.push(f);
      }
    });
  ...
  push(msg: DataFrameJSON) {
    ...
    if (schema) {
      ...
      this.labelsFieldIndex = schema.fields.findIndex((f) => f.name === 'labels' && f.type === FieldType.string);

      let niceSchemaFields = this.niceStruct(schema.fields) as FieldSchema[];

      // create new fields from the schema
      let newFields = niceSchemaFields.map((f, idx) => {
        return {
          config: f.config ?? {},
          name: f.name,
          labels: f.labels,
    ...
    if (data && data.values.length && data.values[0].length) {
      ...
      if (this.labelsFieldIndex >= 0) {
        ...
        [...new Set(values[this.labelsFieldIndex])].forEach((label) => {
        ...
        let niceValues = this.niceStruct(values);
        let labeledTables = transpose([values[this.labelsFieldIndex], ...niceValues]);
