...
import { MatcherType, ThresholdsMode, VariableFormatID } from '@grafana/schema';
...
export function applyFieldOverrides(
  ...
  if (source.overrides) {
    for (const rule of source.overrides) {
      if (matcherType != null && rule.matcher.type !== matcherType) {
        continue;
      }
    ...
    for (const field of newFrame.fields) {
      ...
      if (field.type === FieldType.nestedFrames) {
        ...
        for (let idx = 0; idx < field.values.length; idx++) {
          ...
          newValues[idx] = applyFieldOverrides(options, nestedFrames, MatcherType.Nested);
        }
