...
1. Run `pnpm run packages:clean` script from the root directory. This will delete any previous builds of the packages.
2. Run `pnpm run packages:prepare` script from the root directory. This performs tests on the packages and prompts for the version of the packages. The version should be the same as the one being released.
...
3. Run `pnpm run packages:build` script that compiles distribution code in `packages/grafana-*/dist`.
4. Run `pnpm run packages:pack` script to compress each package into `npm-artifacts/*.tgz` files. This is required for pnpm to replace properties in the package.json files declared in the `publishConfig` property.
...
```
pnpm run packages:build --scope=@grafana/<data|e2e|e2e-selectors|runtime|schema|ui>
```
...
3. To test `@grafana` packages published to your local npm registry uncomment `npmScopes` and `unsafeHttpWhitelist` properties in the `.npmrc` file.
...
1. Run `pnpm run packages:clean`.
2. Run `pnpm run packages:prepare`.
3. Run `pnpm run packages:build`.
4. Run `pnpm run packages:pack`.
