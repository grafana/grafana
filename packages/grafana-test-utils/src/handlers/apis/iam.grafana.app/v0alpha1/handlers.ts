...
import { ListTeamApiResponse, GetDisplayMappingApiResponse } from '@grafana/api-clients/rtkq/iam/v0alpha1';

import { mockTeamsMap } from '../../../../fixtures/teams';

const getDisplayMapping = () =>
  http.get<{ namespace: string }, never, GetDisplayMappingApiResponse>(
    ...
    ({ request }) => {
      ...
      const mockMappings: GetDisplayMappingApiResponse['display'] = keys.map((key) => {
...
const listTeamsHandler = () =>
  http.get<{ namespace: string }, never, ListTeamApiResponse>(
