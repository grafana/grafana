...
var FuncInstance = /** @class */ (function () {
    ...
    FuncInstance.prototype.render = function (metricExp, replaceVariables) {
        ...
        var parameters = map(this.params, function (value, index) {
            var paramType;
            if (index < _this.def.params.length) {
                paramType = _this.def.params[index].type;
            }
            else if (get(last(_this.def.params), 'multiple')) {
                paramType = get(last(_this.def.params), 'type');
            }
            // param types that should never be quoted
            if (includes(['value_or_series', 'boolean', 'int', 'float', 'node', 'int_or_infinity'], paramType)) {
                return value;
            }
            var valueInterpolated = isString(value) ? replaceVariables(value) : value;
...
function getFuncDefs(graphiteVersion, idx) {
    var funcs = {};
    forEach(idx || index, function (funcDef) {
        if (isVersionRelatedFunction(funcDef, graphiteVersion)) {
            funcs[funcDef.name] = assign({}, funcDef, {
                params: filter(funcDef.params, function (param) {
...
function parseFuncDefs(rawDefs) {
    var funcDefs = {};
    forEach(rawDefs || {}, function (funcDef, funcName) {
        ...
        forEach(funcDef.params, function (rawParam) {
            var param = {
                name: rawParam.name,
                type: 'string',
