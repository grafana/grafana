...
describe('Live Stream Tests', () => {
    ...
    it('reads the values into the buffer', (done) => {
        ...
        const tests = [
            ...
            (val) => {
                ...
                const last = Object.assign({}, view.get(view.length - 1));
                expect(last).toEqual({
    ...
    it('returns new subscription when the previous is unsubscribed and correctly unsubscribes from source', () => {
        ...
        jest.spyOn(rxJsWebSocket, 'webSocket').mockReturnValue(fakeSocket);
        const liveStreams = new LiveStreams();
        const stream1 = liveStreams.getStream(makeTarget('url_to_match'));
        const subscription = stream1.subscribe({
            next: noop,
    ...
    it('should reconnect when abnormal error', () => __awaiter(void 0, void 0, void 0, function* () {
        const abnormalError = new Error('weird error');
        ...
        let retries = 0;
        const fakeSocket = of({}).pipe(mergeMap(() => {
        ...
        jest.spyOn(rxJsWebSocket, 'webSocket').mockReturnValue(fakeSocket);
        const liveStreams = new LiveStreams();
        yield expect(liveStreams.getStream(makeTarget('url_to_match'), 100)).toEmitValuesWith((received) => {
            const data = received[0];
            const view = new DataFrameView(data[0]);
            const firstLog = Object.assign({}, view.get(0));
            const secondLog = Object.assign({}, view.get(1));
...
//# sourceMappingURL=LiveStreams.test.js.map