import { createRangeOperation, createRangeOperationWithGrouping, getLineFilterRenderer, isConflictingFilter, labelFilterRenderer, pipelineRenderer, } from './operationUtils';
...
describe('createRangeOperationWithGrouping', () => {
    ...
    it('returns correct query string using range operation definitions for quantile_over_time with by grouping', () => {
        const operations = createRangeOperationWithGrouping('quantile_over_time');
        const query = operations[1].renderer({ id: '__quantile_over_time_by', params: ['[5m]', '0.95', 'source', 'place'] }, operations[1], '{job="grafana"}');
    ...
    it('returns correct query string using range operation definitions for quantile_over_time with without grouping', () => {
        const operations = createRangeOperationWithGrouping('quantile_over_time');
        const query = operations[2].renderer({ id: '__quantile_over_time_without', params: ['[$__interval]', '0.91', 'source', 'place'] }, operations[2], '{job="grafana"}');
    ...
    it('returns correct query string using range operation definitions for avg_over_time with without grouping', () => {
        ...
        const query = operations[2].renderer({ id: '__avg_over_time_without', params: ['[$__interval]', 'source'] }, operations[2], '{job="grafana"}');
...
describe('getLineFilterRenderer', () => {
    ...
    it('lineFilterRenderer returns the correct query for line contains case insensitive', () => {
        ...
        expect(lineFilterRenderer(MOCK_MODEL_INSENSITIVE, MOCK_DEF, MOCK_INNER_EXPR)).toBe('{job="grafana"} !~ `(?i)ERrOR`');
    });
});
describe('labelFilterRenderer', () => {
  ...
  `("value should be of type '$type' when operator is: $operator", ({ operator, expected }) => {
        ...
        expect(labelFilterRenderer(MOCK_MODEL, MOCK_DEF, MOCK_INNER_EXPR)).toBe(`{job="grafana"} | label ${operator} ${expected}`);
    });
});
...
//# sourceMappingURL=operationUtils.test.js.map