import { addLabelFormatToQuery, addLabelToQuery, addNoPipelineErrorToQuery, addParserToQuery, NodePosition, queryHasFilter, removeCommentsFromQuery, removeLabelFromQuery, } from './modifyQuery';
describe('addLabelToQuery()', () => {
  ...
  `('should add label to query:  $query, description: $description', ({ query, description, label, operator, value, expectedResult }) => {
    ...
    it('should always add label as labelFilter if force flag is given', () => {
        expect(addLabelToQuery('{foo="bar"}', 'forcedLabel', '=', 'value', true)).toEqual('{foo="bar"} | forcedLabel=`value`');
    });
    it('should always add label as labelFilter if force flag is given with a parser', () => {
        expect(addLabelToQuery('{foo="bar"} | logfmt', 'forcedLabel', '=', 'value', true)).toEqual('{foo="bar"} | logfmt | forcedLabel=`value`');
    });
});
describe('addParserToQuery', () => {
    describe('when query had line filter', () => {
        ...
        it('should add parser after multiple line filters', () => {
            expect(addParserToQuery('{job="grafana"} |= "error" |= "info" |= "debug"', 'logfmt')).toBe('{job="grafana"} |= "error" |= "info" |= "debug" | logfmt');
        });
    });
...
describe('addNoPipelineErrorToQuery', () => {
    ...
    it('should add error filtering after json parser with expressions', () => {
        expect(addNoPipelineErrorToQuery('{job="grafana"} | json foo="bar", bar="baz"')).toBe('{job="grafana"} | json foo="bar", bar="baz" | __error__=``');
    });
...
describe('addLabelFormatToQuery', () => {
    it('should add label format at the end of log query when parser', () => {
        expect(addLabelFormatToQuery('{job="grafana"} | logfmt', { originalLabel: 'lvl', renameTo: 'level' })).toBe('{job="grafana"} | logfmt | label_format level=lvl');
    });
    it('should add label format at the end of log query when no parser', () => {
        expect(addLabelFormatToQuery('{job="grafana"}', { originalLabel: 'lvl', renameTo: 'level' })).toBe('{job="grafana"} | label_format level=lvl');
    });
    it('should add label format at the end of log query when more label parser', () => {
        expect(addLabelFormatToQuery('{job="grafana"} | logfmt | label_format a=b', { originalLabel: 'lvl', renameTo: 'level' })).toBe('{job="grafana"} | logfmt | label_format a=b | label_format level=lvl');
    });
    it('should add label format at the end of log query part of metrics query', () => {
        expect(addLabelFormatToQuery('rate({job="grafana"} | logfmt | label_format a=b [5m])', {
            originalLabel: 'lvl',
            renameTo: 'level',
    ...
    it('should add label format at the end of multiple log query part of metrics query', () => {
        expect(addLabelFormatToQuery('rate({job="grafana"} | logfmt | label_format a=b [5m]) + rate({job="grafana"} | logfmt | label_format a=b [5m])', { originalLabel: 'lvl', renameTo: 'level' })).toBe('rate({job="grafana"} | logfmt | label_format a=b | label_format level=lvl [5m]) + rate({job="grafana"} | logfmt | label_format a=b | label_format level=lvl [5m])');
    });
});
...
describe('queryHasFilter', () => {
    ...
    ])('should return true if query has a positive filter', (query, value) => {
    ...
    it.each([
        ...
        ['{job!="\\"grafana\\""}', '"grafana"'],
        ['{foo="bar"} | logfmt | job!=`grafana`', 'grafana'],
    ])('should return true if query has a negative filter', (query, value) => {
        expect(queryHasFilter(query, 'job', '!=', value)).toBe(true);
    });
});
describe('removeLabelFromQuery', () => {
    ...
    ])('should remove a positive label matcher from the query', (query, value, expected) => {
    ...
    it.each([
        ...
        ['{job!="\\"grafana\\""}', '"grafana"', '{}'],
        ['{foo="bar"} | logfmt | job!=`grafana`', 'grafana', '{foo="bar"} | logfmt'],
    ])('should remove a negative label matcher from the query', (query, value, expected) => {
        expect(removeLabelFromQuery(query, 'job', '!=', value)).toBe(expected);
    });
});
