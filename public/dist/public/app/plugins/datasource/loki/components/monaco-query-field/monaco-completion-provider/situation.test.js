...
describe('situation', () => {
    ...
    it('identifies AFTER_UNWRAP autocomplete situations', () => {
        ...
        assertSituation('quantile_over_time(0.99, {cluster="ops-tools1",container="ingress-nginx"} | json | __error__ = "" | unwrap ^', {
            type: 'AFTER_UNWRAP',
    ...
    it.each(['count_over_time({job="mysql"}[^])', 'rate({instance="server\\1"}[^])', 'rate({}[^'])('identifies IN_RANGE autocomplete situations in metric query %s', (query) => {
        assertSituation(query, {
            type: 'IN_RANGE',
        });
    });
