...
import { Button, Collapse, Icon, InlineField, InlineFieldRow, InlineSwitch, Label, MultiSelect, RenderUserContentAsHTML, Spinner, Tooltip, useStyles2, } from '@grafana/ui';
import store from 'app/core/store';
import { RawQuery } from '../../prometheus/querybuilder/shared/RawQuery';
import { LOKI_LOG_CONTEXT_PRESERVED_LABELS, SHOULD_INCLUDE_PIPELINE_OPERATIONS, } from '../LogContextProvider';
...
export function LokiContextUi(props) {
    ...
    const [includePipelineOperations, setIncludePipelineOperations] = useState(store.getBool(SHOULD_INCLUDE_PIPELINE_OPERATIONS, false));
    ...
    useAsync(() => __awaiter(this, void 0, void 0, function* () {
        setLoading(true);
        const initContextFilters = yield logContextProvider.getInitContextFilters(row.labels, origQuery);
    ...
    const contextFilterToSelectFilter = useCallback((contextFilter) => {
        return {
    ...
    let queryExpr = logContextProvider.prepareExpression(contextFilters.filter(({ enabled }) => enabled), origQuery);
    return (React.createElement("div", { className: styles.wrapper },
        React.createElement(Tooltip, { content: 'Revert to initial log context query.' },
            React.createElement("div", { className: styles.iconButton },
                React.createElement(Button, { "data-testid": "revert-button", icon: "history-alt", variant: "secondary", disabled: isInitialState, onClick: (e) => {
                        reportInteraction('grafana_explore_logs_loki_log_context_reverted', {
                            logRowUid: row.uid,
                        });
                        setContextFilters((contextFilters) => {
                            return contextFilters.map((contextFilter) => (Object.assign(Object.assign({}, contextFilter), { 
        ...
        React.createElement(Collapse, { collapsible: true, isOpen: isOpen, onToggle: () => {
            ...
            }, label: React.createElement("div", { className: styles.rawQueryContainer }, initialized ? (React.createElement(React.Fragment, null,
                React.createElement(RawQuery, { lang: { grammar: lokiGrammar, name: 'loki' }, query: queryExpr, className: styles.rawQuery }),
                React.createElement(Tooltip, { content: "The initial log context query is created from all labels defining the stream for the selected log line. Use the editor below to customize the log context query." },
                    React.createElement(Icon, { name: "info-circle", size: "sm", className: styles.queryDescription })))) : (React.createElement(Spinner, null))) },
            React.createElement("div", { className: styles.ui },
                React.createElement(Label, { className: styles.label, description: "The initial log context query is created from all labels defining the stream for the selected log line. You can broaden your search by removing one or more of the label filters." }, "Widen the search"),
                React.createElement(MultiSelect, { isLoading: loading, options: realLabels.map(contextFilterToSelectFilter), value: realLabelsEnabled.map(contextFilterToSelectFilter), closeMenuOnSelect: true, maxMenuHeight: 200, noOptionsMessage: "No further labels available", onChange: (keys, actionMeta) => {
                        ...
                        return setContextFilters(contextFilters.map((filter) => {
                ...
                showParsedLabels && (React.createElement(React.Fragment, null,
                    React.createElement(Label, { className: styles.label, description: `By using a parser in your original query, you can use filters for extracted labels. Refine your search by applying extracted labels created from the selected log line.` }, "Refine the search"),
                    React.createElement(MultiSelect, { isLoading: loading, options: parsedLabels.map(contextFilterToSelectFilter), value: parsedLabelsEnabled.map(contextFilterToSelectFilter), closeMenuOnSelect: true, maxMenuHeight: 200, noOptionsMessage: "No further labels available", isClearable: true, onChange: (keys, actionMeta) => {
                            if (actionMeta.action === 'select-option') {
                                reportInteraction('grafana_explore_logs_loki_log_context_filtered', {
                                    logRowUid: row.uid,
                                    type: 'parsed_label',
                            ...
                            setContextFilters(contextFilters.map((filter) => {
                                if (!filter.fromParser) {
                                    return filter;
                                }
                                filter.enabled = keys.some((key) => key.value === filter.label);
                ...
                logContextProvider.queryContainsValidPipelineStages(origQuery) && (React.createElement(InlineFieldRow, { className: styles.operationsToggle },
                    React.createElement(InlineField, { label: "Include LogQL pipeline operations", tooltip: React.createElement(RenderUserContentAsHTML, { content: renderMarkdown("This will include LogQL operations such as `line_format` or `label_format`. It won't include line or label filter operations.") }) },
                        React.createElement(InlineSwitch, { value: includePipelineOperations, showLabel: true, transparent: true, onChange: (e) => {
...
//# sourceMappingURL=LokiContextUi.js.map