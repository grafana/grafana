...
describe('loki result transformer', function () {
    beforeAll(function () {
    ...
    describe('lokiStreamResultToDataFrame', function () {
        it('converts streams to series', function () {
            var data = streamResult.map(function (stream) { return ResultTransformer.lokiStreamResultToDataFrame(stream); });
        ...
        it('should always generate unique ids for logs', function () {
            var streamResultWithDuplicateLogs = [
                {
                    stream: {
                        foo: 'bar',
                    },
            ...
            var data = streamResultWithDuplicateLogs.map(function (stream) { return ResultTransformer.lokiStreamResultToDataFrame(stream); });
        ...
        it('should append refId to the unique ids if refId is provided', function () {
            var data = streamResult.map(function (stream) { return ResultTransformer.lokiStreamResultToDataFrame(stream, false, 'B'); });
            expect(data.length).toBe(2);
    ...
    describe('lokiStreamsToDataFrames', function () {
        it('should enhance data frames', function () {
            jest.spyOn(ResultTransformer, 'enhanceDataFrame');
            var dataFrames = ResultTransformer.lokiStreamsToDataFrames(lokiResponse, { refId: 'B' }, 500, {
                derivedFields: [
                    {
            ...
            dataFrames.forEach(function (frame) {
                expect(frame.fields.filter(function (field) { return field.name === 'test' && field.type === 'string'; }).length).toBeGreaterThanOrEqual(1);
            });
        });
    });
    describe('appendResponseToBufferedData', function () {
        it('should return a dataframe with ts in iso format', function () {
            var tailResponse = {
                streams: [
                    {
                        stream: {
            ...
            var data = new CircularDataFrame({ capacity: 1 });
        ...
        it('should always generate unique ids for logs', function () {
            ...
            var data = new CircularDataFrame({ capacity: 6 });
            data.addField({ name: 'ts', type: FieldType.time, config: { displayName: 'Time' } });
            data.addField({ name: 'tsNs', type: FieldType.time, config: { displayName: 'Time ns' } });
            data.addField({ name: 'line', type: FieldType.string }).labels = { job: 'grafana' };
            data.addField({ name: 'labels', type: FieldType.other });
    ...
    describe('createMetricLabel', function () {
        it('should create correct label based on passed variables', function () {
            var label = ResultTransformer.createMetricLabel({}, {
    ...
    describe('lokiResultsToTableModel', function () {
        it('should correctly set the type of the label column to be a string', function () {
            var lokiResultWithIntLabel = [
                { metric: { test: 1 }, value: [1610367143, 10] },
                { metric: { test: 2 }, value: [1610367144, 20] },
            ];
            var table = ResultTransformer.lokiResultsToTableModel(lokiResultWithIntLabel, 1, 'A', {});
...
describe('enhanceDataFrame', function () {
    it('adds links to fields', function () {
        var df = new MutableDataFrame({ fields: [{ name: 'line', values: ['nothing', 'trace1=1234', 'trace2=foo'] }] });
    ...
    describe('lokiPointsToTimeseriesPoints()', function () {
        /**
        ...
        it('returns data as is if step, start, and end align', function () {
            var options = { start: 1 * 1e9, end: 4 * 1e9, step: 1 };
            var result = ResultTransformer.lokiPointsToTimeseriesPoints(data, options);
