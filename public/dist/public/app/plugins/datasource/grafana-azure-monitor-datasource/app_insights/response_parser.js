...
var ResponseParser = /** @class */ (function () {
    ...
    ResponseParser.prototype.parseQueryResult = function () {
        ...
        for (var i = 0; i < this.results.length; i++) {
            if (this.results[i].query.raw) {
                ...
                var value = this.results[i].result.value;
    ...
    ResponseParser.prototype.parseRawQueryResultRow = function (query, columns, rows, xaxis, yaxises, spliton) {
        var data = [];
        var columnsForDropdown = map(columns, function (column) { return ({ text: column.ColumnName, value: column.ColumnName }); });
        var xaxisColumn = columns.findIndex(function (column) { return column.ColumnName === xaxis; });
        var yaxisesSplit = yaxises.split(',');
        var yaxisColumns = {};
        forEach(yaxisesSplit, function (yaxis) {
        ...
        forEach(rows, function (row) {
            forEach(yaxisColumns, function (yaxisColumn, yaxisName) {
    ...
    ResponseParser.getMetricFieldKey = function (segment) {
        var keys = _keys(segment);
        return filter(without(keys, 'start', 'end'), function (key) {
            return isObject(segment[key]);
        })[0];
    };
    ResponseParser.getKeyForAggregationField = function (dataObj) {
        var keys = _keys(dataObj);
    ...
    ResponseParser.prototype.parseQuerySchema = function () {
        ...
        if (this.results && this.results && this.results.Tables) {
            for (var i = 0; i < this.results.Tables[0].Rows.length; i++) {
                var column = this.results.Tables[0].Rows[i];
