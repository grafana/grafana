...
import { aggregationvariable, labelsVariable, metricVariable, namespaceVariable, } from '../../__mocks__/CloudWatchDataSource';
import { createFunctionWithParameter, createArray, createOperator, createGroupBy, createFunction, createProperty, } from '../../__mocks__/sqlUtils';
...
describe('SQLGenerator', () => {
    ...
    describe('mandatory fields check', () => {
        it('should return undefined if metric and aggregation is missing', () => {
            expect(new SQLGenerator().expressionToSqlQuery({
                from: createFunctionWithParameter('SCHEMA', ['AWS/EC2']),
            })).toBeUndefined();
        });
    ...
    describe('select', () => {
        it('should use statistic and metric name', () => {
            ...
            expect(new SQLGenerator().expressionToSqlQuery(Object.assign(Object.assign({}, baseQuery), { select }))).toEqual(`SELECT COUNT(BytesPerSecond) FROM SCHEMA("AWS/EC2")`);
        });
        it('should wrap in double quotes if metric name contains illegal characters ', () => {
            ...
            expect(new SQLGenerator().expressionToSqlQuery(Object.assign(Object.assign({}, baseQuery), { select }))).toEqual(`SELECT COUNT("Bytes-Per-Second") FROM SCHEMA("AWS/EC2")`);
        });
    });
    describe('from', () => {
        describe('with schema contraint', () => {
            it('should handle schema without dimensions', () => {
                ...
                expect(new SQLGenerator().expressionToSqlQuery(Object.assign(Object.assign({}, baseQuery), { from }))).toEqual(`SELECT SUM(CPUUtilization) FROM SCHEMA("AWS/MQ")`);
            });
            it('should handle schema with dimensions', () => {
                const from = createFunctionWithParameter('SCHEMA', ['AWS/MQ', 'InstanceId', 'InstanceType']);
                expect(new SQLGenerator().expressionToSqlQuery(Object.assign(Object.assign({}, baseQuery), { from }))).toEqual(`SELECT SUM(CPUUtilization) FROM SCHEMA("AWS/MQ", InstanceId, InstanceType)`);
            });
            it('should handle schema with dimensions that has special characters', () => {
                ...
                expect(new SQLGenerator().expressionToSqlQuery(Object.assign(Object.assign({}, baseQuery), { from }))).toEqual(`SELECT SUM(CPUUtilization) FROM SCHEMA("AWS/MQ", "Instance Id", "Instance.Type", "Instance-Group")`);
            });
        });
        describe('without schema', () => {
            it('should use the specified namespace', () => {
                const from = createProperty('AWS/MQ');
                expect(new SQLGenerator().expressionToSqlQuery(Object.assign(Object.assign({}, baseQuery), { from }))).toEqual(`SELECT SUM(CPUUtilization) FROM "AWS/MQ"`);
            });
        });
    });
    function assertQueryEndsWith(rest, expectedFilter) {
        expect(new SQLGenerator().expressionToSqlQuery(Object.assign(Object.assign({}, baseQuery), rest))).toEqual(`SELECT SUM(CPUUtilization) FROM SCHEMA("AWS/EC2") ${expectedFilter}`);
    }
    describe('filter', () => {
        it('should not add WHERE clause in case its empty', () => {
            expect(new SQLGenerator().expressionToSqlQuery(Object.assign({}, baseQuery))).not.toContain('WHERE');
        });
        it('should not add WHERE clause when there is no filter conditions', () => {
            const where = createArray([]);
            expect(new SQLGenerator().expressionToSqlQuery(Object.assign(Object.assign({}, baseQuery), { where }))).not.toContain('WHERE');
        });
        ...
        it('should handle one top level filter with OR', () => {
            assertQueryEndsWith({ where: createArray([createOperator('InstanceId', '=', 'I-123')]) }, `WHERE InstanceId = 'I-123'`);
        });
        it('should handle multiple top level filters combined with AND', () => {
            const filter = createArray([createOperator('InstanceId', '=', 'I-123'), createOperator('Instance-Id', '!=', 'I-456')], QueryEditorExpressionType.And);
        ...
        it('should handle multiple top level filters combined with OR', () => {
            const filter = createArray([createOperator('InstanceId', '=', 'I-123'), createOperator('InstanceId', '!=', 'I-456')], QueryEditorExpressionType.Or);
        ...
        it('should handle one top level filter with two nested filters combined with AND', () => {
            const filter = createArray([
                ...
                createArray([createOperator('InstanceId', '!=', 'I-456'), createOperator('Type', '!=', 'some-type')], QueryEditorExpressionType.And),
            ], QueryEditorExpressionType.And);
            ...
            assertQueryEndsWith({ where: filter }, `WHERE "Instance.Type" = 'I-123' AND (InstanceId != 'I-456' AND Type != 'some-type')`);
        });
        it('should handle one top level filter with two nested filters combined with OR', () => {
            const filter = createArray([
                createOperator('InstanceId', '=', 'I-123'),
                createArray([createOperator('InstanceId', '!=', 'I-456'), createOperator('Type', '!=', 'some-type')], QueryEditorExpressionType.Or),
            ], QueryEditorExpressionType.And);
            assertQueryEndsWith({ where: filter }, `WHERE InstanceId = 'I-123' AND (InstanceId != 'I-456' OR Type != 'some-type')`);
        });
        it('should handle two top level filters with two nested filters combined with AND', () => {
            const filter = createArray([
                createArray([createOperator('InstanceId', '=', 'I-123'), createOperator('Type', '!=', 'some-type')], QueryEditorExpressionType.And),
                createArray([createOperator('InstanceId', '!=', 'I-456'), createOperator('Type', '!=', 'some-type')], QueryEditorExpressionType.Or),
            ], QueryEditorExpressionType.And);
            assertQueryEndsWith({ where: filter }, `WHERE (InstanceId = 'I-123' AND Type != 'some-type') AND (InstanceId != 'I-456' OR Type != 'some-type')`);
        });
        it('should handle two top level filters with two nested filters combined with OR', () => {
            const filter = createArray([
                createArray([createOperator('InstanceId', '=', 'I-123'), createOperator('Type', '!=', 'some-type')], QueryEditorExpressionType.Or),
                createArray([createOperator('InstanceId', '!=', 'I-456'), createOperator('Type', '!=', 'some-type')], QueryEditorExpressionType.Or),
            ], QueryEditorExpressionType.Or);
            assertQueryEndsWith({ where: filter }, `WHERE (InstanceId = 'I-123' OR Type != 'some-type') OR (InstanceId != 'I-456' OR Type != 'some-type')`);
        });
        it('should handle three top level filters with one nested filters combined with OR', () => {
            ...
            assertQueryEndsWith({ where: filter }, `WHERE InstanceId = 'I-123' OR Type != 'some-type' OR InstanceId != 'I-456'`);
        });
        it('should handle three top level filters with one nested filters combined with AND', () => {
            ...
            assertQueryEndsWith({ where: filter }, `WHERE InstanceId = 'I-123' AND Type != 'some-type' AND InstanceId != 'I-456'`);
        });
    });
    describe('group by', () => {
        it('should not add GROUP BY clause in case its empty', () => {
            expect(new SQLGenerator().expressionToSqlQuery(Object.assign({}, baseQuery))).not.toContain('GROUP BY');
        });
        ...
        it('should handle multiple label', () => {
            const groupBy = createArray([createGroupBy('InstanceId'), createGroupBy('Type'), createGroupBy('Group')], QueryEditorExpressionType.And);
            assertQueryEndsWith({ groupBy }, `GROUP BY InstanceId, Type, Group`);
        });
    });
    describe('order by', () => {
        it('should not add ORDER BY clause in case its empty', () => {
            expect(new SQLGenerator().expressionToSqlQuery(Object.assign({}, baseQuery))).not.toContain('ORDER BY');
        });
    ...
    describe('limit', () => {
        it('should not add LIMIT clause in case its empty', () => {
            expect(new SQLGenerator().expressionToSqlQuery(Object.assign({}, baseQuery))).not.toContain('LIMIT');
        });
    ...
    describe('full query', () => {
        it('should not add LIMIT clause in case its empty', () => {
            ...
            expect(new SQLGenerator().expressionToSqlQuery(query)).toEqual(`SELECT COUNT(DroppedBytes) FROM SCHEMA("AWS/MQ", InstanceId, "Instance-Group") WHERE (InstanceId = 'I-123' OR Type != 'some-type') AND (InstanceId != 'I-456' OR Type != 'some-type') GROUP BY InstanceId, InstanceType ORDER BY COUNT() DESC LIMIT 100`);
        });
    });
    describe('using variables', () => {
        ...
        it('should interpolate variables correctly', () => {
            ...
            expect(new SQLGenerator(templateService).expressionToSqlQuery(query)).toEqual(`SELECT $aggregation($metric) FROM SCHEMA(\"$namespace\", $labels) WHERE (InstanceId = 'I-123' OR Type != 'some-type') AND (InstanceId != 'I-456' OR Type != 'some-type') GROUP BY $labels ORDER BY $aggregation() DESC LIMIT 100`);
        });
    });
});
//# sourceMappingURL=SQLGenerator.test.js.map