...
describe('statementPosition', () => {
    function createToken(query, position) {
        ...
        return linkedTokenBuilder(MonacoMock, cloudWatchSqlLanguageDefinition, testModel, position, MetricMathTokenTypes);
    }
    it('returns PredefinedFunction when at the beginning of an empty query', () => {
        const token = createToken(MetricMathTestQueries.singleLineEmptyQuery.query, MetricMathTestQueries.singleLineEmptyQuery.position);
    ...
    it('returns PredefinedFuncSecondArg when in the second arg of a predefined function', () => {
        const token = createToken(MetricMathTestQueries.secondArgQuery.query, MetricMathTestQueries.secondArgQuery.position);
    ...
    it('returns SearchFuncSecondArg when in the second arg of a Search function', () => {
        const token = createToken(MetricMathTestQueries.secondArgAfterSearchQuery.query, MetricMathTestQueries.secondArgAfterSearchQuery.position);
    ...
    it('returns SearchFuncThirdArg when in the third arg of a Search function', () => {
        const token = createToken(MetricMathTestQueries.thirdArgAfterSearchQuery.query, MetricMathTestQueries.thirdArgAfterSearchQuery.position);
    ...
    it('returns AfterFunction when after a function', () => {
        const token = createToken(MetricMathTestQueries.afterFunctionQuery.query, MetricMathTestQueries.afterFunctionQuery.position);
        expect(getStatementPosition(token)).toEqual(StatementPosition.AfterFunction);
    });
    it('returns WithinString when within a string', () => {
        const token = createToken(MetricMathTestQueries.withinStringQuery.query, MetricMathTestQueries.withinStringQuery.position);
