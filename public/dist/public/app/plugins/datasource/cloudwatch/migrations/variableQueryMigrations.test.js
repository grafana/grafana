...
describe('variableQueryMigrations', () => {
        ...
        describe('when dimension_values query is used', () => {
            ...
            describe('and filter param is defined by user', () => {
                it('should use the user defined filter', () => {
                    const query = migrateVariableQuery('dimension_values(us-east-1,AWS/RDS,CPUUtilization,DBInstanceIdentifier,{"InstanceId":"$instance_id"})');
                    expect(query.queryType).toBe(VariableQueryType.DimensionValues);
                    expect(query.region).toBe('us-east-1');
                    expect(query.namespace).toBe('AWS/RDS');
                    expect(query.metricName).toBe('CPUUtilization');
                ...
                it('should migrate json template variables', () => {
                    const query = migrateVariableQuery('dimension_values(us-east-1,AWS/RDS,CPUUtilization,DBInstanceIdentifier,{"role":${role:json},"pop":${pop:json}})');
    ...
    describe('when resource_arns query is used', () => {
        it('should parse the query', () => {
            const query = migrateVariableQuery('resource_arns(eu-west-1,elasticloadbalancing:loadbalancer,{"elasticbeanstalk:environment-name":["myApp-dev","myApp-prod"]})');
        ...
        it('should migrate json template variables', () => {
            const query = migrateVariableQuery('resource_arns(eu-west-1,elasticloadbalancing:loadbalancer,{"elasticbeanstalk:environment-name":[${jsonVar:json},"test-$singleVar"]})');
    ...
    describe('when OldVariableQuery is used', () => {
        ...
        it('should parse ec2 query', () => {
            const oldQuery = Object.assign(Object.assign({}, baseOldQuery), { queryType: VariableQueryType.EC2InstanceAttributes, ec2Filters: '{"environment":["$environment"]}', attributeName: 'rds:db' });
        ...
        it('should parse resource arn query', () => {
            const oldQuery = Object.assign(Object.assign({}, baseOldQuery), { queryType: VariableQueryType.ResourceArns, resourceType: 'elasticloadbalancing:loadbalancer', tags: '{"elasticbeanstalk:environment-name":["myApp-dev","myApp-prod"]}' });
            const query = migrateVariableQuery(oldQuery);
            expect(query.region).toBe('us-east-1');
            expect(query.resourceType).toBe('elasticloadbalancing:loadbalancer');
            expect(query.tags).toStrictEqual({ 'elasticbeanstalk:environment-name': ['myApp-dev', 'myApp-prod'] });
        ...
        it('should parse dimension values query', () => {
            const oldQuery = Object.assign(Object.assign({}, baseOldQuery), { queryType: VariableQueryType.DimensionValues, metricName: 'foo', dimensionKey: 'bar', dimensionFilters: '{"InstanceId":"$instanceid"}' });
            const query = migrateVariableQuery(oldQuery);
...
//# sourceMappingURL=variableQueryMigrations.test.js.map