...
export function setupMockedLogsQueryRunner({ data = {
    results: {},
}, variables, mockGetVariableName = true, settings = CloudWatchSettings, timeSrv = getTimeSrv(), } = {}) {
    let templateService = new TemplateSrv();
    if (variables) {
        templateService = setupMockedTemplateService(variables);
        if (mockGetVariableName) {
            templateService.getVariableName = (name) => name;
        }
    }
...
export function genMockFrames(numResponses) {
...
export function genMockCloudWatchLogsRequest(overrides = {}) {
    const request = Object.assign({ queryString: 'fields @timestamp, @message | sort @timestamp desc', logGroupNames: ['log-group-name-1', 'log-group-name-2'], logGroups: [
            { arn: 'log-group-arn-1', name: 'log-group-name-1' },
            { arn: 'log-group-arn-2', name: 'log-group-name-2' },
        ], refId: 'A', region: 'us-east-1' }, overrides);
    return request;
}
//# sourceMappingURL=LogsQueryRunner.js.map