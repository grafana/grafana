...
describe('CloudWatchLink', () => {
    it('generates a link with log group names', () => __awaiter(void 0, void 0, void 0, function* () {
        const ds = setupMockedDataSource();
        const { rerender } = render(React.createElement(CloudWatchLink, { query: validLogsQuery, datasource: ds.datasource, panelData: undefined }));
        yield waitFor(() => {
            expect(screen.getByText('CloudWatch Logs Insights').closest('a')).toHaveAttribute('href', '');
        });
        yield act(() => __awaiter(void 0, void 0, void 0, function* () {
            rerender(React.createElement(CloudWatchLink, { query: validLogsQuery, datasource: ds.datasource, panelData: {
        ...
        yield waitFor(() => {
            expect(screen.getByText('CloudWatch Logs Insights').closest('a')).toHaveAttribute('href', "https://us-east-2.console.aws.amazon.com/cloudwatch/home?region=us-east-2#logs-insights:queryDetail=~(end~'2016-12-31T16*3a00*3a00.000Z~start~'2016-12-31T15*3a00*3a00.000Z~timeType~'ABSOLUTE~tz~'UTC~editorString~'fields*20*40timestamp*2c*20*40message*20*7c*20sort*20*40timestamp*20desc*20*7c*20limit*2025~isLiveTail~false~source~(~'group-A~'group-B))");
        });
    }));
    it('generates a link with log group names', () => __awaiter(void 0, void 0, void 0, function* () {
        const ds = setupMockedDataSource();
        const query = Object.assign(Object.assign({}, validLogsQuery), { logGroupNames: undefined, logGroups: [
        ...
        const { rerender } = render(React.createElement(CloudWatchLink, { query: query, datasource: ds.datasource, panelData: undefined }));
        yield waitFor(() => {
            expect(screen.getByText('CloudWatch Logs Insights').closest('a')).toHaveAttribute('href', '');
        });
        yield act(() => __awaiter(void 0, void 0, void 0, function* () {
            rerender(React.createElement(CloudWatchLink, { query: query, datasource: ds.datasource, panelData: {
                    timeRange: RequestMock.range,
                    request: RequestMock,
                    state: LoadingState.Done,
                    series: [],
...
//# sourceMappingURL=CloudWatchLink.test.js.map