...
var InfluxSeries = /** @class */ (function () {
    ...
    InfluxSeries.prototype.getTimeSeries = function () {
        ...
        each(this.series, function (series) {
            ...
            var tags = map(series.tags, function (value, key) {
                return key + ': ' + value;
            });
            for (j = 1; j < columns; j++) {
    ...
    InfluxSeries.prototype.getAnnotations = function () {
        ...
        each(this.series, function (series) {
            ...
            var tagsCol = [];
            var textCol = null;
            each(series.columns, function (column, index) {
                if (column === 'time') {
                    timeCol = index;
                    return;
                }
                ...
                if (includes((_this.annotation.tagsColumn || '').replace(' ', '').split(','), column)) {
                    tagsCol.push(index);
                    return;
                }
            ...
            each(series.values, function (value) {
                var data = {
    ...
    InfluxSeries.prototype.getTable = function () {
        ...
        each(this.series, function (series, seriesIndex) {
            if (seriesIndex === 0) {
                var firstCol = series.columns[0];
                // Check the first column's name, if it is `time`, we
                // mark it as having the type time
                var firstTableCol = firstCol === 'time' ? { text: 'Time', type: FieldType.time } : { text: firstCol };
                table.columns.push(firstTableCol);
                each(keys(series.tags), function (key) {
