...
describe('InfluxQueryBuilder', function () {
    describe('when building explore queries', function () {
        ...
        it('should ignore condition if operator is a value operator', function () {
            var builder = new InfluxQueryBuilder({
                measurement: '',
        ...
        it('should handle tag-value=number-ish when getting measurements', function () {
            var builder = new InfluxQueryBuilder({ measurement: undefined, tags: [{ key: 'app', value: '42', operator: '==' }] }, undefined);
            var query = builder.buildExploreQuery('MEASUREMENTS');
            expect(query).toBe("SHOW MEASUREMENTS WHERE \"app\" == 42 LIMIT 100");
        });
        it('should handle tag-value=number-ish getting tag-keys', function () {
            var builder = new InfluxQueryBuilder({ measurement: undefined, tags: [{ key: 'app', value: '42', operator: '==' }] }, undefined);
            var query = builder.buildExploreQuery('TAG_KEYS');
            expect(query).toBe("SHOW TAG KEYS WHERE \"app\" == 42");
        });
        it('should handle tag-value-contains-backslash-character getting tag-keys', function () {
            var builder = new InfluxQueryBuilder({ measurement: undefined, tags: [{ key: 'app', value: 'lab\\el', operator: '==' }] }, undefined);
            var query = builder.buildExploreQuery('TAG_KEYS');
            expect(query).toBe("SHOW TAG KEYS WHERE \"app\" == 'lab\\\\el'");
        });
        it('should handle tag-value-contains-single-quote-character getting tag-keys', function () {
            var builder = new InfluxQueryBuilder({ measurement: undefined, tags: [{ key: 'app', value: "lab'el", operator: '==' }] }, undefined);
            var query = builder.buildExploreQuery('TAG_KEYS');
            expect(query).toBe("SHOW TAG KEYS WHERE \"app\" == 'lab\\'el'");
        });
        it('should handle tag-value=emptry-string when getting measurements', function () {
            var builder = new InfluxQueryBuilder({ measurement: undefined, tags: [{ key: 'app', value: '', operator: '==' }] }, undefined);
            var query = builder.buildExploreQuery('MEASUREMENTS');
            expect(query).toBe("SHOW MEASUREMENTS WHERE \"app\" == '' LIMIT 100");
        });
        it('should handle tag-value=emptry-string when getting tag-keys', function () {
            var builder = new InfluxQueryBuilder({ measurement: undefined, tags: [{ key: 'app', value: '', operator: '==' }] }, undefined);
            var query = builder.buildExploreQuery('TAG_KEYS');
