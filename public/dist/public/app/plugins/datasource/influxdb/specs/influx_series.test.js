...
describe('when generating timeseries from influxdb response', function () {
    ...
    describe('given annotation response', function () {
        ...
        describe('given a time-column in the json-response', function () {
            var options = {
                alias: '',
                series: [
                    {
            ...
            it('the column-names should be correct if the time-column is not renamed', function () {
                var series = new InfluxSeries(options);
                var table = series.getTable();
            ...
            it('the column-names should be correct if the time-column is renamed', function () {
                var renamedOptions = produce(options, function (draft) {
                    // we rename the time-column to `zeit`
                    draft.series[0].columns[0] = 'zeit';
                });
                var series = new InfluxSeries(renamedOptions);
                var table = series.getTable();
                expect(table.columns).toStrictEqual([
                    {
                        text: 'zeit',
