...
describe('influxdb response parser', function () {
    var parser = new ResponseParser();
    describe('SHOW TAG response', function () {
        ...
        it('expects three results', function () {
            expect(size(result)).toBe(3);
        });
    });
    describe('SHOW TAG VALUES response', function () {
        var query = 'SHOW TAG VALUES FROM "cpu" WITH KEY = "hostname"';
        describe('response from 0.10.0', function () {
            var response = {
                results: [
                    {
                        series: [
    ...
    describe('SELECT response', function () {
        ...
        var result = parser.parse(query, response);
        it('should return second column', function () {
            expect(size(result)).toBe(3);
    ...
    describe('SELECT response where ordering matters', function () {
        var query = 'SELECT "val" from "num"';
        var response = {
            results: [
                {
                    series: [
        ...
        it('should keep the order returned by influxdb, even for numbers', function () {
    ...
    describe('SHOW FIELD response', function () {
        var query = 'SHOW FIELD KEYS FROM "cpu"';
        describe('response from pre-1.0', function () {
            var response = {
                results: [
                    {
                        series: [
        ...
        describe('response from 1.0', function () {
            var response = {
                results: [
                    {
                        series: [
            ...
            var result = parser.parse(query, response);
            it('should return first column', function () {
                expect(size(result)).toBe(1);
...
//# sourceMappingURL=response_parser.test.js.map