...
describe('InfluxQuery', () => {
    ...
    describe('render series with measurement only', () => {
        it('should generate correct query', () => {
            ...
            expect(queryText).toBe('SELECT mean("value") FROM "autogen"."cpu" WHERE $timeFilter GROUP BY time($__interval)' + ' fill(null)');
        });
    });
    ...
    describe('render series with math and alias', () => {
        it('should generate correct query', () => {
            ...
            const queryText = query.render();
            expect(queryText).toBe('SELECT mean("value") /100 AS "text" FROM "autogen"."cpu" WHERE $timeFilter GROUP BY time($__interval) fill(null)');
        });
    });
    describe('series with single tag only', () => {
        it('should generate correct query', () => {
            ...
            const queryText = query.render();
            expect(queryText).toBe('SELECT mean("value") FROM "autogen"."cpu" WHERE ("hostname" = \'server\\\\1\') AND $timeFilter' +
        ...
        it('should switch regex operator with tag value is regex', () => {
            ...
            const queryText = query.render();
            expect(queryText).toBe('SELECT mean("value") FROM "autogen"."cpu" WHERE ("app" =~ /e.*/) AND $timeFilter GROUP BY time($__interval)');
        });
    });
    describe('series with multiple tags only', () => {
        it('should generate correct query', () => {
            ...
            const queryText = query.render();
            expect(queryText).toBe('SELECT mean("value") FROM "autogen"."cpu" WHERE ("hostname" = \'server1\' AND "app" = \'email\') AND ' +
    ...
    describe('series with tags OR condition', () => {
        it('should generate correct query', () => {
            ...
            const queryText = query.render();
            expect(queryText).toBe('SELECT mean("value") FROM "autogen"."cpu" WHERE ("hostname" = \'server1\' OR "hostname" = \'server2\') AND ' +
                '$timeFilter GROUP BY time($__interval)');
        });
    });
    describe('field name with single quote should be escaped and', () => {
        it('should generate correct query', () => {
            ...
            const queryText = query.render();
            expect(queryText).toBe('SELECT mean("value") FROM "autogen"."cpu" WHERE ("name" = \'Let\\\'s encrypt.\' OR "hostname" = \'server2\') AND ' +
                '$timeFilter GROUP BY time($__interval)');
        });
    });
    ...
    describe('series with groupByTag', () => {
        it('should generate correct query', () => {
            ...
            const queryText = query.render();
            expect(queryText).toBe('SELECT mean("value") FROM "autogen"."cpu" WHERE $timeFilter GROUP BY time($__interval), "host"');
        });
    });
    ...
    describe('render series without group by and fill', () => {
        it('should generate correct query', () => {
            ...
            const queryText = query.render();
            expect(queryText).toBe('SELECT "value" FROM "autogen"."cpu" WHERE $timeFilter GROUP BY time($__interval) fill(0)');
        });
    });
    describe('when adding group by part', () => {
        it('should add tag before fill', () => {
            ...
            expect((_a = query.target.groupBy) === null || _a === void 0 ? void 0 : _a.length).toBe(3);
        ...
        it('should add tag last if no fill', () => {
            ...
            query.addGroupBy('tag(host)');
            expect((_a = query.target.groupBy) === null || _a === void 0 ? void 0 : _a.length).toBe(1);
...
//# sourceMappingURL=influx_query_model.test.js.map