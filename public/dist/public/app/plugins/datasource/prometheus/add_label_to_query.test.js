...
describe('addLabelToQuery()', function () {
    ...
    it('should convert number Infinity to +Inf', function () {
        expect(addLabelToQuery('sum(rate(prometheus_tsdb_compaction_chunk_size_bytes_bucket[5m])) by (le)', 'le', Infinity)).toBe('sum(rate(prometheus_tsdb_compaction_chunk_size_bytes_bucket{le="+Inf"}[5m])) by (le)');
    });
    it('should handle selectors with punctuation', function () {
        ...
        expect(addLabelToQuery('avg(foo:metric:rate1m{a="b"})', 'bar', 'baz')).toBe('avg(foo:metric:rate1m{a="b",bar="baz"})');
    ...
    it('should not add duplicate labels to a query', function () {
    ...
    it('should not remove filters', function () {
        expect(addLabelToQuery('{x="y"} |="yy"', 'bar', 'baz')).toBe('{bar="baz",x="y"} |="yy"');
        expect(addLabelToQuery('{x="y"} |="yy" !~"xx"', 'bar', 'baz')).toBe('{bar="baz",x="y"} |="yy" !~"xx"');
    });
    it('should add label to query properly with Loki datasource', function () {
        expect(addLabelToQuery('{job="grafana"} |= "foo-bar"', 'filename', 'test.txt', undefined, true)).toBe('{filename="test.txt",job="grafana"} |= "foo-bar"');
    });
    it('should add labels to metrics with logical operators', function () {
    ...
    it('should not add ad-hoc filter to template variables', function () {
        expect(addLabelToQuery('sum(rate({job="foo"}[2m])) by (value $variable)', 'bar', 'baz')).toBe('sum(rate({bar="baz",job="foo"}[2m])) by (value $variable)');
    });
    it('should not add ad-hoc filter to range', function () {
        expect(addLabelToQuery('avg(rate((my_metric{job="foo"} > 0)[3h:])) by (label)', 'bar', 'baz')).toBe('avg(rate((my_metric{bar="baz",job="foo"} > 0)[3h:])) by (label)');
    });
    it('should not add ad-hoc filter to labels in label list provided with the group modifier', function () {
        expect(addLabelToQuery('max by (id, name, type) (my_metric{type=~"foo|bar|baz-test"}) * on(id) group_right(id, type, name) sum by (id) (my_metric) * 1000', 'bar', 'baz')).toBe('max by (id, name, type) (my_metric{bar="baz",type=~"foo|bar|baz-test"}) * on(id) group_right(id, type, name) sum by (id) (my_metric{bar="baz"}) * 1000');
    });
    it('should not add ad-hoc filter to labels in label list provided with the group modifier', function () {
        expect(addLabelToQuery('rate(my_metric[${__range_s}s])', 'bar', 'baz')).toBe('rate(my_metric{bar="baz"}[${__range_s}s])');
    });
    it('should not add ad-hoc filter to labels to math operations', function () {
        expect(addLabelToQuery('count(my_metric{job!="foo"} < (5*1024*1024*1024) or vector(0)) - 1', 'bar', 'baz')).toBe('count(my_metric{bar="baz",job!="foo"} < (5*1024*1024*1024) or vector(0)) - 1');
    });
});
