...
import { escapeLabelValueInExactSelector, escapeLabelValueInRegexSelector, expandRecordingRules, fixSummariesMetadata, getPrometheusTime, getRangeSnapInterval, parseSelector, toPromLikeQuery, truncateResult, } from './language_utils';
...
describe('fixSummariesMetadata', () => {
    ...
    it('returns only synthetics on empty metadata', () => {
        expect(fixSummariesMetadata({})).toEqual(Object.assign({}, synthetics));
    });
    it('returns unchanged metadata if no summary is present', () => {
        ...
        expect(fixSummariesMetadata(metadataRaw)).toEqual(Object.assign(Object.assign({}, metadata), synthetics));
    });
    it('returns metadata with added count and sum for a summary', () => {
        ...
        expect(fixSummariesMetadata(metadata)).toEqual(Object.assign(Object.assign({}, expected), synthetics));
    });
...
describe('getRangeSnapInterval', () => {
    ...
    it('will snap range to closest minute', () => {
        ...
        expect(first).toEqual({
            start: getPrometheusTime(expectedFrom, false).toString(10),
            end: getPrometheusTime(expectedTo, false).toString(10),
        });
        expect(second).toEqual({
            start: getPrometheusTime(expectedFrom.clone().add(1, 'minute'), false).toString(10),
            end: getPrometheusTime(expectedTo.clone().add(1, 'minute'), false).toString(10),
        });
        expect(third).toEqual({
            start: getPrometheusTime(expectedFrom.clone().add(2, 'minute'), false).toString(10),
            end: getPrometheusTime(expectedTo.clone().add(2, 'minute'), false).toString(10),
        });
    });
    it('will snap range to closest 10 minute', () => {
        ...
        const expectedFromFirst = calculateClosest10(nowTime.clone().startOf('minute').subtract(queryDurationMinutes, 'minute'));
        const expectedToFirst = calculateClosest10(nowTime.clone().startOf('minute').add(1, 'minute'));
        const expectedFromSecond = calculateClosest10(nowTimePlusOne.clone().startOf('minute').subtract(queryDurationMinutes, 'minute'));
        const expectedToSecond = calculateClosest10(nowTimePlusOne.clone().startOf('minute').add(1, 'minute'));
        const expectedFromThird = calculateClosest10(nowTimePlusTwo.clone().startOf('minute').subtract(queryDurationMinutes, 'minute'));
        ...
        expect(first).toEqual({
            start: getPrometheusTime(expectedFromFirst, false).toString(10),
            end: getPrometheusTime(expectedToFirst, false).toString(10),
        });
        expect(second).toEqual({
            start: getPrometheusTime(expectedFromSecond.clone(), false).toString(10),
            end: getPrometheusTime(expectedToSecond.clone(), false).toString(10),
        });
        expect(third).toEqual({
            start: getPrometheusTime(expectedFromThird.clone(), false).toString(10),
            end: getPrometheusTime(expectedToThird.clone(), false).toString(10),
        });
    });
    it('will snap range to closest 60 minute', () => {
        ...
        const expectedFromFirst = calculateClosest60(nowTime.clone().startOf('minute').subtract(queryDurationMinutes, 'minute'));
        const expectedToFirst = calculateClosest60(nowTime.clone().startOf('minute').add(1, 'minute'));
        const expectedFromSecond = calculateClosest60(nowTimePlusOne.clone().startOf('minute').subtract(queryDurationMinutes, 'minute'));
        const expectedToSecond = calculateClosest60(nowTimePlusOne.clone().startOf('minute').add(1, 'minute'));
        const expectedFromThird = calculateClosest60(nowTimePlusTwo.clone().startOf('minute').subtract(queryDurationMinutes, 'minute'));
