...
const error = React.createElement(FieldValidationMessage, null, "Value is not valid");
...
describe('promSettings validateInput', () => {
    ...
    ${'1s'}  | ${true}
    ${'1y'}  | ${true}
  `("Single duration regex, when calling the rule with correct formatted value: '$value' then result should be '$expected'", ({ value, expected }) => {
    ...
    ${'1h 2m'} | ${true}
    ${'1m 2s'} | ${true}
  `("Multiple duration regex, when calling the rule with correct formatted value: '$value' then result should be '$expected'", ({ value, expected }) => {
    ...
    ${'w'}    | ${error}
    ${'1.0s'} | ${error}
  `("when calling the rule with incorrect formatted value: '$value' then result should be '$expected'", ({ value, expected }) => {
    ...
    ${'https://'}  | ${error}
    ${'ftp://'}    | ${error}
  `("Url incorrect formatting, when calling the rule with correct formatted value: '$value' then result should be '$expected'", ({ value, expected }) => {
    ...
    ${'http://example'}  | ${true}
    ${'https://example'} | ${true}
  `("Url correct formatting, when calling the rule with correct formatted value: '$value' then result should be '$expected'", ({ value, expected }) => {
    ...
    it('should display a custom validation message', () => {
        ...
        const errorWithCustomMessage = React.createElement(FieldValidationMessage, null, customMessage);
