...
describe('LabelFilters', () => {
    ...
    it('renders empty input when labels are deleted from outside ', () => __awaiter(void 0, void 0, void 0, function* () {
        ...
        expect(screen.getByText(/foo/)).toBeInTheDocument();
        expect(screen.getByText(/bar/)).toBeInTheDocument();
        rerender(React.createElement(LabelFilters, { onChange: jest.fn(), onGetLabelNames: jest.fn(), getLabelValuesAutofillSuggestions: jest.fn(), onGetLabelValues: jest.fn(), labelsFilters: [], debounceDuration: 300 }));
        expect(screen.getAllByText('Select label')).toHaveLength(1);
        expect(screen.getAllByText('Select value')).toHaveLength(1);
        expect(screen.getByText(/=/)).toBeInTheDocument();
        expect(getAddButton()).toBeInTheDocument();
...
function setup(propOverrides) {
    const defaultProps = {
        onChange: jest.fn(),
        getLabelValuesAutofillSuggestions: (query, labelName) => __awaiter(this, void 0, void 0, function* () {
            return [
                { label: 'bar', value: 'bar' },
                { label: 'qux', value: 'qux' },
