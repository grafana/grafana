import { clone, cloneDeep, compact, each, every, filter, findIndex, has, includes, isArray, isEmpty, map as _map, toPairs, } from 'lodash';
...
export default class OpenTsDatasource extends DataSourceApi {
    ...
    suggestTagKeys(query) {
        var _a;
        const metric = (_a = query.metric) !== null && _a !== void 0 ? _a : '';
    ...
    _performSuggestQuery(query, type) {
        return this._get('/api/suggest', { type, q: query, max: this.lookupLimit }).pipe(map((result) => {
            return result.data;
        }));
    }
    _performMetricKeyValueLookup(metric, keys) {
        ...
        return this._get('/api/search/lookup', { m: m, limit: this.lookupLimit }).pipe(map((result) => {
    ...
    _performMetricKeyLookup(metric) {
        ...
        return this._get('/api/search/lookup', { m: metric, limit: 1000 }).pipe(map((result) => {
            result = result.data.results;
            const tagks = [];
            each(result, (r) => {
    ...
    interpolateVariablesInFilters(query, scopedVars) {
        var _a;
        query.filters = (_a = query.filters) === null || _a === void 0 ? void 0 : _a.map((filter) => {
