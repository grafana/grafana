...
describe('Azure Monitor Datasource', () => {
    describe('interpolateVariablesInQueries()', () => {
        it('should interpolate variables in the queries', () => {
            ...
            expect(interpolatedQueries).toContainEqual(expect.objectContaining({
    ...
    describe('When performing targetContainsTemplate', () => {
        it('should return false when no variable is being used', () => {
            const query = Object.assign(Object.assign({}, createMockQuery()), { queryType: AzureQueryType.AzureResourceGraph });
            const ds = new Datasource(createMockInstanceSetttings());
            expect(ds.targetContainsTemplate(query)).toEqual(false);
        });
        it('should return true when resource field is using a variable', () => {
            const query = Object.assign(Object.assign({}, createMockQuery()), { queryType: AzureQueryType.AzureResourceGraph, azureResourceGraph: { query: '$temp-var' } });
            const ds = new Datasource(createMockInstanceSetttings());
        ...
        it('should return true when resource field is using a variable in the subscriptions field', () => {
            const query = Object.assign(Object.assign({}, createMockQuery()), { queryType: AzureQueryType.AzureResourceGraph, subscriptions: ['$temp-var'] });
            const ds = new Datasource(createMockInstanceSetttings());
            expect(ds.targetContainsTemplate(query)).toEqual(true);
        });
        it('should return false when a variable is used in a different part of the query', () => {
            const query = Object.assign(Object.assign({}, createMockQuery()), { queryType: AzureQueryType.AzureResourceGraph, azureMonitor: { metricName: `$${singleVariable.name}` } });
            const ds = new Datasource(createMockInstanceSetttings());
            expect(ds.targetContainsTemplate(query)).toEqual(false);
        });
    });
