...
describe('AzureResourceGraphDatasource', () => {
    ...
    describe('When performing interpolateVariablesInQueries for azure_resource_graph', () => {
        ...
        it('should return a query with any template variables replaced', () => {
            ...
            query.queryType = AzureQueryType.AzureResourceGraph;
            query.azureResourceGraph = Object.assign(Object.assign({}, query.azureResourceGraph), azureResourceGraph);
            const templatedQuery = ctx.datasource.interpolateVariablesInQueries([query], {});
            expect(templatedQuery[0]).toHaveProperty('datasource');
            for (const [path, templateVariable] of templateVariables.entries()) {
                expect(get(templatedQuery[0].azureResourceGraph, path)).toEqual(templateVariable.templateVariable.current.value);
            }
        });
    });
    describe('When applying template variables', () => {
        ...
        it('should expand single value template variable', () => {
            ...
            replace = (target) => target === 'Resources | $var1' ? 'Resources | var1-foo' : target || '';
            expect(ctx.datasource.azureResourceGraphDatasource.applyTemplateVariables(target, {})).toEqual(expect.objectContaining(Object.assign(Object.assign({}, target), { azureResourceGraph: { query: 'Resources | var1-foo', resultFormat: 'table' }, queryType: 'Azure Resource Graph', subscriptions: [] })));
        });
        it('should expand multi value template variable', () => {
            ...
            expect(ctx.datasource.azureResourceGraphDatasource.applyTemplateVariables(target, {})).toEqual(expect.objectContaining(Object.assign(Object.assign({}, target), { azureResourceGraph: {
                    query: `resources | where $__contains(name, 'var3-foo','var3-baz')`,
                    resultFormat: 'table',
                }, queryType: 'Azure Resource Graph', subscriptions: [] })));
        });
    });
    it('should apply subscription variable', () => {
        ...
        getTempVars = () => Array.from([subscriptionsVariable, singleVariable, multiVariable].values()).map((item) => item);
        replace = (target) => (target === '$subs' ? 'sub-foo,sub-baz' : target || '');
        expect(ctx.datasource.azureResourceGraphDatasource.applyTemplateVariables(target, {})).toEqual(expect.objectContaining({
            azureResourceGraph: {
                query: `resources | where $__contains(name)`,
                resultFormat: 'table',
            },
