...
describe('ElasticsearchQueryContext', () => {
    it('Should call onChange and onRunQuery with the default query when the query is empty', () => {
        const datasource = { timeField: 'TIMEFIELD' };
        ...
        const changedQuery = onChange.mock.calls[0][0];
    ...
    describe('useQuery Hook', () => {
        ...
        it('Should return the current query object', () => {
            const wrapper = ({ children }) => (React.createElement(ElasticsearchProvider, { datasource: {}, query: query, onChange: () => { }, onRunQuery: () => { }, range: getDefaultTimeRange() }, children));
            const { result } = renderHook(() => useQuery(), {
                wrapper,
            });
