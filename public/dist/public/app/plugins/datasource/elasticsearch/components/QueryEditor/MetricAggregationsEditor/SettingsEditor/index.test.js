...
describe('Settings Editor', () => {
    describe('Raw Data', () => {
        it('Should correctly render the settings editor and trigger correct state changes', () => {
            ...
            const { rerender } = render(React.createElement(ElasticsearchProvider, { query: query, datasource: {}, onChange: onChange, onRunQuery: () => { }, range: getDefaultTimeRange() },
            ...
            rerender(React.createElement(ElasticsearchProvider, { query: onChange.mock.calls[0][0], datasource: {}, onChange: onChange, onRunQuery: () => { }, range: getDefaultTimeRange() },
    ...
    describe('Rate aggregation', () => {
        it('should render correct settings', () => {
            ...
            const onChange = jest.fn();
            render(React.createElement(ElasticsearchProvider, { query: query, datasource: {}, onChange: onChange, onRunQuery: () => { }, range: getDefaultTimeRange() },
                React.createElement(SettingsEditor, { metric: query.metrics[0], previousMetrics: [] })));
            let settingsButtonEl = screen.getByRole('button');
            fireEvent.click(settingsButtonEl);
