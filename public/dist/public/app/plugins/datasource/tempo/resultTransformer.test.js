import { FieldType, createDataFrame, PluginType, dateTime, } from '@grafana/data';
import { createTableFrame, transformToOTLP, transformFromOTLP, createTableFrameFromSearch, createTableFrameFromTraceQlQuery, } from './resultTransformer';
import { badOTLPResponse, otlpDataFrameToResponse, otlpDataFrameFromResponse, otlpResponse, tempoSearchResponse, traceQlResponse, } from './testResponse';
...
describe('transformFromOTLP()', () => {
    test('transforms OTLP format to dataFrame', () => {
        const res = transformFromOTLP(otlpResponse.batches, false);
        expect(res.data[0]).toMatchObject(Object.assign(Object.assign({}, otlpDataFrameFromResponse), { creator: expect.any(Function) }));
    });
});
describe('createTableFrameFromSearch()', () => {
    ...
    test('transforms search response to dataFrame', () => {
        const frame = createTableFrameFromSearch(tempoSearchResponse.traces, defaultSettings);
...
describe('transformFromOTLP()', () => {
    ...
    test('if passed bad data, will surface an error', () => {
        var _a;
        const res = transformFromOTLP(badOTLPResponse.batches, false);
        expect(res.data[0]).toBeFalsy();
        expect((_a = res.error) === null || _a === void 0 ? void 0 : _a.message).toBeTruthy();
        // if it does have resources, no error will be thrown
        expect(Object.assign(Object.assign({}, res.data[0]), { resources: {
                attributes: [
