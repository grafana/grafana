...
describe('transformTraceList()', function () {
    var lokiDataFrame = new MutableDataFrame({
        fields: [
            {
                name: 'ts',
                type: FieldType.time,
    ...
    test('extracts traceIDs from log lines', function () {
        var frame = createTableFrame(lokiDataFrame, 't1', 'tempo', ['traceID=(\\w+)', 'traceID=(\\w\\w)']);
...
describe('transformToOTLP()', function () {
    test('transforms dataframe to OTLP format', function () {
        var otlp = transformToOTLP(otlpDataFrame);
