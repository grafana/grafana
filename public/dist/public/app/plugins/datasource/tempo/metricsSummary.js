import { createDataFrame, FieldType, MutableDataFrame, sortDataFrame, } from '@grafana/data';
export function createTableFrameFromMetricsSummaryQuery(data, targetQuery, instanceSettings) {
    let frame;
    if (!data.length) {
...
export const transformToMetricsData = (data) => {
...
export const getConfigQuery = (series, targetQuery) => {
    const queryParts = series.map((x) => {
...
const getConfig = (series, query, instanceSettings) => {
    const commonConfig = {
    ...
    if (series.value.type === 7) {
        return Object.assign(Object.assign({}, commonConfig), { unit: 'ns' });
    }
    return Object.assign({}, commonConfig);
};
const NO_VALUE = '';
const getMetricValue = (series) => {
...
const getSpanStatusCode = (statusCode) => {
    if (!statusCode) {
        return NO_VALUE;
    }
    switch (statusCode) {
...
const getNumberForMetric = (metric) => {
