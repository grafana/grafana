...
describe('Graph Panel Migrations', function () {
    it('from 7.0', function () {
        var panel = {
            aliasColors: {},
            bars: false,
            dashLength: 10,
        ...
        var result = graphPanelMigrationHandler(panel);
        var fieldSource = panel.fieldConfig;
        expect(result.dataLinks).toBeUndefined();
        expect(fieldSource.defaults.links).toHaveLength(1);
        var link = fieldSource.defaults.links[0];
        expect(link.url).toEqual('THE DRILLDOWN URL');
    });
    it('from 7.1 it should preserve existing fieldConfig', function () {
        var panel = {
            id: 1,
            fieldConfig: {
                defaults: {
