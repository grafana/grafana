...
describe('grafanaGraph', () => {
    const setupCtx = (beforeRender) => {
        ...
        Object.assign(GraphCtrl.prototype, Object.assign(Object.assign(Object.assign(Object.assign({}, MetricsPanelCtrl.prototype), PanelCtrl.prototype), GraphCtrl.prototype), { height: 200, panel: {
                events: {
                    on: () => { },
                    emit: () => { },
                },
    ...
    describe('when graph is histogram, and xaxis min is set', () => {
        ...
        it('should not contain values lower than min', () => {
            ...
            expect(Math.max.apply(Math, nonZero.map((t) => t[0]))).toBe(280);
        });
    });
    ...
    describe('when graph is histogram, and xaxis min is null', () => {
        ...
        it('xaxis min should not affect the histogram', () => {
            const nonZero = ctx.plotData[0].data.filter((t) => t[1] > 0);
            expect(Math.min.apply(Math, nonZero.map((t) => t[0]))).toBe(-100);
            expect(Math.max.apply(Math, nonZero.map((t) => t[0]))).toBe(250);
        });
    });
    ...
    describe('when graph is histogram, and xaxis max is equal to the minimum value', () => {
        ...
        it('should calculate correct histogram', () => {
            const nonZero = ctx.plotData[0].data.filter((t) => t[1] > 0);
            expect(Math.min.apply(Math, nonZero.map((t) => t[0]))).toBe(90);
            expect(Math.max.apply(Math, nonZero.map((t) => t[0]))).toBe(90);
        });
    });
...
function getGraphElement({ canEdit, canMakeEditable } = {}) {
