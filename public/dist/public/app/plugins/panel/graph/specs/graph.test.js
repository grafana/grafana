...
describe('grafanaGraph', function () {
    var setupCtx = function (beforeRender) {
        ...
        GraphCtrl.prototype = __assign(__assign(__assign(__assign({}, MetricsPanelCtrl.prototype), PanelCtrl.prototype), GraphCtrl.prototype), { height: 200, panel: {
                events: {
                    on: function () { },
                    emit: function () { },
                },
    ...
    describe('when graph is histogram, and xaxis min is set', function () {
        ...
        it('should not contain values lower than min', function () {
            ...
            expect(Math.max.apply(Math, nonZero.map(function (t) { return t[0]; }))).toBe(280);
        });
    });
    ...
    describe('when graph is histogram, and xaxis min is null', function () {
        ...
        it('xaxis min should not affect the histogram', function () {
            var nonZero = ctx.plotData[0].data.filter(function (t) { return t[1] > 0; });
            expect(Math.min.apply(Math, nonZero.map(function (t) { return t[0]; }))).toBe(-100);
            expect(Math.max.apply(Math, nonZero.map(function (t) { return t[0]; }))).toBe(250);
        });
    });
    ...
    describe('when graph is histogram, and xaxis max is equal to the minimum value', function () {
        ...
        it('should calculate correct histogram', function () {
            var nonZero = ctx.plotData[0].data.filter(function (t) { return t[1] > 0; });
            expect(Math.min.apply(Math, nonZero.map(function (t) { return t[0]; }))).toBe(90);
            expect(Math.max.apply(Math, nonZero.map(function (t) { return t[0]; }))).toBe(90);
        });
    });
    ...
    describe('getContextMenuItemsSupplier', function () {
        describe('when called and user can edit the dashboard', function () {
            it('then the correct menu items should be returned', function () {
                var element = getGraphElement({ canEdit: true, canMakeEditable: false });
                var result = element.getContextMenuItemsSupplier({ x: 1, y: 1 })();
        ...
        describe('when called and user can make the dashboard editable', function () {
            it('then the correct menu items should be returned', function () {
                var element = getGraphElement({ canEdit: false, canMakeEditable: true });
                var result = element.getContextMenuItemsSupplier({ x: 1, y: 1 })();
                expect(result.length).toEqual(1);
                expect(result[0].items.length).toEqual(1);
                expect(result[0].items[0].label).toEqual('Add annotation');
        ...
        describe('when called and user can not edit the dashboard and can not make the dashboard editable', function () {
            it('then the correct menu items should be returned', function () {
                var element = getGraphElement({ canEdit: false, canMakeEditable: false });
                var result = element.getContextMenuItemsSupplier({ x: 1, y: 1 })();
...
function getGraphElement(_a) {
    var _b = _a === void 0 ? {} : _a, canEdit = _b.canEdit, canMakeEditable = _b.canMakeEditable;
    ...
    }, { mouseleave: jest.fn(), bind: jest.fn() }, {});
    return element;
}
