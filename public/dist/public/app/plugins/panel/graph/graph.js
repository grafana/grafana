...
class GraphElement {
    constructor(scope, elem, timeSrv) {
        ...
        this.data = [];
        this.getContextMenuItemsSupplier = (flotPosition, linksSupplier) => {
            return () => {
    ...
    onGraphHover(evt) {
        ...
        if (isLegacyGraphHoverEvent(evt)) {
            ...
            if (!this.plot || ((_a = evt.panel) === null || _a === void 0 ? void 0 : _a.id) === this.panel.id || this.ctrl.otherPanelInFullscreenMode()) {
                return;
            }
            this.tooltip.show(evt.pos);
        }
        // DataHoverEvent can come from multiple panels that doesn't include x position
        if (!((_b = evt.point) === null || _b === void 0 ? void 0 : _b.time)) {
            return;
        }
        this.tooltip.show({ x: evt.point.time, panelRelY: (_c = evt.point.panelRelY) !== null && _c !== void 0 ? _c : 1 });
    }
    onPanelTeardown() {
        if (this.plot) {
            this.plot.destroy();
