...
function getRate(yLeft, yRight) {
    if (checkTwoCross(yLeft, yRight)) {
        var rateLeft = yRight.min ? yLeft.min / yRight.min : 0;
        var rateRight = yRight.max ? yLeft.max / yRight.max : 0;
    ...
    if (checkOneSide(yLeft, yRight)) {
        ...
        var upLeft = Math.max(absLeftMin, absLeftMax);
        var downLeft = Math.min(absLeftMin, absLeftMax);
        var upRight = Math.max(absRightMin, absRightMax);
        var downRight = Math.min(absRightMin, absRightMax);
        var rateLeft = downLeft !== 0 ? upLeft / downLeft : upLeft;
        var rateRight = downRight !== 0 ? upRight / downRight : upRight;
        return rateLeft > rateRight ? rateLeft : rateRight;
    }
