...
import { LegendDisplayMode, TooltipDisplayMode, VisibilityMode, GraphGradientMode, StackingMode, SortOrder, } from '@grafana/schema';
...
function mockDataFrame() {
    ...
    const info = prepareBarChartDisplayValues([df1], createTheme(), {});
    if (!('aligned' in info)) {
        throw new Error('Bar chart not prepared correctly');
    }
    return info.aligned;
}
jest.mock('@grafana/data', () => (Object.assign(Object.assign({}, jest.requireActual('@grafana/data')), { DefaultTimeZone: 'utc' })));
describe('BarChart utils', () => {
    describe('preparePlotConfigBuilder', () => {
        ...
        const config = {
            ...
            fullHighlight: false,
            rawValue: (seriesIdx, valueIdx) => frame.fields[seriesIdx].values[valueIdx],
        };
        it.each([VizOrientation.Auto, VizOrientation.Horizontal, VizOrientation.Vertical])('orientation', (v) => {
            const result = preparePlotConfigBuilder(Object.assign(Object.assign({}, config), { orientation: v, frame: frame, theme: createTheme(), timeZones: [DefaultTimeZone], getTimeRange: getDefaultTimeRange, eventBus: new EventBusSrv(), allFrames: [frame] })).getConfig();
            expect(result).toMatchSnapshot();
        });
        it.each([VisibilityMode.Always, VisibilityMode.Auto])('value visibility', (v) => {
            expect(preparePlotConfigBuilder(Object.assign(Object.assign({}, config), { showValue: v, frame: frame, theme: createTheme(), timeZones: [DefaultTimeZone], getTimeRange: getDefaultTimeRange, eventBus: new EventBusSrv(), allFrames: [frame] })).getConfig()).toMatchSnapshot();
        });
        it.each([StackingMode.None, StackingMode.Percent, StackingMode.Normal])('stacking', (v) => {
            expect(preparePlotConfigBuilder(Object.assign(Object.assign({}, config), { stacking: v, frame: frame, theme: createTheme(), timeZones: [DefaultTimeZone], getTimeRange: getDefaultTimeRange, eventBus: new EventBusSrv(), allFrames: [frame] })).getConfig()).toMatchSnapshot();
        });
    });
    describe('prepareGraphableFrames', () => {
        it('will warn when there is no data in the response', () => {
            const result = prepareBarChartDisplayValues([], createTheme(), { stacking: StackingMode.None });
        ...
        it('will warn when there is no string or time field', () => {
            ...
            const result = prepareBarChartDisplayValues([df], createTheme(), { stacking: StackingMode.None });
            const warning = assertIsDefined('warn' in result ? result : null);
