...
export function useLayout(rawNodes, rawEdges, config = defaultConfig, nodeCountLimit, width, rootNodeId) {
    ...
    const { nodes: nodesWithLimit, edges: edgesWithLimit, markers, } = useNodeLimit(config.gridLayout ? nodesGrid : nodesGraph, config.gridLayout ? edgesGrid : edgesGraph, nodeCountLimit, config, rootNodeId);
...
function defaultLayout(nodes, edges, done) {
    ...
    worker.onmessage = (event) => {
        for (let i = 0; i < nodes.length; i++) {
            ...
            event.data.nodes[i] = Object.assign(Object.assign({}, nodes[i]), event.data.nodes[i]);
        }
        done(event.data);
    };
    worker.postMessage({
