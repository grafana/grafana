...
var transformers = {};
export var timeSeriesFormatFilterer = function (data) {
...
export var tableDataFormatFilterer = function (data) {
    if (!Array.isArray(data)) {
...
transformers['timeseries_to_rows'] = {
    ...
    transform: function (data, panel, model) {
        ...
        var filteredData = timeSeriesFormatFilterer(data);
        for (var i = 0; i < filteredData.length; i++) {
            var series = filteredData[i];
...
transformers['table'] = {
    ...
    getColumns: function (data) {
        ...
        if (data.length === 1) {
            return __spreadArray([], __read(data[0].columns), false);
        }
        var filteredData = tableDataFormatFilterer(data);
        // Track column indexes: name -> index
        var columnNames = {};
        // Union of all columns
        var columns = filteredData.reduce(function (acc, series) {
    ...
    transform: function (data, panel, model) {
        ...
        var filteredData = tableDataFormatFilterer(data);
        var noTableIndex = findIndex(filteredData, function (d) { return 'columns' in d && 'rows' in d; });
        if (noTableIndex < 0) {
            throw {
                message: "Result of query #" + String.fromCharCode(65 + noTableIndex) + " is not in table format, try using another transform.",
            };
        }
        mergeTablesIntoModel.apply(void 0, __spreadArray([model], __read(filteredData), false));
    },
};
transformers['json'] = {
