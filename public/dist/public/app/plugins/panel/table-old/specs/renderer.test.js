    ...
    replace: (value, scopedVars) => {
        if (scopedVars) {
            ...
            each(scopedVars, (val, key) => {
                value = value.replace('$' + key, val === null || val === void 0 ? void 0 : val.value);
            });
        }
        return value;
    },
    ...
    describe('given 13 columns', () => {
      ...
      `;
        ...
        it('render_values should ignore hidden columns', () => {
            ...
            expect(columns.filter((col) => col.hidden)).toHaveLength(0);
        });
    });
});
describe('when rendering table with different patterns', () => {
  ...
  `('number column should be formatted for a column:$column with the pattern:$pattern', ({ column, pattern, expected }) => {
        const table = new TableModel();
...
describe('when rendering cells with different alignment options', () => {
    ...
    it.each(cases)('align option:"%s", preformatted:%s columns should be formatted with correct style', (align, preserveFormat, colorMode, expected) => {
        const table = new TableModel();
