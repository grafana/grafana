...
import { dateTimeFormat, formattedValueToString, getColorForTheme, getValueFormat, LegacyGraphHoverClearEvent, LegacyGraphHoverEvent, PanelEvents, toUtc, } from '@grafana/data';
import { graphTimeFormat } from '@grafana/ui';
import { config } from 'app/core/config';
var MIN_CARD_SIZE = 1, CARD_PADDING = 1, CARD_ROUND = 0, DATA_RANGE_WIDING_FACTOR = 1.2, DEFAULT_X_TICK_SIZE_PX = 100, DEFAULT_Y_TICK_SIZE_PX = 22.5, X_AXIS_TICK_PADDING = 10, Y_AXIS_TICK_PADDING = 5, MIN_SELECTION_WIDTH = 2;
...
var HeatmapRenderer = /** @class */ (function () {
    ...
    HeatmapRenderer.prototype.addXAxis = function () {
        ...
        var format = graphTimeFormat(ticks, this.timeRange.from.valueOf(), this.timeRange.to.valueOf());
        var timeZone = this.ctrl.dashboard.getTimezone();
    ...
    HeatmapRenderer.prototype.addYAxisFromBuckets = function () {
        ...
        this.scope.yScale = this.yScale = d3
            ...
            .range([this.chartHeight, 0]);
        var tickValues = map(tsBuckets, function (b, i) { return i; });
        var decimalsAuto = max(map(tsBuckets, ticksUtils.getStringPrecision));
        var decimals = this.panel.yAxis.decimals === null ? decimalsAuto : this.panel.yAxis.decimals;
        this.ctrl.decimals = decimals;
        var tickValueFormatter = this.tickValueFormatter.bind(this);
        function tickFormatter(yAxisWidth) {
            return function (valIndex) {
                var valueFormatted = tsBuckets[valIndex.valueOf()];
                if (!isNaN(toNumber(valueFormatted)) && valueFormatted !== '') {
                    ...
                    if (yAxisWidth) {
                        var scale = 0.15; // how to have a better calculation for this
                        var trimmed = valueFormatted.substring(0, Math.floor(yAxisWidth * scale));
                        var postfix = trimmed.length < valueFormatted.length ? '...' : '';
        ...
        var tsBucketsFormatted = map(tsBuckets, function (v, i) { return tickFormatter(null)(i); });
    ...
    HeatmapRenderer.prototype.tickValueFormatter = function (decimals, scaledDecimals) {
        ...
        return function (value) {
            try {
                if (format !== 'none') {
                    var v = getValueFormat(format)(value, decimals, scaledDecimals);
    ...
    HeatmapRenderer.prototype.addHeatmap = function () {
        ...
        var minValueAuto = Math.max(cardStats.min, 0);
        var maxValue = isNil(this.panel.color.max) ? maxValueAuto : this.panel.color.max;
        var minValue = isNil(this.panel.color.min) ? minValueAuto : this.panel.color.min;
        var colorScheme = find(this.ctrl.colorSchemes, {
    ...
    HeatmapRenderer.prototype.getPanelYAxisWidth = function () {
