    ...
    return {
        ...
        link: function (scope, elem, attrs) {
            ...
            function render() {
                ...
                var legendWidth = Math.floor((_a = legendElem.outerWidth()) !== null && _a !== void 0 ? _a : 10);
                if (panel.color.mode === 'spectrum') {
                    var colorScheme = find(ctrl.colorSchemes, {
        ...
        link: function (scope, elem, attrs) {
            ...
            function render() {
                ...
                if (!isEmpty(ctrl.data) && !isEmpty(ctrl.data.cards)) {
                    ...
                    var rangeFrom = isNil(panel.color.min) ? Math.max(cardStats.min, 0) : panel.color.min;
                    var rangeTo = isNil(panel.color.max) ? cardStats.max : panel.color.max;
...
function drawColorLegend(elem, colorScheme, rangeFrom, rangeTo, maxValue, minValue) {
    ...
    clearLegend(elem);
    var legendWidth = Math.floor((_a = legendElem.outerWidth()) !== null && _a !== void 0 ? _a : 10) - 30;
    ...
    var colorScale = getColorScale(colorScheme, contextSrv.user.lightTheme, rangeTo, rangeFrom);
    legend
        .append('g')
...
function drawOpacityLegend(elem, options, rangeFrom, rangeTo, maxValue, minValue) {
    ...
    var legend = d3.select(legendElem.get(0));
    clearLegend(elem);
    var legendWidth = Math.floor((_a = legendElem.outerWidth()) !== null && _a !== void 0 ? _a : 30) - 30;
    var legendHeight = legendElem.attr('height');
    var rangeStep = ((rangeTo - rangeFrom) / legendWidth) * LEGEND_SEGMENT_WIDTH;
    var widthFactor = legendWidth / (rangeTo - rangeFrom);
    var valuesRange = d3.range(rangeFrom, rangeTo, rangeStep);
    var opacityScale = getOpacityScale(options, rangeTo, rangeFrom);
    legend
        .append('g')
        .attr('class', 'legend-color-bar')
        .attr('transform', 'translate(' + LEGEND_PADDING_LEFT + ',0)')
...
function drawLegendValues(elem, rangeFrom, rangeTo, maxValue, minValue, legendWidth, valuesRange) {
    ...
    var xAxis = d3.axisBottom(legendValueScale).tickValues(ticks).tickSize(LEGEND_TICK_SIZE);
...
function drawSimpleColorLegend(elem, colorScale) {
    ...
    var legendElem = $(elem).find('svg');
    clearLegend(elem);
    var legendWidth = Math.floor((_a = legendElem.outerWidth()) !== null && _a !== void 0 ? _a : 30);
    var legendHeight = legendElem.attr('height');
    if (legendWidth) {
...
function drawSimpleOpacityLegend(elem, options) {
    ...
    if (legendWidth) {
        ...
        if (options.colorScale === 'linear') {
            legendOpacityScale_1 = d3.scaleLinear().domain([0, legendWidth]).range([0, 1]);
        }
        else if (options.colorScale === 'sqrt') {
            legendOpacityScale_1 = d3.scalePow().exponent(options.exponent).domain([0, legendWidth]).range([0, 1]);
        }
        var rangeStep = 10;
        var valuesRange = d3.range(0, legendWidth, rangeStep);
