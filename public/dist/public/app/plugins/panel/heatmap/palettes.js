...
import { defaultOptions, HeatmapColorMode, HeatmapColorScale } from './types';
...
export function quantizeScheme(opts, theme) {
    var _a, _b, _c;
    const options = Object.assign(Object.assign({}, defaultOptions.color), opts);
    const palette = [];
    const steps = ((_a = options.steps) !== null && _a !== void 0 ? _a : 128) - 1;
    if (opts.mode === HeatmapColorMode.Opacity) {
        const fill = tinycolor(theme.visualization.getColorByName(opts.fill)).toPercentageRgb();
        const scale = options.scale === HeatmapColorScale.Exponential
        ...
        const scheme = (_b = colorSchemes.find((scheme) => scheme.name === options.scheme)) !== null && _b !== void 0 ? _b : DEFAULT_SCHEME;
        let fnName = 'interpolate' + ((_c = scheme.name2) !== null && _c !== void 0 ? _c : scheme.name);
        const interpolate = d3ScaleChromatic[fnName];
        for (let i = 0; i <= steps; i++) {
            let rgbStr = interpolate(i / steps);
            let rgb = rgbStr.indexOf('rgb') === 0
                ? '#' + [...rgbStr.matchAll(/\d+/g)].map((v) => (+v[0]).toString(16).padStart(2, '0')).join('')
                : rgbStr;
            palette.push(rgb);
        }
        if (scheme.invert === 'always' ||
