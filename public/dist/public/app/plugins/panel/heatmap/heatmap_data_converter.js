...
function histogramToHeatmap(seriesList) {
    ...
    for (var i = 0; i < seriesList.length; i++) {
        ...
        try {
            for (var _b = (e_1 = void 0, __values(series.datapoints)), _c = _b.next(); !_c.done; _c = _b.next()) {
                var point = _c.value;
...
function convertToCards(buckets, hideZero) {
    ...
    var min = 0, max = 0;
    var cards = [];
    forEach(buckets, function (xBucket) {
        forEach(xBucket.buckets, function (yBucket) {
            var card = {
                x: xBucket.x,
                y: yBucket.y,
...
function convertToHeatMap(seriesList, yBucketSize, xBucketSize, logBase) {
    ...
    var _loop_1 = function (series) {
        ...
        //
        forEach(datapoints, function (point) {
            var bucketBound = getBucketBound(point[TIME_INDEX], xBucketSize);
            pushToXBuckets(heatmap, point, bucketBound, seriesName);
        });
    };
    try {
        for (var seriesList_1 = __values(seriesList), seriesList_1_1 = seriesList_1.next(); !seriesList_1_1.done; seriesList_1_1 = seriesList_1.next()) {
    ...
    // |____|     |0|
    //
    forEach(heatmap, function (xBucket) {
        if (logBase !== 1) {
...
function pushToXBuckets(buckets, point, bucketNum, seriesName) {
    ...
    var pointExt = concat(point, seriesName);
...
function pushToYBuckets(buckets, bucketNum, value, point, bounds) {
    ...
    if (buckets[bucketNum]) {
        buckets[bucketNum].values.push(value);
        (_a = buckets[bucketNum].points) === null || _a === void 0 ? void 0 : _a.push(point);
...
function convertToValueBuckets(xBucket, bucketSize) {
    ...
    var buckets = {};
    forEach(values, function (val, index) {
...
function isHeatmapDataEqual(objA, objB) {
    ...
    forEach(objA, function (xBucket, x) {
        if (objB[x]) {
            if (emptyXOR(xBucket.buckets, objB[x].buckets)) {
                isEql = false;
                return false;
            }
            forEach(xBucket.buckets, function (yBucket, y) {
