...
import { convertToHeatMap, convertToCards, histogramToHeatmap, calculateBucketSize, isHeatmapDataEqual, } from '../heatmap_data_converter';
describe('isHeatmapDataEqual', function () {
    var ctx = {};
    beforeEach(function () {
        ctx.heatmapA = {
            '1422774000000': {
    ...
    it('should proper compare objects', function () {
        var heatmapC = cloneDeep(ctx.heatmapA);
        heatmapC['1422774000000'].buckets['1'].values = [1, 1.5];
        var heatmapD = cloneDeep(ctx.heatmapA);
        heatmapD['1422774000000'].buckets['1'].values = [1.5, 1, 1.6];
        var heatmapE = cloneDeep(ctx.heatmapA);
        heatmapE['1422774000000'].buckets['1'].values = [1, 1.6];
        var empty = {};
        var emptyValues = cloneDeep(ctx.heatmapA);
...
describe('calculateBucketSize', function () {
    var ctx = {};
    describe('when logBase is 1 (linear scale)', function () {
        beforeEach(function () {
            ctx.logBase = 1;
            ctx.bounds_set = [
                { bounds: [], size: 0 },
        ...
        it('should properly calculate bucket size', function () {
            each(ctx.bounds_set, function (b) {
                var bucketSize = calculateBucketSize(b.bounds, ctx.logBase);
                expect(bucketSize).toBe(b.size);
            });
        });
    });
    describe('when logBase is 2', function () {
        beforeEach(function () {
            ctx.logBase = 2;
            ctx.bounds_set = [
                { bounds: [], size: 0 },
...
describe('HeatmapDataConverter', function () {
    var ctx = {};
    beforeEach(function () {
        ctx.series = [];
    ...
    describe('when logBase is 1 (linear scale)', function () {
        ...
        it('should build proper heatmap data', function () {
            var expectedHeatmap = {
                '1422774000000': {
                    x: 1422774000000,
                    buckets: {
            ...
            var heatmap = convertToHeatMap(ctx.series, ctx.yBucketSize, ctx.xBucketSize, ctx.logBase);
            expect(isHeatmapDataEqual(heatmap, expectedHeatmap)).toBe(true);
        });
    });
    describe.skip('when logBase is 2', function () {
        beforeEach(function () {
            ctx.logBase = 2;
        });
        it('should build proper heatmap data', function () {
...
describe('Histogram converter', function () {
    var ctx = {};
    beforeEach(function () {
        ctx.series = [];
        ctx.series.push(new TimeSeries({
    ...
    describe('when converting histogram', function () {
        beforeEach(function () { });
        it('should build proper heatmap data', function () {
            var expectedHeatmap = {
                '1422774000000': {
            ...
            var heatmap = histogramToHeatmap(ctx.series);
            expect(heatmap).toEqual(expectedHeatmap);
        });
        it('should use bucket index as a bound', function () {
            var heatmap = histogramToHeatmap(ctx.series);
            var bucketLabels = map(heatmap['1422774000000'].buckets, function (b, label) { return label; });
            var bucketYs = map(heatmap['1422774000000'].buckets, 'y');
            var bucketBottoms = map(heatmap['1422774000000'].buckets, function (b) { return b.bounds.bottom; });
            var expectedBounds = [0, 1, 2];
            expect(bucketLabels).toEqual(map(expectedBounds, function (b) { return b.toString(); }));
            expect(bucketYs).toEqual(expectedBounds);
            expect(bucketBottoms).toEqual(expectedBounds);
        });
    });
});
describe('convertToCards', function () {
    var buckets = {};
    beforeEach(function () {
        buckets = {
            '1422774000000': {
    ...
    it('should build proper cards data', function () {
        var expectedCards = [
        ...
        var res = convertToCards(buckets);
        expect(res.cards).toMatchObject(expectedCards);
    });
    it('should build proper cards stats', function () {
        var expectedStats = { min: 1, max: 2 };
        var res = convertToCards(buckets);
...
//# sourceMappingURL=heatmap_data_converter.test.js.map