...
describe('filterAlerts', () => {
    ...
    it('Filters by alert instance state ', () => {
        const noNormalStateOptions = Object.assign(Object.assign({}, defaultOption), { stateFilter: { firing: true, pending: true, noData: true, normal: false, error: true } });
        expect(filterAlerts(noNormalStateOptions, alerts).length).toBe(3);
        const noErrorOrNormalStateOptions = Object.assign(Object.assign({}, defaultOption), { stateFilter: { firing: true, pending: true, noData: true, normal: false, error: false } });
        expect(filterAlerts(noErrorOrNormalStateOptions, alerts).length).toBe(1);
    });
    it('Filters by alert instance label', () => {
        const options = Object.assign(Object.assign({}, defaultOption), { alertInstanceLabelFilter: '{severity=low}' });
        const result = filterAlerts(options, alerts);
        expect(result.length).toBe(1);
    });
    it('Filters by alert instance state and label', () => {
        const options = Object.assign(Object.assign(Object.assign({}, defaultOption), { stateFilter: { firing: false, pending: false, noData: false, normal: false, error: true } }), { alertInstanceLabelFilter: '{severity=low}' });
        const result = filterAlerts(options, alerts);
        expect(result.length).toBe(1);
    });
});
