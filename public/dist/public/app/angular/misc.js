...
function tip($compile) {
    return {
        ...
        link: (scope, elem, attrs) => {
            let _t = '<i class="grafana-tip fa fa-' +
                ...
                elem.text().replace(/[\'\"\\{}<>&]/g, (m) => '&amp;#' + m.charCodeAt(0) + ';') +
                '\'"></i>';
...
function editorOptBool($compile) {
    return {
        ...
        link: (scope, elem, attrs) => {
            ...
            const template = '<div class="editor-option gf-form-checkbox text-center"' +
                showIf +
                '>' +
                ' <label for="' +
                attrs.model +
...
function editorCheckbox($compile, $interpolate) {
    return {
        ...
        link: (scope, elem, attrs) => {
            ...
            let template = '<input class="cr1" id="' +
                scope.$id +
                model +
                '" type="checkbox" ' +
                '       ng-model="' +
...
function gfDropdown($parse, $compile, $timeout) {
    ...
    return {
        ...
        link: function postLink(scope, iElement, iAttrs) {
            const getter = $parse(iAttrs.gfDropdown), items = getter(scope);
            $timeout(() => {
