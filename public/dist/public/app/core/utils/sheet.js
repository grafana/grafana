import { read, utils } from 'xlsx';
import { FieldType } from '@grafana/data';
export function readSpreadsheet(file) {
    return workBookToFrames(read(file, { type: 'buffer' }));
}
export function workBookToFrames(wb) {
    return wb.SheetNames.map((name) => workSheetToFrame(wb.Sheets[name], name));
}
export function workSheetToFrame(sheet, name) {
    const columns = sheetAsColumns(sheet);
    if (!(columns === null || columns === void 0 ? void 0 : columns.length)) {
        return {
            fields: [],
            name: name,
            length: 0,
        };
    }
    return {
        fields: columns.map((c, idx) => {
            var _a;
            let type = FieldType.string;
            let values = [];
            switch ((_a = c.type) !== null && _a !== void 0 ? _a : 's') {
                case 'b':
                    type = FieldType.boolean;
                    values = c.data.map((v) => ((v === null || v === void 0 ? void 0 : v.v) == null ? v === null || v === void 0 ? void 0 : v.v : Boolean(v.v)));
                    break;
                case 'n':
                    type = FieldType.number;
                    values = c.data.map((v) => ((v === null || v === void 0 ? void 0 : v.v) == null ? v === null || v === void 0 ? void 0 : v.v : +v.v));
                    break;
                case 'd':
                    type = FieldType.time;
                    values = c.data.map((v) => ((v === null || v === void 0 ? void 0 : v.v) == null ? v === null || v === void 0 ? void 0 : v.v : +v.v)); // ???
                    break;
                default:
                    type = FieldType.string;
                    values = c.data.map((v) => ((v === null || v === void 0 ? void 0 : v.v) == null ? v === null || v === void 0 ? void 0 : v.v : utils.format_cell(v)));
                    break;
            }
            return {
                name: c.name,
                config: {},
                type,
                values,
            };
        }),
        name: name,
        length: columns[0].data.length,
    };
}
function sheetAsColumns(sheet) {
    var _a;
    const r = sheet['!ref'];
    if (!r) {
        return null;
    }
    const columnInfo = sheet['!cols'];
    const cols = [];
    const range = safe_decode_range(r);
    const types = new Set();
    let firstRowIsHeader = true;
    for (let c = range.s.c; c <= range.e.c; ++c) {
        types.clear();
        const info = (_a = columnInfo === null || columnInfo === void 0 ? void 0 : columnInfo[c]) !== null && _a !== void 0 ? _a : {};
        if (info.hidden) {
            continue; // skip the column
        }
        const field = {
            index: c,
            name: utils.encode_col(c),
            data: [],
            info,
        };
        const pfix = utils.encode_col(c);
        for (let r = range.s.r; r <= range.e.r; ++r) {
            const cell = sheet[pfix + utils.encode_row(r)];
            if (cell) {
                if (field.data.length) {
                    types.add(cell.t);
                }
                else if (cell.t !== 's') {
                    firstRowIsHeader = false;
                }
            }
            field.data.push(cell);
        }
        cols.push(field);
        if (types.size === 1) {
            field.type = Array.from(types)[0];
        }
    }
    if (firstRowIsHeader) {
        return cols.map((c) => {
            const first = c.data[0];
            if (first === null || first === void 0 ? void 0 : first.v) {
                c.name = utils.format_cell(first);
            }
            c.data = c.data.slice(1);
            return c;
        });
    }
    return cols;
}
/**
 * Copied from Apache 2 licensed sheetjs:
 * https://git.sheetjs.com/sheetjs/sheetjs/src/branch/master/xlsx.flow.js#L4338
 */
function safe_decode_range(range) {
    let o = { s: { c: 0, r: 0 }, e: { c: 0, r: 0 } };
    let idx = 0, i = 0, cc = 0;
    let len = range.length;
    for (idx = 0; i < len; ++i) {
        if ((cc = range.charCodeAt(i) - 64) < 1 || cc > 26) {
            break;
        }
        idx = 26 * idx + cc;
    }
    o.s.c = --idx;
    for (idx = 0; i < len; ++i) {
        if ((cc = range.charCodeAt(i) - 48) < 0 || cc > 9) {
            break;
        }
        idx = 10 * idx + cc;
    }
    o.s.r = --idx;
    if (i === len || cc !== 10) {
        o.e.c = o.s.c;
        o.e.r = o.s.r;
        return o;
    }
    ++i;
    for (idx = 0; i !== len; ++i) {
        if ((cc = range.charCodeAt(i) - 64) < 1 || cc > 26) {
            break;
        }
        idx = 26 * idx + cc;
    }
    o.e.c = --idx;
    for (idx = 0; i !== len; ++i) {
        if ((cc = range.charCodeAt(i) - 48) < 0 || cc > 9) {
            break;
        }
        idx = 10 * idx + cc;
    }
    o.e.r = --idx;
    return o;
}
//# sourceMappingURL=sheet.js.map