...
export class FetchQueueWorker {
    constructor(fetchQueue, responseQueue, config) {
        const maxParallelRequests = (config === null || config === void 0 ? void 0 : config.http2Enabled) ? 1000 : 5; // for tests that don't mock GrafanaBootConfig the config param will be undefined
        ...
        fetchQueue
            .getUpdates()
            .pipe(filter(({ noOfPending }) => noOfPending > 0), // no reason to act if there is nothing to act upon
