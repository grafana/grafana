...
var TableModel = /** @class */ (function () {
    function TableModel(table) {
        ...
        if (table) {
            ...
            if (table.rows) {
                try {
                    for (var _e = __values(table.rows), _f = _e.next(); !_f.done; _f = _e.next()) {
                        var row = _f.value;
                        this.addRow(row);
                    }
                }
...
export function mergeTablesIntoModel(dst) {
    ...
    if (arguments.length === 2) {
        model.columns = tables[0].hasOwnProperty('columns') ? __spreadArray([], __read(tables[0].columns), false) : [];
        model.rows = tables[0].hasOwnProperty('rows') ? __spreadArray([], __read(tables[0].rows), false) : [];
        return model;
    }
    // Filter out any tables that are not of TableData format
    var tableDataTables = tables.filter(function (table) { return !!table.columns; });
    // Track column indexes of union: name -> index
    var columnNames = {};
    // Union of all non-value columns
    var columnsUnion = tableDataTables.slice().reduce(function (acc, series) {
    ...
    var columnIndexMapper = tableDataTables.map(function (series) { return series.columns.map(function (col) { return columnNames[col.text]; }); });
    // Flatten rows of all series and adjust new column indexes
    var flattenedRows = tableDataTables.reduce(function (acc, series, seriesIndex) {
    ...
    var compactedRows = flattenedRows.reduce(function (acc, row, rowIndex) {
        if (!mergedRows[rowIndex]) {
            ...
            while (offset < flattenedRows.length) {
                ...
                var match = findIndex(flattenedRows, function (otherRow) { return areRowsMatching(columnsUnion, row, otherRow); }, offset);
                if (match > -1) {
