...
var FormDropdownCtrl = /** @class */ (function () {
    /** @ngInject */
    function FormDropdownCtrl($scope, $element, $sce, templateSrv) {
        this.$scope = $scope;
        this.$sce = $sce;
    ...
    FormDropdownCtrl.prototype.$onInit = function () {
        var _this = this;
        if (this.labelMode) {
        ...
        if (isObject(this.model)) {
            ...
            if (this.lookupText) {
                this.getOptionsInternal('').then(function (options) {
                    var item = find(options, { value: _this.model });
    ...
    FormDropdownCtrl.prototype.typeaheadSource = function (query, callback) {
        ...
        this.getOptionsInternal(query).then(function (options) {
            ...
            // extract texts
            var optionTexts = map(options, function (op) {
                return escape(op.text);
            });
            // add custom values
            if (_this.allowCustom && _this.text !== '') {
                if (indexOf(optionTexts, _this.text) === -1) {
    ...
    FormDropdownCtrl.prototype.updateValue = function (text) {
        ...
        this.$scope.$apply(function () {
            var option = find(_this.optionCache, { text: text });
            if (option) {
                if (isObject(_this.model)) {
                    _this.model = option;
                }
                else {
