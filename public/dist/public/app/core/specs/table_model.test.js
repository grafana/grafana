...
describe('mergeTables', function () {
    ...
    var multipleTablesButWithMixedDataFormat = [
        new TableModel({
            type: 'table',
            columns: [{ text: 'Time' }, { text: 'Label Key 1' }, { text: 'Value' }],
            rows: [[time, 'Label Value 1', 42]],
    ...
    it('should return the union of columns for multiple tables', function () {
        var table = mergeTablesIntoModel.apply(void 0, __spreadArray([new TableModel()], __read(multipleTablesSameColumns), false));
    ...
    it('should return 2 rows for multiple tables with different column values', function () {
        var table = mergeTablesIntoModel.apply(void 0, __spreadArray([new TableModel()], __read(multipleTablesDifferentColumns), false));
        expect(table.rows.length).toBe(2);
        expect(table.columns.length).toBe(6);
        expect(table.rows[0][0]).toBe(time);
        expect(table.rows[0][1]).toBe('Label Value 1');
    ...
    it('should not crash if tables array contain non table data', function () {
        expect(function () { return mergeTablesIntoModel.apply(void 0, __spreadArray([new TableModel()], __read(multipleTablesButWithMixedDataFormat), false)); }).not.toThrow();
    });
    it('should should return the single table as is if tables array also contains non table data', function () {
        var table = mergeTablesIntoModel.apply(void 0, __spreadArray([new TableModel()], __read(multipleTablesButWithMixedDataFormat), false));
        expect(table.columns.length).toBe(3);
        expect(table.columns[0].text).toBe('Time');
        expect(table.columns[1].text).toBe('Label Key 1');
        expect(table.columns[2].text).toBe('Value');
    });
    it('should return 1 row for a single table if tables array also contains non table data', function () {
        var table = mergeTablesIntoModel.apply(void 0, __spreadArray([new TableModel()], __read(multipleTablesButWithMixedDataFormat), false));
        expect(table.rows.length).toBe(1);
        expect(table.rows[0][0]).toBe(time);
        expect(table.rows[0][1]).toBe('Label Value 1');
