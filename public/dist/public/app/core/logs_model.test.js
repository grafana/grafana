import { ArrayVector, FieldType, LogLevel, LogsDedupStrategy, LogsMetaKind, MutableDataFrame, toDataFrame, } from '@grafana/data';
import { dataFrameToLogsModel, dedupLogRows, getSeriesProperties, logSeriesToLogsModel, filterLogLevels, LIMIT_LABEL, COMMON_LABELS, } from './logs_model';
...
describe('filterLogLevels()', function () {
    test('should correctly filter out log levels', function () {
        var rows = [
            {
                entry: 'DEBUG 1',
        ...
        var filteredLogs = filterLogLevels(rows, new Set([LogLevel.debug]));
    ...
    test('should correctly filter out log levels and then deduplicate', function () {
        var rows = [
            {
                entry: 'DEBUG 1',
                logLevel: LogLevel.debug,
        ...
        var filteredLogs = filterLogLevels(rows, new Set([LogLevel.error]));
        var deduplicatedLogs = dedupLogRows(filteredLogs, LogsDedupStrategy.exact);
...
describe('dataFrameToLogsModel', function () {
    it('given empty series should return empty logs model', function () {
        expect(dataFrameToLogsModel([], 0)).toMatchObject(emptyLogsModel);
    });
    it('given series without correct series name should return empty logs model', function () {
        var series = [
            toDataFrame({
                fields: [],
            }),
    ...
    it('given series without a time field should return empty logs model', function () {
        var series = [
    ...
    it('given series without a string field should return empty logs model', function () {
        var series = [
            new MutableDataFrame({
                fields: [
                    {
    ...
    it('given one series should return expected logs model', function () {
        var series = [
            new MutableDataFrame({
                fields: [
                    {
        ...
        var logsModel = dataFrameToLogsModel(series, 1);
    ...
    it('given one series with error should return expected logs model', function () {
        var series = [
            new MutableDataFrame({
                fields: [
                    {
    ...
    it('given one series without labels should return expected logs model', function () {
        var series = [
            new MutableDataFrame({
                fields: [
                    {
    ...
    it('given multiple series with unique times should return expected logs model', function () {
        var series = [
            toDataFrame({
                fields: [
                    {
    ...
    it('given multiple series with equal times should return expected logs model', function () {
        var series = [
            toDataFrame({
                fields: [
                    {
    ...
    it('should return expected line limit meta info when returned number of series equal the log limit', function () {
        var series = [
            new MutableDataFrame({
                fields: [
                    {
        ...
        var logsModel = dataFrameToLogsModel(series, 1, { from: 1556270591353, to: 1556289770991 });
        expect(logsModel.meta).toHaveLength(2);
        expect(logsModel.meta[0]).toMatchObject({
            label: COMMON_LABELS,
            value: series[0].fields[1].labels,
    ...
    it('should fallback to row index if no id', function () {
        var series = [
            toDataFrame({
                labels: { foo: 'bar' },
                fields: [
...
describe('logSeriesToLogsModel', function () {
    it('should return correct metaData even if the data is empty', function () {
        var logSeries = [
            {
                fields: [],
                length: 0,
    ...
    it('should return correct metaData when some data frames have empty fields', function () {
        var logSeries = [
            toDataFrame({
                fields: [
                    {
        ...
        var logsModel = dataFrameToLogsModel(logSeries, 0);
    ...
    it('should return empty string if message field is undefined', function () {
        var logSeries = [
            toDataFrame({
                fields: [
                    {
    ...
    it('should correctly get the log level if the message has ANSI color', function () {
        var logSeries = [
            toDataFrame({
                fields: [
                    {
...
describe('getSeriesProperties()', function () {
    it('sets a minimum bucket size', function () {
        var result = getSeriesProperties([], 2, undefined, 3, 123);
        expect(result.bucketSize).toBe(123);
    });
    it('does not adjust the bucketSize if there is no range', function () {
        var result = getSeriesProperties([], 30, undefined, 70);
        expect(result.bucketSize).toBe(2100);
    });
    it('does not adjust the bucketSize if the logs row times match the given range', function () {
        var rows = [
            { entry: 'foo', timeEpochMs: 10 },
            { entry: 'bar', timeEpochMs: 20 },
        ];
        var range = { from: 10, to: 20 };
        var result = getSeriesProperties(rows, 1, range, 2, 1);
        expect(result.bucketSize).toBe(2);
        expect(result.visibleRange).toMatchObject(range);
    });
    it('clamps the range and adjusts the bucketSize if the logs row times do not completely cover the given range', function () {
        var rows = [
            { entry: 'foo', timeEpochMs: 10 },
            { entry: 'bar', timeEpochMs: 20 },
        ];
        var range = { from: 0, to: 30 };
        var result = getSeriesProperties(rows, 3, range, 2, 1);
