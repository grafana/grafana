...
describe('DBClusterAdvancedOptions NetworkAndSecurity::', () => {
    it('render items correctly for create and edit mode', () => {
        render(React.createElement(Form, { initialValues: { [NetworkAndSecurityFields.sourceRanges]: [{}] }, onSubmit: jest.fn(), mutators: Object.assign({}, arrayMutators), render: ({ form }) => React.createElement(NetworkAndSecurity, { form: form }) }));
    ...
    it('the delete button should not delete the first field', () => {
        render(React.createElement(Form, { initialValues: { [NetworkAndSecurityFields.sourceRanges]: [{ sourceRange: '1' }] }, onSubmit: jest.fn(), mutators: Object.assign({}, arrayMutators), render: ({ form }) => React.createElement(NetworkAndSecurity, { form: form }) }));
        expect(screen.getByTestId('toggle-network-and-security')).toBeInTheDocument();
        const checkbox = screen.getByTestId('toggle-network-and-security');
        fireEvent.click(checkbox);
        expect(screen.getByTestId('sourceRanges[0].sourceRange-text-input')).toBeInTheDocument();
    ...
    it('the delete button should delete field from the form if it is not the first one ', () => {
        render(React.createElement(Form, { initialValues: { [NetworkAndSecurityFields.sourceRanges]: [{ sourceRange: '1' }, { sourceRange: '2' }] }, onSubmit: jest.fn(), mutators: Object.assign({}, arrayMutators), render: ({ form }) => React.createElement(NetworkAndSecurity, { form: form }) }));
        expect(screen.getByTestId('toggle-network-and-security')).toBeInTheDocument();
        const checkbox = screen.getByTestId('toggle-network-and-security');
        fireEvent.click(checkbox);
...
//# sourceMappingURL=NetworkAndSecurity.test.js.map