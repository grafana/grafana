...
import { clusterActionsRender, clusterNameRender, clusterStatusRender, connectionRender, databaseTypeRender, parametersRender, } from './ColumnRenderers/ColumnRenderers';
...
export const DBCluster = () => {
    ...
    const addDisabled = (kubernetes === null || kubernetes === void 0 ? void 0 : kubernetes.length) === 0 || isKubernetesListUnavailable(kubernetes) || loading;
    const unAvailableK8s = useMemo(() => kubernetes.filter((k) => k.status === KubernetesClusterStatus.invalid || k.status === KubernetesClusterStatus.unavailable), [kubernetes]);
    ...
    const getDBClusters = useCallback((triggerLoading = true) => __awaiter(void 0, void 0, void 0, function* () {
        ...
        const tokens = availableK8s.map((k) => generateToken(`${GET_CLUSTERS_CANCEL_TOKEN}-${k.kubernetesClusterName}`));
        const result = yield catchFromAsyncThunkAction(dispatch(fetchDBClustersAction({ kubernetes: availableK8s, tokens })));
    ...
    const AddNewClusterButton = useCallback(() => (React.createElement(AddClusterButton, { label: DBaaSMessages.dbcluster.addAction, disabled: addDisabled, action: () => history.push(DB_CLUSTER_CREATION_URL), "data-testid": "dbcluster-add-cluster-button" })), 
    // eslint-disable-next-line react-hooks/exhaustive-deps
    [addDisabled]);
    ...
    useEffect(() => setLoading((prevLoading) => {
    ...
    return (React.createElement(OldPage, { navModel: navModel },
        React.createElement(OldPage.Contents, null,
            ...
            React.createElement(FeatureLoader, { featureName: DBaaSMessages.dbaas, featureSelector: featureSelector },
                ...
                React.createElement("div", null,
                    ...
                    React.createElement(DeleteDBClusterModal, { isVisible: deleteModalVisible, setVisible: setDeleteModalVisible, setLoading: setLoading, onClusterDeleted: getDBClusters, selectedCluster: selectedDBCluster }),
                    logsModalVisible && (React.createElement(DBClusterLogsModal, { isVisible: logsModalVisible, setVisible: setLogsModalVisible, dbCluster: selectedDBCluster })),
                    selectedDBCluster && updateModalVisible && (React.createElement(UpdateDBClusterModal, { dbCluster: selectedDBCluster, isVisible: updateModalVisible, setVisible: setUpdateModalVisible, setLoading: setLoading, onUpdateFinished: getDBClusters })),
                    React.createElement(Table, { columns: columns, data: dbClusters, loading: loading, noData: React.createElement(AddNewClusterButton, null), rowKey: getRowKey }))))));
};
