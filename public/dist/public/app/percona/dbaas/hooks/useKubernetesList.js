...
import { fetchK8sListAction, resetK8SClusterListState, } from '../../shared/core/reducers/dbaas/k8sClusterList/k8sClusterList';
import { getKubernetes as getKubernetesSelector, getPerconaSettingFlag } from '../../shared/core/selectors';
import { CHECK_OPERATOR_UPDATE_CANCEL_TOKEN, GET_KUBERNETES_CANCEL_TOKEN, } from '../components/Kubernetes/Kubernetes.constants';
export const useKubernetesList = () => {
    const [generateToken] = useCancelToken();
    const dispatch = useDispatch();
    const { result: kubernetes, loading } = useSelector(getKubernetesSelector);
    useEffect(() => {
        dispatch(fetchK8sListAction({
            tokens: {
...
export const useUpdateOfKubernetesList = () => {
    const [generateToken] = useCancelToken();
    const dispatch = useDispatch();
...
//# sourceMappingURL=useKubernetesList.js.map