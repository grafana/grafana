...
const setup = (propOverrides) => {
    ...
    return render(React.createElement(TeamGroupSync, Object.assign({}, props)));
};
describe('TeamGroupSync', () => {
    ...
    it('should call add group', () => __awaiter(void 0, void 0, void 0, function* () {
        const mockAddGroup = jest.fn();
        setup({ addTeamGroup: mockAddGroup });
        // Empty List CTA "Add group" button is second in the DOM order
        yield userEvent.click(screen.getAllByRole('button', { name: /add group/i })[1]);
        expect(screen.getByRole('textbox', { name: /add external group/i })).toBeVisible();
        yield userEvent.type(screen.getByRole('textbox', { name: /add external group/i }), 'test/group');
        yield userEvent.click(screen.getAllByRole('button', { name: /add group/i })[0]);
        yield waitFor(() => {
            expect(mockAddGroup).toHaveBeenCalledWith('test/group');
        });
    }));
    it('should call remove group', () => __awaiter(void 0, void 0, void 0, function* () {
        const mockRemoveGroup = jest.fn();
        const mockGroup = { teamId: 1, groupId: 'some/group' };
        setup({ removeTeamGroup: mockRemoveGroup, groups: [mockGroup] });
        yield userEvent.click(screen.getByRole('button', { name: 'Remove group some/group' }));
        yield waitFor(() => {
