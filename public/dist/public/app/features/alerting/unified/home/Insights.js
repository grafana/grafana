...
import { EmbeddedScene, NestedScene, QueryVariable, SceneControlsSpacer, SceneFlexItem, SceneFlexLayout, SceneReactObject, SceneRefreshPicker, SceneTimePicker, SceneTimeRange, SceneVariableSet, VariableValueSelectors, } from '@grafana/scenes';
...
export function overrideToFixedColor(key) {
    return {
        mode: 'fixed',
...
export function getInsightsScenes() {
    ...
    if (showGrafanaManaged) {
        categories.push(new SceneFlexItem({
            ySizing: 'content',
    ...
    return new EmbeddedScene({
        ...
        controls: [
            new SceneReactObject({
                ...
                props: { children: React.createElement("div", null, "Monitor the status of your system.") },
            }),
                                ...
                                new SceneFlexLayout({
                                    ...
                                    children: [
                                        new SceneFlexLayout({
                                            height: '400px',
                                            children: [
                                                getInstanceStatByStatusScene(cloudUsageDs, 'Firing instances', 'The number of currently firing alert rule instances', 'alerting'),
                                                getInstanceStatByStatusScene(cloudUsageDs, 'Pending instances', 'The number of currently pending alert rule instances', 'pending'),
                                            ],
                                        }),
                                        new SceneFlexLayout({
                                            children: [
                                                getInstanceStatByStatusScene(cloudUsageDs, 'No data instances', 'The current number of alert rule instances in No data state', 'nodata'),
                                                getInstanceStatByStatusScene(cloudUsageDs, 'Error instances', 'The current number of alert rule instances in Error state', 'error'),
                                            ],
                                        }),
                                    ],
                                }),
