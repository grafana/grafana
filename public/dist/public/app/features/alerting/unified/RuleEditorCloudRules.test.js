...
import { defaultAlertmanagerChoiceResponse, emptyExternalAlertmanagersResponse, mockAlertmanagerChoiceResponse, mockAlertmanagersResponse, } from './mocks/alertmanagerApi';
...
describe('RuleEditor cloud', () => {
    ...
    it('can create a new cloud alert', () => __awaiter(void 0, void 0, void 0, function* () {
        ...
        yield waitFor(() => expect(screen.getByText('Data source-managed')).toBeInTheDocument());
        ...
        yield user.click(switchToCloudButton);
        ...
        yield user.click(byRole('combobox').get(dataSourceSelect));
        yield clickSelectOption(dataSourceSelect, 'Prom (default)');
        yield waitFor(() => expect(mocks.api.fetchRulerRules).toHaveBeenCalled());
        yield user.type(yield ui.inputs.expr.find(), 'up == 1');
        yield user.type(ui.inputs.name.get(), 'my great new rule');
        yield clickSelectOption(ui.inputs.namespace.get(), 'namespace2');
        yield clickSelectOption(ui.inputs.group.get(), 'group2');
        yield user.type(ui.inputs.annotationValue(0).get(), 'some summary');
        yield user.type(ui.inputs.annotationValue(1).get(), 'some description');
        // TODO remove skipPointerEventsCheck once https://github.com/jsdom/jsdom/issues/3232 is fixed
        yield user.click(ui.buttons.addLabel.get());
        yield user.type(getLabelInput(ui.inputs.labelKey(0).get()), 'severity{enter}');
        yield user.type(getLabelInput(ui.inputs.labelValue(0).get()), 'warn{enter}');
        // save and check what was sent to backend
        yield user.click(ui.buttons.saveAndExit.get());
        yield waitFor(() => expect(mocks.api.setRulerRuleGroup).toHaveBeenCalled());
        expect(mocks.api.setRulerRuleGroup).toHaveBeenCalledWith({ dataSourceName: 'Prom', apiVersion: 'config' }, 'namespace2', {
            name: 'group2',
            rules: [
                {
...
//# sourceMappingURL=RuleEditorCloudRules.test.js.map