...
function option(propertyName, label, description, rest) {
    if (rest === void 0) { rest = {}; }
    return __assign({ propertyName: propertyName, label: label, description: description, element: 'input', inputType: '', required: false, secure: false, placeholder: '', validationRule: '', showWhen: { field: '', is: '' } }, rest);
}
var basicAuthOption = option('basic_auth', 'Basic auth', 'Sets the `Authorization` header with the configured username and password. Password and password_file are mutually exclusive.', {
    element: 'subform',
    subformOptions: [
...
var tlsConfigOption = option('tls_config', 'TLS config', 'Configures the TLS settings.', {
    element: 'subform',
    subformOptions: [
...
var httpConfigOption = option('http_config', 'HTTP Config', 'Note that `basic_auth`, `bearer_token` and `bearer_token_file` options are mutually exclusive.', {
    element: 'subform',
    subformOptions: [
...
export var globalConfigOptions = [
    // email
