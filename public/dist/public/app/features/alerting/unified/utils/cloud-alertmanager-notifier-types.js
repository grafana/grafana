...
const httpConfigOption = option('http_config', 'HTTP Config', 'Note that `basic_auth`, `bearer_token` and `bearer_token_file` options are mutually exclusive.', {
    ...
    subformOptions: [
        ...
        option('bearer_token_file', 'Bearer token file', 'Sets the `Authorization` header with the bearer token read from the configured file.'),
        option('proxy_url', 'Proxy URL', 'Optional proxy URL.'),
        basicAuthOption,
        tlsConfigOption,
    ],
});
export const cloudNotifierTypes = [
    {
        name: 'Email',
        ...
        options: [
            option('to', 'To', 'The email address to send notifications to. You can enter multiple addresses using a "," separator', { required: true }),
            ...
            option('headers', 'Headers', 'Further headers email header key/value pairs. Overrides any headers previously set by the notification implementation.', { element: 'key_value_map' }),
            tlsConfigOption,
        ],
    },
    {
        ...
        options: [
            option('routing_key', 'Routing key', 'The PagerDuty integration key (when using PagerDuty integration type `Events API v2`)'),
            option('service_key', 'Service key', 'The PagerDuty integration key (when using PagerDuty integration type `Prometheus`).'),
    ...
    },
    {
        ...
        options: [
            ...
            option('max_alerts', 'Max alerts', 'The maximum number of alerts to include in a single webhook message. Alerts above this threshold are truncated. When leaving this at its default value of 0, all alerts are included.', { placeholder: '0', validationRule: '(^\\d+$|^$)' }),
            httpConfigOption,
        ],
    },
    {
        ...
        options: [
            ...
            option('message', 'Message Content', 'Mention a group using @ or a user using <@ID> when notifying in a channel', { placeholder: '{{ template "discord.default.message" . }}' }),
    ...
    },
    {
        ...
        options: [
            ...
            Object.assign(Object.assign({}, httpConfigOption), { required: true }),
        ],
    },
    {
        name: 'Telegram',
    ...
    },
    {
        ...
        options: [
            ...
            option('sigv4', 'SigV4 authentication', "Configures AWS's Signature Verification 4 signing process to sign requests", {
                element: 'subform',
                subformOptions: [
                    option('region', 'Region', 'The AWS region. If blank, the region from the default credentials chain is used'),
                    option('access_key', 'Access key', 'The AWS API access_key. If blank the environment variable "AWS_ACCESS_KEY_ID" is used'),
                    option('secret_key', 'Secret key', 'The AWS API secret_key. If blank the environment variable "AWS_ACCESS_SECRET_ID" is used'),
            ...
            option('topic_arn', 'SNS topic ARN', "If you don't specify this value, you must specify a value for the phone_number or target_arn. If you are using a FIFO SNS topic you should set a message group interval longer than 5 minutes to prevent messages with the same group key being deduplicated by the SNS default deduplication window"),
            option('phone_number', 'Phone number', "Phone number if message is delivered via SMS in E.164 format. If you don't specify this value, you must specify a value for the topic_arn or target_arn"),
            option('target_arn', 'Target ARN', "The  mobile platform endpoint ARN if message is delivered via mobile notifications. If you don't specify this value, you must specify a value for the topic_arn or phone_number"),
...
export const globalConfigOptions = [
    ...
    option('smtp_smarthost', 'SMTP smarthost', 'The default SMTP smarthost used for sending emails, including port number. Port number usually is 25, or 587 for SMTP over TLS (sometimes referred to as STARTTLS). Example: smtp.example.org:587'),
    option('smtp_hello', 'SMTP hello', 'The default hostname to identify to the SMTP server.', {
        placeholder: 'localhost',
    }),
    option('smtp_auth_username', 'SMTP auth username', "SMTP Auth using CRAM-MD5, LOGIN and PLAIN. If empty, Alertmanager doesn't authenticate to the SMTP server."),
    ...
    option('smtp_require_tls', 'SMTP require TLS', 'The default SMTP TLS requirement. Note that Go does not support unencrypted connections to remote SMTP endpoints.', {
        element: 'checkbox',
    }),
    // slack
    ...
    httpConfigOption,
    option('resolve_timeout', 'Resolve timeout', 'ResolveTimeout is the default value used by alertmanager if the alert does not include EndsAt, after this time passes it can declare the alert as resolved if it has not been updated. This has no impact on alerts from Prometheus, as they always include EndsAt.', {
        placeholder: '5m',
    }),
];
//# sourceMappingURL=cloud-alertmanager-notifier-types.js.map