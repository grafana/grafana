...
describe('Alert rules searchParser', () => {
    describe('getSearchFilterFromQuery', () => {
        ...
        it('should parse filter values with whitespaces when in quotes', () => {
            const query = 'datasource:"prom dev" namespace:"node one" label:"team=frontend us" group:"cpu alerts" rule:"cpu failure"';
            const filter = getSearchFilterFromQuery(query);
        ...
        it('should parse filter values with special characters', () => {
            const query = 'datasource:prom::dev/linux>>; namespace:"[{node}] (#20+)" label:_region=apac|emea\\nasa group:$20.00%$ rule:"cpu!! & memory.,?"';
            const filter = getSearchFilterFromQuery(query);
    ...
    describe('applySearchFilterToQuery', () => {
        it('should apply filters to an empty query', () => {
            ...
            expect(query).toBe('datasource:"Mimir Dev" namespace:/etc/prometheus group:cpu-usage rule:"cpu > 80%" state:firing type:alerting health:ok label:team label:region=apac cpu eighty');
        });
        it('should update filters in existing query', () => {
            ...
            expect(query).toBe('datasource:"Mimir Dev" namespace:/etc/prometheus group:cpu-usage rule:"cpu > 80%" label:team label:region=apac');
        });
        it('should preserve the order of parameters when updating', () => {
            ...
            const query = applySearchFilterToQuery(baseQuery, filter);
            expect(query).toBe('label:region=emea rule:"cpu > 80%" group:cpu-usage namespace:/etc/prometheus datasource:"Mimir Dev"');
        });
    });
});
//# sourceMappingURL=rulesSearchParser.test.js.map