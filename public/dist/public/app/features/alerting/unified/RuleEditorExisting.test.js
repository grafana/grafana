...
describe('RuleEditor grafana managed rules', () => {
    ...
    it('can edit grafana managed rule', () => __awaiter(void 0, void 0, void 0, function* () {
        const uid = 'FOOBAR123';
        const folder = {
            title: 'Folder A',
            uid: 'abcd',
        ...
        jest.spyOn(backendSrv, 'getFolderByUid').mockResolvedValue(Object.assign(Object.assign({}, mockFolder()), { accessControl: {
        ...
        mocks.searchFolders.mockResolvedValue([folder, slashedFolder]);
        renderRuleEditor(uid);
        // check that it's filled in
        const nameInput = yield ui.inputs.name.find();
        ...
        // expect(within(folderInput).queryByText("Folders with '/' character are not allowed.")).not.toBeInTheDocument();
        // add an annotation
        yield userEvent.click(screen.getByText('Add custom annotation'));
        yield userEvent.type(screen.getByPlaceholderText('Enter custom annotation name...'), 'custom');
        yield userEvent.type(screen.getByPlaceholderText('Enter custom annotation content...'), 'value');
        //add a label
        yield userEvent.type(getLabelInput(ui.inputs.labelKey(2).get()), 'custom{enter}');
        yield userEvent.type(getLabelInput(ui.inputs.labelValue(2).get()), 'value{enter}');
        // save and check what was sent to backend
        yield userEvent.click(ui.buttons.save.get());
...
//# sourceMappingURL=RuleEditorExisting.test.js.map