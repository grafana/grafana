...
jest.mock('../../hooks/useAbilities', () => (Object.assign(Object.assign({}, jest.requireActual('../../hooks/useAbilities')), { useAlertmanagerAbilities: jest.fn() })));
...
describe('Policy', () => {
    ...
    it('should render a policy tree', () => __awaiter(void 0, void 0, void 0, function* () {
        const onEditPolicy = jest.fn();
        const onAddPolicy = jest.fn();
        const onDeletePolicy = jest.fn();
        const onShowAlertInstances = jest.fn((alertGroups, matchers) => { });
        const routeTree = mockRoutes;
        const user = userEvent.setup();
        renderPolicy(React.createElement(Policy, { routeTree: routeTree, currentRoute: routeTree, alertManagerSourceName: GRAFANA_RULES_SOURCE_NAME, onEditPolicy: onEditPolicy, onAddPolicy: onAddPolicy, onDeletePolicy: onDeletePolicy, onShowAlertInstances: onShowAlertInstances }));
        ...
        yield user.click(within(defaultPolicy).getByTestId('more-actions'));
        // should be editable
        ...
        yield user.click(editDefaultPolicy);
        ...
        // for timing options
        expect(within(defaultPolicy).getByTestId('timing-options')).toHaveTextContent('Wait30s to group instances,5m before sending updates');
        ...
        for (const container of customPolicies) {
            ...
            yield user.click(policy.getByTestId('more-actions'));
            ...
            yield user.click(screen.getByRole('menuitem', { name: 'Delete' }));
        ...
        expect(within(thirdPolicy).getByTestId('label-matchers')).toHaveTextContent(/^foo = barbar = bazbaz = quxasdf = asdftype = diskand 1 more$/);
    }));
    it('should show export option when export is allowed and supported returns true', () => __awaiter(void 0, void 0, void 0, function* () {
        const onEditPolicy = jest.fn();
        const onAddPolicy = jest.fn();
        const onDeletePolicy = jest.fn();
        const onShowAlertInstances = jest.fn((alertGroups, matchers) => { });
    ...
    it('should not show export option when is not supported', () => __awaiter(void 0, void 0, void 0, function* () {
        const onEditPolicy = jest.fn();
        const onAddPolicy = jest.fn();
        const onDeletePolicy = jest.fn();
        const onShowAlertInstances = jest.fn((alertGroups, matchers) => { });
    ...
    it('should not show export option when is not allowed', () => __awaiter(void 0, void 0, void 0, function* () {
        const onEditPolicy = jest.fn();
        const onAddPolicy = jest.fn();
        const onDeletePolicy = jest.fn();
        const onShowAlertInstances = jest.fn((alertGroups, matchers) => { });
    ...
    it('should not allow editing readOnly policy tree', () => {
        const routeTree = { id: '0', routes: [{ id: '1' }] };
        renderPolicy(React.createElement(Policy, { readOnly: true, routeTree: routeTree, currentRoute: routeTree, alertManagerSourceName: GRAFANA_RULES_SOURCE_NAME, onEditPolicy: noop, onAddPolicy: noop, onDeletePolicy: noop, onShowAlertInstances: noop }));
    ...
    it.skip('should show matching instances', () => {
        ...
        renderPolicy(React.createElement(Policy, { readOnly: true, alertGroups: matchingGroups, routeTree: routeTree, currentRoute: routeTree, alertManagerSourceName: GRAFANA_RULES_SOURCE_NAME, onEditPolicy: noop, onAddPolicy: noop, onDeletePolicy: noop, onShowAlertInstances: noop }));
        const defaultPolicy = screen.getByTestId('am-root-route-container');
    ...
    it('should show warnings and errors', () => {
        ...
        const receiversState = mockReceiversState();
        renderPolicy(React.createElement(Policy, { readOnly: true, routeTree: routeTree, currentRoute: routeTree, contactPointsState: receiversState, alertManagerSourceName: GRAFANA_RULES_SOURCE_NAME, onEditPolicy: noop, onAddPolicy: noop, onDeletePolicy: noop, onShowAlertInstances: noop }));
        const defaultPolicy = screen.getByTestId('am-root-route-container');
...
const renderPolicy = (element) => render(React.createElement(Router, { history: locationService.getHistory() },
    React.createElement(AlertmanagerProvider, { accessType: "notification" }, element)));
