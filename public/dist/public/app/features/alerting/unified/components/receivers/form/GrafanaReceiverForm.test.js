...
const ui = {
    ...
    existingOnCallIntegrationSelect: (index) => byTestId(`items.${index}.settings.url`),
};
describe('GrafanaReceiverForm', () => {
    ...
    describe('OnCall contact point', () => {
        it('OnCall contact point should be disabled if OnCall integration is not enabled', () => __awaiter(void 0, void 0, void 0, function* () {
            mockApi(server).grafanaNotifiers(grafanaAlertNotifiersMock);
            mockApi(server).plugins.getPluginSettings(Object.assign(Object.assign({}, onCallPluginMetaMock), { enabled: false }));
            const amConfig = getAmCortexConfig((_) => { });
            render(React.createElement(GrafanaReceiverForm, { alertManagerSourceName: GRAFANA_RULES_SOURCE_NAME, config: amConfig }), {
                wrapper: TestProvider,
            });
            yield waitFor(() => expect(ui.loadingIndicator.query()).not.toBeInTheDocument());
            yield clickSelectOption(byTestId('items.0.type').get(), 'Grafana OnCall');
            ...
            yield clickSelectOption(byTestId('items.0.type').get(), 'Alertmanager');
            expect(ui.integrationType.get().closest('form')).toHaveFormValues({ 'items.0.type': 'prometheus-alertmanager' });
        }));
        it('OnCall contact point should support new and existing integration options if OnCall integration V2 is enabled', () => __awaiter(void 0, void 0, void 0, function* () {
            mockApi(server).grafanaNotifiers(grafanaAlertNotifiersMock);
            mockApi(server).plugins.getPluginSettings(Object.assign(Object.assign({}, onCallPluginMetaMock), { enabled: true }));
            ...
            yield user.click(newIntegrationRadio.get());
            expect(newIntegrationRadio.get()).toBeChecked();
            yield user.type(ui.newOnCallIntegrationName.get(), 'emea-oncall');
            ...
            yield user.click(existingIntegrationRadio.get());
            expect(existingIntegrationRadio.get()).toBeChecked();
            yield clickSelectOption(ui.existingOnCallIntegrationSelect(0).get(), 'apac-oncall');
            expect(ui.integrationType.get().closest('form')).toHaveFormValues({
                'items.0.settings.url': 'https://apac.oncall.example.com',
                'items.0.settings.integration_name': undefined,
            });
        }));
        it('Should render URL text input field for OnCall concact point if OnCall plugin uses legacy integration', () => __awaiter(void 0, void 0, void 0, function* () {
            mockApi(server).grafanaNotifiers(grafanaAlertNotifiersMock);
            mockApi(server).plugins.getPluginSettings(Object.assign(Object.assign({}, onCallPluginMetaMock), { enabled: true }));
            mockApi(server).oncall.features([]);
            mockApi(server).oncall.getOnCallIntegrations([]);
            const amConfig = getAmCortexConfig((config) => config.addReceivers((receiver) => receiver.addGrafanaReceiverConfig((receiverConfig) => receiverConfig.withType('oncall').withName('emea-oncall').addSetting('url', 'https://oncall.example.com'))));
            render(React.createElement(GrafanaReceiverForm, { alertManagerSourceName: GRAFANA_RULES_SOURCE_NAME, config: amConfig, existing: amConfig.alertmanager_config.receivers[0] }), {
                wrapper: TestProvider,
            });
            yield waitFor(() => expect(ui.loadingIndicator.query()).not.toBeInTheDocument());
...
function getAmCortexConfig(configure) {
...
//# sourceMappingURL=GrafanaReceiverForm.test.js.map