...
export const SilencePeriod = () => {
    ...
    const { field: { onChange: onChangeStartsAt, value: startsAt }, fieldState: { invalid: startsAtInvalid }, } = useController({
        name: 'startsAt',
        control,
        rules: {
    ...
    const { field: { onChange: onChangeEndsAt, value: endsAt }, fieldState: { invalid: endsAtInvalid }, } = useController({
        name: 'endsAt',
        control,
        rules: {
    ...
    const { field: { onChange: onChangeTimeZone, value: timeZone }, } = useController({
        name: 'timeZone',
        control,
    });
    ...
    return (React.createElement(Field, { className: styles.timeRange, label: "Silence start and end", error: invalid ? 'To is before or the same as from' : '', invalid: invalid },
        React.createElement(TimeRangeInput, { value: {
            ...
            }, timeZone: timeZone, onChange: (newValue) => {
                onChangeStartsAt(dateTime(newValue.from));
                onChangeEndsAt(dateTime(newValue.to));
            }, onChangeTimeZone: (newValue) => onChangeTimeZone(newValue), hideTimeZone: false, hideQuickRanges: true, placeholder: 'Select time range' })));
};
const styles = {
    timeRange: css `
    width: 400px;
