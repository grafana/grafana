...
const EMPTY_FRAME = toDataFrame([]);
const NAMED_FRAME = Object.assign({ name: 'MyFrame' }, toDataFrame([]));
const DATA_FRAME = toDataFrame({
    fields: [{ name: 'value', type: FieldType.number, values: [1, 2, 3] }],
});
const DATA_FRAME_LARGE_DECIMAL = toDataFrame({
    fields: [{ name: 'value', type: FieldType.number, values: [1.23456789] }],
});
const DATA_FRAME_WITH_LABELS = toDataFrame({
...
describe('getSeriesName', () => {
    ...
    it('should give preference to displayNameFromDS', () => {
        const frame = Object.assign({ name: 'MyFrame' }, toDataFrame({
            fields: [
                {
                    name: 'value',
