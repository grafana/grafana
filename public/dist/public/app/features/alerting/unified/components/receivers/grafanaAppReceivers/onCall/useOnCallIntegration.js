...
export var OnCallIntegrationType;
(function (OnCallIntegrationType) {
    OnCallIntegrationType["NewIntegration"] = "new_oncall_integration";
    OnCallIntegrationType["ExistingIntegration"] = "existing_oncall_integration";
})(OnCallIntegrationType || (OnCallIntegrationType = {}));
export var OnCallIntegrationSetting;
(function (OnCallIntegrationSetting) {
    OnCallIntegrationSetting["IntegrationType"] = "integration_type";
    OnCallIntegrationSetting["IntegrationName"] = "integration_name";
})(OnCallIntegrationSetting || (OnCallIntegrationSetting = {}));
var OnCallIntegrationStatus;
(function (OnCallIntegrationStatus) {
    OnCallIntegrationStatus["Disabled"] = "disabled";
    ...
    OnCallIntegrationStatus["V2"] = "v2";
})(OnCallIntegrationStatus || (OnCallIntegrationStatus = {}));
function useOnCallPluginStatus() {
    const { installed: isOnCallEnabled, loading: isPluginBridgeLoading, error: pluginError, } = usePluginBridge(SupportedPlugin.OnCall);
    const { data: onCallFeatures = [], error: onCallFeaturesError, isLoading: isOnCallFeaturesLoading, } = onCallApi.endpoints.features.useQuery(undefined, { skip: !isOnCallEnabled });
    ...
    const isAlertingV2IntegrationEnabled = useMemo(() => integrationStatus === OnCallIntegrationStatus.V2, [integrationStatus]);
    return {
        isOnCallEnabled,
        integrationStatus,
        isAlertingV2IntegrationEnabled,
        isOnCallStatusLoading: isPluginBridgeLoading || isOnCallFeaturesLoading,
        onCallError: pluginError !== null && pluginError !== void 0 ? pluginError : onCallFeaturesError,
    };
}
export function useOnCallIntegration() {
    const notifyApp = useAppNotification();
    const { isOnCallEnabled, integrationStatus, isAlertingV2IntegrationEnabled, isOnCallStatusLoading, onCallError } = useOnCallPluginStatus();
    const { useCreateIntegrationMutation, useGrafanaOnCallIntegrationsQuery, useLazyValidateIntegrationNameQuery } = onCallApi;
    ...
    const { data: grafanaOnCallIntegrations = [], isLoading: isLoadingOnCallIntegrations, isError: isIntegrationsQueryError, } = useGrafanaOnCallIntegrationsQuery(undefined, { skip: !isAlertingV2IntegrationEnabled });
    const onCallFormValidators = useMemo(() => {
        return {
            integration_name: (value) => __awaiter(this, void 0, void 0, function* () {
                try {
                    yield validateIntegrationNameQuery(value).unwrap();
                    return true;
                }
                catch (error) {
    ...
    const extendOnCallReceivers = useCallback((receiver) => {
        if (!isAlertingV2IntegrationEnabled) {
            return receiver;
        }
        return produce(receiver, (draft) => {
            var _a;
            (_a = draft.grafana_managed_receiver_configs) === null || _a === void 0 ? void 0 : _a.forEach((config) => {
    ...
    const createOnCallIntegrations = useCallback((receiver) => __awaiter(this, void 0, void 0, function* () {
        var _a, _b;
        if (!isAlertingV2IntegrationEnabled) {
            return receiver;
        }
        const onCallIntegrations = (_b = (_a = receiver.grafana_managed_receiver_configs) === null || _a === void 0 ? void 0 : _a.filter((c) => c.type === 'oncall')) !== null && _b !== void 0 ? _b : [];
        const newOnCallIntegrations = onCallIntegrations.filter((c) => c.settings[OnCallIntegrationSetting.IntegrationType] === OnCallIntegrationType.NewIntegration);
        const createNewOnCallIntegrationJobs = newOnCallIntegrations.map((c) => __awaiter(this, void 0, void 0, function* () {
            const newIntegration = yield createIntegrationMutation({
        ...
        yield Promise.all(createNewOnCallIntegrationJobs);
        return produce(receiver, (draft) => {
            var _a;
            (_a = draft.grafana_managed_receiver_configs) === null || _a === void 0 ? void 0 : _a.forEach((c) => {
    ...
    const extendOnCallNotifierFeatures = useCallback((notifier) => {
        ...
        if (notifier.type === ReceiverTypes.OnCall && isAlertingV2IntegrationEnabled) {
            ...
            const existingIntegrationOption = {
            ...
            options.unshift(option(OnCallIntegrationSetting.IntegrationType, 'How to connect to OnCall', '', {
                required: true,
                element: 'radio',
                defaultValue: newIntegrationOption,
                selectOptions: [newIntegrationOption, existingIntegrationOption],
            }), option(OnCallIntegrationSetting.IntegrationName, 'Integration name', 'The name of the new OnCall integration', {
                required: true,
            ...
            return Object.assign(Object.assign({}, notifier), { options });
        }
        return notifier;
...
//# sourceMappingURL=useOnCallIntegration.js.map