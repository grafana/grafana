...
jest.spyOn(dataSource, 'getDatasourceAPIUid').mockImplementation((ds) => ds);
const useGetAlertManagersSourceNamesAndImageMock = useGetAlertManagersSourceNamesAndImage;
const ui = {
...
function mockOneAlertManager() {
    ...
    mockApi(server).getAlertmanagerConfig(GRAFANA_RULES_SOURCE_NAME, (amConfigBuilder) => amConfigBuilder
        .withRoute((routeBuilder) => routeBuilder
        .withReceiver('email')
...
function mockTwoAlertManagers() {
    ...
    mockApi(server).getAlertmanagerConfig(GRAFANA_RULES_SOURCE_NAME, (amConfigBuilder) => amConfigBuilder
        ...
        .addReceivers((b) => b.withName('slack'))
        .addReceivers((b) => b.withName('opsgenie')));
    mockApi(server).getAlertmanagerConfig('OTHER_AM', (amConfigBuilder) => amConfigBuilder
        .withRoute((routeBuilder) => routeBuilder
        .withReceiver('email')
        .addRoute((rb) => rb.withReceiver('slack').addMatcher('tomato', MatcherOperator.equal, 'red'))
        .addRoute((rb) => rb.withReceiver('opsgenie').addMatcher('team', MatcherOperator.equal, 'operations')))
...
function mockHasEditPermission(enabled) {
...
describe('NotificationPreview', () => {
    it('should render notification preview without alert manager label, when having only one alert manager configured to receive alerts', () => __awaiter(void 0, void 0, void 0, function* () {
        mockOneAlertManager();
        mockPreviewApiResponse(server, [{ labels: [{ tomato: 'red', avocate: 'green' }] }]);
        render(React.createElement(NotificationPreview, { alertQueries: [alertQuery], customLabels: [], condition: "A", folder: folder }), {
            wrapper: TestProvider,
        });
        yield userEvent.click(ui.previewButton.get());
        yield waitFor(() => {
    ...
    it('should render notification preview with alert manager sections, when having more than one alert manager configured to receive alerts', () => __awaiter(void 0, void 0, void 0, function* () {
    ...
    it('should render details modal when clicking see details button', () => __awaiter(void 0, void 0, void 0, function* () {
        // two alert managers configured  to receive alerts
        mockOneAlertManager();
        mockPreviewApiResponse(server, [{ labels: [{ tomato: 'red', avocate: 'green' }] }]);
        ...
        yield userEvent.click(ui.seeDetails.get());
    ...
    it('should not render contact point link in details modal if user has no permissions for editing contact points', () => __awaiter(void 0, void 0, void 0, function* () {
        // two alert managers configured  to receive alerts
        mockOneAlertManager();
        mockPreviewApiResponse(server, [{ labels: [{ tomato: 'red', avocate: 'green' }] }]);
...
describe('NotificationPreviewByAlertmanager', () => {
    it('should render route matching preview for alertmanager', () => __awaiter(void 0, void 0, void 0, function* () {
        const potentialInstances = [
        ...
        render(React.createElement(NotificationPreviewByAlertManager, { alertManagerSource: { name: GRAFANA_RULES_SOURCE_NAME, img: '' }, potentialInstances: potentialInstances, onlyOneAM: true }), { wrapper: TestProvider });
        yield waitFor(() => {
            expect(ui.loadingIndicator.query()).not.toBeInTheDocument();
        });
        ...
        yield user.click(ui.routeButton.get(routeElements[0]));
        yield user.click(ui.routeButton.get(routeElements[1]));
    ...
    it('should render route matching preview for alertmanager without errors if receiver is inherited from parent route (no receiver) ', () => __awaiter(void 0, void 0, void 0, function* () {
        const potentialInstances = [
            { foo: 'bar', severity: 'critical' },
            { job: 'prometheus', severity: 'warning' },
        ];
    ...
    it('should render route matching preview for alertmanager without errors if receiver is inherited from parent route (empty string receiver)', () => __awaiter(void 0, void 0, void 0, function* () {
        const potentialInstances = [
            { foo: 'bar', severity: 'critical' },
            { job: 'prometheus', severity: 'warning' },
        ];
...
//# sourceMappingURL=NotificationPreview.test.js.map