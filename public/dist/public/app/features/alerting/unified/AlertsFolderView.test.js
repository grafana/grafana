...
const combinedNamespaceMock = jest.fn();
jest.mock('./hooks/useCombinedRuleNamespaces', () => ({
    useCombinedRuleNamespaces: () => combinedNamespaceMock(),
}));
const mockFolder = (folderOverride = {}) => {
    return Object.assign({ id: 1, title: 'Folder with alerts', uid: 'folder-1', hasChanged: false, canSave: false, url: '/folder-1', version: 1, permissions: [], canViewFolderPermissions: false, canDelete: false }, folderOverride);
};
describe('AlertsFolderView tests', () => {
    it('Should display grafana alert rules when the namespace name matches the folder name', () => {
        // Arrange
        const folder = mockFolder();
        const grafanaNamespace = {
            name: folder.title,
    ...
    it('Should filter alert rules by the name, case insensitive', () => __awaiter(void 0, void 0, void 0, function* () {
        // Arrange
        const folder = mockFolder();
        const grafanaNamespace = {
            name: folder.title,
        ...
        render(React.createElement(TestProvider, null,
            React.createElement(AlertsFolderView, { folder: folder })));
        yield userEvent.type(ui.filter.name.get(), 'cpu');
        // Assert
        expect(ui.ruleList.row.queryAll()).toHaveLength(1);
        expect(ui.ruleList.row.get()).toHaveTextContent('CPU Alert');
    }));
    it('Should filter alert rule by labels', () => __awaiter(void 0, void 0, void 0, function* () {
        // Arrange
        const folder = mockFolder();
        const grafanaNamespace = {
            name: folder.title,
        ...
        render(React.createElement(TestProvider, null,
            React.createElement(AlertsFolderView, { folder: folder })));
        yield userEvent.type(ui.filter.label.get(), 'severity=critical');
        // Assert
        expect(ui.ruleList.row.queryAll()).toHaveLength(1);
        expect(ui.ruleList.row.get()).toHaveTextContent('RAM usage alert');
    }));
});
//# sourceMappingURL=AlertsFolderView.test.js.map