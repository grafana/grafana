    ...
    work(options) {
        ...
        const observables = annotations.map((annotation) => {
            ...
            if (config.publicDashboardAccessToken) {
                ...
                datasourceObservable = from(getDataSourceSrv().get(annotation.datasource)).pipe(catchError(handleDatasourceSrvError) // because of the reduce all observables need to be completed, so an erroneous observable wont do
                );
            }
            return datasourceObservable.pipe(mergeMap((datasource) => {
                ...
                return runner.run({ annotation, datasource, dashboard, range }).pipe(takeUntil(getDashboardQueryRunner()
                    .cancellations()
                    .pipe(filter((a) => a === annotation))), map((results) => {
