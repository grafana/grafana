...
const dashboardModel = createDashboardModelFixture({ id: 1, title: 'Test Dashboard', uid: 'test' }, { folderTitle: 'Test Folder' });
...
jest.mock('@grafana/runtime', () => (Object.assign(Object.assign({}, jest.requireActual('@grafana/runtime')), { reportMetaAnalytics: jest.fn() })));
const mockGetUrlSearchParams = jest.fn(() => {
    return {};
});
jest.mock('@grafana/data', () => (Object.assign(Object.assign({}, jest.requireActual('@grafana/data')), { urlUtil: {
...
function getTestData(requestApp, series = []) {
    const now = dateTime();
    return {
        request: {
            app: requestApp,
...
function getTestDataForExplore(requestApp, series = []) {
    const now = dateTime();
