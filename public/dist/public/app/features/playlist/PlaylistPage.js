...
export const PlaylistPage = () => {
    ...
    const playlists = useMemo(() => { var _a; return searchPlaylists((_a = allPlaylists.value) !== null && _a !== void 0 ? _a : [], searchQuery); }, [searchQuery, allPlaylists]);
    ...
    const emptyListBanner = (React.createElement(EmptyListCTA, { title: t('playlist-page.empty.title', 'There are no playlists created yet'), buttonIcon: "plus", buttonLink: "playlists/new", buttonTitle: t('playlist-page.empty.button', 'Create Playlist'), buttonDisabled: !contextSrv.isEditor, proTip: t('playlist-page.empty.pro-tip', 'You can use playlists to cycle dashboards on TVs without user control'), proTipLink: "http://docs.grafana.org/reference/playlist/", proTipLinkTitle: t('playlist-page.empty.pro-tip-link-title', 'Learn more'), proTipTarget: "_blank" }));
    const showSearch = playlists.length > 0 || searchQuery.length > 0;
    return (React.createElement(Page, { navId: "dashboards/playlists" },
        React.createElement(Page.Contents, { isLoading: allPlaylists.loading },
            showSearch && (React.createElement(PageActionBar, { searchQuery: searchQuery, linkButton: contextSrv.isEditor
                    ? { title: t('playlist-page.create-button.title', 'New playlist'), href: '/playlists/new' }
                    : undefined, setSearchQuery: setSearchQuery })),
            ...
            playlistToDelete && (React.createElement(ConfirmModal, { title: playlistToDelete.name, confirmText: t('playlist-page.delete-modal.confirm-text', 'Delete'), body: t('playlist-page.delete-modal.body', 'Are you sure you want to delete {{name}} playlist?', {
                    name: playlistToDelete.name,
                }), onConfirm: onDeletePlaylist, isOpen: Boolean(playlistToDelete), onDismiss: onDismissDelete })),
            startPlaylist && React.createElement(StartModal, { playlist: startPlaylist, onDismiss: () => setStartPlaylist(undefined) }))));
};
