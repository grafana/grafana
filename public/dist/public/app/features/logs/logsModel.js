...
import { dateTimeFormat, dateTimeFormatTimeAgo, FieldCache, FieldColorModeId, FieldType, findCommonLabels, findUniqueLabels, getTimeField, LoadingState, LogLevel, LogsDedupStrategy, LogsMetaKind, LogsVolumeType, rangeUtil, sortDataFrame, textUtil, toDataFrame, toUtc, } from '@grafana/data';
...
export function dataFrameToLogsModel(dataFrame, intervalMs, absoluteRange, queries) {
    ...
    if (logsModel) {
        ...
        if (intervalMs && logsModel.rows.length > 0) {
            ...
            const { visibleRange, bucketSize, visibleRangeMs, requestedRangeMs } = getSeriesProperties(sortedRows, intervalMs, absoluteRange);
...
function parseTime(timeField, timeNsField, index) {
 ...
 * like common labels.
 */
export function logSeriesToLogsModel(logSeries, queries = []) {
    ...
    if (seriesWithFields.length) {
        seriesWithFields.forEach((series) => {
            ...
            if (logsFrame != null) {
                ...
                const frameLabels = (_a = logsFrame.getAttributesAsLabels()) !== null && _a !== void 0 ? _a : undefined;
                const info = {
                    rawFrame: series,
                    logsFrame: logsFrame,
                    frameLabels,
    ...
    for (const info of allSeries) {
        ...
        for (let j = 0; j < series.length; j++) {
            const { ts, timeEpochMs, timeEpochNs } = parseTime(timeField, timeNanosecondField !== null && timeNanosecondField !== void 0 ? timeNanosecondField : undefined, j);
            // In edge cases, this can be undefined. If undefined, we want to replace it with empty string.
            const messageValue = (_a = stringField.values[j]) !== null && _a !== void 0 ? _a : '';
            ...
            const searchWords = (_f = (_d = (_c = (_b = series.meta) === null || _b === void 0 ? void 0 : _b.custom) === null || _c === void 0 ? void 0 : _c.searchWords) !== null && _d !== void 0 ? _d : (_e = series.meta) === null || _e === void 0 ? void 0 : _e.searchWords) !== null && _f !== void 0 ? _f : [];
            const entry = hasAnsi ? ansicolor.strip(message) : message;
            const labels = frameLabels === null || frameLabels === void 0 ? void 0 : frameLabels[j];
            ...
            const datasourceType = (_h = (_g = queries.find((query) => query.refId === series.refId)) === null || _g === void 0 ? void 0 : _g.datasource) === null || _h === void 0 ? void 0 : _h.type;
            const row = {
    ...
    const limits = logSeries.filter((series) => { var _a, _b, _c, _d; return (_c = (_b = (_a = series === null || series === void 0 ? void 0 : series.meta) === null || _a === void 0 ? void 0 : _a.custom) === null || _b === void 0 ? void 0 : _b.limit) !== null && _c !== void 0 ? _c : (_d = series === null || series === void 0 ? void 0 : series.meta) === null || _d === void 0 ? void 0 : _d.limit; });
    const lastLimitPerRef = limits.reduce((acc, elem) => {
        var _a, _b, _c, _d, _e, _f;
        acc[(_a = elem.refId) !== null && _a !== void 0 ? _a : ''] = (_f = (_d = (_c = (_b = elem.meta) === null || _b === void 0 ? void 0 : _b.custom) === null || _c === void 0 ? void 0 : _c.limit) !== null && _d !== void 0 ? _d : (_e = elem.meta) === null || _e === void 0 ? void 0 : _e.limit) !== null && _f !== void 0 ? _f : 0;
        return acc;
    }, {});
    ...
    let totalBytes = 0;
    const queriesVisited = {};
    // To add just 1 error message
    let errorMetaAdded = false;
    for (const series of logSeries) {
        const totalBytesKey = (_k = (_j = series.meta) === null || _j === void 0 ? void 0 : _j.custom) === null || _k === void 0 ? void 0 : _k.lokiQueryStatKey;
        const { refId } = series; // Stats are per query, keeping track by refId
        if (!errorMetaAdded && ((_m = (_l = series.meta) === null || _l === void 0 ? void 0 : _l.custom) === null || _m === void 0 ? void 0 : _m.error)) {
            meta.push({
                label: '',
                value: (_o = series.meta) === null || _o === void 0 ? void 0 : _o.custom.error,
                kind: LogsMetaKind.Error,
            });
            errorMetaAdded = true;
        }
        if (refId && !queriesVisited[refId]) {
            if (totalBytesKey && ((_p = series.meta) === null || _p === void 0 ? void 0 : _p.stats)) {
...
function adjustMetaInfo(logsModel, visibleRangeMs, requestedRangeMs) {
    ...
    const limit = limitIndex >= 0 && ((_a = logsModelMeta[limitIndex]) === null || _a === void 0 ? void 0 : _a.value);
    if (limit && limit > 0) {
        let metaLimitValue;
...
const updateLogsVolumeConfig = (dataFrame, extractLevel, oneLevelDetected) => {
    dataFrame.fields = dataFrame.fields.map((field) => {
        if (field.type === FieldType.number) {
            field.config = Object.assign(Object.assign({}, field.config), getLogVolumeFieldConfig(extractLevel(dataFrame), oneLevelDetected));
        }
        return field;
    });
    return dataFrame;
            ...
            next: (dataQueryResponse) => {
                ...
                if (error !== undefined) {
                    ...
                    logsVolumeData = dataQueryResponse.data.map((dataFrame) => {
                        ...
                        const logsVolumeCustomMetaData = {
                            logsVolumeType: LogsVolumeType.FullRange,
                            absoluteRange: { from: options.range.from.valueOf(), to: options.range.to.valueOf() },
                            datasourceName: datasource.name,
                            sourceQuery: options.targets.find((dataQuery) => dataQuery.refId === sourceRefId),
                        };
                        dataFrame.meta = Object.assign(Object.assign({}, dataFrame.meta), { custom: Object.assign(Object.assign({}, (_a = dataFrame.meta) === null || _a === void 0 ? void 0 : _a.custom), logsVolumeCustomMetaData) });
 ...
 */
export function queryLogsSample(datasource, logsSampleRequest) {
