...
jest.mock('@grafana/runtime', () => (Object.assign(Object.assign({}, jest.requireActual('@grafana/runtime')), { reportInteraction: (interactionName, properties) => reportInteraction(interactionName, properties) })));
const theme = createTheme();
const styles = getLogRowStyles(theme);
const setup = (propOverrides, rowOverrides) => {
    const props = Object.assign({ row: createLogRow(Object.assign({ entry: 'test123', uid: 'log-row-id', logLevel: LogLevel.error, timeEpochMs: 1546297200000 }, rowOverrides)), enableLogDetails: false, getRows: () => [], onOpenContext: () => { }, prettifyLogMessage: false, app: CoreApp.Explore, showDuplicates: false, showLabels: false, showTime: false, wrapLogMessage: false, timeZone: 'utc', styles }, (propOverrides || {}));
    const { container } = render(React.createElement("table", null,
        React.createElement("tbody", null,
            React.createElement(LogRow, Object.assign({}, props)))));
...
describe('LogRow', () => {
    ...
    describe('with permalinking', () => {
        ...
        it('does not highlight row details with different permalink-id', () => __awaiter(void 0, void 0, void 0, function* () {
            const { container } = setup({
                permalinkedRowId: 'log-row-id',
            ...
            const row = container.querySelector('tr');
            yield userEvent.click(row);
            const allRows = container.querySelectorAll('tr');
            expect(row).toHaveStyle(`background-color: ${tinycolor(theme.colors.info.transparent).setAlpha(0.25).toString()}`);
            expect(allRows[allRows.length - 1]).not.toHaveStyle(`background-color: ${tinycolor(theme.colors.info.transparent).setAlpha(0.25).toString()}`);
        }));
        it('not highlights row with different permalink-id', () => {
            const { container } = setup({ permalinkedRowId: 'wrong-log-row-id' });
            const row = container.querySelector('tr');
            expect(row).not.toHaveStyle(`background-color: ${tinycolor(theme.colors.info.transparent).setAlpha(0.25).toString()}`);
        });
        ...
        it('calls `scrollIntoView` once', () => __awaiter(void 0, void 0, void 0, function* () {
            const scrollIntoView = jest.fn();
            setup({ permalinkedRowId: 'log-row-id', scrollIntoView, containerRendered: true });
    ...
    it('should render the menu cell on mouse over', () => __awaiter(void 0, void 0, void 0, function* () {
    ...
    it('should render the menu cell on mouse over with displayed fields', () => __awaiter(void 0, void 0, void 0, function* () {
        setup({ showContextToggle: jest.fn().mockReturnValue(true), displayedFields: ['test'] }, { labels: { test: 'field value' } });
        expect(screen.queryByLabelText('Show context')).not.toBeInTheDocument();
        yield userEvent.hover(screen.getByText('test=field value'));
        expect(screen.getByLabelText('Show context')).toBeInTheDocument();
    }));
    it('should highlight the original log row when showing its context', () => __awaiter(void 0, void 0, void 0, function* () {
        const { container } = setup({ showContextToggle: jest.fn().mockReturnValue(true) });
        yield userEvent.hover(screen.getByText('test123'));
        yield userEvent.click(screen.getByLabelText('Show context'));
        yield userEvent.unhover(screen.getByText('test123'));
        const row = container.querySelector('tr');
        expect(row).toHaveStyle(`background-color: ${tinycolor(theme.colors.info.transparent).setAlpha(0.25).toString()}`);
    }));
});
//# sourceMappingURL=LogRow.test.js.map