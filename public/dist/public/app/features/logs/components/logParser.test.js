...
describe('logParser', () => {
    describe('getAllFields', () => {
        it('should filter out fields with data links that have a nullish value', () => {
            const createScenario = (value) => createLogRow({
                entryFieldIndex: 1,
                rowIndex: 0,
                dataFrame: {
                    refId: 'A',
        ...
        it('should filter out field with config hidden field', () => {
            const testField = Object.assign({}, testStringField);
            testField.config = {
                custom: {
                    hidden: true,
                },
            ...
            const logRow = createLogRow({
                ...
                dataFrame: new MutableDataFrame({
                    refId: 'A',
                    fields: [testTimeField, testLineField, Object.assign({}, testField)],
                }),
            });
            const fields = getAllFields(logRow);
        ...
        it('should filter out field with null values', () => {
            const logRow = createLogRow({
                ...
                dataFrame: new MutableDataFrame({
                    refId: 'A',
                    fields: [testTimeField, testLineField, Object.assign({}, testFieldWithNullValue)],
                }),
            });
            const fields = getAllFields(logRow);
            expect(fields.length).toBe(0);
        ...
        it('should not filter out field with string values', () => {
            const logRow = createLogRow({
                ...
                dataFrame: new MutableDataFrame({
                    refId: 'A',
                    fields: [testTimeField, testLineField, Object.assign({}, testStringField)],
                }),
            });
            const fields = getAllFields(logRow);
            expect(fields.length).toBe(1);
        ...
        describe('dataplane frames', () => {
            const makeLogRow = (fields, entryFieldIndex) => {
                var _a;
                return createLogRow({
                    entryFieldIndex,
                    rowIndex: 0,
                    dataFrame: {
                        refId: 'A',
                        fields,
                        length: (_a = fields[0]) === null || _a === void 0 ? void 0 : _a.values.length,
                        meta: {
            ...
            const expectHasField = (defs, name) => {
                expect(defs.find((field) => field.keys[0] === name)).not.toBe(undefined);
            };
            it('should filter out fields with data links that have a nullish value', () => {
                const createScenario = (value) => makeLogRow([
                    testTimeField,
                    testLineField,
                    {
                        name: 'link',
            ...
            it('should keep system fields with data-links', () => {
                ...
                const row = makeLogRow([
                    Object.assign(Object.assign({}, testTimeField), { config: { links } }),
                    Object.assign(Object.assign({}, testLineField), { config: { links } }),
                    {
                        config: { links },
                        name: 'id',
                        type: FieldType.string,
            ...
            it('should filter out config-hidden fields', () => {
                const row = makeLogRow([
                    testTimeField,
                    testLineField,
                    Object.assign(Object.assign({}, testStringField), { config: {
                            custom: {
                                hidden: true,
                            },
                        } }),
