...
function getTestContext(variable) {
    ...
    variable = variable !== null && variable !== void 0 ? variable : queryBuilder().withId('query').withRootStateKey(key).withName('query').build();
    const datasource = { metricFindQuery: jest.fn().mockResolvedValue([]) };
    ...
    const templatingState = {
        transaction: Object.assign(Object.assign({}, initialTransactionState), { uid: key }),
        variables: {
...
describe('VariableQueryRunner', () => {
    describe('happy case', () => {
        it('then it should work as expected', (done) => {
            const { key, identifier, runner, datasource, getState, getVariable, queryRunners, queryRunner, dispatch } = getTestContext();
            expectOnResults({
                identifier,
                runner,
                expect: (results) => {
                    ...
                    expect(dispatch.mock.calls[0][0]).toEqual(toKeyedAction(key, updateVariableOptions({
                        id: 'query',
                        type: 'query',
    ...
    describe('error cases', () => {
        describe('queryRunners.getRunnerForDatasource throws', () => {
            it('then it should work as expected', (done) => {
                const { identifier, runner, datasource, getState, getVariable, queryRunners, queryRunner, dispatch } = getTestContext();
