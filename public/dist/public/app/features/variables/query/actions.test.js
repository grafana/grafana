...
import { addVariableEditorError, changeVariableEditorExtended, initialVariableEditorState, removeVariableEditorError, variableEditorMounted, } from '../editor/reducer';
...
const mocks = {
    ...
    dataSourceSrv: {
        get: (ref) => Promise.resolve(mocks[ref.uid]),
        getList: jest.fn().mockReturnValue([]),
    },
    pluginLoader: {
        importDataSourcePlugin: jest.fn().mockResolvedValue({ components: {} }),
    },
    VariableQueryEditor(props) {
        return React.createElement("div", null, "this is a variable query editor");
    },
};
...
describe('query actions', () => {
    ...
    describe('when updateQueryVariableOptions is dispatched for variable without both tags and includeAll', () => {
        it('then correct actions are dispatched', () => __awaiter(void 0, void 0, void 0, function* () {
            ...
            tester.thenDispatchedActionsShouldEqual(toKeyedAction('key', updateVariableOptions(toVariablePayload(variable, update))), toKeyedAction('key', setCurrentVariableValue(toVariablePayload(variable, { option }))));
        }));
    });
    ...
    describe('when updateQueryVariableOptions is dispatched for variable open in editor', () => {
        it('then correct actions are dispatched', () => __awaiter(void 0, void 0, void 0, function* () {
            ...
            const option = createOption(ALL_VARIABLE_TEXT, ALL_VARIABLE_VALUE);
            const update = { results: optionsMetrics, templatedRegex: '' };
            tester.thenDispatchedActionsShouldEqual(toKeyedAction('key', removeVariableEditorError({ errorProp: 'update' })), toKeyedAction('key', updateVariableOptions(toVariablePayload(variable, update))), toKeyedAction('key', setCurrentVariableValue(toVariablePayload(variable, { option }))));
        }));
    });
    describe('when updateQueryVariableOptions is dispatched for variable with searchFilter', () => {
        it('then correct actions are dispatched', () => __awaiter(void 0, void 0, void 0, function* () {
            ...
            const update = { results: optionsMetrics, templatedRegex: '' };
            tester.thenDispatchedActionsShouldEqual(toKeyedAction('key', updateVariableOptions(toVariablePayload(variable, update))));
        }));
    });
    describe('when updateQueryVariableOptions is dispatched and fails for variable open in editor', () => {
        ...
        it('then correct actions are dispatched', () => __awaiter(void 0, void 0, void 0, function* () {
            ...
            tester.thenDispatchedActionsPredicateShouldEqual((dispatchedActions) => {
                ...
                expect(dispatchedActions[1]).toEqual(toKeyedAction('key', variableStateFailed(toVariablePayload(variable, { error }))));
    ...
    describe('when initQueryVariableEditor is dispatched', () => {
        it('then correct actions are dispatched', () => __awaiter(void 0, void 0, void 0, function* () {
            ...
            tester.thenDispatchedActionsShouldEqual(toKeyedAction('key', changeVariableEditorExtended({ dataSource: mocks.datasource, VariableQueryEditor: editor })));
        }));
    });
    describe('when initQueryVariableEditor is dispatched and metricsource without value is available', () => {
        it('then correct actions are dispatched', () => __awaiter(void 0, void 0, void 0, function* () {
            const variable = createVariable({ includeAll: true });
            const testMetricSource = { name: 'test', value: null, meta: {} };
            const editor = mocks.VariableQueryEditor;
            mocks.dataSourceSrv.getList = jest.fn().mockReturnValue([testMetricSource]);
            mocks.pluginLoader.importDataSourcePlugin = jest.fn().mockResolvedValue({
                components: { VariableQueryEditor: editor },
    ...
    describe('when changeQueryVariableDataSource is dispatched', () => {
        it('then correct actions are dispatched', () => __awaiter(void 0, void 0, void 0, function* () {
            ...
            const tester = yield reduxTester()
                ...
                .whenActionIsDispatched(toKeyedAction('key', addVariable(toVariablePayload(variable, { global: false, index: 0, model: variable }))))
                .whenActionIsDispatched(toKeyedAction('key', variablesInitTransaction({ uid: 'key' })))
                .whenAsyncActionIsDispatched(changeQueryVariableDataSource(toKeyedVariableIdentifier(variable), { uid: 'datasource' }), true);
            tester.thenDispatchedActionsShouldEqual(toKeyedAction('key', changeVariableEditorExtended({ dataSource: mocks.datasource, VariableQueryEditor: editor })));
        }));
        describe('and data source type changed', () => {
            it('then correct actions are dispatched', () => __awaiter(void 0, void 0, void 0, function* () {
                const variable = createVariable({ datasource: { uid: 'other' } });
                const editor = mocks.VariableQueryEditor;
                const previousDataSource = { type: 'previous' };
                const templatingState = {
                    editor: Object.assign(Object.assign({}, initialVariableEditorState), { extended: { dataSource: previousDataSource, VariableQueryEditor: editor } }),
                };
                ...
                const tester = yield reduxTester({ preloadedState })
                    ...
                    .whenActionIsDispatched(toKeyedAction('key', variablesInitTransaction({ uid: 'key' })))
                    .whenAsyncActionIsDispatched(changeQueryVariableDataSource(toKeyedVariableIdentifier(variable), { uid: 'datasource' }), true);
                tester.thenDispatchedActionsShouldEqual(toKeyedAction('key', changeVariableProp(toVariablePayload(variable, { propName: 'query', propValue: '' }))), toKeyedAction('key', changeVariableEditorExtended({ dataSource: mocks.datasource, VariableQueryEditor: editor })));
            }));
        });
    });
    ...
    describe('when changeQueryVariableQuery is dispatched', () => {
        it('then correct actions are dispatched', () => __awaiter(void 0, void 0, void 0, function* () {
            ...
            mockDatasourceMetrics(Object.assign(Object.assign({}, variable), { query }), optionsMetrics);
            const tester = yield reduxTester()
                .givenRootReducer(getRootReducer())
                .whenActionIsDispatched(toKeyedAction('key', addVariable(toVariablePayload(variable, { global: false, index: 0, model: variable }))))
                .whenActionIsDispatched(toKeyedAction('key', variablesInitTransaction({ uid: 'key' })))
                .whenAsyncActionIsDispatched(changeQueryVariableQuery(toKeyedVariableIdentifier(variable), query, definition), true);
            const option = createOption(ALL_VARIABLE_TEXT, ALL_VARIABLE_VALUE);
            const update = { results: optionsMetrics, templatedRegex: '' };
            tester.thenDispatchedActionsShouldEqual(toKeyedAction('key', removeVariableEditorError({ errorProp: 'query' })), toKeyedAction('key', changeVariableProp(toVariablePayload(variable, { propName: 'query', propValue: query }))), toKeyedAction('key', changeVariableProp(toVariablePayload(variable, { propName: 'definition', propValue: definition }))), toKeyedAction('key', variableStateFetching(toVariablePayload(variable))), toKeyedAction('key', updateVariableOptions(toVariablePayload(variable, update))), toKeyedAction('key', setCurrentVariableValue(toVariablePayload(variable, { option }))), toKeyedAction('key', variableStateCompleted(toVariablePayload(variable))));
        }));
    });
    ...
    describe('when changeQueryVariableQuery is dispatched with invalid query', () => {
        it('then correct actions are dispatched', () => __awaiter(void 0, void 0, void 0, function* () {
            ...
            tester.thenDispatchedActionsShouldEqual(toKeyedAction('key', addVariableEditorError({ errorProp: 'query', errorText })));
        }));
    });
...
function mockDatasourceMetrics(variable, optionsMetrics) {
    ...
    const { metricFindQuery } = mocks[(_a = variable.datasource) === null || _a === void 0 ? void 0 : _a.uid];
    metricFindQuery.mockReset();
    metricFindQuery.mockImplementation((query) => { var _a; return Promise.resolve((_a = metrics[query]) !== null && _a !== void 0 ? _a : []); });
}
function createVariable(extend) {
    return Object.assign({ type: 'query', id: '0', rootStateKey: 'key', global: false, current: createOption(''), options: [], query: 'options-query', name: 'Constant', label: '', hide: VariableHide.dontHide, skipUrlSync: false, index: 0, datasource: { uid: 'datasource' }, definition: '', sort: VariableSort.alphabeticalAsc, refresh: VariableRefresh.onDashboardLoad, regex: '', multi: true, includeAll: true, state: LoadingState.NotStarted, error: null, description: null }, (extend !== null && extend !== void 0 ? extend : {}));
}
function createOption(text, value) {
    const metric = createMetric(text);
    return Object.assign(Object.assign({}, metric), { value: value !== null && value !== void 0 ? value : metric.text, selected: false });
}
