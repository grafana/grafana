...
import { keyedVariablesReducer } from './keyedVariablesReducer';
import { getInitialTemplatingState } from './reducers';
export const getVariableState = (noOfVariables, inEditorIndex = -1, includeEmpty = false, includeSystem = false) => {
    const variables = {};
    if (includeSystem) {
...
export const getVariableTestContext = (adapter, variableOverrides = {}) => {
    const defaults = {
        id: '0',
        rootStateKey: 'key',
        index: 0,
    ...
    const defaultVariable = Object.assign(Object.assign({}, adapter.initialState), defaults);
    const initialState = {
        '0': Object.assign(Object.assign({}, defaultVariable), variableOverrides),
    };
    return { initialState };
};
...
export const getTemplatingRootReducer = () => combineReducers({
    templating: keyedVariablesReducer,
});
export function getPreloadedState(key, templatingState) {
    return {
        templating: {
            lastKey: key,
            keys: {
                [key]: Object.assign(Object.assign({}, getInitialTemplatingState()), templatingState),
            },
        },
    };
}
// function to find a query variable node in the list of all variables
export function findVariableNodeInList(allVariables, nodeName) {
 ...
 */
export function isVariableOnTimeRangeConfigured(variable) {
    return ((variable.type === 'query' || variable.type === 'datasource' || variable.type === 'interval') &&
