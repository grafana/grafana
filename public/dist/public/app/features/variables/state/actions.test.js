...
import { cancelVariables, changeVariableMultiValue, cleanUpVariables, fixSelectedInconsistency, initDashboardTemplating, isVariableUrlValueDifferentFromCurrent, processVariables, validateVariableSelectionState, } from './actions';
import { getPreloadedState, getTemplatingRootReducer } from './helpers';
...
describe('shared actions', () => {
    describe('when initDashboardTemplating is dispatched', () => {
        it('then correct actions are dispatched', () => {
            ...
            reduxTester()
                ...
                expect(dispatchedActions[0]).toEqual(toKeyedAction(key, addVariable(toVariablePayload(query, { global: false, index: 0, model: query }))));
                expect(dispatchedActions[1]).toEqual(toKeyedAction(key, addVariable(toVariablePayload(constant, { global: false, index: 1, model: constant }))));
                expect(dispatchedActions[2]).toEqual(toKeyedAction(key, addVariable(toVariablePayload(custom, { global: false, index: 2, model: custom }))));
                expect(dispatchedActions[3]).toEqual(toKeyedAction(key, addVariable(toVariablePayload(textbox, { global: false, index: 3, model: textbox }))));
                ...
                expect(dispatchedActions[4]).toEqual(toKeyedAction(key, variableStateNotStarted(toVariablePayload(Object.assign(Object.assign({}, query), { id: dispatchedActions[4].payload.action.payload.id })))));
                expect(dispatchedActions[5]).toEqual(toKeyedAction(key, variableStateNotStarted(toVariablePayload(Object.assign(Object.assign({}, constant), { id: dispatchedActions[5].payload.action.payload.id })))));
                expect(dispatchedActions[6]).toEqual(toKeyedAction(key, variableStateNotStarted(toVariablePayload(Object.assign(Object.assign({}, custom), { id: dispatchedActions[6].payload.action.payload.id })))));
                expect(dispatchedActions[7]).toEqual(toKeyedAction(key, variableStateNotStarted(toVariablePayload(Object.assign(Object.assign({}, textbox), { id: dispatchedActions[7].payload.action.payload.id })))));
                return true;
            });
        });
    });
    describe('when processVariables is dispatched', () => {
        it('then circular dependencies fail gracefully', () => __awaiter(void 0, void 0, void 0, function* () {
            const key = 'key';
            const var1 = queryBuilder().withName('var1').withQuery('$var2').build();
            const var2 = queryBuilder().withName('var2').withQuery('$var1').build();
            const dashboard = { templating: { list: [var1, var2] } };
            const preloadedState = getPreloadedState(key, {});
            yield expect(() => __awaiter(void 0, void 0, void 0, function* () {
                yield reduxTester({ preloadedState })
                    .givenRootReducer(getTemplatingRootReducer())
        ...
        it('then correct actions are dispatched', () => __awaiter(void 0, void 0, void 0, function* () {
            ...
            const dashboard = { templating: { list } };
            const preloadedState = getPreloadedState(key, {});
            const locationService = { getSearchObject: () => ({}) };
            ...
            yield tester.thenDispatchedActionsPredicateShouldEqual((dispatchedActions) => {
                ...
                expect(dispatchedActions[0]).toEqual(toKeyedAction(key, variableStateFetching(toVariablePayload(Object.assign(Object.assign({}, query), { id: dispatchedActions[0].payload.action.payload.id })))));
                expect(dispatchedActions[1]).toEqual(toKeyedAction(key, variableStateCompleted(toVariablePayload(Object.assign(Object.assign({}, constant), { id: dispatchedActions[1].payload.action.payload.id })))));
                expect(dispatchedActions[2]).toEqual(toKeyedAction(key, variableStateFetching(toVariablePayload(Object.assign(Object.assign({}, custom), { id: dispatchedActions[2].payload.action.payload.id })))));
                expect(dispatchedActions[3]).toEqual(toKeyedAction(key, variableStateCompleted(toVariablePayload(Object.assign(Object.assign({}, textbox), { id: dispatchedActions[3].payload.action.payload.id })))));
                expect(dispatchedActions[4]).toEqual(toKeyedAction(key, createCustomOptionsFromQuery(toVariablePayload(Object.assign(Object.assign({}, custom), { id: dispatchedActions[4].payload.action.payload.id })))));
                expect(dispatchedActions[5]).toEqual(toKeyedAction(key, variableStateCompleted(toVariablePayload(Object.assign(Object.assign({}, custom), { id: dispatchedActions[5].payload.action.payload.id })))));
                expect(dispatchedActions[6]).toEqual(toKeyedAction(key, variableStateCompleted(toVariablePayload(Object.assign(Object.assign({}, query), { id: dispatchedActions[6].payload.action.payload.id })))));
                return true;
            });
        }));
        // Fix for https://github.com/grafana/grafana/issues/28791
        it('fix for https://github.com/grafana/grafana/issues/28791', () => __awaiter(void 0, void 0, void 0, function* () {
            ...
            const locationService = { getSearchObject: () => query };
            runtime.setLocationService(locationService);
            const preloadedState = getPreloadedState(key, {});
            const tester = yield reduxTester({ preloadedState })
                .givenRootReducer(getTemplatingRootReducer())
                ...
                .whenActionIsDispatched(initDashboardTemplating(key, dashboard))
                .whenAsyncActionIsDispatched(processVariables(key), true);
            yield tester.thenDispatchedActionsShouldEqual(toKeyedAction(key, variableStateFetching(toVariablePayload(stats))), toKeyedAction(key, updateVariableOptions(toVariablePayload(stats, { results: [{ text: 'responses' }, { text: 'timers' }], templatedRegex: '' }))), toKeyedAction(key, setCurrentVariableValue(toVariablePayload(stats, {
                option: { text: ALL_VARIABLE_TEXT, value: ALL_VARIABLE_VALUE, selected: false },
            }))), toKeyedAction(key, variableStateCompleted(toVariablePayload(stats))), toKeyedAction(key, setCurrentVariableValue(toVariablePayload(stats, { option: { text: ['response'], value: ['response'], selected: false } }))), toKeyedAction(key, variableStateFetching(toVariablePayload(substats))), toKeyedAction(key, updateVariableOptions(toVariablePayload(substats, { results: [{ text: '200' }, { text: '500' }], templatedRegex: '' }))), toKeyedAction(key, setCurrentVariableValue(toVariablePayload(substats, {
                option: { text: [ALL_VARIABLE_TEXT], value: [ALL_VARIABLE_VALUE], selected: true },
            }))), toKeyedAction(key, variableStateCompleted(toVariablePayload(substats))), toKeyedAction(key, setCurrentVariableValue(toVariablePayload(substats, {
                option: { text: [ALL_VARIABLE_TEXT], value: [ALL_VARIABLE_VALUE], selected: false },
            }))));
        }));
    });
    describe('when validateVariableSelectionState is dispatched with a custom variable (no dependencies)', () => {
      ...
      `('then correct actions are dispatched', ({ withOptions, withCurrent, defaultValue, expected }) => __awaiter(void 0, void 0, void 0, function* () {
                let custom;
                const key = 'key';
                if (!withOptions) {
                ...
                const tester = yield reduxTester()
                    .givenRootReducer(getTemplatingRootReducer())
                    .whenActionIsDispatched(toKeyedAction(key, addVariable(toVariablePayload(custom, { global: false, index: 0, model: custom }))))
                    .whenAsyncActionIsDispatched(validateVariableSelectionState(toKeyedVariableIdentifier(custom), defaultValue), true);
                yield tester.thenDispatchedActionsPredicateShouldEqual((dispatchedActions) => {
                    const expectedActions = withOptions
                        ? [
                            toKeyedAction(key, setCurrentVariableValue(toVariablePayload({ type: 'custom', id: '0' }, { option: { text: expected, value: expected, selected: false } }))),
                        ]
                        : [];
      ...
      `('then correct actions are dispatched', ({ withOptions, currentText, currentValue, defaultValue, expected }) => __awaiter(void 0, void 0, void 0, function* () {
                let custom;
                const key = 'key';
                if (!withOptions) {
                    custom = customBuilder()
                ...
                yield tester.thenDispatchedActionsPredicateShouldEqual((dispatchedActions) => {
                    const expectedActions = withOptions
                        ? [
                            toKeyedAction(key, setCurrentVariableValue(toVariablePayload({ type: 'custom', id: '0' }, { option: { text: expected.text, value: expected.value, selected: false } }))),
                        ]
                        : [];
                    expect(dispatchedActions).toEqual(expectedActions);
                    return true;
        ...
        ${['A', 'B', 'C']} | ${['X']}      | ${'C'}       | ${'A'}        | ${false}
      `('then correct actions are dispatched', ({ withOptions, withCurrent, defaultValue, expectedText, expectedSelected }) => __awaiter(void 0, void 0, void 0, function* () {
                let custom;
                const key = 'key';
                if (!withOptions) {
                    custom = customBuilder()
                ...
                yield tester.thenDispatchedActionsPredicateShouldEqual((dispatchedActions) => {
                    const expectedActions = withOptions
                        ? [
                            toKeyedAction(key, setCurrentVariableValue(toVariablePayload({ type: 'custom', id: '0' }, { option: { text: expectedText, value: expectedText, selected: expectedSelected } }))),
                        ]
                        : [];
                    expect(dispatchedActions).toEqual(expectedActions);
                    return true;
    ...
    describe('changeVariableName', () => {
        describe('when changeVariableName is dispatched with the same name', () => {
            it('then the correct actions are dispatched', () => {
                ...
                reduxTester()
                    .givenRootReducer(getTemplatingRootReducer())
                    .whenActionIsDispatched(toKeyedAction(key, addVariable(toVariablePayload(textbox, { global: false, index: 0, model: textbox }))))
                    .whenActionIsDispatched(toKeyedAction(key, addVariable(toVariablePayload(constant, { global: false, index: 1, model: constant }))))
                    .whenActionIsDispatched(changeVariableName(toKeyedVariableIdentifier(constant), constant.name), true)
                    .thenDispatchedActionsShouldEqual(toKeyedAction(key, changeVariableNameSucceeded({ type: 'constant', id: 'constant', data: { newName: 'constant' } })));
            });
        });
        describe('when changeVariableName is dispatched with an unique name', () => {
            it('then the correct actions are dispatched', () => {
                ...
                reduxTester()
                    ...
                    .thenDispatchedActionsShouldEqual(toKeyedAction(key, addVariable({
                    type: 'constant',
                    id: 'constant1',
                    data: {
                        global: false,
                        index: 1,
                        model: Object.assign(Object.assign({}, constant), { name: 'constant1', id: 'constant1', global: false, index: 1, current: { selected: true, text: '', value: '' }, options: [{ selected: true, text: '', value: '' }] }),
                    },
                })), toKeyedAction(key, changeVariableNameSucceeded({ type: 'constant', id: 'constant1', data: { newName: 'constant1' } })), toKeyedAction(key, removeVariable({ type: 'constant', id: 'constant', data: { reIndex: false } })));
            });
        });
        describe('when changeVariableName is dispatched with an unique name for a new variable', () => {
            it('then the correct actions are dispatched', () => {
                ...
                })), toKeyedAction(key, changeVariableNameSucceeded({ type: 'constant', id: 'constant1', data: { newName: 'constant1' } })), toKeyedAction(key, removeVariable({ type: 'constant', id: NEW_VARIABLE_ID, data: { reIndex: false } })));
            });
        });
        describe('when changeVariableName is dispatched with __newName', () => {
            it('then the correct actions are dispatched', () => {
                ...
                reduxTester()
                    ...
                    .thenDispatchedActionsShouldEqual(toKeyedAction(key, changeVariableNameFailed({
                    newName: '__newName',
        ...
        describe('when changeVariableMultiValue is dispatched for variable with multi enabled', () => {
            it('then correct actions are dispatched', () => {
                ...
                reduxTester()
                    ...
                    .thenDispatchedActionsShouldEqual(toKeyedAction(key, changeVariableProp(toVariablePayload(custom, {
                    propName: 'multi',
                    propValue: false,
                }))), toKeyedAction(key, changeVariableProp(toVariablePayload(custom, {
                    propName: 'current',
                    propValue: {
                        value: 'A',
                        text: 'A',
        ...
        describe('when called', () => {
            it('then correct actions are dispatched', () => __awaiter(void 0, void 0, void 0, function* () {
                ...
                reduxTester()
                    ...
                    .thenDispatchedActionsShouldEqual(toKeyedAction(key, cleanVariables()), toKeyedAction(key, cleanEditorState()), toKeyedAction(key, cleanPickerState()), toKeyedAction(key, variablesClearTransaction()));
            }));
        });
    });
    describe('cancelVariables', () => {
        ...
        describe('when called', () => {
            it('then cancelAllInFlightRequests should be called and correct actions are dispatched', () => __awaiter(void 0, void 0, void 0, function* () {
                const key = 'key';
                reduxTester()
                    .givenRootReducer(getTemplatingRootReducer())
