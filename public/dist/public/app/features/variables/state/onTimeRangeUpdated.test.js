...
const getTestContext = (dashboard) => {
    jest.clearAllMocks();
    const key = 'key';
    ...
    const templateSrvMock = { updateTimeRange: updateTimeRangeMock };
    const dependencies = { templateSrv: templateSrvMock, events: appEvents };
    ...
    const templatingState = {
        variables: {
            'interval-0': Object.assign({}, interval),
            'constant-1': Object.assign({}, constant),
        },
    };
    const preloadedState = Object.assign({ dashboard: dashboardState }, getPreloadedState(key, templatingState));
    return {
        key,
        interval,
        range,
...
const getTestContextVariables = (dashboard, customeVariables) => {
    jest.clearAllMocks();
    const key = 'key';
    const interval = intervalBuilder()
        .withId('interval-0')
    ...
    const adapterQuery = variableAdapters.get('query');
    const templatingState = {
        variables: Object.assign({ 'interval-0': Object.assign({}, interval), 'constant-1': Object.assign({}, constant), a: Object.assign({}, queryA), b: Object.assign({}, queryB), c: Object.assign({}, queryC), d: Object.assign({}, varQueryWithNoDependentNodes) }, customeVariables),
    };
    const preloadedState = Object.assign({ dashboard: dashboardState }, getPreloadedState(key, templatingState));
    return {
        key,
...
describe('when onTimeRangeUpdated is dispatched', () => {
    describe('and options are changed by update', () => {
        it('then correct actions are dispatched and correct dependencies are called', () => __awaiter(void 0, void 0, void 0, function* () {
            const { key, preloadedState, range, dependencies, updateTimeRangeMock, templateVariableValueUpdatedMock, startRefreshMock, } = getTestContext(getDashboardModel());
            ...
            tester.thenDispatchedActionsShouldEqual(toKeyedAction(key, variablesInitTransaction({ uid: key })), toKeyedAction(key, variableStateFetching(toVariablePayload({ type: 'interval', id: 'interval-0' }))), toKeyedAction(key, createIntervalOptions(toVariablePayload({ type: 'interval', id: 'interval-0' }))), toKeyedAction(key, setCurrentVariableValue(toVariablePayload({ type: 'interval', id: 'interval-0' }, { option: { text: '1m', value: '1m', selected: false } }))), toKeyedAction(key, variableStateCompleted(toVariablePayload({ type: 'interval', id: 'interval-0' }))));
    ...
    describe('and options are not changed by update', () => {
        it('then correct actions are dispatched and correct dependencies are called', () => __awaiter(void 0, void 0, void 0, function* () {
            const { key, interval, preloadedState, range, dependencies, updateTimeRangeMock, templateVariableValueUpdatedMock, startRefreshMock, } = getTestContext(getDashboardModel());
            const base = yield reduxTester({ preloadedState })
                .givenRootReducer(getRootReducer())
                .whenActionIsDispatched(toKeyedAction(key, variablesInitTransaction({ uid: key })))
                .whenAsyncActionIsDispatched(setOptionAsCurrent(toKeyedVariableIdentifier(interval), interval.options[0], false));
            const tester = yield base.whenAsyncActionIsDispatched(onTimeRangeUpdated(key, range, dependencies), true);
            tester.thenDispatchedActionsShouldEqual(toKeyedAction(key, variableStateFetching(toVariablePayload({ type: 'interval', id: 'interval-0' }))), toKeyedAction(key, createIntervalOptions(toVariablePayload({ type: 'interval', id: 'interval-0' }))), toKeyedAction(key, setCurrentVariableValue(toVariablePayload({ type: 'interval', id: 'interval-0' }, { option: { text: '1m', value: '1m', selected: false } }))), toKeyedAction(key, variableStateCompleted(toVariablePayload({ type: 'interval', id: 'interval-0' }))));
            expect(updateTimeRangeMock).toHaveBeenCalledTimes(1);
            expect(updateTimeRangeMock).toHaveBeenCalledWith(range);
    ...
    describe('and updateOptions throws', () => {
        ...
        it('then correct actions are dispatched and correct dependencies are called', () => __awaiter(void 0, void 0, void 0, function* () {
            const { key, adapterInterval, preloadedState, range, dependencies, updateTimeRangeMock, templateVariableValueUpdatedMock, startRefreshMock, } = getTestContext(getDashboardModel());
            adapterInterval.updateOptions = jest.fn().mockRejectedValue(new Error('Something broke'));
            const tester = yield reduxTester({ preloadedState, debug: true })
                .givenRootReducer(getRootReducer())
                .whenActionIsDispatched(toKeyedAction(key, variablesInitTransaction({ uid: key })))
                .whenAsyncActionIsDispatched(onTimeRangeUpdated(key, range, dependencies), true);
            tester.thenDispatchedActionsPredicateShouldEqual((dispatchedActions) => {
                expect(dispatchedActions[0]).toEqual(toKeyedAction(key, variableStateFetching(toVariablePayload({ type: 'interval', id: 'interval-0' }))));
                expect(dispatchedActions[1]).toEqual(toKeyedAction(key, variableStateFailed(toVariablePayload({ type: 'interval', id: 'interval-0' }, { error: new Error('Something broke') }))));
    ...
    describe('When onTimeRangeUpdated is dispatched without refactorVariablesTimeRange feature flag ', () => {
        silenceConsoleOutput();
        it('then with old getVariablesThatNeedRefresh we call all of them in pararell', () => __awaiter(void 0, void 0, void 0, function* () {
            ...
            yield reduxTester({ preloadedState, debug: true })
                .givenRootReducer(getRootReducer())
                .whenActionIsDispatched(toKeyedAction(key, variablesInitTransaction({ uid: key })))
                .whenAsyncActionIsDispatched(onTimeRangeUpdated(key, range, dependencies), true);
    ...
    describe('On dispatch of onTimeRangeUpdated with refactorVariablesTimeRange feature flag', () => {
        ...
        it('should refresh only the independent query variables and those with dependents, optimising the number of calls when working with chained query variables', () => __awaiter(void 0, void 0, void 0, function* () {
            const { key, preloadedState, range, dependencies } = getTestContextVariables(getDashboardModel());
            // Spying on timeRangeUpdated action
            const spyTimeRangeUpdated = jest.spyOn(actions, 'timeRangeUpdated');
            // Initiating the Redux testing environment
        ...
        it('Should ensure query variable is refreshed when dependent on a non-refreshing datasource variable', () => __awaiter(void 0, void 0, void 0, function* () {
            ...
            const { key, preloadedState, range, dependencies } = getTestContextVariables(getDashboardModel(), customVariables);
            // Spying on timeRangeUpdated action
            const spyTimeRangeUpdated = jest.spyOn(actions, 'timeRangeUpdated');
            // Initiating the Redux testing environment
            yield reduxTester({ preloadedState, debug: true })
