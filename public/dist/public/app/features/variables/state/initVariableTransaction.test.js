...
import { LoadingState } from '@grafana/data/src';
...
import { getPreloadedState, getRootReducer } from './helpers';
import { toKeyedAction } from './keyedVariablesReducer';
import { addVariable, changeVariableProp, setCurrentVariableValue, variableStateCompleted, variableStateFetching, variableStateNotStarted, } from './sharedReducer';
import { initialTransactionState, variablesClearTransaction, variablesCompleteTransaction, variablesInitTransaction, } from './transactionReducer';
...
function getTestContext(variables) {
    const key = 'key';
    const constant = constantBuilder().withId('constant').withName('constant').build();
    const templating = { list: variables !== null && variables !== void 0 ? variables : [constant] };
    ...
    const dashboard = { title: 'Some dash', uid: key, templating };
...
describe('initVariablesTransaction', () => {
    describe('when called and the previous dashboard has completed', () => {
        it('then correct actions are dispatched', () => __awaiter(void 0, void 0, void 0, function* () {
            ...
            tester.thenDispatchedActionsPredicateShouldEqual((dispatchedActions) => {
                ...
                expect(dispatchedActions[4]).toEqual(toKeyedAction(key, addVariable(toVariablePayload(constant, { global: false, index: 0, model: constant }))));
        ...
        describe('and there are variables that have data source that need to be migrated', () => {
            it('then correct actions are dispatched', () => __awaiter(void 0, void 0, void 0, function* () {
                ...
                tester.thenDispatchedActionsPredicateShouldEqual((dispatchedActions) => {
                    ...
                    expect(dispatchedActions[3].payload.action.type).toEqual(addVariable.type);
                    expect(dispatchedActions[3].payload.action.payload.id).toEqual('__user');
                    expect(dispatchedActions[4]).toEqual(toKeyedAction(key, addVariable(toVariablePayload(ds, { global: false, index: 0, model: ds }))));
                    expect(dispatchedActions[5]).toEqual(toKeyedAction(key, addVariable(toVariablePayload(query, { global: false, index: 1, model: query }))));
                    expect(dispatchedActions[6]).toEqual(toKeyedAction(key, addVariable(toVariablePayload(adhoc, { global: false, index: 2, model: adhoc }))));
                    ...
                    expect(dispatchedActions[10]).toEqual(toKeyedAction(key, changeVariableProp(toVariablePayload(query, { propName: 'datasource', propValue: { uid: '${ds}' } }))));
                    expect(dispatchedActions[11]).toEqual(toKeyedAction(key, changeVariableProp(toVariablePayload(adhoc, { propName: 'datasource', propValue: { uid: '${ds}' } }))));
                    ...
                    expect(dispatchedActions[14]).toEqual(toKeyedAction(key, createDataSourceOptions(toVariablePayload(ds, { sources: [], regex: undefined }))));
                    expect(dispatchedActions[15]).toEqual(toKeyedAction(key, setCurrentVariableValue(toVariablePayload(ds, { option: { selected: false, text: 'No data sources found', value: '' } }))));
    ...
    describe('when called and the previous dashboard is still processing variables', () => {
        it('then correct actions are dispatched', () => __awaiter(void 0, void 0, void 0, function* () {
            const { constant, key, dashboard } = getTestContext();
            const transactionState = Object.assign(Object.assign({}, initialTransactionState), { uid: 'previous-uid', status: TransactionStatus.Fetching });
            const preloadedState = getPreloadedState(key, { transaction: transactionState });
            const tester = yield reduxTester({ preloadedState })
                .givenRootReducer(getRootReducer())
                .whenAsyncActionIsDispatched(initVariablesTransaction(key, dashboard));
            tester.thenDispatchedActionsPredicateShouldEqual((dispatchedActions) => {
                ...
                expect(dispatchedActions[8]).toEqual(toKeyedAction(key, addVariable(toVariablePayload(constant, { global: false, index: 0, model: constant }))));
...
//# sourceMappingURL=initVariableTransaction.test.js.map