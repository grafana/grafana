...
export var QuickFeedbackType;
(function (QuickFeedbackType) {
    QuickFeedbackType["Shorter"] = "Even shorter";
    QuickFeedbackType["MoreDescriptive"] = "More descriptive";
    QuickFeedbackType["Regenerate"] = "Regenerate";
})(QuickFeedbackType || (QuickFeedbackType = {}));
/**
 ...
 */
export const sanitizeReply = (reply) => {
 ...
 */
export function getDashboardChanges(dashboard) {
 ...
 */
export function isLLMPluginEnabled() {
    return __awaiter(this, void 0, void 0, function* () {
        // Check if the LLM plugin is enabled and configured.
        // If not, we won't be able to make requests, so return early.
        return yield openai.enabled();
    });
}
/**
 *
 ...
 */
export function getDashboardPanelPrompt(dashboard) {
    const getPanelString = (panel, idx) => `
  - Panel ${idx}\n
  - Title: ${panel.title}\n
  ${panel.description ? `- Description: ${panel.description}` : ''}
  `;
    const panelStrings = dashboard.panels.map(getPanelString);
    let panelPrompt;
