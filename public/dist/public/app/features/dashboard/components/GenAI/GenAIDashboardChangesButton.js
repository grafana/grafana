...
export const GenAIDashboardChangesButton = ({ dashboard, onGenerate, disabled }) => {
    const messages = useMemo(() => getMessages(dashboard), [dashboard]);
    return (React.createElement(GenAIButton, { messages: messages, onGenerate: onGenerate, loadingText: 'Generating changes summary', temperature: 0, eventTrackingSrc: EventTrackingSrc.dashboardChanges, toggleTipTitle: 'Improve your dashboard changes summary', disabled: disabled }));
};
function getMessages(dashboard) {
    ...
    return [
        ...
        {
            content: `This is the list of panel names, when referring to a panel, please use the title: ${JSON.stringify(dashboard.panels.map((panel) => panel.title))}`,
            role: Role.system,
        },
        {
...
//# sourceMappingURL=GenAIDashboardChangesButton.js.map