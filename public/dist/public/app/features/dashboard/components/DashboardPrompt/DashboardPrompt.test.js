...
function getTestContext() {
    const contextSrv = { isSignedIn: true, isEditor: true };
...
describe('DashboardPrompt', () => {
    ...
    describe('ignoreChanges', () => {
        ...
        it('Should ignore panel schema migrations', () => __awaiter(void 0, void 0, void 0, function* () {
            const { original, dash } = getTestContext();
            const plugin = getPanelPlugin({}).setMigrationHandler((panel) => {
                delete panel.legend;
                return { option1: 'Aasd' };
            });
            yield dash.panels[0].pluginLoaded(plugin);
            expect(hasChanges(dash, original)).toBe(false);
        }));
