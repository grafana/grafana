...
describe('AnnotationsSettings', function () {
    ...
    beforeAll(function () {
        setDataSourceSrv({
            getList: function () {
                return Object.values(datasources).map(function (d) { return d; });
            },
        ...
        setAngularLoader({
            load: function () { return ({
                destroy: jest.fn(),
                digest: jest.fn(),
                getScope: function () { return ({ $watch: function () { } }); },
            }); },
        });
    });
    beforeEach(function () {
    ...
    test('it renders a header and cta if no annotations or only builtIn annotation', function () {
        render(React.createElement(AnnotationsSettings, { dashboard: dashboard }));
        expect(screen.getByRole('heading', { name: /annotations/i })).toBeInTheDocument();
        expect(screen.queryByRole('table')).toBeInTheDocument();
        expect(screen.getByRole('row', { name: /annotations & alerts \(built\-in\) grafana cancel delete/i })).toBeInTheDocument();
        expect(screen.queryByLabelText(selectors.components.CallToActionCard.button('Add annotation query'))).toBeInTheDocument();
        ...
        var nameInput = screen.getByRole('textbox', { name: /name/i });
    ...
    test('it renders a sortable table of annotations', function () {
        var annotationsList = __spreadArray(__spreadArray([], __read(dashboard.annotations.list), false), [
            {
                builtIn: 0,
        ...
        var dashboardWithAnnotations = __assign(__assign({}, dashboard), { annotations: {
                list: __spreadArray([], __read(annotationsList), false),
            } });
        render(React.createElement(AnnotationsSettings, { dashboard: dashboardWithAnnotations }));
    ...
    test('it renders a form for adding/editing annotations', function () {
        render(React.createElement(AnnotationsSettings, { dashboard: dashboard }));
        ...
        expect(screen.queryByRole('row', { name: /my prometheus annotation prometheus cancel delete/i })).toBeInTheDocument();
        expect(screen.queryByRole('button', { name: /new query/i })).toBeInTheDocument();
        expect(screen.queryByLabelText(selectors.components.CallToActionCard.button('Add annotation query'))).not.toBeInTheDocument();
