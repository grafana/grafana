...
describe('LinksSettings', function () {
    var dashboard = {};
    var links = [
        {
            asDropdown: false,
    ...
    var getTableBody = function () { return screen.getAllByRole('rowgroup')[1]; };
    var getTableBodyRows = function () { return within(getTableBody()).getAllByRole('row'); };
    var assertRowHasText = function (index, text) {
    ...
    beforeEach(function () {
        dashboard = {
            id: 74,
            version: 7,
            links: __spreadArray([], __read(links), false),
        };
    });
    test('it renders a header and cta if no links', function () {
        var linklessDashboard = __assign(__assign({}, dashboard), { links: [] });
        // @ts-ignore
        render(React.createElement(LinksSettings, { dashboard: linklessDashboard }));
        expect(screen.getByRole('heading', { name: 'Dashboard links' })).toBeInTheDocument();
        expect(screen.getByLabelText(selectors.components.CallToActionCard.button('Add dashboard link'))).toBeInTheDocument();
        expect(screen.queryByRole('table')).not.toBeInTheDocument();
    });
    test('it renders a table of links', function () {
        // @ts-ignore
        render(React.createElement(LinksSettings, { dashboard: dashboard }));
        expect(getTableBodyRows().length).toBe(links.length);
        expect(screen.queryByLabelText(selectors.components.CallToActionCard.button('Add dashboard link'))).not.toBeInTheDocument();
    });
    test('it rearranges the order of dashboard links', function () {
        // @ts-ignore
        render(React.createElement(LinksSettings, { dashboard: dashboard }));
    ...
    test('it duplicates dashboard links', function () {
        // @ts-ignore
        render(React.createElement(LinksSettings, { dashboard: dashboard }));
        expect(getTableBodyRows().length).toBe(links.length);
    ...
    test('it deletes dashboard links', function () {
        // @ts-ignore
        render(React.createElement(LinksSettings, { dashboard: dashboard }));
        expect(getTableBodyRows().length).toBe(links.length);
    ...
    test('it renders a form which modifies dashboard links', function () {
        // @ts-ignore
        render(React.createElement(LinksSettings, { dashboard: dashboard }));
        ...
        userEvent.click(within(screen.getByRole('heading', { name: /dashboard links edit/i })).getByText(/dashboard links/i));
        expect(getTableBodyRows().length).toBe(links.length + 1);
