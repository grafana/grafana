...
describe('timeSrv', function () {
    ...
    beforeEach(function () {
        _dashboard = {
            ...
            refresh: false,
            timeRangeUpdated: jest.fn(function () { }),
        };
        ...
        beforeEach(function () {
            ...
            var origPush = locationService.push;
            locationService.push = function (args) {
                pushSpy();
                origPush(args);
            };
        });
    ...
    describe('init time from url', function () {
        ...
        it('should handle epochs that look like formatted date without time', function () {
        ...
        it('should handle epochs that look like formatted date', function () {
        ...
        it('should handle refresh_intervals=null when refresh is enabled', function () {
            locationService.push('/d/id?refresh=30s');
            timeSrv = new TimeSrv(new ContextSrvStub());
            _dashboard.timepicker = {
                refresh_intervals: null,
            };
            expect(function () { return timeSrv.init(_dashboard); }).not.toThrow();
        });
        describe('data point windowing', function () {
            ...
            it('corrects inverted from/to dates in ms', function () {
            ...
            it('corrects inverted from/to dates as relative times', function () {
    ...
    describe('setTime', function () {
        ...
        it('should update location only once for consecutive calls with the same range', function () {
            timeSrv.setTime({ from: 'now-1h', to: 'now-10s' });
            timeSrv.setTime({ from: 'now-1h', to: 'now-10s' });
            expect(pushSpy).toHaveBeenCalledTimes(1);
        });
    });
    describe('pauseAutoRefresh', function () {
        it('should set refresh to empty value', function () {
            _dashboard.refresh = '10s';
            timeSrv.pauseAutoRefresh();
            expect(_dashboard.refresh).toBe('');
        });
        it('should set previousAutoRefresh value', function () {
            _dashboard.refresh = '10s';
            timeSrv.pauseAutoRefresh();
            expect(timeSrv.previousAutoRefresh).toBe('10s');
        });
    });
    describe('resumeAutoRefresh', function () {
        it('should set refresh to empty value', function () {
