...
import { DataLinkBuiltInVars, FieldMatcherID, FieldType, getActiveThreshold, getDataSourceRef, isDataSourceRef, isEmptyObject, MappingType, SpecialValueMatch, standardEditorsRegistry, standardFieldConfigEditorRegistry, urlUtil, } from '@grafana/data';
    ...
    updateSchema(old) {
        ...
        if (oldVersion < 20) {
            const updateLinks = (link) => {
                return Object.assign(Object.assign({}, link), { url: updateVariablesSyntax(link.url) });
            };
            panelUpgrades.push((panel) => {
                // For graph panel
        ...
        if (oldVersion < 21) {
            const updateLinks = (link) => {
                return Object.assign(Object.assign({}, link), { url: link.url.replace(/__series.labels/g, '__field.labels') });
            };
            panelUpgrades.push((panel) => {
                // For graph panel
                if (panel.options && panel.options.dataLinks && isArray(panel.options.dataLinks)) {
        ...
        if (oldVersion < 27) {
            ...
            this.dashboard.templating.list = this.dashboard.templating.list.map((variable) => {
                ...
                const newVariable = Object.assign({}, variable);
                newVariable.current = { selected: true, text: (_a = newVariable.query) !== null && _a !== void 0 ? _a : '', value: (_b = newVariable.query) !== null && _b !== void 0 ? _b : '' };
                newVariable.options = [newVariable.current];
                if (newVariable.hide === VariableHide.dontHide || newVariable.hide === VariableHide.hideLabel) {
                    return Object.assign(Object.assign({}, newVariable), { type: 'textbox' });
                }
                return newVariable;
            });
        }
        ...
        if (oldVersion < 29) {
            for (const variable of this.dashboard.templating.list) {
                ...
                if ((_a = variable.options) === null || _a === void 0 ? void 0 : _a.length) {
                    variable.options = [];
                }
            }
        }
                    ...
                    if (panel.targets) {
                        ...
                        for (const target of panel.targets) {
                            if (target.datasource == null || target.datasource.uid == null) {
                                if (((_a = panel.datasource) === null || _a === void 0 ? void 0 : _a.uid) !== MIXED_DATASOURCE_NAME) {
                            ...
                            if (panelDataSourceWasDefault && ((_b = target.datasource) === null || _b === void 0 ? void 0 : _b.uid) !== '__expr__') {
        ...
        if (oldVersion < 37) {
            panelUpgrades.push((panel) => {
                var _a, _b, _c;
                if (((_a = panel.options) === null || _a === void 0 ? void 0 : _a.legend) &&
                ...
                else if ((_b = panel.options) === null || _b === void 0 ? void 0 : _b.legend) {
                    panel.options.legend = Object.assign(Object.assign({}, (_c = panel.options) === null || _c === void 0 ? void 0 : _c.legend), { showLegend: true });
                }
                return panel;
            });
        }
        ...
        if (oldVersion < 38) {
            panelUpgrades.push((panel) => {
                ...
                if (panel.type === 'table' && panel.fieldConfig !== undefined) {
                    const displayMode = (_b = (_a = panel.fieldConfig.defaults) === null || _a === void 0 ? void 0 : _a.custom) === null || _b === void 0 ? void 0 : _b.displayMode;
                    ...
                    if ((_c = panel.fieldConfig) === null || _c === void 0 ? void 0 : _c.overrides) {
                        for (const override of panel.fieldConfig.overrides) {
                            for (let j = 0; (_e = j < ((_d = override.properties) === null || _d === void 0 ? void 0 : _d.length)) !== null && _e !== void 0 ? _e : 0; j++) {
        ...
        for (const row of old.rows) {
            ...
            for (const panel of row.panels) {
                ...
                if (rowPanelModel && rowPanel.collapsed) {
                    (_a = rowPanelModel.panels) === null || _a === void 0 ? void 0 : _a.push(panel);
                }
                else {
    ...
    for (const old of oldMappings) {
        ...
        if (old.type && old.options) {
            ...
            if (old.type === MappingType.ValueToText) {
                valueMaps.options = Object.assign(Object.assign({}, valueMaps.options), old.options);
            }
            else {
...
function migrateTooltipOptions(panel) {
    if (panel.type === 'timeseries' || panel.type === 'xychart') {
        if (panel.options.tooltipOptions) {
            panel.options = Object.assign(Object.assign({}, panel.options), { tooltip: panel.options.tooltipOptions });
...
function ensureXAxisVisibility(panel) {
    ...
    if (panel.type === 'timeseries') {
        if (((_b = (_a = panel.fieldConfig) === null || _a === void 0 ? void 0 : _a.defaults.custom) === null || _b === void 0 ? void 0 : _b.axisPlacement) ===
            AxisPlacement.Hidden) {
            panel.fieldConfig = Object.assign(Object.assign({}, panel.fieldConfig), { overrides: [
