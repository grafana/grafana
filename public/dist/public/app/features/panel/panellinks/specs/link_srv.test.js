    ...
    describe('getDataLinkUIModel', () => {
        describe('built in variables', () => {
            it('should not trim white space from data links', () => {
                ...
                }, (v) => v, {}).href).toEqual('www.google.com?query=some query');
            });
            it('should remove new lines from data link', () => {
                expect(linkSrv.getDataLinkUIModel({
                    title: 'New line',
                    url: 'www.google.com?query=some\nquery',
                }, (v) => v, {}).href).toEqual('www.google.com?query=somequery');
            });
        });
        ...
        ${true}             | ${url}
        ${false}            | ${'about:blank'}
      `("when disable disableSanitizeHtml set to '$disableSanitizeHtml' then result should be '$expected'", ({ disableSanitizeHtml, expected }) => {
                updateConfig({
                    disableSanitizeHtml,
                });
      ...
      `("when link '$url' and config.appSubUrl set to '$appSubUrl' then result should be '$expected'", ({ url, appSubUrl, expected }) => {
...
describe('getDataFrameVars', () => {
    ...
    describe('when called with a DataFrame that contains fields with displayName', () => {
        it('then it should return correct suggestions', () => {
            ...
            frame.fields[1].config = Object.assign(Object.assign({}, frame.fields[1].config), { displayName: 'Indoor Temperature' });
            const suggestions = getDataFrameVars([frame]);
            expect(suggestions).toEqual([
                {
                    value: '__data.fields.time',
    ...
    describe('when called with a DataFrame that contains fields with duplicate names', () => {
        it('then it should ignore duplicates', () => {
            ...
            frame.fields[2].config = Object.assign(Object.assign({}, frame.fields[2].config), { displayName: 'Indoor Temperature' });
            const suggestions = getDataFrameVars([frame]);
            expect(suggestions).toEqual([
                {
                    value: '__data.fields.time',
