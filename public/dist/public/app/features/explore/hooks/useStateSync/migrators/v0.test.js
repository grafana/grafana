...
describe('v0 migrator', () => {
    describe('parse', () => {
        ...
        it('returns a valid Explore state from a compact URL parameter', () => {
            const paramValue = '["now-1h","now","Local",{"expr":"metric"},{"ui":[true,true,true,"none"],"__panelsState":{"logs":"1"}}]';
            expect(v0Migrator.parse({ left: paramValue })).toMatchObject({
                left: {
                    datasource: 'Local',
                    queries: [{ expr: 'metric' }],
        ...
        it('should not return a query for mode in the url', () => {
            ...
            const paramValue = '["now-1h","now","x-ray-datasource",{"queryType":"getTraceSummaries"},{"mode":"Metrics"},{"ui":[true,true,true,"none"]}]';
            expect(v0Migrator.parse({ left: paramValue })).toMatchObject({
                left: {
        ...
        it('should return queries if queryType is present in the url', () => {
            const paramValue = '["now-1h","now","x-ray-datasource",{"queryType":"getTraceSummaries"},{"ui":[true,true,true,"none"]}]';
            expect(v0Migrator.parse({ left: paramValue })).toMatchObject({
                left: {
                    datasource: 'x-ray-datasource',
                    queries: [{ queryType: 'getTraceSummaries' }],
...
//# sourceMappingURL=v0.test.js.map