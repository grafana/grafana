...
var setup = function (propOverrides) {
    var props = __assign({ pages: [
        ...
        ], currentPageIndex: 0, oldestLogsFirst: false, timeZone: 'local', loading: false, changeTime: jest.fn() }, propOverrides);
    return render(React.createElement(LogsNavigationPages, __assign({}, props)));
};
describe('LogsNavigationPages', function () {
    it('should render logs navigation pages', function () {
        setup();
        expect(screen.getByTestId('logsNavigationPages')).toBeInTheDocument();
    });
    it('should render logs pages with correct range if normal order', function () {
        setup();
    ...
    it('should render logs pages with correct range if flipped order', function () {
