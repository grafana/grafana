...
const setup = (rows) => render(React.createElement(LiveLogsWithTheme, { logRows: rows, timeZone: 'utc', stopLive: () => { }, onPause: () => { }, onResume: () => { }, onClear: () => { }, clearedAtIndex: null, isPaused: true }));
describe('LiveLogs', () => {
    it('renders logs', () => {
    ...
    it('renders new logs only when not paused', () => {
        ...
        rerender(React.createElement(LiveLogsWithTheme, { logRows: secondLogs, timeZone: 'utc', stopLive: () => { }, onPause: () => { }, onClear: () => { }, clearedAtIndex: null, onResume: () => { }, isPaused: true }));
        expect(screen.getByRole('cell', { name: 'log message 1' })).toBeInTheDocument();
        expect(screen.getByRole('cell', { name: 'log message 2' })).toBeInTheDocument();
        expect(screen.getByRole('cell', { name: 'log message 3' })).toBeInTheDocument();
        ...
        rerender(React.createElement(LiveLogsWithTheme, { logRows: secondLogs, timeZone: 'utc', stopLive: () => { }, onPause: () => { }, onResume: () => { }, onClear: () => { }, clearedAtIndex: null, isPaused: false }));
