...
const isClientOtel = (span) => span.kind === 'client';
const isClient = spanHasTag.bind(null, 'span.kind', 'client');
export const isClientSpan = (span) => isClientOtel(span) || isClient(span);
const isServerOtel = (span) => span.kind === 'server';
const isServer = spanHasTag.bind(null, 'span.kind', 'server');
export const isServerSpan = (span) => isServerOtel(span) || isServer(span);
const isErrorOtel = (span) => span.statusCode === 2;
const isErrorBool = spanHasTag.bind(null, 'error', true);
const isErrorStr = spanHasTag.bind(null, 'error', 'true');
export const isErrorSpan = (span) => isErrorOtel(span) || isErrorBool(span) || isErrorStr(span);
/**
...
export const isKindClient = (span) => {
    if (span.kind) {
