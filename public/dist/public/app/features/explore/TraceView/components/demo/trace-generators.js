...
function attachReferences(spans, depth, spansPerLevel) {
    ...
    return spans.map((span) => {
        ...
        return parentSpanId
            ? Object.assign(Object.assign({}, span), { references: [
                    {
                        refType: 'CHILD_OF',
...
export default chance.mixin({
    trace({ numberOfSpans = Math.max(Math.ceil(chance.pickone([
        chance.normal({ mean: 200, dev: 10 }),
    ...
    ), numberOfProcesses = chance.integer({ min: 1, max: 10 }), maxDepth = chance.integer({ min: 1, max: 10 }), spansPerLevel = null, }) {
        ...
        const processes = processArray.reduce((pMap, p) => (Object.assign(Object.assign({}, pMap), { [p.processID]: p })), {});
    ...
    span({ traceID = chance.guid(), processes = {}, traceStartTime = 0, traceEndTime = 0, operations = OPERATIONS_LIST, }) {
...
//# sourceMappingURL=trace-generators.js.map