...
it('getSpanReferenceByType() should return the span reference requested', () => {
    var _a;
    expect((_a = spanSelectors.getSpanReferenceByType({
        span: generatedTrace.spans[1],
        type: 'CHILD_OF',
    })) === null || _a === void 0 ? void 0 : _a.refType).toBe('CHILD_OF');
});
...
it('getSpanParentId() should return the spanID of the parent span', () => {
    expect(spanSelectors.getSpanParentId(generatedTrace.spans[1])).toBe(generatedTrace.spans[1].references.find(({ refType }) => refType === 'CHILD_OF').spanID);
});
