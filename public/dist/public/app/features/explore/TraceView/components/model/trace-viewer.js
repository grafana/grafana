...
export function _getTraceNameImpl(spans) {
    ...
    for (let i = 0; i < spans.length; i++) {
        const hasInternalRef = spans[i].references &&
        ...
        if (thisRefLength < candidateRefLength ||
...
export function findHeaderTags(spans) {
    for (let i = 0; i < spans.length; i++) {
...
export const getHeaderTags = memoize(findHeaderTags, (spans) => {
    if (!spans.length) {
        return 0;
    }
    return spans[0].traceID;
