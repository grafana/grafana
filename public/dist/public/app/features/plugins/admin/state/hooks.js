...
import { selectPlugins, selectById, selectIsRequestPending, selectRequestError, selectIsRequestNotFetched, selectDisplayMode, selectPluginErrors, } from './selectors';
export const useGetAll = (filters, sortBy = Sorters.nameAsc) => {
    useFetchAll();
...
export const useGetSingleLocalWithoutDetails = (id) => {
    useFetchAllLocal();
    return useSelector((state) => selectById(state, id));
};
export const useGetErrors = (filterByPluginType) => {
    useFetchAll();
    return useSelector(selectPluginErrors(filterByPluginType));
};
export const useInstall = () => {
    const dispatch = useDispatch();
    return (id, version, isUpdating) => dispatch(install({ id, version, isUpdating }));
};
...
export const useFetchDetailsLazy = () => {
    const dispatch = useDispatch();
    return (id) => dispatch(fetchDetails(id));
};
