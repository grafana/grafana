 ...
 */
export function transformPluginSourceForCDN({ url, source, transformSourceMapURL = false, transformAssets = true, }) {
    ...
    if (transformSourceMapURL) {
        newSource = newSource.replace(/(\/\/#\ssourceMappingURL=)(.+)\.map/g, `$1${baseAddress}/public/plugins/${pluginId}/$2.map`);
    }
    return newSource;
}
