...
export function DataSourcesListHeader() {
    ...
    const setSearchQuery = useCallback((q) => dispatch(setDataSourcesSearchQuery(q)), [dispatch]);
    const searchQuery = useSelector(({ dataSources }) => getDataSourcesSearchQuery(dataSources));
    const setSort = useCallback((sort) => dispatch(setIsSortAscending(sort.value === ascendingSortValue)), [dispatch]);
    const isSortAscending = useSelector(({ dataSources }) => getDataSourcesSort(dataSources));
    const sortPicker = {
        onChange: setSort,
        value: isSortAscending ? ascendingSortValue : descendingSortValue,
        getSortOptions: () => Promise.resolve(sortOptions),
    };
    return (React.createElement(PageActionBar, { searchQuery: searchQuery, setSearchQuery: setSearchQuery, key: "action-bar", sortPicker: sortPicker }));
}
//# sourceMappingURL=DataSourcesListHeader.js.map