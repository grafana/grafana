...
transformers['table'] = {
    ...
    getColumns: function (data) {
        ...
        if (data.length === 1) {
            return tslib_1.__spread(data[0].columns);
        }
    ...
    transform: function (data, panel, model) {
        ...
        var noTableIndex = _.findIndex(data, function (d) { return d.type !== 'table'; });
        if (noTableIndex > -1) {
            throw {
                message: "Result of query #" + String.fromCharCode(65 + noTableIndex) + " is not in table format, try using another transform.",
            };
        }
        mergeTablesIntoModel.apply(void 0, tslib_1.__spread([model], data));
    },
};
transformers['json'] = {
    ...
    transform: function (data, panel, model) {
        ...
        var i, y, z;
        try {
            for (var _b = tslib_1.__values(panel.columns), _c = _b.next(); !_c.done; _c = _b.next()) {
                var column = _c.value;
