jest.mock('app/core/utils/kbn', function () {
    return {
...
describe('AzureMonitorFilterBuilder', function () {
    var builder;
    var timefilter = 'timespan=2017-08-22T06:00:00Z/2017-08-22T07:00:00Z';
    var metricFilter = 'metricnames=Percentage CPU';
    beforeEach(function () {
        builder = new AzureMonitorFilterBuilder('Percentage CPU', moment.utc('2017-08-22 06:00'), moment.utc('2017-08-22 07:00'), 'PT1H', '3m');
    });
    describe('with a metric name and auto time grain of 3 minutes', function () {
        ...
        it('should always add datetime filtering and a time grain rounded to the closest allowed value to the filter', function () {
            var filter = timefilter + '&interval=PT5M&' + metricFilter;
    ...
    describe('with a metric name and auto time grain of 30 seconds', function () {
        ...
        it('should always add datetime filtering and a time grain in ISO_8601 format to the filter', function () {
            var filter = timefilter + '&interval=PT1M&' + metricFilter;
            expect(builder.generateFilter()).toEqual(filter);
        });
    });
    describe('with a metric name and auto time grain of 10 minutes', function () {
        ...
        it('should always add datetime filtering and a time grain rounded to the closest allowed value to the filter', function () {
            var filter = timefilter + '&interval=PT15M&' + metricFilter;
            expect(builder.generateFilter()).toEqual(filter);
        });
    });
    describe('with a metric name and auto time grain of 2 day', function () {
        ...
        it('should always add datetime filtering and a time grain rounded to the closest allowed value to the filter', function () {
            var filter = timefilter + '&interval=P1D&' + metricFilter;
            expect(builder.generateFilter()).toEqual(filter);
        });
    });
    describe('with a metric name and 1 hour time grain', function () {
        it('should always add datetime filtering and a time grain in ISO_8601 format to the filter', function () {
            var filter = timefilter + '&interval=PT1H&' + metricFilter;
            expect(builder.generateFilter()).toEqual(filter);
        });
    });
    ...
    describe('with a metric name and 1 day time grain and an aggregation', function () {
        ...
        it('should add time grain to the filter in ISO_8601 format', function () {
            var filter = timefilter + '&interval=P1D&aggregation=Maximum&' + metricFilter;
            expect(builder.generateFilter()).toEqual(filter);
        });
    });
    describe('with a metric name and 1 day time grain and an aggregation and a dimension', function () {
        ...
        it('should add dimension to the filter', function () {
            var filter = timefilter + '&interval=PT1H&' + metricFilter + "&$filter=aDimension eq 'aFilterValue'";
            expect(builder.generateFilter()).toEqual(filter);
        });
    });
});
//# sourceMappingURL=azure_monitor_filter_builder.test.js.map