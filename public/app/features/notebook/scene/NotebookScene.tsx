...
import { useIsNotebookRenderTarget } from './NotebookRenderTargetContext';
...
function NotebookSceneRenderer({ model }: SceneComponentProps<NotebookScene>) {
  ...
  // Real kiosk mode, i.e. a display someone has put a notebook on: the editing affordances below go
  // away, but the time controls stay, since a live display may well want the range visible. A PDF
  // capture is a separate question, answered by `isRenderTarget` below.
  const isKioskFull = kioskMode === KioskMode.Full;
  // From the tree, like `embedded` above: the render route provides it, and it is the route's
  // answer to give rather than something this component re-derives from the url.
  const isRenderTarget = useIsNotebookRenderTarget();

  return (
    <div className={styles.container}>
      <NotebookHiddenVariables model={model} />
      {/* Left out entirely when the notebook is being captured, rather than emptied: everything
            it holds is either an editing affordance or a time control, none of which mean anything
            in a document — and an empty row still spends its own padding, pushing the first heading
