...
import { useNotebookPickerData } from './useNotebookPickerData';
...
describe('useNotebookPickerData', () => {
  ...
  it('bounds the page so the server cannot silently truncate the library', () => {
    setupHook();

    expect(mockUseListNotebookQuery).toHaveBeenCalledWith({ limit: 500 });
  });
});
