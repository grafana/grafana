...
export function validateVariableSelection(args: {
  ...
  return (source) =>
    source.pipe(
      mergeMap(() => {
        const { dispatch, visitedVariables, variable, searchFilter } = args;
        ...
        if (!searchFilter) {
          return from(dispatch(validateVariableSelectionState(toKeyedVariableIdentifier(variable), visitedVariables)));
        }
