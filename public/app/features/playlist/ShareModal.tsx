...
export const ShareModal = ({ playlistUid, onDismiss }: Props) => {
  ...
  const modes: Array<SelectableValue<PlaylistMode>> = [
    { label: t('share-modal.playlist.mode-normal', 'Normal'), value: false },
    { label: t('share-modal.playlist.mode-tv', 'TV'), value: 'tv' },
    { label: t('share-modal.playlist.mode-kiosk', 'Kiosk'), value: true },
  ];
  ...
  return (
    <Modal isOpen={true} title={t('share-modal.playlist.title', 'Share playlist')} onDismiss={onDismiss}>
      <FieldSet>
        <Field label={t('share-modal.playlist.mode', 'Mode')}>
          <RadioButtonGroup value={mode} options={modes} onChange={setMode} />
        </Field>
        <Field>
          <Checkbox
            label={t('share-modal.playlist.checkbox-label', 'Autofit')}
            description={t(
              'share-modal.playlist.checkbox-description',
        ...
        <Field label={t('share-modal.playlist.link-url-label', 'Link URL')}>
          <Input
            id="link-url-input"
            value={shareUrl}
            readOnly
            addonAfter={
              <ClipboardButton icon="copy" variant="primary" getText={() => shareUrl}>
                <Trans i18nKey="share-modal.playlist.copy-link-button">Copy</Trans>
              </ClipboardButton>
            }
          />
        </Field>
