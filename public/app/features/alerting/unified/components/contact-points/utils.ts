...
import { type ReceiverPluginMetadata, getOnCallMetadata } from '../receivers/grafanaAppReceivers/useReceiversMetadata';

import { RECEIVER_META_KEY, RECEIVER_PLUGIN_META_KEY, RECEIVER_STATUS_KEY } from './constants';
import type { NotificationTemplate } from './useNotificationTemplates';
...
/** Plain-text summary of where notifications go (destination, addresses, etc.). Consumers may render the string or wrap it in richer React trees (e.g. icons). */
export function getReceiverDescription(receiver: ReceiverConfigWithMetadata): string | undefined {
...
export function isLegacyTemplate(template: NotificationTemplate): boolean {
  return template.kind === 'mimir';
}
