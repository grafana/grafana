...
const Policy = (props: PolicyComponentProps) => {
  ...
  const { filtersApplied: isFilteringResults, matchedRoutes, matchedRoutesWithPath } = routesMatchingFilters;

  //check if this route matches the filters
  const hasFocus = matchedRoutes.some((route) => route.id === currentRoute.id);
  ...
  const childPolicies = isFilteringResults
  ...
  const hideCurrentPolicyForFilters = isFilteringResults && !belongsToMatchPath;
  ...
  const childPoliciesToRender = !isFilteringResults
    ? childPolicies.slice(0, visibleChildPolicies) // no filters, show all
    : hasSomeChildPoliciesBelongingToMatchPath // if we have filters, show all that match the filters
