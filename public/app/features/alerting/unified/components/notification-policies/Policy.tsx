...
interface PolicyComponentProps {
  ...
  openDetailModal: (route: RouteWithID, isDefault?: boolean, readOnly?: boolean) => void;
  ...
  isAutogeneratedChunkOpen: boolean;
  toggleAutogeneratedChunk: () => void;
}
...
const Policy: FC<PolicyComponentProps> = ({
  ...
  isAutogeneratedChunkOpen,
  toggleAutogeneratedChunk,
}) => {
  ...
  const showEditAction = updatePoliciesSupported && updatePoliciesAllowed && !isAutoGenerated;
  const showDeleteAction = !isAutoGenerated && deletePolicySupported && deletePolicyAllowed && !isDefaultPolicy;
  const isAllowedToSeeAutogeneratedChunk = updatePoliciesSupported && updatePoliciesAllowed; //todo: ARE WE GOING TO HAVE ANOTHER PERMISSION FOR THIS?
  const showAutogeneratedPolicyChunk = !isAutoGenerated || (isAutoGenerated && isAllowedToSeeAutogeneratedChunk);

  // build the menu actions for our policy
  const dropdownMenuActions: JSX.Element[] = [];
  const readOnlyPolicy = !showEditAction;
      ...
      <Fragment key="edit-policy">
        <ConditionalWrap shouldWrap={provisioned} wrap={ProvisionedTooltip}>
          <Menu.Item
            ...
            label="Edit"
            onClick={() => openDetailModal(currentRoute, isDefaultPolicy, readOnlyPolicy)}
          />
        </ConditionalWrap>
      </Fragment>
    );
  ...
  return showAutogeneratedPolicyChunk ? (
    // we only render this policy if user can see the autogenerated chunk
    // and, this Policy will be collapsible only when it's the autogenerated root
    <ConditionalWrap
      shouldWrap={isAutoGeneratedRootAndSimplifiedEnabled(currentRoute)}
      wrap={(children) => CollapsiblePolicy(children, isAutogeneratedChunkOpen, toggleAutogeneratedChunk)}
    >
            ...
            <Stack direction="column" gap={1}>
              ...
              <div>
                <Stack direction="row" alignItems="center" gap={1}>
                  {isDefaultPolicy || isAutogeneratedPolicyRoot ? (
                    <DefaultPolicyIndicator isAutogeneratedDefaultPolicy={isAutogeneratedPolicyRoot} />
                  ) : hasMatchers ? (
            ...
            {childPolicies.map((child) => {
              ...
              return (
                <Policy
                  ...
                  readOnly={readOnly || provisioned || isAutoGenerated}
                  ...
                  // isAutoGenerated is the parent's , so if the parent is auto-generated then the child is too
                  isAutoGenerated={isAutoGenerated || isAutoGeneratedRootAndSimplifiedEnabled(child)}
                  isAutogeneratedChunkOpen={isAutogeneratedChunkOpen}
                  toggleAutogeneratedChunk={toggleAutogeneratedChunk}
                />
              );
            })}
          </>
...
const AUTOGENERATED_ROOT_LABEL_NAME = '__grafana_autogenerated__';

function isAutoGeneratedRootAndSimplifiedEnabled(route: RouteWithID) {
  ...
  return (
    route.object_matchers.some((objectMatcher) => {
      return (
        objectMatcher[0] === AUTOGENERATED_ROOT_LABEL_NAME && objectMatcher[1] === '=' && objectMatcher[2] === 'true'
      );
    }) ?? false
  );
  //  return route.receiver === 'contact_point_5';
}
...
function DefaultPolicyIndicator({ isAutogeneratedDefaultPolicy }: { isAutogeneratedDefaultPolicy: boolean }) {
  const styles = useStyles2(getStyles);
  if (isAutogeneratedDefaultPolicy) {
    return <strong>Default autogenerated policy</strong>;
  } else {
    return (
      <>
...
function CollapsiblePolicy(children: ReactNode, isOpen: boolean, onToggle: () => void) {
  return (
    <Collapse label={'Auto-generated policies'} collapsible isOpen={isOpen} onToggle={onToggle}>
      {children}
    </Collapse>
  );
}
