...
const Policy = (props: PolicyComponentProps) => {
  const {
    ...
    routeTree,
    inheritedProperties,
    routesMatchingFilters = { matchedRoutes: [], matchedRoutesWithPath: new Map<RouteWithID, RouteWithID[]>() },
  ...
  const hasFocus = routesMatchingFilters.matchedRoutes.some((route) => route.id === currentRoute.id);
  // check if this route belongs to a path that matches the filters
  const routesPath = Array.from(routesMatchingFilters.matchedRoutesWithPath.values()).flat();
  ...
  const childPolicies = hasSomeFilter
  ...
  const hideCurrentPolicyForFilters = hasSomeFilter && !belongsToMatchPath;
  ...
  //const childPoliciesMatchingFilters = childPolicies.filter((child) => routesMatchingFilters.routeIds.has(child.id));
  ...
  const hasSomeChildPoliciesBelongingToMatchPath = childPoliciesBelongingToMatchPath.length > 0;
  const childPoliciesToRender = !hasSomeFilter
    ? // no filters, show all
      childPolicies.slice(0, visibleChildPolicies)
    : // if we have filters, show all that match the filters
      hasSomeChildPoliciesBelongingToMatchPath
      ? childPoliciesBelongingToMatchPath.slice(0, visibleChildPolicies)
      : [];

  const showMore = hasSomeChildPoliciesBelongingToMatchPath
    ? visibleChildPolicies < childPoliciesBelongingToMatchPath.length
    : visibleChildPolicies < childPolicies.length;
  const showLess = visibleChildPolicies > POLICIES_PER_PAGE;
    ...
    <>
      <Stack direction="column" gap={1.5}>
        ...
        <div className={styles.childPolicies}>
          {renderChildPolicies && (
            <>
              {childPoliciesToRender.slice(0, visibleChildPolicies).map((child) => {
