...
interface PolicyComponentProps {
  ...
  testLabelsMatchingRouteId?: string | null;
}
...
const Policy = (props: PolicyComponentProps) => {
  const {
    ...
    testLabelsMatchingRouteId = null,
  } = props;
  ...
  const isTestingLabels = testLabelsMatchingRouteId !== null;

  const isInTestLabelsPath = (routeId: string): boolean => {
    if (!isTestingLabels) {
      return false;
    }
    if (routeId === testLabelsMatchingRouteId) {
      return true;
    }
    const route = allChildPolicies.find((r) => r.id === routeId);
    if (route?.routes) {
      return route.routes.some((child) => isInTestLabelsPath(child.id));
    }
    return false;
  };
  ...
  if (isTestingLabels) {
    childPolicies = allChildPolicies.filter(
      (policy) => policy.id === testLabelsMatchingRouteId || isInTestLabelsPath(policy.id)
    );
          ...
          <Stack direction="column" gap={1}>
            ...
            <div>
              <Stack direction="row" alignItems="center" gap={1}>
                ...
                {isTestingLabels && currentRoute.id === testLabelsMatchingRouteId && (
                  <Badge
            ...
            {pageOfChildren.map((child) => {
              ...
              return (
                <Policy
                  ...
                  routesMatchingFilters={routesMatchingFilters}
                  matchingInstancesPreview={matchingInstancesPreview}
                  testLabelsMatchingRouteId={testLabelsMatchingRouteId}
