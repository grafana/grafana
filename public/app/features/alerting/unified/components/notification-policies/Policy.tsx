...
const Policy = (props: PolicyComponentProps) => {
  ...
  const isTopLevelPolicy = depth === 0;
  const isGeneratedRoot = isTopLevelPolicy && currentRoute.id === 'generated-root';
  // Can't exist outside of depth 0 if viewing directly, or depth 1 if viewing as part of a generated root.
  const isManagedSubtree = currentRoute.name !== '' && depth === 0;
  const isUserDefinedManagedSubtree = isManagedSubtree && currentRoute.name === ROOT_ROUTE_NAME;

  // we collapse the auto-generated policies by default
  const isAutogeneratedPolicyRoot = isAutoGeneratedRoot(currentRoute);
  // const showChildren = !(isAutogeneratedPolicyRoot || (isManagedSubtree && !isUserDefinedManagedSubtree && (parentRoute?.routes?.length ?? 0) > 1));
  const [showPolicyChildren, togglePolicyChildren] = useToggle(showChildren);

  // Initial values only update on first render.
  // if (showChildren !== showPolicyChildren) {
  //   togglePolicyChildren(showChildren);
  // }
  ...
  const showMatchesAllLabelsWarning = !hasMatchers && !isDefaultPolicy && !continueMatching && !isUserDefinedManagedSubtree;
          ...
          <Stack direction="column" gap={1}>
            ...
            <div>
              <Stack direction="row" alignItems="center" gap={1}>
                ...
                {isUserDefinedManagedSubtree && (
                  <DefaultPolicyIndicator route={currentRoute} />
                )}
                {isManagedSubtree && !isUserDefinedManagedSubtree && (
                  <NamedPolicySubrouteIndicator route={currentRoute} />
                )}
                {!isAutogeneratedPolicyRoot && !isManagedSubtree && !isTopLevelPolicy && (
                  <>
                    {hasMatchers ? (
                      ...
                      <ConditionalWrap shouldWrap={provisioned} wrap={ProvisionedTooltip}>
                        { isGeneratedRoot ? (
                          <Button
                            ...
                            type="button"
                            onClick={() => onAddPolicy(currentRoute, 'child')} // TODO: Call api.
                          >
                            New managed subroute
                          </Button>
                        ) : (
                          isManagedSubtree || isTopLevelPolicy ? (
                            <Button
                              variant="secondary"
                              icon="plus"
                              size="sm"
        ...
        {showPolicyChildren && (
          <>
            {pageOfChildren.map((child) => {
              ...
              // const isManagedSubtree = child.name !== '' && depth === 0;
              // const isUserDefinedManagedSubtree = isManagedSubtree && child.name === ROOT_ROUTE_NAME;
              // const shouldShowChildren = !(isManagedSubtree && !isUserDefinedManagedSubtree && (currentRoute?.routes?.length ?? 0) > 1);
...
export const useCreateDropdownMenuActions = (
  ...
  const showExportAction = exportPoliciesAllowed && exportPoliciesSupported && (isDefaultPolicy || isNamedPolicySubroute) && !isAutoGenerated;
      ...
      <Fragment key="edit-policy">
        <ConditionalWrap shouldWrap={provisioned} wrap={ProvisionedTooltip}>
          <Menu.Item
            ...
            onClick={() => onEditPolicy(currentRoute, isDefaultPolicy || isNamedPolicySubroute)}
          />
        </ConditionalWrap>
      </Fragment>
    );
        ...
        <Fragment key="delete-policy">
          ...
          <ConditionalWrap shouldWrap={provisioned} wrap={ProvisionedTooltip}>
            <Menu.Item
              ...
              icon="trash-alt"
              disabled={provisioned || isAutoGenerated}
              label={'Delete managed subroute'}
              onClick={() => onDeletePolicy(currentRoute)} // TODO: Delete actual named subtree.
            />
          </ConditionalWrap>
        </Fragment>
      );
...
export const NAMED_ROOT_LABEL_NAME = '__grafana_named_subtree__';
...
const NamedPolicySubrouteIndicator: FC<DefaultPolicyIndicatorProps> = ({route}) => {
  const styles = useStyles2(getStyles);
  return (
    <>
      <Text element="h2" variant="body" weight="medium">{route.name == ROOT_ROUTE_NAME ? "User Defined" : (route.name ?? '')}</Text>
      <span className={styles.metadata}>{route.name == ROOT_ROUTE_NAME ? "Default subroute that matches when no prior managed subroutes do." : "Managed subroute"}</span>
    </>
  );
}
