...
const Policy = (props: PolicyComponentProps) => {
  ...
  const childrenNodesIds = currentRoute.routes?.map((route) => route.id) ?? [];
  const someChildrenMatchSearch =
    !isDefaultPolicy && routesMatchingFilters.some((route) => childrenNodesIds.includes(route.id));
  ...
  return (
    <>
      <Stack direction="column" gap={1.5}>
        <div
          className={styles.policyWrapper(hasFocus, someChildrenMatchSearch)}
        ...
        <div className={styles.childPolicies}>
          {renderChildPolicies && (
            <>
              {childPolicies.slice(0, visibleChildPolicies).map((child) => {
              ...
              <div className={styles.moreButtons}>
                ...
                {visibleChildPolicies > POLICIES_PER_PAGE && (
                  <Button
                    variant="secondary"
                    onClick={() => setVisibleChildPolicies(visibleChildPolicies - POLICIES_PER_PAGE)}
                  >
...
const getStyles = (theme: GrafanaTheme2) => ({
  ...
  policyWrapper: (hasFocus = false, someChildrenHasFocus = false) =>
    css({
      flex: 1,
      position: 'relative',
      background: theme.colors.background.secondary,
      ...
      ...((hasFocus || someChildrenHasFocus) && { borderColor: theme.colors.primary.border }),
    }),
  metadata: css({
