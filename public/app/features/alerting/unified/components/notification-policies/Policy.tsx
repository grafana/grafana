...
interface PolicyComponentProps {
  ...
  testLabelsMatchResult?: { matchingRouteId: string; pathRouteIds: Set<string> } | null;
}
...
const Policy = (props: PolicyComponentProps) => {
  ...
  const isTestingLabels = testLabelsMatchResult !== null;

  // filter child policies that match
  let childPolicies: RouteWithID[];
  if (isTestingLabels) {
    childPolicies = allChildPolicies.filter((policy) => testLabelsMatchResult.pathRouteIds.has(policy.id));
  } else if (filtersApplied) {
    childPolicies = allChildPolicies.filter((policy) =>
      routesPath.some((route: RouteWithID) => route.id === policy.id)
    );
  } else {
          ...
          <Stack direction="column" gap={1}>
            ...
            <>
              ...
              <Stack direction="row" alignItems="center" gap={1} wrap="wrap-reverse">
                ...
                {/* Show indicator when this policy matches the test labels */}
                {isTestingLabels && currentRoute.id === testLabelsMatchResult.matchingRouteId && (
                  <Badge
                    text={t('alerting.policies.test-labels-match', 'Alert would be routed here')}
                    color="green"
                    icon="check"
                  />
                )}
