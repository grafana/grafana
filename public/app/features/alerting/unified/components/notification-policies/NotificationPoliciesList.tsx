...
import { parseBackendLabelFilters } from '../rules/central-state-history/utils';
...
export const NotificationPoliciesList = () => {
  ...
  const { getRouteGroupsMap, matchInstancesToRoute } = useRouteGroupsMatcher();
  ...
  const [{ value: testLabelsMatchingMap }, triggerTestLabelsMatching] = useAsyncFn(matchInstancesToRoute, [
    matchInstancesToRoute,
  ]);

  const testLabels = useMemo(() => {
    return testLabelsFilter ? parseBackendLabelFilters(testLabelsFilter) : null;
  }, [testLabelsFilter]);

  useEffect(() => {
    if (rootRoute && testLabels) {
      triggerTestLabelsMatching(rootRoute, [testLabels], { unquoteMatchers: !isGrafanaAlertmanager });
    }
  }, [rootRoute, testLabels, triggerTestLabelsMatching, isGrafanaAlertmanager]);

  const testLabelsMatchingRouteId = useMemo(() => {
    if (!testLabelsMatchingMap || !testLabels) {
      return null;
    }

    for (const [routeId, matches] of testLabelsMatchingMap.entries()) {
      if (matches.length > 0) {
        return routeId;
      }
    }
    return null;
  }, [testLabelsMatchingMap, testLabels]);
