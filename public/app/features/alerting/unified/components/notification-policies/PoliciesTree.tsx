...
import { type Label, computeInheritedTree, findMatchingRoutes } from '@grafana/alerting';
...
export const PoliciesTree = ({ routeName }: PoliciesTreeProps) => {
  ...
  const [testLabelsFilter, setTestLabelsFilter] = useState<string | undefined>();
  ...
  const testLabelsMatchResult = useMemo(() => {
    if (!rootRoute || !testLabelsFilter) {
      return null;
    }

    const matchers = parsePromQLStyleMatcherLooseSafe(testLabelsFilter);
    const labels: Label[] = matchers.filter((m) => !m.isRegex && m.isEqual).map((m) => [m.name, m.value]);

    if (labels.length === 0) {
      return null;
    }

    const packageRoute = routeAdapter.toPackage(rootRoute);
    const results = findMatchingRoutes(packageRoute, labels);

    if (results.length === 0) {
      return null;
    }

    const matchingRouteId = results[0].route.id;
    const pathRouteIds = new Set(
      results[0].matchingJourney.filter((info) => info.matched).map((info) => info.route.id)
    );

    return { matchingRouteId, pathRouteIds };
  }, [rootRoute, testLabelsFilter]);
    ...
    <>
      ...
      {hasPoliciesData && (
        <Stack direction="column" gap={1}>
          <AlertRoutingTest onChangeTestLabels={setTestLabelsFilter} />
          <NotificationPoliciesFilter
          ...
          <Policy
            ...
            testLabelsMatchResult={testLabelsMatchResult}
