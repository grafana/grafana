...
import { DEFAULT_ROUTING_TREE_NAME_ALIAS, USER_DEFINED_TREE_NAME, findMatchingRoutes } from '@grafana/alerting';
import {
  API_GROUP,
  API_VERSION,
  type RoutingTree,
...
describe('k8sRouteToRoute default-tree matcher', () => {
  ...
  it.each(['user-defined', 'default', '', undefined])(
    'emits the empty catch-all matchers for the default tree named %p',
    (name) => {
      const result = k8sRouteToRoute(treeNamed(name));
      expect(result.object_matchers).toEqual([
  ...
  it('does not send the root matchers back to the API', () => {
    const spec = createKubernetesRoutingTreeSpec(k8sRouteToRoute(treeNamed('user-defined')));
    expect(spec.spec.defaults).not.toHaveProperty('object_matchers');
    expect(spec.spec.defaults).not.toHaveProperty('matchers');
  });
});

describe('default tree matching for simplified routing', () => {
  const defaultTree: RoutingTree = {
    apiVersion: `${API_GROUP}/${API_VERSION}`,
    kind: 'RoutingTree',
  ...
  it('does not match an instance delivered by simplified routing', () => {
    const root = routeAdapter.toPackage(normalizeRoute(addUniqueIdentifierToRoute(k8sRouteToRoute(defaultTree))));

    const matches = findMatchingRoutes(root, [
      ['alertname', 'asserts_up'],
      [AUTOGENERATED_ROOT_LABEL_NAME, 'true'],
      ['__grafana_receiver__', 'asserts-contact-point'],
    ]);

    expect(matches).toHaveLength(0);
  });

  it('still matches an instance that goes through the notification policy tree', () => {
    const root = routeAdapter.toPackage(normalizeRoute(addUniqueIdentifierToRoute(k8sRouteToRoute(defaultTree))));

    const matches = findMatchingRoutes(root, [['alertname', 'asserts_up']]);

    expect(matches).toHaveLength(1);
    expect(matches[0].route.receiver).toBe('grafana-default-email');
  });
});
