...
export enum StdFunctions {
  extVar = 'extVar',
}

export const availableAlertManagerFunctions = Object.values(AlertmanagerTemplateFunction);
export const availableStdFunctions = Object.values(StdFunctions);

// built-in functions for Jsonnet
export const builtinFunctions = ['std'];

// Go template keywords
export const keywords = ['if', 'else', 'end', 'range', 'break', 'continue', 'true', 'false', 'local'];
...
export const language: monacoType.languages.IMonarchLanguage = {
  ...
  keywords: keywords,
  functions: [...builtinFunctions, ...availableAlertManagerFunctions, ...availableStdFunctions],
  operators: ['|'],
  tokenizer: {
    root: [
      // strings
      ...
      [/-?}}/, TokenType.Delimiter],
      // variables
      [/local ([A-Za-z]+)?/, TokenType.Variable],
