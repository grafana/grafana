...
import { useCreateOrUpdateContactPoint } from 'app/features/alerting/unified/components/contact-points/useContactPoints';
...
export const CloudReceiverForm = ({ contactPoint, alertManagerSourceName, readOnly = false }: Props) => {
  ...
  const createOrUpdateContactPoint = useCreateOrUpdateContactPoint({ alertmanager: alertManagerSourceName });
  ...
  const onSubmit = async (values: ReceiverFormValues<CloudChannelValues>) => {
    ...
    await createOrUpdateContactPoint({ contactPoint: newReceiver, existingName: contactPoint?.name });
