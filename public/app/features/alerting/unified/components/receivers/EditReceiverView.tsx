...
export const EditReceiverView = ({ contactPoint, alertmanagerName }: Props) => {
  ...
  if (alertmanagerName === GRAFANA_RULES_SOURCE_NAME) {
    return <GrafanaReceiverForm contactPoint={contactPoint} readOnly={readOnly} />;
  } else {
    return (
      <CloudReceiverForm alertManagerSourceName={alertmanagerName} contactPoint={contactPoint} readOnly={readOnly} />
    );
  }
};
