...
import { isInternalRootMatcher } from '../../notification-policies/useNotificationPolicyRoute';
...
export function JourneyPolicyCard({ route, isRoot = false, isFinalRoute = false, policyName }: JourneyPolicyCardProps) {
  ...
  // Convert route matchers to ObjectMatcher format, hiding the internal routing labels we add to a root route
  // (__grafana_managed_route__ and __grafana_autogenerated__) since they aren't part of the stored configuration.
  const matchers: ObjectMatcher[] = (route.matchers?.map(labelMatcherToObjectMatcher) ?? []).filter(
    (m) => !isRoot || !isInternalRootMatcher(m)
  );
