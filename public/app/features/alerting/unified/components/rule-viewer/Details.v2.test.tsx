...
describe('Details v2', () => {
  ...
  it('should render evaluation section first with interval', () => {
    const rule = mockCombinedRule({
      rulerRule: alertingFactory.ruler.grafana.alertingRule.build(),
      group: { name: 'test-group', interval: '5m', rules: [], totals: {} },
    });
  ...
  it('should render rule metadata section last with title', () => {
    const rule = mockCombinedRule({
      rulerRule: alertingFactory.ruler.grafana.alertingRule.build(),
    });
    ...
    expect(screen.getByText('Rule metadata')).toBeInTheDocument();
    expect(screen.getByText('Grafana-managed alert rule')).toBeInTheDocument();
  });
