...
export const FolderActionsButton = ({ folderUID }: Props) => {
  ...
  const onConfirmDelete = async () => {
    ...
    const message = getFolderActionResultMessage('delete', result.deleted ?? 0, result.skipped ?? 0);
        ...
        <>
          <PauseUnpauseActionMenuItem
            ...
            executeAction={async (folderUID) => {
              ...
              const message = getFolderActionResultMessage('pause', result.updated ?? 0, result.skipped ?? 0);
              appEvents.emit(AppEvents.alertSuccess, [message]);
              await redirectToListView();
            }}
          ...
          <PauseUnpauseActionMenuItem
            ...
            executeAction={async (folderUID) => {
              ...
              const message = getFolderActionResultMessage('resume', result.updated ?? 0, result.skipped ?? 0);
              appEvents.emit(AppEvents.alertSuccess, [message]);
              await redirectToListView();
            }}
