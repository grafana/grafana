...
const AmRoutes = () => {
  ...
  const routesMatchingFilters = useMemo(() => {
    if (!rootRoute) {
      const emptyResult: RoutesMatchingFilters = { matchedRoutes: [], matchedRoutesWithPath: new Map() };
      return emptyResult;
    }
                ...
                <Stack direction="column" gap={1}>
                  ...
                  {rootRoute && (
                    <Policy
                      ...
                      hasSomeFilter={Boolean(contactPointFilter) || labelMatchersFilter.length > 0}
                    />
                  )}
                </Stack>
                {addModal}
...
export const findRoutesMatchingFilters = (rootRoute: RouteWithID, filters: RouteFilters): RoutesMatchingFilters => {
  ...
  if (!hasFilter) {
    return { matchedRoutes: [], matchedRoutesWithPath: new Map() };
  }
  ...
  return { matchedRoutes: intersectedRoutes, matchedRoutesWithPath: routesForAllFilterResults };
};
