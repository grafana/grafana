...
/**
 * Grouped-view folder card backed by the single cross-kind k8s `/search` endpoint. Unlike
 * {@link K8sFolderCard} (which issues two list queries and offers six split layouts), this
 * renders one flat, definition-only list mixing alert + recording rules — demonstrating the
 * `/search` path. Selected via the "Single /search request" recording-split mode.
 */
      ...
      <div className={styles.folderHead}>
        <Stack alignItems="center">
          ...
          <span className={styles.frules}>
            {t('alerting.k8s-folder.rule-count', '{{count}} rules', { count: countLabel })}
          </span>
          ...
          {hasMore && (
            <div className={styles.loadMore}>
              ...
              <span className={styles.loadMoreMeta}>
                {t('alerting.k8s-folder.loaded-count', '{{count}} loaded', { count: loadedCount })}
              </span>
            </div>
          )}
        </div>
