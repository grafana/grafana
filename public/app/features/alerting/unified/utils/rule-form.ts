...
/**
 * Converts a standalone VizPanel to alert rule form values.
 *
 ...
 *
 * Key differences from `scenesPanelToRuleFormValues`:
 * - Does NOT require a dashboard parent (no getDashboardSceneFor call)
 * - Does NOT auto-populate folder (user selects in alert form)
 * - Does NOT add __dashboardUid__ or __panelId__ annotations
 *
 * Use `scenesPanelToRuleFormValues` when the panel IS part of a dashboard.
 * Use this function when the panel is standalone (e.g., from a drilldown plugin app).
 */
export const vizPanelToRuleFormValues = async (vizPanel: VizPanel): Promise<Partial<RuleFormValues> | undefined> => {
  if (!vizPanel.state.key) {
    return undefined;
  }
  ...
  // No alerting-capable queries found
  if (!grafanaQueries.length || !grafanaQueries.find((query) => query.datasourceUid !== ExpressionDatasourceUID)) {
    return undefined;
  }
  ...
  return {
    type: RuleFormType.grafana,
    // folder: undefined - user selects in alert form
    queries: grafanaQueries,
    name: vizPanel.state.title || '',
    condition: grafanaQueries[grafanaQueries.length - 1].refId,
    // No dashboard/panel annotations for standalone panels
    annotations: [],
  };
};
