...
export const getTimeRangeForExpression = (query: ExpressionQuery, queries: GrafanaAlertQuery[]): RelativeTimeRange => {
...
const getReferencedIds = (model: ExpressionQuery, queries: GrafanaAlertQuery[]): string[] | undefined => {
  switch (model.type) {
...
const getTimeRanges = (referencedRefIds: string[], queries: GrafanaAlertQuery[]) => {
  let from: number[] = [];
...
const getReferencedIdsForMath = (model: ExpressionQuery, queries: GrafanaAlertQuery[]) => {
  return (
    queries
