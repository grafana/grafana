...
export async function getAlertmanagerConfig(alertmanagerSourceName: string): Promise<AlertmanagerCortexConfig> {
  const conf: AlertmanagerCortexConfigDTO = await getBackendSrv().get(`/alertmanager/${getDatasourceAPIId(alertmanagerSourceName)}/config/api/v1/alerts`);
  return {
...
export async function updateAlertmanagerConfig(alertmanagerSourceName: string, config: AlertmanagerCortexConfig): Promise<void> {
  ...
  await getBackendSrv().post(`/alertmanager/${getDatasourceAPIId(alertmanagerSourceName)}/config/api/v1/alerts`, payload)
}