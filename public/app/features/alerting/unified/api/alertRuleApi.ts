...
function getProvisioningRuleExportUrl(ruleUid: string, format: 'yaml' | 'json' | 'hcl' = 'yaml') {
...
export const alertRuleApi = alertingApi.injectEndpoints({
  endpoints: (build) => ({
    ...
    exportRule: build.query<string, { uid: string; format: ExportFormats }>({
      query: ({ uid, format }) => ({ url: getProvisioningRuleExportUrl(uid, format), responseType: 'text' }),
    }),
