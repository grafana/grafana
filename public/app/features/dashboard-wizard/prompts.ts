...
function formatSectionForPrompt(section: WizardSummarySection, indent: string, lines: string[]): void {
  const sectionRepeat = section.repeatFor ? `, repeated for each value of $${section.repeatFor}` : '';
  lines.push(`${indent}${section.kind === 'tab' ? 'Tab' : 'Row'} "${section.title}"${sectionRepeat}:`);
  for (const panel of section.panels) {
    const notes = [panel.visualization, panel.repeatFor ? `repeated for each value of $${panel.repeatFor}` : '']
      .filter((note) => note !== '')
      .join('; ');
    lines.push(`${indent}  - ${panel.title}${notes ? ` (${notes})` : ''}`);
  }
...
function formatSummaryForPrompt(summary: WizardSummary): string {
  ...
  if (summary.variables.length > 0) {
    lines.push(`Template variables: ${summary.variables.map((name) => `$${name}`).join(', ')}`);
  }
  return lines.join('\n');
}

const JSON_ONLY_RULE =
  'Your ENTIRE response must be a single JSON object matching the schema below — no markdown fences, no prose, ' +
  'no commentary. NEVER reply conversationally and NEVER ask clarifying questions as text. When the request is ' +
  'ambiguous, underspecified, or refers to something you could not resolve, still return the complete JSON ' +
  'object: make the best plan you can from verified data and express every clarification through the ' +
  '"questions" array, using real values you verified with the tools as the answer options.';
...
- Showcase interactivity: template variables that scope the dashboard (datasource, namespace, instance — chained where the data supports it), reused in queries and legends — and where the data has a natural per-value structure, one row or tab repeated by a variable instead of hand-built duplicates.`;
...
${JSON_ONLY_RULE}
{
  ...
  "summary": {
    ...
    "description": string,
    "structure": "tabs" | "rows",
    "sections": [{ "title": string, "kind": "tab" | "row", "repeatFor": string /* optional */, "panels": [{ "title": string, "visualization": string, "repeatFor": string /* optional */ }], "sections": [ /* optional: nested sections of the same shape, INSTEAD of "panels" */ ] }],
    "variables": [string]
  },
  "dataNotes": string,
...
- "summary": a plain-language preview of the dashboard laid out panel by panel, shown to the user to review before building. It must describe the SAME dashboard as "prompt".
  ...
  - "variables": the template variables the dashboard will offer to scope the panels (e.g. "namespace", "instance"), as bare label names without a $ prefix, in the order they'll appear. Only list variables for labels you verified exist on the planned metrics — never invent one. Empty array when the dashboard needs none.
  - "repeatFor" (optional, on a section or a panel): a variable name from "variables" whose every value gets its own copy of that section or panel. PREFER repetition over enumeration: when the dashboard covers several instances of the same kind of thing and their panels would be identical apart from one label value — one row per namespace, one tab per cluster, one panel per instance — plan ONE section or panel with "repeatFor" instead of near-duplicate copies. Put the variable in the repeated section's title (e.g. "Latency — $endpoint") so each copy identifies itself. Only repeat by a variable listed in "variables" whose label you verified on the planned metrics, and never nest repeats — nothing inside a repeating section may set "repeatFor". Leave it out when the sections genuinely differ.
...
- "questions": 0-3 clarifying questions, ONLY when the answer genuinely changes the dashboard (e.g. which environment, cluster, namespace, or service to focus on; fleet-wide vs single-instance). If the request leaves a value unresolved (a placeholder, a name you could not find, a vague reference), resolve it with a question: look up the real candidates with list_label_values and offer those as the options. Each question has a short slug "id" and 2-5 short answer options — real values you verified beat generic placeholders. No free-text questions, no questions about layout/visualization taste, and never ask about something the request or attached context already answers. If the request is already specific enough to build a good dashboard, return [].
...
- A section or panel marked "repeated for each value of $var" must be realized with the layout repeat option: create it ONCE and set its repeat to that variable (which must exist as a dashboard variable, defined before the section that repeats by it). Its queries filter by $var so each copy shows only that value's data. NEVER hand-build one copy per value.`
    : '- Structure the dashboard into sections — never one flat grid of panels. An overview row of KPI stats first, then detail sections grouped by domain (e.g. traffic, errors, latency, resources); use tabs when the domains are distinct, and auto grid inside every section so panels arrange themselves. Aim for roughly 8-16 panels across 3-5 sections — deep enough to be genuinely useful, without filler panels. When several instances of the same kind of thing need identical panels (per namespace, per service, per instance), prefer ONE row, tab, or panel repeated by a template variable over hand-built near-duplicates.';
