...
describe('FolderReadmeHint', () => {
  ...
  it('renders an inline link that scrolls to the README panel anchor', () => {
    setReadmeResult();

    renderWithRouter(<FolderReadmeHint folderUID="test-folder" />);
    ...
    // The TextLink resolves the hash against the current path; we only care
    // that it ends with the panel's anchor.
  ...
  it('renders nothing when the feature toggle is off', () => {
    ...
    const { container } = renderWithRouter(<FolderReadmeHint folderUID="test-folder" />);
