...
/**
 * Build a URL that opens a specific file in the provider's web editor.
 * GitHub and GitLab support this directly. Bitbucket has no public edit URL,
 * so we fall back to the source view, where the user can click Edit themselves.
 */
 ...
 * supplied path. GitHub and GitLab support a true new-file URL with prefilled
 * content; Bitbucket has no documented new-file URL, so we link to the source
 * view of the parent directory at the target branch and let the user create
 * the file from there.
 */
export function getRepoNewFileUrl({
  ...
  switch (repoType) {
    ...
    case 'bitbucket': {
      // Bitbucket has no public new-file URL with prefilled content.
      // Link to the parent directory's source view so the user can use the UI
      // to add the file.
