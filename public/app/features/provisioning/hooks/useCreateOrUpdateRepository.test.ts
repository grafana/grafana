...
describe('useCreateOrUpdateRepository', () => {
  ...
  it('includes commitSigningKey in secure payload when provided', async () => {
    const captured = captureRequests();
    const { result } = renderHook(() => useCreateOrUpdateRepository(), { wrapper: getWrapper({}) });
    ...
    await waitFor(() => expect(captured.create).toBeDefined());
    expect(captured.create?.secure).toEqual({ commitSigningKey: { create: 'PGP-KEY' } });
  });

  it('includes both token and commitSigningKey when both provided', async () => {
    const captured = captureRequests();
    const { result } = renderHook(() => useCreateOrUpdateRepository(), { wrapper: getWrapper({}) });
    ...
    expect(captured.create?.secure).toEqual({
      ...
      commitSigningKey: { create: 'PGP-KEY' },
    });
  });

  it('omits secure when neither token nor commitSigningKey provided', async () => {
    const captured = captureRequests();
    const { result } = renderHook(() => useCreateOrUpdateRepository(), { wrapper: getWrapper({}) });
