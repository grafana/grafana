...
export class K8sDashboardAPI implements DashboardAPI<DashboardDTO, Dashboard> {
  ...
  saveDashboard(options: SaveDashboardCommand<Dashboard>): Promise<SaveDashboardResponseDTO> {
    ...
    const queryParams = '?fieldValidation=Ignore';

    if (dashboard.uid) {
      obj.metadata.name = dashboard.uid;
      return this.client.update(obj, queryParams).then((v) => this.asSaveDashboardResponseDTO(v));
    }
    return this.client.create(obj, queryParams).then((v) => this.asSaveDashboardResponseDTO(v));
  }
