 ...
 *
 * This test compares backend and frontend migration results for all community dashboards
...
describe('Community Dashboard Backend / Frontend result comparison', () => {
  beforeEach(() => {
    jest.clearAllMocks();
  ...
  const { inputDir: communityDashboardInputDir } = getCommunityDashboardTestDirectories();
  const communityDashboardOutputDir = getOutputDirectory('community-dashboards');

  // Find all JSON files in community-dashboards directory
  const communityDashboardFiles = findJSONFiles(communityDashboardInputDir);

  communityDashboardFiles.forEach((inputFile) => {
    const relativeInputPath = path.relative(communityDashboardInputDir, inputFile);

    it(`should migrate community dashboard ${relativeInputPath} correctly`, async () => {
      ...
      const backendOutputPath = getRelativeOutputPath(
        inputFile,
        communityDashboardInputDir,
        communityDashboardOutputDir
      );
