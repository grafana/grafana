...
import {
  ...
  getHistoricalDevDashboardTestDirectories,
  getOutputDirectory,
} from './__tests__/migrationTestUtils';

/*
 * Historical Dev Dashboard Backend / Frontend Migration Comparison Test
 *
 * This test compares backend and frontend migration results for all historical dev dashboards
...
describe('Historical Dev Dashboard Backend / Frontend result comparison', () => {
  beforeEach(() => {
    jest.clearAllMocks();
    // Setup dev dashboard datasources to match backend DevDashboardConfig
    setupDevDashboardDataSources();
  });

  const { inputDir: historicalDevDashboardInputDir } = getHistoricalDevDashboardTestDirectories();
  const historicalDevDashboardOutputDir = getOutputDirectory('historical-dev-dashboards');

  // Find all JSON files in historical-dev-dashboards directory
  const historicalDevDashboardFiles = findJSONFiles(historicalDevDashboardInputDir);

  historicalDevDashboardFiles.forEach((inputFile) => {
    const relativeInputPath = path.relative(historicalDevDashboardInputDir, inputFile);

    it(`should migrate historical dev dashboard ${relativeInputPath} correctly`, async () => {
      ...
      const backendOutputPath = getRelativeOutputPath(
        inputFile,
        historicalDevDashboardInputDir,
        historicalDevDashboardOutputDir
      );
