...
export class HistorySrv {
  getHistoryList(dashboard: DashboardModel, options: HistoryListOpts) {
    ...
    return id ? backendSrv.get(`api/dashboards/id/${id}/versions`, options) : Promise.resolve([]);
  }

  calculateDiff(options: CalculateDiffOptions) {
    return backendSrv.post('api/dashboards/calculate-diff', options);
  }
  ...
  restoreDashboard(dashboard: DashboardModel, version: number) {
    ...
    return id && _.isNumber(version) ? backendSrv.post(url, { version }) : Promise.resolve({});
  }
}
