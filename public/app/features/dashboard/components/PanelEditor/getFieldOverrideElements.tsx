...
export function getFieldOverrideCategories2(
  searchQuery: string,
  panel: VizPanel,
  panelData: PanelData
): OptionsPaneCategoryDescriptor[] {
  const registry = panel.getPlugin()?.fieldConfigRegistry;
  ...
  const overrides = currentFieldConfig.overrides;

  const onOverrideChange = (index: number, override: ConfigOverrideRule) => {
    const overridesClone = cloneDeep(overrides);
    overridesClone[index] = override;
    panel.setState({ fieldConfig: { ...currentFieldConfig, overrides: overridesClone } });
  };

  const onOverrideRemove = (overrideIndex: number) => {
    const overridesClone = cloneDeep(overrides);
    overridesClone.splice(overrideIndex, 1);
    panel.setState({ fieldConfig: { ...currentFieldConfig, overrides: overridesClone } });
  };
  ...
  for (let idx = 0; idx < currentFieldConfig.overrides.length; idx++) {
    ...
    const category = new OptionsPaneCategoryDescriptor({
      title: overrideName,
      id: overrideName,
      forceOpen: override.properties.length === 0 ? 1 : 0,
      renderTitle: function renderOverrideTitle(isExpanded: boolean) {
        return (
          <OverrideCategoryTitle
            override={override}
