...
function calculateDashboardHeight(dashboard: Dashboard): number {
  ...
  const GRID_UNIT_HEIGHT = 30;
  const totalHeight = maxY * GRID_UNIT_HEIGHT + maxHeight * GRID_UNIT_HEIGHT;

  // Add padding and margins
  return totalHeight + TOP_PADDING + BOTTOM_PADDING + ROW_MARGIN * Math.ceil(maxY / 24);
}

export function buildDashboardImageUrl(
  useCurrentTimeRange: boolean,
  dashboardUid: string,
  selectedTheme?: string,
  dashboard?: Dashboard
  ...
  baseUrl = baseUrl.replace(config.appSubUrl + '/d-solo/', config.appSubUrl + '/d/');

  // Add render prefix
  baseUrl = baseUrl.replace(config.appSubUrl + '/d/', config.appSubUrl + '/render/d/');

  // Add required parameters
  const params = new URLSearchParams();
  params.set('orgId', String(config.bootData.user.orgId));
  params.set('theme', selectedTheme || 'light');
  params.set('kiosk', 'true');
  params.set('fullPageImage', 'true');

  // Calculate and set height if dashboard is provided
  if (dashboard) {
    const height = calculateDashboardHeight(dashboard);
    params.set('height', String(height));
  }

  // Add scale and timezone
  params.set('scale', String(config.rendererDefaultImageScale));
  params.set('timezone', getLocalTimeZone());
