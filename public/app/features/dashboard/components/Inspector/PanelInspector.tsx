...
export class PanelInspector extends PureComponent<Props, State> {
  ...
  async componentDidMount() {
    ...
    const errors = getDataQueryErrors(data);
    ...
    this.setState(prevState => ({
      ...
      data,
      metaDS,
      tab: errors ? InspectTab.Error : prevState.tab,
    }));
  }
  ...
  render() {
    const { panel } = this.props;
    const { last, tab, data } = this.state;
    const styles = getStyles();
    const errors = getDataQueryErrors(data);
              ...
              {({ width, height }) => {
                ...
                return (
                  <div style={{ width, height }}>
                    ...
                    {tab === InspectTab.Error && this.renderErrorTab(errors[0])}
                  </div>
                );
              }}
            </AutoSizer>
