...
describe('findLatestDatasourceCheck', () => {
  ...
  it('should return undefined when no datasource checks exist', () => {
    const items = [
      makeCheck({ metadata: { name: 'c1', labels: { 'advisor.grafana.app/type': 'plugin' }, creationTimestamp: '2024-01-01T00:00:00Z', annotations: {} } }),
    ];
    expect(findLatestDatasourceCheck(items)).toBeUndefined();
  });

  it('should return the only datasource check', () => {
    const check = makeCheck({ metadata: { name: 'ds-check', labels: { 'advisor.grafana.app/type': 'datasource' }, creationTimestamp: '2024-01-01T00:00:00Z', annotations: {} } });
    expect(findLatestDatasourceCheck([check])).toBe(check);
  });

  it('should return the latest datasource check by creationTimestamp', () => {
    const older = makeCheck({ metadata: { name: 'old', labels: { 'advisor.grafana.app/type': 'datasource' }, creationTimestamp: '2024-01-01T00:00:00Z', annotations: {} } });
    const newer = makeCheck({ metadata: { name: 'new', labels: { 'advisor.grafana.app/type': 'datasource' }, creationTimestamp: '2024-06-01T00:00:00Z', annotations: {} } });
  ...
  it('should ignore non-datasource checks when finding latest', () => {
    const plugin = makeCheck({ metadata: { name: 'plugin', labels: { 'advisor.grafana.app/type': 'plugin' }, creationTimestamp: '2025-01-01T00:00:00Z', annotations: {} } });
    const ds = makeCheck({ metadata: { name: 'ds', labels: { 'advisor.grafana.app/type': 'datasource' }, creationTimestamp: '2024-01-01T00:00:00Z', annotations: {} } });
