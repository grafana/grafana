...
export const convertDataQueryResponseToQueryTemplates2 = (result: ListQueryTemplateApiResponse): QueryTemplate[] => {
  if (!result.items) {
    return [];
  }
  return result.items.map((spec) => {
...
export const convertAddQueryTemplateCommandToDataQuerySpec2 = (
  addQueryTemplateCommand: AddQueryTemplateCommand
): ComGithubGrafanaGrafanaPkgApisPeakqV0Alpha1QueryTemplate => {
  const { title, targets } = addQueryTemplateCommand;
  return {
