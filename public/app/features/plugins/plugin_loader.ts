...
import { DataSourcePlugin, AppPlugin, VizPanelPlugin, AngularPanelPlugin, PluginMeta } from '@grafana/ui/src/types';
...
export function importPanelPlugin(path: string): Promise<AngularPanelPlugin | VizPanelPlugin> {
  return importPluginModule(path).then(pluginExports => {
    if (pluginExports.plugin) {
      return pluginExports.plugin as VizPanelPlugin;
    } else {
