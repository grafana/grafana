 ...
 *
 * Core logic for rendering nodes in the dependency graph.
 */
...
import { polishVersion } from './nodeUtils';

interface NodeRenderingLogicProps {
  theme: GrafanaTheme2;
  ...
  onContentProviderLeftClick: (event: React.MouseEvent, contentProviderId: string) => void;
  onContentProviderContextMenu: (event: React.MouseEvent, contentProviderId: string) => void;
}

/**
 * Renders the main node elements
 */
export function NodeRenderingLogic({
  theme,
  nodes,
  data,
  width,
  ...
  onContentProviderLeftClick,
  onContentProviderContextMenu,
}: NodeRenderingLogicProps): React.JSX.Element {
          ...
          return (
            <g key={node.id}>
              ...
              <rect
                ...
                onClick={(event) => onContentProviderLeftClick(event, node.id)}
                onContextMenu={(event) => onContentProviderContextMenu(event, node.id)}
