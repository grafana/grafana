  ...
  for (const { pluginId, extensionConfigs, error } of pluginPreloadResults) {
    ...
    for (const extensionConfig of extensionConfigs) {
      ...
      if (!extensionsPerPlugin.allowedToAdd(extensionConfig)) {
        logWarning(
          `"${pluginId}" plugin has reached the limit of ${MAX_EXTENSIONS_PER_EXTENSION_POINT_PER_PLUGIN} for "${extensionPointId}", skip registering extension "${extensionConfig.title}".`
        );
        continue;
      }
