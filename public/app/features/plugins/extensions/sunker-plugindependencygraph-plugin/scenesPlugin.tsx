...
import { PluginDependencyGraphScenesViz } from './components/PluginDependencyGraphScenesViz';
import {
...
const ContentProviderMultiSelect: React.FC<any> = ({ value, onChange, context }) => {
...
const ContentConsumerMultiSelect: React.FC<any> = ({ value, onChange, context }) => {
  // Get visualization mode from panel options
  const visualizationMode = context.options?.visualizationMode || 'add';
...
// Create the scenes panel plugin
const pluginDependencyGraphScenesPlugin = new PanelPlugin(PluginDependencyGraphScenesViz).setPanelOptions((builder) => {
  return (
    builder
      .addSelect({
...
// Register the runtime panel plugin with scenes
sceneUtils.registerRuntimePanelPlugin({
  pluginId: 'grafana-extensions-plugin-dependency-graph',
  plugin: pluginDependencyGraphScenesPlugin,
});

export { pluginDependencyGraphScenesPlugin };
