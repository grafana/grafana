...
export function assertIsFunction(fn: Function) {
  if (typeof fn !== 'function') {
    throw new Error(
      `Invalid function extension, the "functino" property needs to be a valid javascript function definition.`
    );
  }
}
...
export function isPluginExtensionConfigValid(pluginId: string, extension: PluginExtensionConfig): boolean {
  try {
    ...
    // Function
    if (isPluginExtensionFunctionConfig(extension)) {
      assertIsFunction(extension.function);
    }
