...
export function getPluginRepositoryUrl(
  pluginLinks: Array<{
    name: string;
    url: string;
  }>
  ...
  pluginLinks.map((item) => {
    try {
      ...
      if (pathSegments.length >= 2) {
        repositoryURL = `${url.protocol}//${url.hostname}/${pathSegments[0]}/${pathSegments[1]}`;
      }
    } catch (error) {
...
export const fetchDefaultBranchFromRepo = async (repoLink: string): Promise<string | null> => {
  if (!repoLink.startsWith('https://github.com/') && !repoLink.startsWith('https://gitlab.com/')) {
    return null;
  }

  try {
    const isGitHub = repoLink.startsWith('https://github.com/');
    const baseUrl = isGitHub ? 'https://api.github.com/repos/' : 'https://gitlab.com/api/v4/projects/';

    const repoPath = repoLink
      .replace(isGitHub ? 'https://github.com/' : 'https://gitlab.com/', '')
      .split('/')
      .slice(0, 2)
      .join('/');
    ...
    if (!response.ok) {
      console.error(`${isGitHub ? 'GitHub' : 'GitLab'} API error: ${response.status} ${response.statusText}`);
      return null;
    }
...
export const getCustomLink = (
  ...
  if (!repositoryLink || !defaultBranch) {
    return null;
  }
  const predefinedLink = links.find((link) => link.name.toLowerCase() === source.toLowerCase());

  if (source === 'Documentation' || source === 'Docs') {
    return predefinedLink ? predefinedLink.url : `${repositoryLink}/blob/${defaultBranch}/README.md`;
  }
  return predefinedLink ? predefinedLink.url : `${repositoryLink}/blob/${defaultBranch}/${source}`;
};
