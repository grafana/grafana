...
interface AddServiceAccountRoleModalProps {
  isOpen: boolean;
...
export const AddServiceAccountRoleModal = ({
  isOpen,
  serviceAccountId,
  serviceAccountOrgId,
  currentRoleUids,
  onDismiss,
  onRoleAdded,
}: AddServiceAccountRoleModalProps) => {
  ...
  const { data: allRoles, isLoading } = useListRolesQuery({ delegatable: true, targetOrgId: serviceAccountOrgId });
  ...
  const handleAddRole = useCallback(
    async (roleUid: string) => {
      try {
        ...
        console.error('Error adding role to service account:', error);
      }
    },
    [serviceAccountId, serviceAccountOrgId, currentRoleUids, updateUserRoles, onRoleAdded]
  );
  ...
  return (
    <>
      <Modal
        title="Add Role to Service Account"
        isOpen={isOpen}
      ...
      >
        <div className={styles.content}>
          ...
          {!isLoading && filteredRoles.length === 0 && availableRoles.length === 0 && (
            <div>
              <Trans i18nKey="serviceaccounts.add-role-modal.no-roles">No delegatable roles available</Trans>
            </div>
          )}

          {!isLoading && filteredRoles.length === 0 && availableRoles.length > 0 && (
            <div>
              <Trans i18nKey="serviceaccounts.add-role-modal.no-matches">
