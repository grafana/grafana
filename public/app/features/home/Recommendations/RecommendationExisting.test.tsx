...
beforeEach(() => {
  mockUsePluginBridge.mockImplementation((pluginId) => ({
    loading: false,
    installed: true,
    settings: { id: pluginId } as PluginMeta<{}>,
  }));
...
describe('RecommendationExisting', () => {
  ...
  it('shows Metrics & infrastructure without Kubernetes Monitoring', async () => {
    const overview = {
      activeSeries: 4200000,
      activeSeriesSource: 'oss' as const,
      dataPointsPerMinute: 5160000,
      datasourceUid: 'team-prom',
    };
    mockUsePluginBridge.mockImplementation((pluginId) => ({
      loading: false,
      installed: pluginId === METRICS_DRILLDOWN_APP_ID,
      settings: pluginId === METRICS_DRILLDOWN_APP_ID ? ({ id: pluginId } as PluginMeta<{}>) : undefined,
    }));
    ...
    expect(screen.getByText('5.2M data points/min')).toBeInTheDocument();
  ...
  it('omits a metrics secondary label when active series is unavailable', async () => {
    mockUsePluginBridge.mockImplementation((pluginId) => ({
      loading: false,
      installed: pluginId === METRICS_DRILLDOWN_APP_ID,
      settings: pluginId === METRICS_DRILLDOWN_APP_ID ? ({ id: pluginId } as PluginMeta<{}>) : undefined,
    }));
    mockFetchMetricsOverview.mockResolvedValue({
      activeSeries: null,
      dataPointsPerMinute: 142500,
      datasourceUid: 'team-prom',
    });

    render(<RecommendationExisting />);

    expect(await screen.findByText('142.5K data points/min')).toBeInTheDocument();
    expect(screen.queryByText('Active series · last 24h')).not.toBeInTheDocument();
  });
