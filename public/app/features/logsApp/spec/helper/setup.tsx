...
import LogsAppPage from '../../LogsAppPage';
...
export function setupExplore(options?: SetupOptions): {
  ...
  const history = createMemoryHistory({
    initialEntries: [{ pathname: '/logs', search: stringify(options?.urlParams) }],
  });
      ...
      <GrafanaContext.Provider value={contextMock}>
        <Router history={history}>
          <Route
            path="/logs"
            exact
            render={(props) => <GrafanaRoute {...props} route={{ component: LogsAppPage, path: '/logs' }} />}
          />
        </Router>
