...
export class CentrifugeLiveChannel<TMessage = any, TPublish = any> implements LiveChannel<TMessage, TPublish> {
  ...
  initalize(config: LiveChannelConfig): SubscriptionEvents {
    ...
    if (config.hasPresense) {
      events.join = (ctx: JoinLeaveContext) => {
        const joinleave: LiveChannelJoinLeave = {
          user: ctx.info.user,
        };
        this.stream.next({
          type: 'join',
  ...
  private sendStatus() {
    this.stream.next({ type: 'status', status: { ...this.currentStatus } });
  }
   ...
   */
  getStream() {
    const status: LiveChannelMessage<TMessage> = { type: 'status', status: { ...this.currentStatus } };
