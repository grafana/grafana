  ...
  constructor($scope: any, datasourceSrv: DatasourceSrv, variableSrv: VariableSrv, templateSrv: TemplateSrv) {
    ...
    $scope.isValid = () => {
      ...
      if (!$scope.current.name.match(/^\w+$/)) {
        appEvents.emit(alertWarning, ['Validation', 'Only word and digit characters are allowed in variable names']);
        return false;
      }

      const sameName: any = _.find($scope.variables, { name: $scope.current.name });
      if (sameName && sameName !== $scope.current) {
        appEvents.emit(alertWarning, ['Validation', 'Variable with the same name already exists']);
        return false;
      }
    ...
    $scope.runQuery = () => {
      ...
      return variableSrv.updateOptions($scope.current).catch((err: { data: { message: any }; message: string }) => {
        ...
        appEvents.emit(alertError, ['Templating', 'Template variables could not be initialized: ' + err.message]);
      });
    };
