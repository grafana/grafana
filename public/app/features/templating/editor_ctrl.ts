...
export class VariableEditorCtrl {
  ...
  constructor($scope: any, datasourceSrv: DatasourceSrv, variableSrv: VariableSrv, templateSrv: TemplateSrv) {
    ...
    $scope.duplicate = (variable: { getSaveModel: () => void; name: string }) => {
      ...
      const index = variableSrv.variables.length;
      $scope.current = variableSrv.createVariableFromModel(clone, index);
    ...
    $scope.reset = () => {
      ...
      const index = variableSrv.variables.length;
      $scope.current = variableSrv.createVariableFromModel({ type: 'query' }, index);
    ...
    $scope.typeChanged = function() {
      ...
      $scope.current = variableSrv.createVariableFromModel({ type: $scope.current.type }, oldIndex);
