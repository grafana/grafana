...
const BrowseDashboardsPage = memo(({ queryParams }: { queryParams: Record<string, string> }) => {
  ...
  const { value: isRecentlyViewedEnabledValue } = useFlag('recentlyViewedDashboards', false);
  const { value: isExperimentRecentlyViewedDashboards } = useFlag('experimentRecentlyViewedDashboards', false);
