...
describe('browse-dashboards utils', () => {
  ...
  describe('getFolderIsEmpty', () => {
    ...
    it('returns true when the only affected item is the selected folder itself', () => {
      const affected = { folders: 1, dashboards: 0, librarypanels: 0, alertrules: 0, recordingrules: 0, variables: 0 };
      expect(getFolderIsEmpty(affected, selection)).toBe(true);
    });

    it('returns false when the selected folder contains a child folder', () => {
      const affected = { folders: 2, dashboards: 0, librarypanels: 0, alertrules: 0, recordingrules: 0, variables: 0 };
      expect(getFolderIsEmpty(affected, selection)).toBe(false);
    });

    it('returns false when the selected folder contains a library panel', () => {
      const affected = { folders: 1, dashboards: 0, librarypanels: 1, alertrules: 0, recordingrules: 0, variables: 0 };
      expect(getFolderIsEmpty(affected, selection)).toBe(false);
    });

    it('returns false when the selected folder contains an alert rule', () => {
      const affected = { folders: 1, dashboards: 0, librarypanels: 0, alertrules: 1, recordingrules: 0, variables: 0 };
      expect(getFolderIsEmpty(affected, selection)).toBe(false);
    });

    it('returns false when the selected folder contains a recording rule', () => {
      const affected = { folders: 1, dashboards: 0, librarypanels: 0, alertrules: 0, recordingrules: 1, variables: 0 };
      expect(getFolderIsEmpty(affected, selection)).toBe(false);
    });

    it('returns false when the selected folder contains a variable', () => {
      const affected = { folders: 1, dashboards: 0, librarypanels: 0, alertrules: 0, recordingrules: 0, variables: 1 };
      expect(getFolderIsEmpty(affected, selection)).toBe(false);
    });

    it('subtracts selected dashboards so they are not counted as descendants', () => {
      const affected = { folders: 1, dashboards: 1, librarypanels: 0, alertrules: 0, recordingrules: 0, variables: 0 };
    ...
    it('returns true when the affected counts are lower than the selection counts', () => {
      const affected = { folders: 0, dashboards: 0, librarypanels: 0, alertrules: 0, recordingrules: 0, variables: 0 };
      expect(getFolderIsEmpty(affected, selection)).toBe(true);
    });
  });
});
