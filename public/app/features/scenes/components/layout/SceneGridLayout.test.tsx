  ...
  describe('rendering', () => {
    it('should render all grid children', async () => {
      const scene = new Scene({
        ...
        body: new SceneGridLayout({
          children: [
            new TestObject({ layout: { x: 0, y: 0, width: 12, height: 5 } }),
            new TestObject({ layout: { x: 0, y: 5, width: 12, height: 5 } }),
          ],
        }),
      });
    ...
    it('should not render children of a collapsed row', async () => {
      const scene = new Scene({
        ...
        body: new SceneGridLayout({
          children: [
            new TestObject({ key: 'a', layout: { x: 0, y: 0, width: 12, height: 5 } }),
            new TestObject({ key: 'b', layout: { x: 0, y: 5, width: 12, height: 5 } }),
            new SceneGridRow({
              title: 'Row A',
              key: 'Row A',
              isCollapsed: true,
              layout: { y: 10 },
              children: [new TestObject({ key: 'c', layout: { x: 0, y: 11, width: 12, height: 5 } })],
            }),
          ],
        }),
      });
  ...
  describe('when moving a panel', () => {
    it('shoud update layout children placement and order ', () => {
      const layout = new SceneGridLayout({
        children: [
          new TestObject({ key: 'a', layout: { x: 0, y: 0, width: 1, height: 1 } }),
          new TestObject({ key: 'b', layout: { x: 1, y: 0, width: 1, height: 1 } }),
          new TestObject({ key: 'c', layout: { x: 0, y: 1, width: 1, height: 1 } }),
        ],
      });
      layout.onDragStop(
        [
      ...
      expect(layout.state.children[0].state.layout).toEqual({ x: 0, y: 0, width: 1, height: 1 });
      expect(layout.state.children[1].state.key).toEqual('a');
      expect(layout.state.children[1].state.layout).toEqual({ x: 0, y: 1, width: 1, height: 1 });
      expect(layout.state.children[2].state.key).toEqual('c');
      expect(layout.state.children[2].state.layout).toEqual({ x: 0, y: 2, width: 1, height: 1 });
    });
  });

  describe('when using rows', () => {
    it('should update objects relations when moving object out of a row', () => {
      const rowAChild1 = new TestObject({ key: 'row-a-child1', layout: { x: 0, y: 1, width: 1, height: 1 } });
      const rowAChild2 = new TestObject({ key: 'row-a-child2', layout: { x: 1, y: 1, width: 1, height: 1 } });
    ...
    it('should update objects relations when moving objects between rows', () => {
      const rowAChild1 = new TestObject({ key: 'row-a-child1', layout: { x: 0, y: 0, width: 1, height: 1 } });
      const rowAChild2 = new TestObject({ key: 'row-a-child2', layout: { x: 1, y: 0, width: 1, height: 1 } });
      ...
      const panelOutsideARow = new TestObject({ key: 'a', layout: { x: 0, y: 0, width: 1, height: 1 } });
      const layout = new SceneGridLayout({
    ...
    it('should update position of objects when row is expanded', () => {
      ...
      const panelOutsideARow = new TestObject({ key: 'outsider', layout: { x: 0, y: 1, width: 1, height: 1 } });

      const rowBChild1 = new TestObject({ key: 'row-b-child1', layout: { x: 0, y: 3, width: 1, height: 1 } });
      ...
      expect(panelOutsideARow.state!.layout!.y).toEqual(2);
      expect(rowB.state!.layout!.y).toEqual(3);
      expect(rowBChild1.state!.layout!.y).toEqual(4);
    });
  });
});
