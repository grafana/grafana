...
describe('Explore Components i18n Verification', () => {
  ...
  describe('ErrorContainer', () => {
    ...
    it('handles complex string inputs: Unicode, emojis, special characters, and long payloads', () => {
      ...
      const { unmount: unmount1 } = render(
        <ErrorContainer queryError={{ message: unicodeStr, refId: 'STRESS1' }} />
      );
      ...
      const { unmount: unmount2 } = render(
        <ErrorContainer queryError={{ message: xssStr, refId: 'STRESS2' }} />
      );
      ...
      const { unmount: unmount3 } = render(
        <ErrorContainer queryError={{ message: multilineStr, refId: 'STRESS3' }} />
      );
      ...
      const { unmount: unmount4 } = render(
        <ErrorContainer queryError={{ message: longStr, refId: 'STRESS4' }} />
      );
  ...
  describe('NoData', () => {
    ...
    it('verifies exact DOM tag hierarchy: PanelContainer > span > text', () => {
      render(<NoData />);

      const containerEl = screen.getByTestId('explore-no-data');
      expect(containerEl.tagName.toLowerCase()).toBe('div');

      const spanChildren = containerEl.getElementsByTagName('span');
      expect(spanChildren).toHaveLength(1);

      const spanEl = spanChildren[0];
      expect(spanEl.textContent).toBe('No data');
      expect(spanEl.parentElement).toBe(containerEl);
    });

    it('verifies Emotion CSS classes are attached to both container and message elements', () => {
      render(<NoData />);

      const containerEl = screen.getByTestId('explore-no-data');
      const spanEl = screen.getByText('No data');

      // Check that classes are generated and attached
      expect(containerEl.className).toBeTruthy();
      expect(typeof containerEl.className).toBe('string');
      expect(containerEl.className.length).toBeGreaterThan(0);

      expect(spanEl.className).toBeTruthy();
      expect(typeof spanEl.className).toBe('string');
      expect(spanEl.className.length).toBeGreaterThan(0);

      // Wrapper and message classes must be distinct
      expect(containerEl.className).not.toEqual(spanEl.className);
    });
