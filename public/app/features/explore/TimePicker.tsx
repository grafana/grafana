...
export default class TimePicker extends PureComponent<TimePickerProps, TimePickerState> {
  ...
  move(direction: number, scanning?: boolean): RawTimeRange {
    ...
    const nextTimeRange = {
      from: this.props.isUtc ? toUtc(from) : momentWrapper(from),
      to: this.props.isUtc ? toUtc(to) : momentWrapper(to),
    };
    if (onChangeTime) {
    ...
    this.setState(
      state => {
        ...
        if (rawRange.from.indexOf('now') === -1) {
          rawRange.from = isUtc ? toUtc(rawRange.from, TIME_FORMAT) : momentWrapper(rawRange.from, TIME_FORMAT);
        }

        if (rawRange.to.indexOf('now') === -1) {
          rawRange.to = isUtc ? toUtc(rawRange.to, TIME_FORMAT) : momentWrapper(rawRange.to, TIME_FORMAT);
        }
