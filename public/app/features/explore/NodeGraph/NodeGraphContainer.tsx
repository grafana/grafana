...
export function UnconnectedNodeGraphContainer(props: Props) {
  ...
  const countWarning =
    withTraceView && nodes[0]?.length > 1000 ? (
      <span className={styles.warningText}>
				<Trans i18nKey='explorer.node-graph.count-warning' count={nodes[0].length}>
					({{count:nodes[0].length}} nodes, can be slow to load)
				</Trans>
				</span>
    ) : null;
  ...
  return (
    <PanelChrome
      title={t('explorer.node-graph.planel-chrome.title', 'Node graph')}
