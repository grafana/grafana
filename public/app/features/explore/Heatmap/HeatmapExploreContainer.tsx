...
export function HeatmapExploreContainer({
  ...
  const preparedAnnotations = useMemo(() => {
    ...
    return filteredAnnotations.map((frame) => {
      ...
      // Use the datasource UID embedded in the frame metadata (set by the Go backend).
      // This correctly resolves to the Pyroscope datasource even when Explore uses Mixed.
      const frameDatasourceUid = frame.meta?.custom?.['datasourceUID'] ?? datasourceUid;
      ...
      const newFields = frame.fields.map((field) => {
        ...
        const dataLink: DataLink<PyroscopeQuery> = {
          ...
          url: '',
          internal: {
            datasourceUid: frameDatasourceUid,
  ...
  }, [filteredAnnotations, splitOpenFn, datasourceUid]);
  ...
  const yAxisRange = useMemo(() => {
    ...
    for (const frame of data) {
      ...
      if (yMinField) {
        for (const v of yMinField.values) {
      ...
      if (yMaxField) {
        for (const v of yMaxField.values) {
