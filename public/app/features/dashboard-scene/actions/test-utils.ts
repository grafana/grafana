 ...
 * hosts the undo/redo history (currently DashboardEditActionsHistory under the sidebar).
 */
export function setupEditActionTest(spec?: Partial<DashboardV2Spec>): EditActionTestContext {
  ...
  return {
    ...
    undo() {
      dashboard.state.sidebar.state.editHistory.undoAction();
    },

    redo() {
      dashboard.state.sidebar.state.editHistory.redoAction();
    },

    undoAll() {
      const history = dashboard.state.sidebar.state.editHistory;
      while (history.state.undoStack.length > 0) {
        history.undoAction();
      }
    },

    getHistorySizes() {
      const { undoStack, redoStack } = dashboard.state.sidebar.state.editHistory.state;
