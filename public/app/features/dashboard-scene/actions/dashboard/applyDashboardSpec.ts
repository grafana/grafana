...
export function applyDashboardSpec({ scene, spec, description }: ApplyDashboardSpecProps): void {
  ...
  const { isModalLoading: rebuiltLoading, ...newState } = sceneUtils.cloneSceneObjectState(rebuilt.state, {
    key: scene.state.key,
    sidebar: scene.state.sidebar,
  });
  const { isModalLoading: previousLoading, ...previousState } = scene.state;
  ...
  edit({
    ...
    perform: () => {
      ...
      scene.updateView({ ...newState, editPanel: undefined, isDirty: true });
    ...
    undo: () => {
      scene.updateView(previousState);
      scene.state.sidebar.closePane();
      scene.forEachChild((child) => scene.publishEvent(new NewSceneObjectAddedEvent(child), true));
    },
  });
