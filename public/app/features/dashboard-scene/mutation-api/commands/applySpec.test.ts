...
function emptyGrid() {
  return { kind: 'GridLayout' as const, spec: { items: [] } };
}

function gridWithPanels(...names: string[]) {
  return {
    ...
    spec: {
      items: names.map((name, i) => ({
        ...
        spec: {
          ...
          width: 12,
          height: 8,
          element: { kind: 'ElementReference' as const, name },
        },
      })),
    },
  };
...
/** Two tabs, so `?dtab=` is meaningful, and a variable whose spec value the URL overrides. */
function makeTabsSpec(secondTabPanels: string[] = ['panel-1']): DashboardV2Spec {
  return makeSpec((spec) => {
    spec.layout = {
      kind: 'TabsLayout',
      spec: {
        tabs: [
          { kind: 'TabsLayoutTab', spec: { title: 'Overview', layout: emptyGrid() } },
          { kind: 'TabsLayoutTab', spec: { title: 'human-2', layout: gridWithPanels(...secondTabPanels) } },
        ],
      },
    };
    spec.variables = [
      {
        ...
        spec: {
          ...
          current: { text: 'option1', value: 'option1' },
          options: [],
          multi: false,
          includeAll: false,
          hide: 'dontHide',
...
function tabsBody(scene: DashboardScene): TabsLayoutManager {
  // eslint-disable-next-line @typescript-eslint/consistent-type-assertions -- these specs always build a tabs layout
  return scene.state.body as TabsLayoutManager;
}

/**
 * Every object in the tree the command swaps in is new and has never seen the URL, so state the
 * spec does not carry is re-initialized from it after the swap. Without that, editing any panel
 * bounces the view to the first tab, which is the reported symptom.
 */
describe('APPLY_SPEC with url state the rebuilt tree has never seen', () => {
  /** Sync from the URL the way a page load does, so the tab and the variable value come from it. */
  function initUrlSync(scene: DashboardScene, search: string) {
    locationService.replace({ search });
    new UrlSyncManager().initSync(scene);
  }

  it.each([
    { when: 'the spec is unchanged', before: ['panel-1'], after: ['panel-1'] },
    { when: 'a panel is added to the selected tab', before: ['panel-1'], after: ['panel-1', 'panel-2'] },
  ])('keeps the selected tab and the variable value when $when', async ({ before: from, after: to }) => {
    const scene = buildScene(makeTabsSpec(from));
    initUrlSync(scene, 'dtab=human-2&var-customVar=option2');
    expect(tabsBody(scene).state.currentTabSlug).toBe('human-2');

    expect((await applySpec(scene, makeTabsSpec(to))).success).toBe(true);

    expect(tabsBody(scene).state.currentTabSlug).toBe('human-2');
    expect(scene.state.$variables!.getByName('customVar')!.getValue()).toBe('option2');
  });
});
