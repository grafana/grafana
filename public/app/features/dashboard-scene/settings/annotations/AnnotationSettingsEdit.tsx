...
export const AnnotationSettingsEdit = ({ editIdx, dashboard }: Props) => {
  ...
  const [annotation, setAnnotation] = useState(dashboard.state.annotations.list[editIdx]);
  ...
  const onUpdate = (annotation: AnnotationQuery) => {
    const list = [...dashboard.state.annotations.list];
    list.splice(editIdx, 1, annotation);
    setAnnotation(annotation);
    dashboard.state.annotations.list = list;
  };
  ...
  const onDelete = () => {
    const annotations = dashboard.state.annotations.list;
    dashboard.setState({
      annotations: {
        list: [...annotations.slice(0, editIdx), ...annotations.slice(editIdx + 1)],
      },
    });
    goBackToList();
  };
  ...
  return (
    <div>
      ...
      <FieldSet>
        ...
        {/* TODO - DO WE STILL NEED THIS? {ds && !ds.annotations && <AngularEditorLoader datasource={ds} annotation={annotation} onChange={onUpdate} />} */}
      </FieldSet>
      <Stack>
        {!annotation.builtIn && (
