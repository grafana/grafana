...
/**
 * What the pipeline runs ahead of `selected`: the transformations before it in the list.
 *
...
export function precedingTransformations(selected: Transformation, all: Transformation[]): TransformationConfigs {
  ...
  return all.slice(0, Math.max(selectedIndex, 0)).map(({ transformConfig }) => transformConfig);
}
