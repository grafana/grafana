...
import { getLibraryPanelByName } from 'app/features/library-panels/state/api';
import { usePanelSave } from 'app/features/library-panels/utils/usePanelSave';
...
export const AddLibraryPanelContents = ({ panel, initialFolderUid, onDismiss }: AddLibraryPanelContentsProps) => {
  const [panelName, setPanelName] = useState(panel.state.title);
  const [debouncedPanelName, setDebouncedPanelName] = useState(panel.state.title);
  ...
  return (
    <>
      ...
      <Modal.ButtonRow>
        ...
        <Button onClick={() => {}} disabled={invalidInput}>
