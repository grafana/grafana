 ...
 *
 * They live in `transformations` alongside the user's, tagged `origin: 'system'` by
...
export class PanelDataTransformer extends SceneDataTransformer {
  ...
  private _installSystemTransformations(plugin: PanelPlugin) {
    ...
    //
    // Passing no groups clears this origin while preserving any other, which is what lets URL-provided
    // transformations be added later without touching this.
