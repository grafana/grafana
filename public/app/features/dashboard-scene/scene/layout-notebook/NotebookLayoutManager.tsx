  ...
  implements DashboardLayoutManager
{
  ...
  // Panel cells are serialized via getVizPanels(); markdown/code cells carry their content in
  // NotebookCellItem.state.content and must be written into `elements` here so it round-trips.
  public getContributedElements(): DashboardV2Spec['elements'] {
    const elements: DashboardV2Spec['elements'] = {};
    for (const cell of this.state.cells) {
      const { elementName, content } = cell.state;
      if (content) {
