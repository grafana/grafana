...
Feature toggle: `grafana.pinnedFilters` (experimental, default off).

Verification status: all items below were verified on 2026-07-10 against a local build with the
toggle enabled, using a combination of unit tests (`pinned-filters/`, `PinnedFiltersEditor`,
`setDashboardPanelContext`, serialization), the Playwright spec
(`e2e-playwright/dashboards-suite/pinned-filters.spec.ts`), and a scripted browser walkthrough
against a synthetic Prometheus-compatible sales dataset (see the demo video on the PR).
...
- [x] Given a dashboard with an ad hoc filter variable, when the author opens the variable
      settings with the toggle enabled, then a "Pinned filters" editor is shown (replacing the
      "Default filters" editor).
- [x] Given the pinned filters editor, when the author adds a pinned filter and picks a field
      from the key dropdown, then the field list matches the data source's filterable keys.
- [x] Given a pinned field with no default values, when the dashboard is viewed, then the pinned
      control is visible at the top of the dashboard showing the "All" (no restriction) state.
- [x] Given two pinned fields out of N filterable fields, when the dashboard is viewed, then
...
- [x] Given a pinned field with default values, when the dashboard loads, then the pinned control
      shows those values and panels are filtered by them.
- [x] Given a viewer changed a pinned control away from its defaults, when they use the restore
...
- [x] Given a pinned control, when the viewer opens it, then value options are loaded from the
      data source for that field.
- [x] Given a pinned control with multi-select (v1 default), when the viewer selects two values,
      then all panels using that data source re-query with the filter applied (`=|` OR semantics).
- [x] Given a pinned selection, when the viewer clears all values from the control, then the
...
- [x] Given a selection in pinned control A, when the viewer opens pinned control B, then B's
      value options are constrained by A's selection.
- [x] Given a selection in pinned control A, when the viewer opens the value dropdown of a bulk
      filter, then options are constrained by A's selection.
- [x] Given a bulk filter selection, when the viewer opens a pinned control, then its options are
...
- [x] Given pinned fields, when the viewer opens the bulk filter key dropdown, then pinned keys
      are not suggested.
...
- [x] Given a pinned control, then the viewer has no way to remove the control itself (only to
      clear its values, returning it to "All").
- [x] Given viewer changes to pinned and bulk filters, when the viewer uses "Clear filters",
...
- [x] Given a table panel cell on a pinned field, when the viewer clicks "Filter for value",
      then the pinned control's selection is replaced with the clicked value (no duplicate bulk
      filter is created).
- [x] Given a bar chart bar on a pinned field, when the viewer uses filter-for from the tooltip,
      then the pinned control's selection is replaced with the clicked value.
- [x] Given "Filter out value" on any field (pinned or not), then v1 behavior is unchanged from
...
- [x] Given a table cell on a non-pinned field, when the viewer clicks "Filter for value", then a
...
- [x] Given pinned selections and bulk filters, when the URL is copied and reopened, then both
...
- [x] Given pinned fields with custom labels and defaults, when the dashboard is saved and
      reloaded, then pinned config (fields, labels, defaults, operator) round-trips intact.
- [x] Viewer-picked values are NOT persisted on save; only author-configured defaults are
...
- [x] Given a custom label on a pinned field, when the dashboard is viewed, then the control
      shows the custom label.
- [x] Given no custom label, then the control shows the raw field name.
...
- [x] Given the toggle disabled, then the ad hoc filter UI (including the existing "Default
