...
export class DashboardModelCompatibilityWrapper {
  ...
  public removePanel(panel: PanelCompatibilityWrapper) {
    ...
    if (layout instanceof SceneGridLayout) {
    ...
    } else if (layout instanceof SceneGridRow) {
      // Updating row is a bit more complicated, first clone row with updated children that does not contain the panel we want to remove
      const newRow = layout.clone({
        children: layout.state.children.filter((child) => child !== gridItem),
      });

      // Now update the parent grid layout and replace the row with the updated one
      ...
      console.error('DashboardModelCompatibilityWrapper.removePanel only supports removing root objects', panel);
    }
  }
