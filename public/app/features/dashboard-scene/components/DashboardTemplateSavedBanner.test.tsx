...
function UrlSpy() {
  ...
  return <div data-testid="url-spy">{location.search}</div>;
}
...
describe('DashboardTemplateSavedBanner', () => {
  ...
  it('clears the templateSaved URL param when dismissed', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByTestId('url-spy').textContent).not.toContain('templateSaved=tmpl-1');
    });
  });
  ...
  it('adds templateDashboards=true to the URL when the gallery link is clicked', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByTestId('url-spy').textContent).toContain('templateDashboards=true');
    });
  });
});
