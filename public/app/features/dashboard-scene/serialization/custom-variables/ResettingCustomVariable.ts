...
export class ResettingCustomVariable extends CustomVariable {
  // Identifies the most recent resolution. A delayed reset from a superseded one must not
  ...
  public getValueOptions(args: VariableGetOptionsArgs): Observable<VariableValueOption[]> {
    ...
    return options$.pipe(
      switchMap((options) => {
        ...
        // needs delaying, so an ordinary value change is unaffected.
        if (options.length > 0) {
          return of(options);
        }

        // Defence in depth. MultiValueUrlSyncHandler.updateFromUrl also sets this flag, for
        // any variable inactive at URL-sync time and regardless of its dependencies, so it is
        // not covered by the condition upstream. No natural path reaches a set flag here,
        // since updateFromUrl only fires while inactive and activation consumes the flag
        // first, but left set it would revert this reset. Only the empty case clears it, so a
        // non-empty resolution still lets the URL-value protection stand.
