...
import { isFloat, searchForFloatGridItems, truncateFloatGridItems } from './floatingGridItems';
...
describe('floatingGridItems', () => {
  ...
  describe('truncateFloatGridItems', () => {
    describe('GridLayout', () => {
      it('truncates all float dimensions toward zero', () => {
        const layout = defaultGridLayoutKind();
        layout.spec.items = [
          {
            kind: 'GridLayoutItem',
        ...
        const result = truncateFloatGridItems(layout) as GridLayoutKind;
      ...
      it('processes multiple items', () => {
        ...
        expect(result.spec.items[0].spec.width).toBe(12);
        expect(result.spec.items[0].spec.height).toBe(8);
        expect(result.spec.items[1].spec.x).toBe(13);
        expect(result.spec.items[1].spec.y).toBe(0);
        expect(result.spec.items[1].spec.width).toBe(11);
        expect(result.spec.items[1].spec.height).toBe(7);
      });
    });

    describe('RowsLayout', () => {
      it('truncates float dimensions in nested grid layouts', () => {
        const gridLayout = defaultGridLayoutKind();
        gridLayout.spec.items = [
          {
            kind: 'GridLayoutItem',
    ...
    describe('TabsLayout', () => {
      it('truncates float dimensions in nested grid layouts', () => {
        ...
        const result = truncateFloatGridItems(layout) as TabsLayoutKind;
        ...
        expect(resultGridLayout.spec.items[0].spec.y).toBe(1);
        expect(resultGridLayout.spec.items[0].spec.width).toBe(11);
