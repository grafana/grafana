    ...
    return {
      ...
      link: function(scope, elem) {
        ...
        function render_panel() {
           ...
           * @related issues:  OWL-159, OWL-062
           * @param:           object elem
          ...
          function drawBar(elem, sortedSeries) {
            ...
            elem.attr('id', barId);
            var datapoints = sortedSeries[0].datapoints[0];
            var option = datapoints['option'];
          ...
          function drawMap(elem, sortedSeries) {
            ...
            var datapoints = sortedSeries[0].datapoints[0];
            var option = datapoints['option'];
            var series = datapoints['option']['series'];
            var seriesProvinces = series[0];
            var seriesCities = series[1];
          ...
          /**
           * @function name:   function drawRose(elem, sortedSeries)
           * @description:     This function draws a map chart.
           * @related issues:  OWL-159, OWL-062
           * @param:           object elem
           * @param:           object sortedSeries
           ...
           * @called by:       function callPlot(incrementRenderCounter)
           */
          function drawRose(elem, sortedSeries) {
            var timestamp = Math.floor(Date.now() / 1000).toString();
            var rand = Math.random() * 100000;
            var roseId = 'roseChart' + '_' + timestamp + rand.toString().substring(0, 5);
            elem.attr('id', roseId);

            var datapoints = sortedSeries[0].datapoints[0];
            var option = datapoints['option'];
            var myChart = ec.init(document.getElementById(roseId));
            myChart.setOption(option, true);
          }
           ...
           * @related issues:  OWL-159, OWL-062, OWL-063, OWL-052, OWL-030
            ...
            if (sortedSeries.length && 'datapoints' in sortedSeries[0]) {
              try {
                if ('chartType' in sortedSeries[0].datapoints[0]) {
                  ...
                  } else if (sortedSeries[0].datapoints[0].chartType === 'rose') {
                    if (!options.series.rose.show) {
                      alert(str);
                    } else {
                      drawRose(elem, sortedSeries);
                    }
                  }
                } else {
