  ...
  function CloudWatchDatasource(instanceSettings, $q, backendSrv, templateSrv) {
    ...
    this.expandTemplateVariable = function(targets, scopedVars, templateSrv) {
      ...
      .map(function(target) {
        ...
        if (dimensionKey) {
          var variableName = templateSrv.getVariableName(target.dimensions[dimensionKey]);
          if (scopedVars && _.has(scopedVars, variableName)) {
            var variable = scopedVars[variableName];
