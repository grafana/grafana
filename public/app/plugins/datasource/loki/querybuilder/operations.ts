...
[Read the docs](https://grafana.com/docs/loki/latest/logql/log_queries/#labels-format-expression) for more.
    ...
    },
    {
      ...
      renderer: getLineFilterRenderer('|~'),
      addOperationHandler: addLokiOperation,
      explainHandler: (op) => `Return log lines that match a \`Go language\` regex pattern. \`${op.params[0]}\`.`,
    },
    {
      ...
      addOperationHandler: addLokiOperation,
      explainHandler: (op) =>
        `Return log lines that doesn't match a \`Go language\` regex pattern. \`${op.params[0]}\`.`,
    },
    {
