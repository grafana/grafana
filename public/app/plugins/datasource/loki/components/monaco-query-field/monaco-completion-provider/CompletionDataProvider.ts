...
export class CompletionDataProvider {
  ...
  getLogInfo(labels: Label[]) {
    return this.languageProvider.getLogInfo(this.buildSelector(labels));
  }

  getSeriesLabels(labels: Label[]) {
    return this.languageProvider.getSeriesLabels(this.buildSelector(labels)).then((data) => data ?? {});
  }
}
