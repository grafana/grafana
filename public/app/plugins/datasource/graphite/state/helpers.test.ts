...
describe('handleTargetChanged', () => {
    ...
    it('should refresh if target changes and state is not paused', () => {
        ...
        (state.queryModel.updateModelTarget as jest.Mock).mockImplementation(() => {state.queryModel.target.target = 'newTarget'})
        handleTargetChanged(state);
    ...
    it('should refresh if fullTarget changes and state is not paused', () => {
        ...
        (state.queryModel.updateModelTarget as jest.Mock).mockImplementation(() => {state.queryModel.target.targetFull = 'newTargetFull'})
        handleTargetChanged(state);
        expect(state.refresh).toHaveBeenCalled();
    });
