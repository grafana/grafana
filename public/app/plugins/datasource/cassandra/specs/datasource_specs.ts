...
describe('CQLDatasource', function() {
  var ctx = new helpers.ServiceTestContext();
  var instanceSettings = {name: 'cassandra'};
  ...
  beforeEach(angularMocks.inject(function($q, $rootScope, $httpBackend, $injector) {
    ...
    ctx.ds = $injector.instantiate(CQLDatasource, {instanceSettings: instanceSettings});
  ...
  describe('When performing annotationQuery', function() {
    ...
    const options = {
      annotation: {
        name: annotationName,
        rawQuery: 'select time_ms, title, text, tags from table;'
      },
      range: {
    ...
    const response = {
      results: {
        MyAnno: {
          ...
          tables: [
            {
              columns: [{text: 'time_ms'}, {text: 'title'}, {text: 'text'}, {text: 'tags'}],
              rows: [
