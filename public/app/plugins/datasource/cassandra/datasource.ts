...
export class CQLDatasource {
  ...
  transformAnnotationResponse(options, data) {
    ...
    for (let i = 0; i < table.columns.length; i++) {
      if (table.columns[i].text === 'time_ms') {
        timeColumnIndex = i;
    ...
    if (timeColumnIndex === -1) {
      return this.$q.reject({message: 'Missing mandatory time column (with time_ms column alias) in annotation query.'});
    }
    ...
    return this.backendSrv.datasourceRequest({
      ...
      data: {
        ...
        queries: [{
          ...
          maxDataPoints: 1,
          datasourceId: this.id,
          rawSql: "select now() from system.local",
          format: 'table',
        }],
      }
    }).then(res => {
