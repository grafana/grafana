...
export class ElasticDatasource extends DataSourceApi<ElasticsearchQuery, ElasticsearchOptions> {
  ...
  private request(method: string, url: string, data?: unknown): Observable<any> {
    const options: any = {
      url,
      method: method,
      data: data,
  ...
  query(options: DataQueryRequest<ElasticsearchQuery>): Observable<DataQueryResponse> {
    ...
    return this.request('POST', '/api/tsdb/query', {
      ...
      map((res) => {
        ...
        return this.responseTransformer.transformTimeSeriesQueryResult(res);
      })
    );
  }
  ...
  getFields(fieldTypeFilter?: string, range: TimeRange = getDefaultTimeRange()): Observable<MetricFindValue[]> {
    return this.post('/api/tsdb/query', {
    ...
    }).pipe(map((data) => this.responseTransformer.transformFieldsQueryResponse('A', data)));
  }
