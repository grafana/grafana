...
export type MetricsConfiguration = {
  [P in Schema.MetricAggregationType]: MetricConfiguration<P>;
};

export type BucketsConfiguration = {
  [P in Schema.BucketAggregationType]: BucketConfiguration<P>;
};

export interface ElasticsearchAggregation {
  id: string;
  type: Schema.MetricAggregationType | Schema.BucketAggregationType;
  settings?: unknown;
  field?: string;
  hide: boolean;
}
...
// OVERRIDES because of incorrect type generation in dataquery.gen.ts
export interface BasePipelineMetricAggregation extends Schema.BasePipelineMetricAggregation {
  type: Schema.PipelineMetricAggregationType;
}
...
export type MovingAverageModelSettings<T extends Schema.MovingAverageModel = Schema.MovingAverageModel> = Partial<
  Extract<
    | Schema.MovingAverageSimpleModelSettings
    | Schema.MovingAverageLinearModelSettings
    | Schema.MovingAverageEWMAModelSettings
    | Schema.MovingAverageHoltModelSettings
    | Schema.MovingAverageHoltWintersModelSettings,
    { model: T }
  >
>;

export interface MovingAverage<T extends Schema.MovingAverageModel = Schema.MovingAverageModel>
  extends Schema.MovingAverage {
