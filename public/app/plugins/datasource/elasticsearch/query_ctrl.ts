...
export class ElasticQueryCtrl extends QueryCtrl {
  ...
  queryUpdated() {
    ...
    if (this.rawQueryOld && newJson !== this.rawQueryOld) {
      this.refresh();
      // To switch between legacy and new table format, we need to always refresh Raw Document query
    } else if (this.target.metrics.some((metric: any) => metric.type === 'raw_document')) {
      this.refresh();
    }
