  ...
  describe('When performing query', () => {
    ...
    it('should return a list of datapoints', () => {
      return ctx.ds.query(options).then((results: any) => {
        ...
        expect(data.fields[1].values[0]).toEqual(1558278660000);
        expect(data.fields[0].values[0]).toEqual(2.2075);
        expect(data.fields[1].values[1]).toEqual(1558278720000);
        expect(data.fields[0].values[1]).toEqual(2.29);
      });
    });
  });
