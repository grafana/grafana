...
export class PrometheusDatasource extends DataSourceApi<PromQuery, PromOptions> {
  ...
  createQuery(target: PromQuery, options: DataQueryRequest<PromQuery>, start: number, end: number) {
    ...
    const adjusted = alignRange(start, end, query.step, this.timeSrv.timeRange().to.utcOffset() * 60, (target.stepAlignment || '0m'));
 ...
 * @param userStepAlignment String time quantity entered by user for additional step alignment offset
 */
export function alignRange(
  ...
  step: number,
  utcOffsetSec: number,
  userStepAlignment: any
): { end: number; start: number } {
  const stepOffsetSeconds = utcOffsetSec - kbn.interval_to_seconds(userStepAlignment);  //combine UTC offset and user setpoint offset
  const alignedEnd = Math.floor((end + stepOffsetSeconds) / step) * step - stepOffsetSeconds;
  const alignedStart = Math.floor((start + stepOffsetSeconds) / step) * step - stepOffsetSeconds;
  return {
    end: alignedEnd,
    start: alignedStart,
  };
