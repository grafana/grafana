...
it('should return trace, nodes, and edges when querying by trace ID', async () => {
...
it('should show error when no service is selected for search', async () => {
...
it('should resolve template variables in trace ID', async () => {
  const mock = setupQueryMock('trace');
  const ds = new JaegerDatasource(defaultSettings);
...
it('should resolve template variables in search tags', async () => {
  const mock = setupQueryMock('search');
  const ds = new JaegerDatasource(defaultSettings);
  await lastValueFrom(
    ds.query({
...
it('should interpolate template variables in search parameters', async () => {
  const mock = setupQueryMock('search');
  const ds = new JaegerDatasource(defaultSettings);
