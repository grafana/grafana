...
export const prepConfig = (xySeries: XYSeries[], theme: GrafanaTheme2) => {
  ...
  builder.addScale({
    ...
    min: typeof fieldConfig.min === 'string' ? undefined : fieldConfig.min,
    max: typeof fieldConfig.max === 'string' ? undefined : fieldConfig.max,
  ...
  xySeries.forEach((s, si) => {
    ...
    builder.addScale({
      ...
      log: scaleDistr?.log,
      linearThreshold: scaleDistr?.linearThreshold,
      min: typeof config.min === 'string' ? undefined : config.min,
      max: typeof config.max === 'string' ? undefined : config.max,
      softMin: customConfig?.axisSoftMin,
      softMax: customConfig?.axisSoftMax,
      centeredZero: customConfig?.axisCenteredZero,
