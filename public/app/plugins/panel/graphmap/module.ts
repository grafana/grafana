...
class GraphCtrl extends MetricsPanelCtrl {
  ...
  constructor($scope, $injector, private annotationsSrv) {
    ...
    _.defaults(this.panel.valueMaps, this.panelDefaults.valueMaps);
  ...
  onInitEditMode() {
    ...
    this.addEditorTab('Value Mappings', 'public/app/plugins/panel/singlestat/mappings.html', 6);
	  ...
	  for (var i = 0; i < this.panel.valueMaps.length; i++) {
        ...
        if (map.value !== 'null') {
			for(var j=0; j < dataList.length; j++ {
				if (dataList[j].target === value){
					dataList[j].target = txt;
					break;
				}
			}
		}
