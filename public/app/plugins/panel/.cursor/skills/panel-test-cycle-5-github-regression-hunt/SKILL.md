...
description: >-
  ...
  may lack test coverage; return PR and linked issue links; ask whether to add a
  regression test. Use with panel test meta workflow, regression discovery, or
...
3. For **each candidate** the prompter confirms (or that you help shortlist), **deliver to the prompter**:
   - **Pull request**: Full HTTPS URL to the merged PR, e.g. `https://github.com/grafana/grafana/pull/<number>`.
   - **Linked issue** (if available): Full HTTPS URL from the PR description (`Fixes #`, `Closes #`), linked sidebar issue, or commit message—e.g. `https://github.com/grafana/grafana/issues/<number>`. If none is linked, say **no linked issue found** and still keep the PR.
...
Use a small table or bullet list so the prompter can scan:

| Candidate | PR | Issue (if any) |
| --------- | -- | -------------- |
| Short label (e.g. “threshold drag bug”) | link | link or “—” |
...
- Only pasting issue numbers without full **https://github.com/grafana/grafana/...** links.
