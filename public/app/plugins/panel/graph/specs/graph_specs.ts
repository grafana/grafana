...
describe("grafanaGraph", function() {
        ...
        beforeEach(
          angularMocks.inject(($rootScope, $compile) => {
            ...
            var element = angular.element(
              "<div style='width:" + elementWidth + "px' grafana-graph><div>"
            );
  ...
  graphScenario(
    "sorting stacked series as legend. min descending order",
    ctx => {
      ctx.setup(ctrl => {
  ...
  graphScenario(
    "sorting stacked series as legend. min ascending order",
    ctx => {
      ctx.setup((ctrl, data) => {
        ctrl.panel.legend.sort = "min";
  ...
  graphScenario(
    "sorting stacked series as legend. current descending order",
    ctx => {
      ctx.setup(ctrl => {
  ...
  graphScenario("when logBase is log 10", function(ctx) {
    ...
    it("should apply axis transform, autoscaling (if necessary) and ticks", function() {
      ...
      expect(axisAutoscale.inverseTransform(-3)).to.within(
        0.00099999999,
        0.00100000001
      );
      ...
      if (axisAutoscale.ticks.length === 7) {
        expect(axisAutoscale.ticks[axisAutoscale.ticks.length - 1]).to.within(
          999.9999,
          1000.0001
        );
      } else {
        expect(axisAutoscale.ticks[axisAutoscale.ticks.length - 1]).to.be(
          10000
        );
      }
  ...
  graphScenario(
    "when logBase is log 10 and y-min is set to 0 and auto min is > 0.1",
    function(ctx) {
      ctx.setup(function(ctrl, data) {
        ctrl.panel.yaxes[0].logBase = 10;
  ...
  graphScenario(
    "when logBase is log 2 and y-min is set to 0 and num of ticks exceeds max",
    function(ctx) {
      ctx.setup(function(ctrl, data) {
