...
import { AnnotationMarker2Clustering } from './annotations2Clustering/AnnotationMarker2Clustering';
import { AnnotationVals, XYAnnoVals } from './annotations2Clustering/types';
import { ClusteringMode, useAnnotationClustering } from './annotations2Clustering/useAnnotationClustering';
import { useAnnotations } from './annotations2Clustering/useAnnotations';
import { ANNOTATION_LANE_SIZE } from './utils';

interface AnnotationsPlugin2ClusteringProps {
...
function getVals<T = AnnotationVals | Record<string, unknown>>(frame: DataFrame) {
...
export const AnnotationsPlugin2Clustering = ({
  annotations,
  timeZone,
  config,
  newRange,
  ...
  options,
}: AnnotationsPlugin2ClusteringProps) => {
  ...
  const clusteringMode: ClusteringMode | null =
    options?.clustering && (typeof options.clustering === 'number' ? options.clustering > 0 : options.clustering)
      ? ClusteringMode.Render
      : null;
  ...
  if (plot) {
    ...
    const markers = xAnnoRef.current.flatMap((frame, frameIdx) => {
      ...
      for (let i = 0; i < vals.time.length; i++) {
        ...
        const left = Math.round(plot.valToPos(vals.time[i], 'x')) || 0;

        let style: React.CSSProperties | null = null;
        let isVisible = true;

        if (isRegion && timeEnd != null) {
          let right = Math.round(plot.valToPos(timeEnd, 'x')) || 0;
