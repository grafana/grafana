...
export function getAnnotationTooltip(
  ...
  const canEdit = canUpdateAnno && dashboardUID != null && canEditAnnotations(dashboardUID);
  const canDelete =
    canUpdateAnno && dashboardUID != null && canDeleteAnnotations(dashboardUID) && onAnnotationDelete != null;

  let time: string = timeFormatter(annoVals.time[annoIdx], timeZone);
  let text: string = annoVals.text?.[annoIdx] ?? '';

  if (annoVals.isRegion?.[annoIdx] && annoVals.timeEnd?.[annoIdx] != null) {
