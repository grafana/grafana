...
describe('when deleting rows', () => {
  ...
  it('should soft delete selected rows', () => {
    ...
    expect(newDf.fields[0].values).toEqual(['a', null, 'c', null, 'e']);
    expect(newDf.fields[1].values).toEqual([1, null, 3, null, 5]);
    expect(newDf.fields[2].values).toEqual(['a', null, 'c', null, 'e']);
  ...
  it('should remove selected rows', () => {
    ...
    expect(newDf.fields[0].values).toEqual(['a', 'c', 'e']);
    expect(newDf.fields[1].values).toEqual([1, 3, 5]);
    expect(newDf.fields[2].values).toEqual(['a', 'c', 'e']);
    expect(newDf.length).toEqual(3);

    newDf = deleteRows(newDf, [2], true);

    expect(newDf.fields[0].values).toEqual(['a', 'c']);
    expect(newDf.fields[1].values).toEqual([1, 3]);
    expect(newDf.fields[2].values).toEqual(['a', 'c']);
  ...
  it('should remove all rows when all rows are selected', () => {
    ...
    expect(newDf.fields[0].values).toEqual([]);
    expect(newDf.fields[1].values).toEqual([]);
    expect(newDf.fields[2].values).toEqual([]);
...
describe('when clearing cells from range selection', () => {
  ...
  it('should clear cells from range selection', () => {
    ...
    expect(newDf.fields[0].values).toEqual([null, null, 'c', 'd', 'e']);
    expect(newDf.fields[1].values).toEqual([null, null, 3, 4, 5]);
    expect(newDf.fields[2].values).toEqual(['a', 'b', 'c', 'd', 'e']);
    expect(newDf.length).toEqual(5);
  });
  ...
  it('should clear single cell when only one is selected', () => {
    ...
    expect(newDf.fields[0].values).toEqual(['a', 'b', 'c', 'd', 'e']);
    expect(newDf.fields[1].values).toEqual([1, null, 3, 4, 5]);
    expect(newDf.fields[2].values).toEqual(['a', 'b', 'c', 'd', 'e']);
    expect(newDf.length).toEqual(5);
  });
