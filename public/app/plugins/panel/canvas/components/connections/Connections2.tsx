  ...
  updateConnectionsAfterIndividualMove = (movedElement: ElementState) => {
    ...
    this.state.forEach(connectionState => {
      ...
      if (isSource || isTarget) {
        ...
        const { x1, y1, x2, y2 } = calculateCoordinates2(connectionState.source, connectionState.target, connectionState.info);
  ...
  updateConnectionsAfterGroupMove = (movedElements: ElementState[], selectedTargets: Array<HTMLElement | SVGElement>) => {
    ...
    this.state.forEach(connectionState => {
      ...
      if (sourceDiv && targetDiv && selectedTargets.includes(sourceDiv) && selectedTargets.includes(targetDiv)) {
        ...
        const sourceElement = movedElements.find((el: ElementState) => el.div === sourceDiv);
        const targetElement = movedElements.find((el: ElementState) => el.div === targetDiv);

        if (sourceElement && targetElement) {
          // Get current positions of source and target
          const { x1, y1, x2, y2 } = calculateCoordinates2(sourceElement, targetElement, connectionState.info);
