...
// Mock the utils module — real implementations except coords helpers used by Connections
jest.mock('../../utils', () => {
  const originalModule = jest.requireActual('../../utils');
  return {
    ...originalModule,
    calculateCoordinates: jest.fn((sourceRect, parentRect, info, target, transformScale) => ({
      x1: 100,
      y1: 100,
      x2: 200,
      y2: 200,
...
// Helper to create a mock scene (only fields touched by Connections under test)
...
describe('Connections', () => {
  describe('updateConnectionsAfterIndividualMove - integration', () => {
    it('should call calculateCoordinates with correct parameters', () => {
      ...
      // Verify calculateCoordinates was called
      expect(calculateCoordinates).toHaveBeenCalled();

      // Verify it was called with the expected parameters (sourceRect, parentRect, info, target, scale)
      const call = calculateCoordinates.mock.calls[0];
      expect(call[2]).toBe(connectionsObj.state[0].info); // info parameter
      expect(call[3]).toBe(targetElement); // target parameter
      expect(call[4]).toBe(1); // transformScale parameter
    });
