...
function Mousetrap(targetElement) {
  ...
  function _getMatches(character, modifiers, e, sequenceName, combination, level) {
    console.group('get matches for', { character, sequenceName, combination });
    let i;
    let callback;
    let matches = [];
    let action = e.type;
    ...
    for (i = 0; i < self._callbacks[character].length; ++i) {
      ...
      console.log('checking', { i, character, callback, sequenceLevel: _sequenceLevels[callback.seq] });
  ...
  self._handleKey = function (character, modifiers, e) {
    function logSeq() {
      ...
      for (let key in _sequenceLevels) {
        if (_sequenceLevels[key] > 0) {
          toReturn[key] = _sequenceLevels[key];
        }
      }
      return toReturn;
    }

    console.groupCollapsed('_handleKey', e.type, character);
    console.log('sequences before', logSeq(_sequenceLevels));
    ...
    console.log('matched callbacks', callbacks);
    ...
    for (i = 0; i < callbacks.length; ++i) {
      ...
      if (callbacks[i].seq) {
        ...
        // Don't reset sequences that share a prefix with the current one
        // This allows for rapid `t left` `t left` `t right` to work
        const suffixPrefixIndex = callbacks[i].seq.lastIndexOf(character);
        const sequencePrefix = callbacks[i].seq.slice(0, suffixPrefixIndex);
        for (const seq of Object.keys(_sequenceLevels)) {
          if (seq.startsWith(sequencePrefix)) {
            doNotReset[seq] = 1;
          }
        }
    ...
    console.log('sequences after', logSeq(_sequenceLevels));
    console.groupEnd();
  };
  ...
  function _bindSequence(combo, keys, callback, action) {
    ...
    function _increaseSequence(nextAction) {
      return function () {
        ...
        console.log('increaseSequence', combo, _sequenceLevels[combo]);
        _resetSequenceTimer();
      };
    }
