...
describe('MegaMenu with the client-built nav tree (grafana.multiTenantNavTree)', () => {
  ...
  it('renders the static tree without plugin items when the plugins.useMTPlugins flag is off', async () => {
    setTestFlags({ 'grafana.multiTenantNavTree': true, 'plugins.useMTPlugins': false });

    renderMegaMenu();

    // multiTenantNavTree builds the static tree client-side, but without
    // useMTPlugins there is no metas fetch and no skeleton: the static tree
    // renders as-is, with no plugin section merged in.
    ...
    expect(screen.queryByRole('link', { name: 'More apps' })).not.toBeInTheDocument();
