...
export function AppChromeMenu({}: Props) {
  ...
  const [prevMegaMenuState, setPrevMegaMenuState] = useState(state.megaMenu);
  const searchBarHidden = state.searchBarHidden || state.kioskMode === KioskMode.TV;

  useEffect(() => {
    setPrevMegaMenuState(state.megaMenu);
  }, [state.megaMenu]);
  ...
  const animationSpeed =
    prevMegaMenuState === 'docked' && state.megaMenu === 'open' ? 0 : theme.transitions.duration.shortest;
