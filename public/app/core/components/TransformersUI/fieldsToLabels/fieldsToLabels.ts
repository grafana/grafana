...
export const fieldsToLabelsTransformer: DataTransformerInfo<FieldsToLabelsTransformOptions> = {
  ...
  operator: (options) => (source) =>
    source.pipe(
      map((data) => {
        return fieldsToLabels(options, data);
      })
    ),
};

export function fieldsToLabels(options: FieldsToLabelsTransformOptions, data: DataFrame[]): DataFrame[] {
  ...
  for (const frame of data) {
    ...
    for (let rowIndex = 0; rowIndex < frame.length; rowIndex++) {
      ...
      if (rowFrame == null) {
        ...
        framesByLabels[jsonRowLabels] = rowFrame;
      }
