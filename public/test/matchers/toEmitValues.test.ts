...
describe('toEmitValues matcher', () => {
  describe('failing test (need to skip these for obvious reasons)', () => {
    ...
    describe.skip('wrong number of emitted values', () => {
      it('should fail with correct message', async () => {
        const observable = interval(10).pipe(take(3));
        await expect(observable).toEmitValues([0, 1]);
      });
    });

    describe.skip('wrong emitted values', () => {
      it('should fail with correct message', async () => {
        const observable = interval(10).pipe(take(3));
        await expect(observable).toEmitValues([1, 2, 3]);
      });
    ...
    describe.skip('wrong emitted value types', () => {
      it('should fail with correct message', async () => {
