/*import {
  ...
  export function homeScene2(templatised = true, seriesToShow = '__server_names') {
...
import {
  ...
  import { DATASOURCE_REF } from '../../constants';
  import { CustomSceneObject } from './CustomSceneObject';
  import { fetchDataFromJSON } from '../../utils/api'; // Import the function to fetch data from JSON
  
  export async function homeScene2(templatised = true, seriesToShow = '__server_names') {
    const timeRange = new SceneTimeRange({
      from: 'now-6h',
      to: 'now',
    });
