...
for (const file of m1Files) {
  ...
  for (const k of keysInFile) {
    ...
    if (val.valid) {
      console.log(`      ✓ ${k.key} [${k.type}] (valid 3 segments)`);
    } else {
...
if (keyFormatViolations.length > 0) {
  ...
  const m1Violations = keyFormatViolations.filter((v) => m1Files.includes(v.file));
  if (m1Violations.length > 0) {
    console.log(`  --> ATTENTION: ${m1Violations.length} M1 violations found in NoDataSourceCallToAction.tsx:`);
    for (const v of m1Violations) {
