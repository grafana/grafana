...
def verify_linux_DEB_packages_step(depends_on = ["publish-linux-packages-deb"]):
    return {
        ...
        "commands": [
            ...
            'if dpkg-query -W -f=\'${Status}\' grafana | grep -q "ok installed"; then',
...
def verify_linux_RPM_packages_step(depends_on = ["publish-linux-packages-rpm"]):
    return {
        ...
        "commands": [
            ...
            'echo "Step 4: Installing Grafana..."',
            "if dnf install -y https://dl.grafana.com/oss/release/grafana-${TAG}-1.aarch64.rpm >/dev/null 2>&1; then",
            '    echo "Successfully installed Grafana version ${TAG}"',
            "else",
            '    echo "Failed to install Grafana version ${TAG}"',
            "    exit 1",
