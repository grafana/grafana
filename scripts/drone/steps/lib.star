...
def verify_linux_RPM_packages_step(depends_on = ["publish-linux-packages-rpm"]):
    return {
        ...
        "commands": [
            ...
            "dnf install -y https://dl.grafana.com/oss/release/grafana-${TAG}-1.aarch64.rpm | grep -v '^(Updating|Last metadata|Dependencies|Package|Installed)'",
            'echo "Step 4: Verifying Grafana installation..."',
