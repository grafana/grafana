...
def store_storybook_step(edition, ver_mode, trigger=None):
    ...
    if ver_mode == 'release':
        commands.extend([
            './bin/build store-storybook --deployment latest --src-bucket grafana-prerelease --src-dir artifacts/storybook',
            './bin/build store-storybook --deployment ${DRONE_TAG} --src-bucket grafana-prerelease --src-dir artifacts/storybook',
        ])

    else:
        # main pipelines should deploy storybook to grafana-storybook/canary public bucket
        commands = ['./bin/build store-storybook --deployment canary --src-bucket grafana-storybook', ]
