...
def pr_pipelines(edition):
    ...
    return [
         pipeline(
            name='pr-build-e2e', edition=edition, trigger=trigger, services=[], steps=[download_grabpl_step(), restore_cache_step()] + initialize_step(edition, platform='linux', ver_mode=ver_mode)
                + build_steps,
        ),
    ]
