...
//   those read integrity from the manifest, which the manifest callback hashes itself.
// - webpack-assets-manifest → rspack-manifest-plugin, with the same `generate` callback
//   rspack.dev.ts uses. The callback digests the emitted bytes itself rather than reading
//   FileDescriptor.integrity, so dev and prod produce the manifest the same way even though
//   only prod registers an SRI plugin. See plugins/assetsManifest.ts.
