...
- **Node.js** — version pinned in `.nvmrc` (currently v24.11.0). Installed via nvm and set as the nvm default. **PATH gotcha:** the infra injects `/exec-daemon/node` (v22) ahead of nvm, so the plain non-login shell resolves `node` to v22 — which still satisfies `engines` (`>= 22 <25`) so builds/tests work, but is not the pinned version. Login shells (tmux sessions, `bash -lc '...'`) get v24 because `~/.bashrc` prepends the nvm v24 bin. Run `yarn` / `yarn start` / `jest` / webpack via a login shell (tmux or `bash -lc`) to use the pinned Node.
- **Go** — version pinned in `go.mod` (currently 1.26.5), installed at `/usr/local/go` and symlinked to `/usr/local/bin/go`. The distro `/usr/bin/go` is older; `/usr/local/bin` wins in PATH so `go` resolves correctly. If `go.mod` bumps Go, reinstall a matching toolchain into `/usr/local/go`.
- **Yarn** via corepack — version pinned by `package.json` `packageManager` (currently `yarn@4.17.1`). Run `corepack enable` if `yarn` is not found. `.yarnrc.yml` sets `enableScripts: false`, so dependency build/lifecycle scripts are disabled by default.
