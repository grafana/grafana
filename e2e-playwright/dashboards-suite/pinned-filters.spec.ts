...
test.describe(
  ...
  () => {
    ...
    test('Pinned filters render as standalone controls, cascade, and are populated by panel clicks', async ({
      ...
      // Both pinned filters render as standalone labeled controls (rendered by @grafana/scenes)
      const sliceControl = page.getByTestId('origin-filter-control-slice');
      const jobControl = page.getByTestId('origin-filter-control-job');
      ...
      // The control is a MultiSelect that commits on blur, so close the menu and blur the input.
      await sliceControl.getByTestId('origin-filter-value-slice').click();
      ...
      await jobControl.getByTestId('origin-filter-value-job').click();
      ...
      await sliceControl.getByRole('button', { name: /Restore the filter set by this dashboard/ }).click();
