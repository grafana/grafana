...
| File            | Responsibility                                                                                                                                                                   | Add a new helper when…                                                                                                    |
| --------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------- |
| `flows.ts`      | Multi-step user flows (setup, navigation) composed from page objects, exposed as the namespaced `flows` object (`flows.dashboards.*`, `flows.variables.*`, `flows.navigation.*`) | A second spec needs the same verbatim sequence (import, save+toast, add variable, go to explore). Not for a single click. |
| `utils.ts`      | Pixel/timing-sensitive mechanics: drag-and-drop and `boundingBox()` geometry                                                                                                     | Several specs need the same mouse/geometry sequence. Keep one-off `toPass()` / mouse steps inline in the spec.            |
| `assertions.ts` | Reusable assertion bundles (`expect` calls over page-object locators)                                                                                                            | The same multi-step or multi-element `expect` bundle appears in a second spec. Single expects stay in the spec.           |
