...
| `Controls`         | `Controls.ts`                 | Top nav bar (edit, save, timepicker, ...) and variable submenu         | `enterEditMode()`, `exitEditMode()`, `saveDashboard(title?)`, `setTimeRange(from, to)`, `openControlsMenu()`; `variables` sub-object: `getLabel(variableLabel)`, `getInput(variableLabel)`, `setValue(variableLabel, text)`, `getDropdownTrigger(variableLabel)`, `openDropdown(variableLabel)`, `getOption(optionLabel)`, `selectOption(variableLabel, optionLabel)`, `addFilter(variableLabel, [label, operator, value])`                                                                                                                                                                                                                                                                    |
