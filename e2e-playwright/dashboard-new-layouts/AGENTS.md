...
test.describe(
  ...
  () => {
    test('describes the user-visible behavior', async ({ gotoDashboardPage, page, controls, sidebar }) => {
      ...
      // ... re-assert persistence (reload separately if the next step needs a fresh page)
    });
  }
);
```
