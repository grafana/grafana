...
| `Controls`         | `Controls.ts`                 | Top nav bar (edit, save, ...) and variable submenu                     | `enterEditMode()`, `exitEditMode()`; `variables` sub-object: `getVariableLabel(label)`, `selectVariableOption(variableLabel, optionLabel)`, `addFilter(variableLabel, [label, operator, value])`                                                 |
...
| `VariableOptions`  | `sidebar/VariableOptions.ts`  | Variable edit pane — via `sidebar.variableOptions`                     | `selectVariableType(type)`, `setName(name)`, `setLabel(label)`; type-specific sub-objects: `datasource.selectDatasourceType(dsType)`, `datasource.setDatasourceNameFilter(filter)`, `groupby.selectDatasource(ds)`, `adhoc.selectDatasource(ds)` |
