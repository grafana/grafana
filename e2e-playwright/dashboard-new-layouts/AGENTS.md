...
| `Controls`                    | `Controls.ts`                                   | Top nav bar (edit, save, timepicker, share, ...) and variable submenu                          | `getContainer()`, `enterEditMode()`, `exitEditMode()`, `saveDashboard(title?)`, `goBackToDashboard()`, `openControlsMenu()`, `openShareSnapshotDrawer()`, `refresh()`; `timeRange` sub-object: `set(from, to)`, `selectPreset(presetLabel)`; `variables` sub-object: `getLabel(variableLabel)`, `getInput(variableLabel)`, `setValue(variableLabel, text)`, `getDropdownTrigger(variableLabel)`, `openDropdown(variableLabel)`, `getOption(optionLabel)`, `selectOption(variableLabel, optionLabel)`, `deselectOption(variableLabel, optionLabel)`, `addFilter(variableLabel, [label, operator, value])`                                                                               |
| `Sidebar`                     | `sidebar/Sidebar.ts`                            | Whole sidebar region (toolbar + open pane)                                                     | `.toolbar`, `.addOptions`, `.dashboardOptions`, `.panelOptions`, `.rowOptions`, `.tabOptions`, `.groupOptions`, `.variableOptions`, `.contentOutline` sub-objects; `getContainer()`, `goBack()`, `getDockToggle()`, `closePane()`, `copySelection()`, `duplicateSelection()`, `deleteSelection({ confirm? })`                                                                                                                                                                                                                                                                                                                                                                          |
| `Toolbar`                     | `sidebar/Toolbar.ts`                            | Icon strip — accessed via `sidebar.toolbar`                                                    | `getButton(name)`, `clickButton(name)` with `name: 'Add' \| 'Options' \| 'Code' \| 'Undo' \| 'Redo' \| 'Outline' \| 'Filters'`; `getVisibilityToggle()`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| `AddOptions`                  | `sidebar/AddOptions.ts`                         | "Add" pane (default pane on new dashboards) — via `sidebar.addOptions`                         | `addPanel()`, `addTab()`, `addVariable()`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| `ContentOutline`              | `sidebar/ContentOutline.ts`                     | Content outline pane — via `sidebar.contentOutline`                                            | `getTree()`, `getItem(name)`, `clickItem(name)`, `toggleNode(name)`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
...
| `RowOptions`                  | `sidebar/RowOptions.ts`                         | Row options pane — via `sidebar.rowOptions`                                                    | `.conditionalRenderingOptions`, `.repeatOptions`; `getTitleInput()`, `setTitle(rowTitle)`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| `TabOptions`                  | `sidebar/TabOptions.ts`                         | Tab options pane — via `sidebar.tabOptions`                                                    | `.conditionalRenderingOptions`, `.repeatOptions`; `getTitleInput()`, `setTitle(tabTitle)`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| `GroupOptions`                | `sidebar/GroupOptions.ts`                       | "Group" category shown for a multi-selection — via `sidebar.groupOptions`                      | `getGroupElementsIntoButton('row' \| 'tab')`, `groupElementsInto('row' \| 'tab')`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| `ConditionalRenderingOptions` | `sidebar/shared/ConditionalRenderingOptions.ts` | Shared conditional rendering rules — via `*.conditionalRenderingOptions`                       | `getVisibilityRadio('show' \| 'hide')`, `selectVisibility('show' \| 'hide')`, `getMatchTypeRadio('all' \| 'any')`, `selectMatchType('all' \| 'any')`, `getRule('data' \| 'timeRangeSize' \| 'variable')`, `removeRule(ruleId)`, `getAddRuleButton()`; rule-type sub-objects: `templateVariable.getRuleNameSelect()`, `templateVariable.getRuleOperatorSelect()`, `templateVariable.getRuleValueInput()`, `templateVariable.addRule(name, operator, value)`, `timeRange.getRuleSelect()`, `timeRange.addRule(optionLabel)`, `queryResult.getRuleSelect()`, `queryResult.addRule('Has data' \| 'No data')`                                                                               |
| `GridLayoutOptions`           | `sidebar/shared/GridLayoutOptions.ts`           | Layout options on the dashboard options pane, via `sidebar.dashboardOptions.gridLayoutOptions` | `getLayoutType(layoutType)`, `switchLayout(layoutType, { confirm? })` with `layoutType: 'Auto' \| 'Custom' \| 'Rows' \| 'Tabs'`; auto grid sizing: `getMinColumnWidthSelect()`, `selectMinColumnWidth(option)` / `selectMinColumnWidth('Custom', customWidth)`, `getCustomMinColumnWidthInput()`, `clearCustomMinColumnWidth()`, `getMaxColumnsSelect()`, `selectMaxColumns(option)`, `getRowHeightSelect()`, `selectRowHeight(option)` / `selectRowHeight('Custom', customHeight)`, `getCustomRowHeightInput()`, `clearCustomRowHeight()`, `getFillScreenSwitch()`, `toggleFillScreen()`                                                                                              |
| `RepeatOptions`               | `sidebar/shared/RepeatOptions.ts`               | Shared repeat options — via `*.repeatOptions`                                                  | `repeatByVariable(variableName)`, `disableRepeatByVariable()`; custom grid panels only: `setRepeatDirection('Horizontal' \| 'Vertical')`, `getMaxPerRowSelect()`, `selectMaxPerRow(value)` with `value: 2 \| 3 \| 4 \| 6 \| 8 \| 12`; constructor takes the options-group id (`'repeat-options'` for panels/tabs, `'dash-row-repeat'` for rows) — already wired by `PanelOptions`/`TabOptions`/`RowOptions`                                                                                                                                                                                                                                                                            |
| `VariableOptions`             | `sidebar/VariableOptions.ts`                    | Variable sidebar — via `sidebar.variableOptions`                                               | `selectVariableType(type)`, `setName(name)`, `setLabel(label)`, `selectDisplay(displayLabel)`; type-specific sub-objects: `datasource.selectType(dsType)`, `datasource.setNameFilter(filter)`, `custom.openEditor()`, `custom.selectFormat(format)`, `custom.setValues(values)`, `custom.getPreviewOfValues()`, `custom.getPreviewTable()`, `custom.applyChanges()`, `groupby.selectDatasource(ds)`, `adhoc.selectDatasource(ds)`, `query.openEditor()`, `query.selectTargetDatasource(ds)`, `query.setTestDataQuery(query)`, `query.runQuery()`, `query.getPreviewOfValues()`, `query.applyChanges()`, `constant.setValue(value)`, `textbox.setValue(value)`, `interval.toggleAuto()` |
...
| `Tabs`                        | `Tabs.ts`                                       | Tab bar of a tabs layout (top-level or nested in a row)                                        | `getTitle(tabTitle, scope?)`, `getContent(tabTitle, scope?)` — pass `rows.getContent(rowTitle)` as `scope` to look up a tab (or its content) inside a specific row; `select(tabTitle \| tabTitle[])` — click the tab title, an array extends the selection via shift-clicks                                                                                                                                                                                                                                                                                                                                                                                                            |
| `Canvas`                      | `Canvas.ts`                                     | Edit canvas add-actions strip (per grid, revealed on hover)                                    | `getContainer()`, `getAddPanelButton(scope?)`, `addPanel(scope?)`, `getAddTabButton(scope?)`, `addTab(scope?)`, `pasteTab(scope?)`, `ungroupTabs(scope?)`, `getAddRowButton(scope?)`, `addRow(scope?)`, `pasteRow(scope?)`, `ungroupRows(scope?)`, `getGroupPanelsButton(scope?)`, `groupPanels('row' \| 'tab', scope?)` — pass `scope` (e.g. `tabs.getContent(...)`, `rows.getContent(...)`) to target the add-actions strip of a nested grid                                                                                                                                                                                                                                         |

> The show/hide visibility toggle is a **Toolbar** control (`sidebar.toolbar.getVisibilityToggle()`), even though its selector lives under `components.Sidebar.*`. `Toolbar.getButton(name)` / `clickButton(name)` resolve buttons by accessible name (`'Add' | 'Options' | 'Code' | 'Undo' | 'Redo' | 'Outline' | 'Filters'`), scoped to the sidebar container.
...
Shared spec helpers live in `helpers/` and are imported from its barrel (`import { flows, movePanel, expectRowVisibility } from './helpers'`). Each file has one responsibility: put new helpers in the right one:

| File            | Responsibility                                                                                                                                                                   | Key helpers                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        | Add a new helper when…                                                                                          |
| --------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------- |
| `flows.ts`      | Multi-step user flows (setup, navigation) composed from page objects, exposed as the namespaced `flows` object (`flows.dashboards.*`, `flows.variables.*`, `flows.navigation.*`) | `flows.dashboards.importTestDashboard(page, selectors, title, dashInput?, options?)`, `flows.dashboards.saveDashboard(page, controls, options?)` (save + close toast, reloads by default; pass `{ title }` for Save as, `{ reloadPageAfterSave: false }` to keep the current page); `flows.variables.addNewGenericVariable(page, sidebar, controls, variable, skipEnterEditMode?)`, `addNewTextBoxVariable(...)`, `fillVariableValue(page, controls, varName, text)`, `delete(sidebar, variableLabel)`; `flows.navigation.goToEmbeddedPanel(page)` | A second spec needs the same verbatim sequence. Not for a single click.                                         |
| `utils.ts`      | Pixel/timing-sensitive mechanics: drag-and-drop and `boundingBox()` geometry                                                                                                     | `dragTo`, `getPanelBox`, `movePanel`, `getTabBox`, `moveTab`, `getRowBox`, `moveRow`                                                                                                                                                                                                                                                                                                                                                                                                                                                               | Several specs need the same mouse/geometry sequence. Keep one-off `toPass()` / mouse steps inline in the spec.  |
| `assertions.ts` | Reusable assertion bundles (`expect` calls over page-object locators)                                                                                                            | `expectRepeatedPanelTitlesToBe`, `expectRepeatedRowTitlesToBe`, `expectRepeatedTabTitlesToBe`, `expectTabVisibility`, `expectRowVisibility`, `expectDashboardChangesToContain`                                                                                                                                                                                                                                                                                                                                                                     | The same multi-step or multi-element `expect` bundle appears in a second spec. Single expects stay in the spec. |

> `expectTabVisibility(tabTitle, tabs, visibility?)` and `expectRowVisibility(rowTitle, rows, visibility?)` assert that the element's title **and** content are both `'visible'` (the default) or both `'hidden'`, and return `{ title, content }` locators: reuse those instead of calling `getTitle()` / `getContent()` again. A mixed state (a collapsed row keeps its title but unmounts its content) is not expressible with the single `visibility` argument: assert it inline on the returned locators. `expectTabVisibility` only works on a tab that is **active** and hosts a **grid**: `Tabs.getContent()` resolves the `LayoutContainer('tab ...')` testid, which is not rendered for an inactive tab nor for a tab wrapping a nested rows layout. Assert the title alone in those cases.

Do not put UI interactions that belong on a page object into helpers. Do not put the behavior-under-test assertion into `flows.ts` (flows may only assert to keep their own sequence reliable, e.g. waiting for a save toast).
...
test.describe(
  ...
  () => {
    test('describes the user-visible behavior', async ({ gotoDashboardPage, page, controls, sidebar }) => {
      await gotoDashboardPage({ uid: 'dashboard-uid' });

      await controls.enterEditMode();
      // ... action + assert (the toolbar is reached via sidebar.toolbar)

      await flows.dashboards.saveDashboard(page, controls);
      // ... re-assert persistence (reload is the default; pass { reloadPageAfterSave: false } to keep the current page)
    });
  }
);
```
...
- **Action methods** (e.g. `enterEditMode()`, `closePane()`) wrap interactions — multi-step flows or single clicks on act-only elements — and use `test.step()` so the HTML report shows named steps.
- **When a spec needs both**, pair them: the action method delegates to the getter (see `Toolbar.getButton()` / `clickButton()`).
- **No speculative methods.** Only add methods needed by the current (or new) spec.
...
- **One raw `getByGrafanaSelector` is allowed** for one-off assertions that aren't reusable interactions (e.g. a breadcrumb check). When a second spec needs the same raw lookup, promote it into a page-object getter or action.
...
- **Viewport**: use `test.use({ viewport: { width: 1920, height: 1080 } })` when tests involve layout-sensitive operations (drag-and-drop, panel positioning).
- **Persistence**: prefer `flows.dashboards.saveDashboard(page, controls)` over `controls.saveDashboard` plus a manual toast close / `page.reload()`.
- **Do not mix** page-object calls and raw selectors for the same UI region in one spec.

### Adding a method to a page object

1. Find the raw selector chain in the spec you're writing or updating.
...
5. Run `--repeat-each=3` on the spec.
