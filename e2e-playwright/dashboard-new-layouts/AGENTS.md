...
| `Sidebar`                     | `sidebar/Sidebar.ts`                            | Whole sidebar region (toolbar + open pane)                               | `.toolbar`, `.addOptions`, `.dashboardOptions`, `.panelOptions`, `.rowOptions`, `.tabOptions`, `.variableOptions`, `.contentOutline` sub-objects; `getContainer()`, `clickGoBackButton()`, `getDockToggle()`, `clickCloseButton()`, `clickDeleteButton({ confirm? })`                                                                                                                                                                                                                                                                                                                                                                                                                          |
...
| `Canvas`                      | `Canvas.ts`                                     | Edit canvas add-actions strip (per grid, revealed on hover)              | `getContainer()`, `getAddPanelButton(panelsContainer?)`, `addPanel(panelsContainer?)`, `getAddTabButton(panelsContainer?)`, `addTab()`, `getAddRowButton(panelsContainer?)`, `addRow()`, `groupPanels('row' \| 'tab', panelsContainer?)` — pass `panelsContainer` (e.g. `tabs.getContent(...)`, `rows.getContent(...)`) to target the add-actions strip of a nested grid                                                                                                                                                                                                                                                                                                                       |
...
**30 of 31 specs migrated.** Non-migrated specs are listed first by descending selectors usage count (a rough proxy for migration effort). "Selectors usage count" is the number of times the spec accesses the `selectors` object (`selectors.components...`, `selectors.pages...`, etc.).
