...
| `Controls`         | `Controls.ts`                 | Top nav bar (edit, save, ...) and variable submenu                     | `enterEditMode()`, `exitEditMode()`; `variables` sub-object: `getVariableLabel(label)`, `selectVariableOption(variableLabel, optionLabel)`                                                                                              |
...
| `VariableOptions`  | `sidebar/VariableOptions.ts`  | Variable edit pane — via `sidebar.variableOptions`                     | `selectVariableType(type)`, `setName(name)`, `setLabel(label)`; type-specific sub-objects: `datasource.selectDatasourceType(dsType)`, `datasource.setDatasourceNameFilter(filter)`, `groupby.selectDatasource(ds)`                      |
...
**12 of 30 specs migrated.** Non-migrated specs are listed by descending selectors usage count (a rough proxy for migration effort). "Selectors usage count" is the number of times the spec accesses the `selectors` object (`selectors.components...`, `selectors.pages...`, etc.).
