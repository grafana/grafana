...
test.describe('Panels test: Table - Nested', { tag: ['@panels', '@table'] }, () => {
  test('a11y', { tag: ['@a11y'] }, async ({ scanForA11yViolations, selectors, page }) => {
    await page.goto(selectors.pages.SoloPanel.url(`${DASHBOARD_UID}/panel-tests-table-kitchen-sink?orgId=1&panelId=4`));
    await expect(page.getByTestId(selectors.components.Panels.Panel.title('Nested tables'))).toBeVisible();
    await waitForTableLoad(page);

    await expect(page.locator('[role="row"]').filter({ visible: true })).toHaveCount(3); // header + 2 rows

    const firstRowExpander = page.getByTestId(selectors.components.Panels.Visualization.TableNG.RowExpander).first();
