...
test.describe('plugin-e2e-api-tests admin', { tag: ['@plugins'] }, () => {
  test('should evaluate to false if entire request returns 200 but partial query result is invalid', async ({
    ...
    await fillMonacoEditor(
      queryA.getByGrafanaSelector(e2e.selectors.components.QueryField.container).locator('.monaco-editor'),
      'topk(5,'
    );
    ...
    await fillMonacoEditor(
      queryB.getByGrafanaSelector(e2e.selectors.components.QueryField.container).locator('.monaco-editor'),
