...
import { DROPZONE_TEST_ID, dragFilesOntoDropzone, writeFilesForDrag } from '../utils/dropzone-helpers';
...
test.describe(
  ...
  () => {
    test.beforeEach(async ({ page }) => {
      ...
      await expect(templateDropzone(page)).toBeVisible();
    });

    test('lists every template file added through the file picker', async ({ page }) => {
      await templateDropzone(page)
      ...
      await expect(page.getByRole('main').getByText('slack.tmpl', { exact: true })).toBeVisible();
      await expect(page.getByRole('main').getByText('email.tmpl', { exact: true })).toBeVisible();
    });
    ...
    test('lists template files dragged onto the dropzone', async ({ page }, testInfo) => {
      ...
      await dragFilesOntoDropzone(page, templateDropzone(page), filePaths);
    ...
    test('removes a single template file and keeps the rest', async ({ page }) => {
      ...
      await expect(page.getByRole('main').getByText('email.tmpl', { exact: true })).toBeVisible();
      await expect(page.getByRole('main').getByText('slack.tmpl', { exact: true })).toBeHidden();
    });

    test('flags two template files sharing a name', async ({ page }) => {
      await templateDropzone(page)
        .locator('input[type="file"]')
        .setInputFiles([{ name: 'slack.tmpl', mimeType: 'text/plain', buffer: Buffer.from(SLACK_TEMPLATE) }]);
      await expect(page.getByRole('main').getByText('slack.tmpl', { exact: true })).toBeVisible();

      // A second file with the same name would overwrite the first template on import.
      await templateDropzone(page)
        .locator('input[type="file"]')
        .setInputFiles([{ name: 'slack.tmpl', mimeType: 'text/plain', buffer: Buffer.from(EMAIL_TEMPLATE) }]);
...
function templateDropzone(page: Page) {
  // The YAML configuration upload uses the same test id.
  return page.getByTestId(DROPZONE_TEST_ID).filter({ hasText: 'Drop template files here or click to upload' });
}
