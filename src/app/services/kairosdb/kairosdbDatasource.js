    ...
    function (angular, _, kbn) {
        ...
        module.factory('KairosDBDatasource', function($q, $http) {
            ...
            KairosDBDatasource.prototype.query = function(filterSrv,options) {
                ...
                var queries = _.compact(_.map(options.targets, convertTargetToQuery,options));
            ...
            KairosDBDatasource.prototype.performTagSuggestQuery = function(metricname,range,type,keyValue) {
                if(tagList && (metricname == tagList.metricName) && (range.from == tagList.range.from) && (range.to == tagList.range.to) ){
            ...
            function convertTargetToQuery(target) {
                ...
                if(target.downsampling!='(NONE)')
                    query.aggregators.push({name: target.downsampling, align_sampling: true, sampling: KairosDBDatasource.prototype.convertToKairosInterval(target.sampling||this.interval)});
                if(target.horizontalAggregators){
                    _.each(target.horizontalAggregators,function(chosenAggregator){
                        ...
                        if(chosenAggregator.unit) returnedAggregator.unit = chosenAggregator.unit+'s';
                        if(chosenAggregator.factor && chosenAggregator.name=='div') returnedAggregator.divisor = chosenAggregator.factor;
                        else if(chosenAggregator.factor && chosenAggregator.name=='scale') returnedAggregator.factor  = chosenAggregator.factor;
                        if(chosenAggregator.percentile) returnedAggregator.percentile = chosenAggregator.percentile;
                ...
                if(target.groupByTags || target.nonTagGroupBys){
                    ...
                    if(target.nonTagGroupBys){
                        _.each(target.nonTagGroupBys,function(rawGroupBy){
                            ...
                            if(formattedGroupBy.name==='time') formattedGroupBy.range_size=KairosDBDatasource.prototype.convertToKairosInterval(formattedGroupBy.range_size);
            ...
            function milliSecondsToHms(milliSeconds) {
                var numyears = Math.floor(milliSeconds / 31536000 / 1000);
                if(numyears){
                    return numyears + 'y';
                }
                var numdays = Math.floor(milliSeconds / 86400 / 1000);
                if(numdays){
                    return numdays + 'd';
                }
                var numhours = Math.floor(milliSeconds / 3600 /1000);
                if(numhours){
                    return numhours + 'h';
                }
                var numminutes = Math.floor(milliSeconds / 60 / 1000);
                if(numminutes){
                    return numminutes + 'm';
                }
                ...
                return milliSeconds+'ms'; //'just now' //or other string you like;
            }
