    ...
    function (angular, _, kbn) {
        ...
        module.controller('KairosDBTargetCtrl', function($scope, $timeout) {
            ...
            $scope.changeHorAggregationInput = function(){
                $scope.hasSamplingRate = _.contains(['avg','dev','max','min','sum','least_squares','count','percentile'], $scope.target.currentHorizontalAggregatorName);
            ...
            $scope.validateHorizontalAggregator = function() {
                ...
                if($scope.hasFactor ){
                    ...
                    else if($scope.target.horAggregator.factor==0 && $scope.target.currentHorizontalAggregatorName=='div') {
                        $scope.target.errors.horAggregator.factor = 'Cannot divide by 0';
                        $scope.isAggregatorValid = false;
                    }
                }
                if($scope.hasPercentile){
                    if(!$scope.target.horAggregator.percentile || $scope.target.horAggregator.percentile<=0 || $scope.target.horAggregator.percentile>1) {
            ...
            function hasNoError(error){
                return _.isEmpty(_.omit($scope.target.errors,['horAggregator','groupBy'])) && _.isEmpty($scope.target.errors.horAggregator)&& _.isEmpty($scope.target.errors.groupBy);
            }
