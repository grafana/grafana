...
docker_tag_all () {
  ...
  if [ $BUILD_FAST = "0" ]; then
    ...
    docker tag "${_docker_repo}-ppc64le-linux:${_grafana_version}${TAG_SUFFIX}" "${_docker_repo}-ppc64le-linux:${tag}${TAG_SUFFIX}"
  fi
}
