...
func (m *SubscribeRequest) MarshalToSizedBufferVT(dAtA []byte) (int, error) {
	...
	if m.Tf != nil {
		size, err := m.Tf.MarshalToSizedBufferVT(dAtA[:i])
		if err != nil {
			return 0, err
		}
		i -= size
...
func (m *SubscribeRequest) UnmarshalVT(dAtA []byte) error {
	...
	for iNdEx < l {
		...
		case 13:
			...
			if err := m.Tf.UnmarshalVT(dAtA[iNdEx:postIndex]); err != nil {
				return err
			}
			iNdEx = postIndex
		case 14:
...
func (m *FilterNode) UnmarshalVT(dAtA []byte) error {
	l := len(dAtA)
	iNdEx := 0
	for iNdEx < l {
		preIndex := iNdEx
		...
		wireType := int(wire & 0x7)
		if wireType == 4 {
			return fmt.Errorf("proto: FilterNode: wiretype end group for non-group")
		}
		if fieldNum <= 0 {
			return fmt.Errorf("proto: FilterNode: illegal tag %d (wire type %d)", fieldNum, wire)
		}
		switch fieldNum {
		case 1:
			if wireType != 2 {
