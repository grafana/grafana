// The code in this file is copied from k8s.io/client-go/tools/cache/reflector.go.
...
type Reflector struct {
	...
	watchErrorHandler func(ctx context.Context, r *Reflector, err error)
...
func NewReflectorWithOptions(
	lw cache.ListerWatcherWithContext, expectedType any, store cache.ReflectorStore, options ReflectorOptions,
) *Reflector {
	...
	r := &Reflector{
		...
		watchErrorHandler: func(ctx context.Context, r *Reflector, err error) {
			klog.FromContext(ctx).Error(err, "cache reflector watch error", "reflector", r.name, "type", r.typeDescription)
		},
...
func (r *Reflector) RunWithContext(ctx context.Context) {
	...
	logger.Info("Starting reflector", "type", r.typeDescription, "resyncPeriod", r.resyncPeriod, "reflector", r.name)
	...
	logger.Info("Stopping reflector", "type", r.typeDescription, "resyncPeriod", r.resyncPeriod, "reflector", r.name)
}
...
func (r *Reflector) ListAndWatchWithContext(ctx context.Context) error {
	logger := klog.FromContext(ctx)
	logger.Info("Listing and watching", "type", r.typeDescription, "reflector", r.name)
	var err error
	...
	logger.Info("Caches populated", "type", r.typeDescription, "reflector", r.name)
...
func (r *Reflector) list(ctx context.Context) error {
	...
	go func() {
		...
		exp := isExpiredError(err)
		tooLarge := isTooLargeResourceVersionError(err)
		if exp || tooLarge {
			klog.FromContext(ctx).Error(
				err, "error fetching list",
		...
		if w == nil {
			...
			if err != nil {
				if canRetry := isWatchErrorRetriable(err); canRetry {
					logger.Info("Watch failed - backing off", "reflector", r.name, "type", r.typeDescription, "err", err)
					select {
					case <-stopCh:
						return nil
		...
		if err != nil {
			if !errors.Is(err, errorStopRequested) {
				...
				case isExpiredError(err):
					...
					logger.Info("Watch closed", "reflector", r.name, "type", r.typeDescription, "err", err)
				case apierrors.IsTooManyRequests(err):
					logger.Info("Watch returned 429 - backing off", "reflector", r.name, "type", r.typeDescription)
					select {
					case <-stopCh:
						return nil
					case <-r.backoffManager.Backoff().C():
				...
				case apierrors.IsInternalError(err) && retry.ShouldRetry():
					logger.Info("Retrying watch after internal error", "reflector", r.name, "type", r.typeDescription, "err", err)
					continue
				default:
...
func (r *Reflector) watchList(ctx context.Context) (watch.Interface, error) {
	...
	isErrorRetriableWithSideEffectsFn := func(err error) bool {
		if canRetry := isWatchErrorRetriable(err); canRetry {
			logger.Info("watch-list failed - backing off", "reflector", r.name, "type", r.typeDescription, "err", err)
...
func (r *Reflector) startResync(ctx context.Context, resyncerrc chan error) {
	...
	for {
		...
		if r.ShouldResync == nil || r.ShouldResync() {
			logger.Info("Forcing resync", "reflector", r.name)
...
loop:
	for {
		...
		case event, ok := <-w.ResultChan():
			...
			if exitOnWatchListBookmarkReceived && watchListBookmarkReceived {
				...
				klog.FromContext(ctx).Info("Exiting watch because received the bookmark that marks the end of initial events stream", "reflector", name, "totalItems", eventCount, "duration", watchDuration)
	...
	klog.FromContext(ctx).Info("Watch close", "reflector", name, "type", expectedTypeName, "totalItems", eventCount)
	return watchListBookmarkReceived, nil
}
