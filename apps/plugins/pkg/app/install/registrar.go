...
const (
	...
	SourcePluginPipeline  Source = "plugin-pipeline"
	SourcePreinstallSync  Source = "preinstall-sync"
	SourcePreinstallAsync Source = "preinstall-async"
)
...
type InstallSourceAPIRegistration struct {
	clientGenerator resource.ClientGenerator
}

func NewInstallSourceAPIRegistration(clientGenerator resource.ClientGenerator) *InstallSourceAPIRegistration {
	return &InstallSourceAPIRegistration{
		clientGenerator: clientGenerator,
	}
}

func (r *InstallSourceAPIRegistration) getClient() (*pluginsv0alpha1.PluginInstallClient, error) {
	return pluginsv0alpha1.NewPluginInstallClientFromGenerator(r.clientGenerator)
}

func (r *InstallSourceAPIRegistration) Register(ctx context.Context, namespace string, install *PluginInstall) error {
	log := logging.FromContext(ctx)
	log.Debug("Registering plugin install", "pluginId", install.ID, "version", install.Version, "source", install.Source)
