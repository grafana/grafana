...
const TAB_PAGE_MAP: Record<TabView, React.ReactElement> = {
  ...
  [TabView.VERSIONS]: <Version versions={mockDetails.versions} installedVersion={mockDetails.installedVersion} />,
};

function Overview() {
  return (
    <div>
      <Trans i18nKey="upgrades.overveiw.header">Overview of grafana version.</Trans>
    </div>
  );
}
...
function UpgradePage() {
  ...
  return (
    <div>
      <TabsBar>
        {TABS.map((tab) => (
          <Tab key={tab.id} label={tab.label} active={activeTab === tab.id} onChangeTab={() => setActiveTab(tab.id)} />
        ))}
      </TabsBar>
