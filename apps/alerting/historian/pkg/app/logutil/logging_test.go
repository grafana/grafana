...
func TestToGoKitLogger(t *testing.T) {
	t.Run("debug / 1 args", func(t *testing.T) {
		fake, gk := setup()
		level.Debug(gk).Log("msg", "hello world", "foo")
	...
	t.Run("no level / no args / no msg", func(t *testing.T) {
		fake, gk := setup()
		gk.Log()
		require.Equal(t, []ent(nil), fake.logs)
	})
}
