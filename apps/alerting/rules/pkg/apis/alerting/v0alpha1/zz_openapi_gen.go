		...
		Schema: spec.Schema{
			SchemaProps: spec.SchemaProps{
				...
				Properties: map[string]spec.Schema{
					...
					"lastAppliedHash": {
						SchemaProps: spec.SchemaProps{
							Description: "lastAppliedHash is the dedup key (upstream config hash combined with the resolved uid) from the last successful sync via this resource. The worker reads it back (API path only) to skip an unchanged re-apply across restarts and replicas, where an in-memory-only dedup cache would otherwise start empty. Internal bookkeeping; not user-facing.",
							Type:        []string{"string"},
							Format:      "",
						},
					},
