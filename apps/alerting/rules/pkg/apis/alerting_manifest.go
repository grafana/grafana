...
var (
	...
	rawSchemaPrometheusRulev0alpha1     = []byte(`{"OperatorState":{"additionalProperties":false,"properties":{"descriptiveState":{"description":"descriptiveState is an optional more descriptive state field which has no requirements on format","type":"string"},"details":{"additionalProperties":true,"description":"details contains any extra information that is operator-specific","type":"object"},"lastEvaluation":{"description":"lastEvaluation is the ResourceVersion last evaluated","type":"string"},"state":{"description":"state describes the state of the lastEvaluation.\nIt is limited to three possible states for machine evaluation.","enum":["success","in_progress","failed"],"type":"string"}},"required":["lastEvaluation","state"],"type":"object"},"PromDuration":{"not":{"pattern":"hmuµn"},"pattern":"^((([0-9]+)y)?(([0-9]+)w)?(([0-9]+)d)?(([0-9]+)h)?(([0-9]+)m)?(([0-9]+)s)?|0)$","type":"string"},"PrometheusRule":{"properties":{"spec":{"$ref":"#/components/schemas/spec"},"status":{"$ref":"#/components/schemas/status"}},"required":["spec"]},"PrometheusRuleEntry":{"additionalProperties":false,"properties":{"alert":{"type":"string"},"annotations":{"additionalProperties":{"type":"string"},"type":"object"},"expr":{"type":"string"},"for":{"$ref":"#/components/schemas/PromDuration"},"keepFiringFor":{"$ref":"#/components/schemas/PromDuration"},"labels":{"additionalProperties":{"type":"string"},"type":"object"},"record":{"type":"string"}},"required":["expr"],"type":"object"},"PrometheusRuleGroup":{"additionalProperties":false,"properties":{"interval":{"$ref":"#/components/schemas/PromDuration"},"labels":{"additionalProperties":{"type":"string"},"type":"object"},"limit":{"minimum":0,"type":"integer"},"name":{"type":"string"},"queryOffset":{"$ref":"#/components/schemas/PromDuration"},"rules":{"items":{"$ref":"#/components/schemas/PrometheusRuleEntry"},"type":"array"}},"required":["name","rules"],"type":"object"},"spec":{"additionalProperties":false,"properties":{"groups":{"items":{"$ref":"#/components/schemas/PrometheusRuleGroup"},"type":"array"}},"required":["groups"],"type":"object"},"status":{"additionalProperties":false,"properties":{"additionalFields":{"additionalProperties":true,"description":"additionalFields is reserved for future use","type":"object"},"operatorStates":{"additionalProperties":{"$ref":"#/components/schemas/OperatorState"},"description":"operatorStates is a map of operator ID to operator state evaluations.\nAny operator which consumes this kind SHOULD add its state evaluation information to this field.","type":"object"}},"type":"object"}}`)
	versionSchemaPrometheusRulev0alpha1 app.VersionSchema
	_                                   = json.Unmarshal(rawSchemaPrometheusRulev0alpha1, &versionSchemaPrometheusRulev0alpha1)
)
	...
	Versions: []app.ManifestVersion{
		{
			...
			Kinds: []app.ManifestVersionKind{
...

				{
					...
					Schema: &versionSchemaPrometheusRulev0alpha1,
				},
			},
...
var kindVersionToGoType = map[string]resource.Kind{
	...
	"PrometheusRule/v0alpha1": v0alpha1.PrometheusRuleKind(),
}
