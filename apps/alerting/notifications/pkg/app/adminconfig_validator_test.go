...
func adminConfigObj(name string, uid *string) *v0alpha1.AdminConfig {
	o := &v0alpha1.AdminConfig{}
	o.SetName(name)
	if uid != nil {
		o.Spec.Alertmanager = &v0alpha1.AdminConfigAlertmanagerSpec{
			ExternalSync: &v0alpha1.AdminConfigV0alpha1AlertmanagerSpecExternalSync{DatasourceUid: uid},
		}
	}
	return o
}
...
func TestNewAdminConfigValidator(t *testing.T) {
	ctx := context.Background()

	t.Run("rejects non-singleton name", func(t *testing.T) {
		v := newAdminConfigValidator(&Config{ValidateExternalSyncDatasource: failIfCalled(t)})
		err := v.ValidateFunc(ctx, &app.AdmissionRequest{Object: adminConfigObj("not-default", nil)})
	...
	t.Run("allows singleton name with no UID change", func(t *testing.T) {
		v := newAdminConfigValidator(&Config{ValidateExternalSyncDatasource: failIfCalled(t)})
		if err := v.ValidateFunc(ctx, &app.AdmissionRequest{Object: adminConfigObj(v0alpha1.AdminConfigSingletonName, nil)}); err != nil {
	...
	t.Run("runs datasource validator when UID set to non-empty", func(t *testing.T) {
		...
		err := newAdminConfigValidator(cfg).ValidateFunc(ctx, &app.AdmissionRequest{Object: adminConfigObj(v0alpha1.AdminConfigSingletonName, ptr("uid-1"))})
		if !called {
			t.Fatal("expected datasource validator to be called")
		}
		if err == nil || !strings.Contains(err.Error(), "alertmanager.externalSync.datasourceUid") {
	...
	t.Run("allows clearing UID without running validator", func(t *testing.T) {
		req := &app.AdmissionRequest{
			Object:    adminConfigObj(v0alpha1.AdminConfigSingletonName, nil),
			OldObject: adminConfigObj(v0alpha1.AdminConfigSingletonName, ptr("uid-1")),
		}
		if err := newAdminConfigValidator(&Config{ValidateExternalSyncDatasource: failIfCalled(t)}).ValidateFunc(ctx, req); err != nil {
