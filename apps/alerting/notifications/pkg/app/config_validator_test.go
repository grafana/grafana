...
func configObjV0alpha1(name string, uid *string) resource.Object {
	o := &v0alpha1.Config{}
	o.SetName(name)
	if uid != nil {
...
func configObjV1beta1(name string, uid *string) resource.Object {
	o := &v1beta1.Config{}
	o.SetName(name)
	if uid != nil {
		o.Spec.ExternalAlertmanagerSync = &v1beta1.ConfigV1beta1SpecExternalAlertmanagerSync{DatasourceUid: uid}
	}
	return o
}
...
func TestNewConfigValidator(t *testing.T) {
	...
	// The same validator is attached to every served version of the kind, so
	// each case runs against both.
	versions := map[string]func(name string, uid *string) resource.Object{
		"v0alpha1": configObjV0alpha1,
		"v1beta1":  configObjV1beta1,
	}

	for version, configObj := range versions {
		t.Run(version, func(t *testing.T) {
			...
			t.Run("allows singleton name with no UID change", func(t *testing.T) {
				v := newConfigValidator(&Config{ValidateExternalSyncDatasource: failIfCalled(t)})
				if err := v.ValidateFunc(ctx, &app.AdmissionRequest{Object: configObj(v1beta1.ConfigSingletonName, nil)}); err != nil {
	...
	t.Run("rejects an object that is not a Config", func(t *testing.T) {
		v := newConfigValidator(&Config{ValidateExternalSyncDatasource: failIfCalled(t)})
		err := v.ValidateFunc(ctx, &app.AdmissionRequest{Object: &v1beta1.Receiver{}})
		if err == nil || !strings.Contains(err.Error(), "not a Config") {
