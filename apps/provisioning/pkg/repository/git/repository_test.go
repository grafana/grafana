...
func TestGitRepository_CompareFiles_FallbackRef(t *testing.T) {
	t.Run("falls back to next ref when first is missing", func(t *testing.T) {
		mockClient := &mocks.FakeClient{}
		mockClient.GetRefReturnsOnCall(0, nanogit.Ref{
			Name: "refs/heads/main",
			Hash: hash.MustFromHex("0102030405060708090a0b0c0d0e0f1011121314"),
		...
		mockClient.GetRefReturnsOnCall(1, nanogit.Ref{}, nanogit.ErrObjectNotFound)
		mockClient.GetRefReturnsOnCall(2, nanogit.Ref{
			Name: "refs/heads/feature",
			Hash: hash.MustFromHex("1415161718191a1b1c1d1e1f2021222324252627"),
		}, nil)
		mockClient.CompareCommitsReturns([]nanogit.CommitFile{
			{
				Path:   "configs/new-file.yaml",
		...
		changes, err := gitRepo.CompareFiles(context.Background(), "main", "refs/pull/123/merge", "feature")

		require.NoError(t, err)
		require.Len(t, changes, 1)
		require.Equal(t, "feature", changes[0].Ref)

		_, gotRef := mockClient.GetRefArgsForCall(1)
		require.Equal(t, "refs/pull/123/merge", gotRef)
		_, gotRef = mockClient.GetRefArgsForCall(2)
		require.Equal(t, "refs/heads/feature", gotRef)
	})

	t.Run("errors when no ref resolves", func(t *testing.T) {
		mockClient := &mocks.FakeClient{}
		mockClient.GetRefReturnsOnCall(0, nanogit.Ref{
			Name: "refs/heads/main",
			Hash: hash.MustFromHex("0102030405060708090a0b0c0d0e0f1011121314"),
		...
		mockClient.GetRefReturnsOnCall(1, nanogit.Ref{}, nanogit.ErrObjectNotFound)
		mockClient.GetRefReturnsOnCall(2, nanogit.Ref{}, nanogit.ErrObjectNotFound)
		...
		_, err := gitRepo.CompareFiles(context.Background(), "main", "refs/pull/123/merge", "feature")

		require.Error(t, err)
		require.ErrorIs(t, err, repository.ErrRefNotFound)
		require.Equal(t, 0, mockClient.CompareCommitsCallCount())
	})
}
