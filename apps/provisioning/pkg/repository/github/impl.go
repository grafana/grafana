...
func (r *githubClient) CreateWebhook(ctx context.Context, owner, repository string, cfg WebhookConfig) (WebhookConfig, error) {
	...
	if err != nil {
		...
		if errors.As(err, &ghErr) && isWebhookAlreadyExists(ghErr) {
			return r.adoptExistingWebhook(ctx, owner, repository, cfg)
		}
...
func (r *githubClient) adoptExistingWebhook(ctx context.Context, owner, repository string, cfg WebhookConfig) (WebhookConfig, error) {
	...
	for {
		...
		for _, h := range hooks {
			...
			if _, _, err := r.gh.Repositories.EditHook(ctx, owner, repository, h.GetID(), edit); err != nil {
