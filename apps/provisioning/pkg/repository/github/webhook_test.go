...
func TestParseWebhooks(t *testing.T) {
	...
	gh.WebhookHandler = repo.NewWebhookHandler(gh.processRequest, cfg.Status.Webhook, cfg.GetName(),
		"grafana/git-ui-sync-demo", cfg.Spec.GitHub.Branch, cfg.Spec.Sync.Enabled, repo.IncrementalSyncPolicy{})
...
func TestParsePushEvent_LargeDiffForcesFullSync(t *testing.T) {
	...
	gh.WebhookHandler = repo.NewWebhookHandler(gh.processRequest, cfg.Status.Webhook, cfg.GetName(),
		"grafana/git-ui-sync-demo", cfg.Spec.GitHub.Branch, cfg.Spec.Sync.Enabled, repo.NewIncrementalSyncPolicy(false, 5))
...
func TestGitHubRepository_Webhook_ReplayProtection(t *testing.T) {
	...
	newRepo := func(cache *replayCache, secret string) *githubWebhookRepository {
		...
		r.WebhookHandler = repo.NewWebhookHandler(r.processRequest, cfg.Status.Webhook, cfg.GetName(),
			"grafana/grafana", cfg.Spec.GitHub.Branch, cfg.Spec.Sync.Enabled, repo.IncrementalSyncPolicy{})
		return r
	}
