...
func NewGithubWebhookRepository(
	...
	// the repository with a nil cache to dereference.
	if replay == nil {
	...
	return &githubWebhookRepository{
		...
		WebhookHandler: repository.NewWebhookHandler(
			requestProcessor{secret: secret, replay: replay}, cfg.Status.Webhook, cfg.GetName(), slug,
			cfg.Spec.GitHub.Branch, cfg.Spec.Sync.Enabled, incrementalPolicy,
		),
		WebhookManager: repository.NewWebhookManager(basic.Client(), cfg.Status.Webhook, webhookURL, subscribedEvents,
...
func (p requestProcessor) ProcessRequest(ctx context.Context, req *http.Request) (repository.WebhookEvent, error) {
	if p.secret.IsZero() {
		return repository.WebhookEvent{}, fmt.Errorf("missing webhook secret")
	}

	payload, err := github.ValidatePayload(req, []byte(p.secret))
	if err != nil {
		return repository.WebhookEvent{}, apierrors.NewUnauthorized("invalid request")
	}
	...
	//
	// A replayed delivery is dropped as a ping so the handler returns a generic
	// 200, avoiding confirming to an attacker that the payload was real.
	signature := req.Header.Get(github.SHA256SignatureHeader)
	if signature == "" {
		signature = req.Header.Get(github.SHA1SignatureHeader)
	}
	if p.replay.seenOrAdd(signature) {
