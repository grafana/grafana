...
// RequestProcessor authenticates and normalizes a provider's inbound webhook
// delivery into a provider-agnostic event. It is the only provider-specific
// part of the inbound flow.
//
//go:generate mockery --name RequestProcessor --structname MockRequestProcessor --inpackage --filename mock_request_processor.go --with-expecter
type RequestProcessor interface {
	ProcessRequest(ctx context.Context, req *http.Request) (WebhookEvent, error)
}

func NewWebhookHandler(processor RequestProcessor, status *provisioning.WebhookStatus, repoName, repoSlug, branch string, syncEnabled bool, incrementalPolicy IncrementalSyncPolicy) *WebhookHandler {
...
// job response. The provider supplies authentication and event normalization
// via the RequestProcessor.
func (m *WebhookHandler) Webhook(ctx context.Context, req *http.Request) (*provisioning.WebhookResponse, error) {
	...
	event, err := m.processor.ProcessRequest(ctx, req)
	if err != nil {
		return nil, err
	}
