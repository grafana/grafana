...
func (m *mockStorageLister) List(ctx context.Context, options *internalversion.ListOptions) (runtime.Object, error) {
	...
	// Capture the field selector for testing
	if options.FieldSelector != nil {
		m.lastFieldSel = options.FieldSelector.String()
	}
...
func TestStorageLister_ListByConnection(t *testing.T) {
	t.Run("uses field selector for connection name", func(t *testing.T) {
		store := &mockStorageLister{
			pages: [][]provisioning.Repository{
				{
					{ObjectMeta: metav1.ObjectMeta{Name: "repo1"}},
		...
		_, err := lister.ListByConnection(context.Background(), "my-connection")

		require.NoError(t, err)
		assert.Equal(t, "spec.connection.name=my-connection", store.lastFieldSel)
	})

	t.Run("returns repositories matching connection", func(t *testing.T) {
		store := &mockStorageLister{
			pages: [][]provisioning.Repository{
				{
					{
		...
		repos, err := lister.ListByConnection(context.Background(), "my-connection")
	...
	t.Run("returns empty list when no matches", func(t *testing.T) {
		...
		repos, err := lister.ListByConnection(context.Background(), "no-match")
