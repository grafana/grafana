...
func (c *Connection) Test(ctx context.Context) (*provisioning.TestResults, error) {
	...
	permissionErrors := validatePermissions(permissionTargetApp, c.obj.Spec.GitHub.AppID, app.Permissions, c.obj.Spec.GitHub.PollingOnly)
	...
	installationPermErrors := validatePermissions(permissionTargetInstallation, c.obj.Spec.GitHub.InstallationID, installation.Permissions, c.obj.Spec.GitHub.PollingOnly)
...
// When pollingOnly is true, the webhooks:write check is skipped because Grafana will poll
// instead of receiving push events from GitHub.
func validatePermissions(target permissionTarget, id string, permissions Permissions, pollingOnly bool) []provisioning.ErrorDetails {
