...
- **Backend migration tests**: Schema version migration tests
- **Frontend parity tests**: TypeScript conversion tests that compare against backend golden files
...
1. The test always writes the golden file to disk (so frontend tests can read it).
2. It computes the SHA-256 of the written data.
3. In normal mode, it compares against the stored checksum and fails on mismatch.
4. In regenerate mode (`REGENERATE_CHECKSUMS=true`), it records the new hash instead.
...
- **Test data**: Uses golden files from `apps/dashboard/pkg/migration/conversion/testdata/` and migrated dashboards
...
- **Test data**: Uses golden files from `apps/dashboard/pkg/migration/conversion/testdata/`
...
// Conversion duration histogram (labels: source_version_api, target_version_api, outcome)
