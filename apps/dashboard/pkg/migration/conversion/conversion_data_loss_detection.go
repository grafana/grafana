...
//
// Beyond the automated validation implemented here, consider these approaches:
//
// ## 1. Schema Validation
// Use CUE or JSON Schema to validate the structure of converted dashboards:
//   - Ensures all required fields are present
//   - Validates field types and constraints
//   - Catches malformed data early
//
// ## 2. Transformation Data Loss Detection
// For complex transformations (like V1->V2 panel restructuring):
//   - Detect if data transformation configurations are lost
//   - Check that field mappings are maintained
//   - Verify calculation expressions survive conversion
//
// ## 3. Visual Regression Testing
// Render dashboards before and after conversion:
//   - Compare screenshots for visual differences
//   - Validate chart configurations produce same output
//   - Detect layout changes that affect readability
//
// ## 4. Alert Configuration Data Loss Detection
// For dashboards with alerts:
//   - Detect if alert rules are lost during conversion
//   - Check alert thresholds and conditions
//   - Validate notification channel configurations
//
// ## 5. Variable Data Loss Detection
// Dashboard variables enable dynamic queries:
//   - Count template variables and their types
//   - Detect if variables are lost
//   - Check variable dependencies and chaining
//
// ## 6. Templating Data Loss Detection
// Dashboards use templating in queries and panel titles:
//   - Detect if variable references (${varName}) are lost
//   - Check that template syntax remains valid
//   - Verify repeated panels/rows maintain template bindings
//
// ## 7. Time Range and Refresh Data Loss Detection
// Dashboard-level settings:
//   - Detect if time range settings are lost
//   - Check auto-refresh intervals
//   - Validate time zone configurations
//
// ## 8. Permission and Folder Metadata
// Organizational aspects:
//   - Verify folder assignments are preserved
//   - Check dashboard tags and metadata
//   - Validate permission inheritance
//
// ## 9. Data Source Reference Data Loss Detection
// Critical for query execution:
//   - Detect if data source references are lost
//   - Check that data source types are preserved
//   - Validate data source-specific query properties
//
// ## 10. Round-Trip Testing
// Convert forward and backward:
//   - V0 -> V1 -> V0 should yield original
//   - Identifies lossy transformations
//   - Helps isolate conversion bugs
//
// ## 11. Checksum/Hash Validation
// For detecting subtle changes:
//   - Hash critical sections of dashboard JSON
//   - Compare hashes before/after conversion
//   - Useful for detecting unexpected modifications
//
// ## 12. Diff Analysis
// Compare JSON structures:
//   - Use semantic diff tools on dashboard JSON
//   - Identify added, removed, or changed fields
//   - Filter expected changes from unexpected ones
//
// # Usage Example
//
//   // Data loss detection is automatically applied in withConversionMetrics:
//   withConversionMetrics(sourceAPI, targetAPI, func(a, b interface{}, scope conversion.Scope) error {
//       return ConvertDashboard(a, b, scope)
//   })
//
// # Error Handling
//
// When data loss is detected, a ConversionDataLossError is returned containing:
//   - Function name where data loss was detected
//   - Detailed error message listing all losses
//   - Structured logging with all count details
//
// The error is also logged with ERROR level (or WARN for expected failures like minimum version errors or unsupported downgrades from V2).
//
// # Performance Considerations
//
// Data loss detection adds minimal overhead:
//   - Single pass through panels/queries/annotations/links
//   - No deep cloning or JSON serialization
//   - Counts are computed in O(n) time where n is number of items
//   - Memory overhead is constant (just the dashboardStats struct)
//
// # Future Enhancements
//
// Potential additions to the data loss detection system:
//   - Variable count detection
//   - Alert rule count detection
//   - Data transformation loss detection
//   - Plugin-specific detection hooks
//   - Configurable severity levels
//   - Detection result caching for performance
//
// # Known Limitations
//
// Conversions from V2 (v2alpha1, v2beta1) to earlier versions (v0, v1) are not yet fully implemented.
// Data loss is expected in these downgrade paths and should be handled appropriately (either by
// blocking the conversion or warning the user).
