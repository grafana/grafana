...
func TestMigratedDashboardsConversion(t *testing.T) {
	...
	// These are generated by running migration tests first (make generate-golden-files).
	inputDir := filepath.Join("..", "testdata", "output", "latest_version")
	files, err := os.ReadDir(inputDir)
	if err != nil {
		t.Skipf("Skipping: migration golden files not found at %s. Run 'make generate-golden-files' from apps/dashboard/ to generate them.", inputDir)
	}
