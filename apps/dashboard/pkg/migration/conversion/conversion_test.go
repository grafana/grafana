...
func TestMigratedDashboardsConversion(t *testing.T) {
	...
	// These are generated by running the migration tests first (make generate-golden-files).
	inputDir := filepath.Join("..", "testdata", "output", "latest_version")
	files, err := os.ReadDir(inputDir)
	require.NoError(t, err, "Migration golden files not found at %s. Run 'make generate-golden-files' from apps/dashboard/", inputDir)
