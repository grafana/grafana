...
func TestV36(t *testing.T) {
	...
	tests := []migrationTestCase{
		...
		// FIXME: Template variable migration test fails - backend incorrectly converts strings to objects
		// Issue: Backend converts ALL template variable datasources to object format, but frontend only converts null datasources
		// Expected: Template variables should keep string datasource references like "Existing Target Name" and "existing-target-uid"
		// Actual: Backend converts them to objects like {"apiVersion":"v2", "type":"elasticsearch", "uid":"existing-target-uid"}
		// Frontend behavior: Only converts variable.datasource === null to object format (using default datasource)
		/*
			{
