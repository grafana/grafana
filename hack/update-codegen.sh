...
CODEGEN_PKG=${CODEGEN_PKG:-$(cd "${SCRIPT_ROOT}"; ls -d -1 ./vendor/k8s.io/code-generator 2>/dev/null || echo $(go env GOPATH)/pkg/mod/k8s.io/code-generator@v0.35.0)}
...
grafana::codegen:run() {
  ...
  for api_pkg in $(grafana:codegen:lsdirs ./${generate_root}/apis); do
   ...
   for pkg_version in $(grafana:codegen:lsdirs ./${generate_root}/apis/${api_pkg}); do
      # v1beta1 aliases v1; hand-maintained openapi_definitions.go / defaults.go; no +k8s:openapi-gen tags.
